...
describe("profile settings media visual snapshots", () => {
  let container;
  let root;
  ...
  it("matches media tab active story state", async () => {
    await act(async () => {
    ...
    expect(mediaSection?.outerHTML || "").toMatchSnapshot();
  });

  it("matches expanded story modal state", async () => {
    await act(async () => {
      root.render(React.createElement(ProfileSettingsPage, { initialTab: "media" }));
    });
