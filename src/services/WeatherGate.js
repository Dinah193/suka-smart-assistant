...
(function () {
  ...
  try { SafetyRules = require("@/libraries/SafetyRules"); } catch (e) {}
  ...
  function registerProvider(name, fetchFn){ providers[name] = fetchFn; }
  ...
  function cacheKey(p, lat, lon, start, end){
    ...
    return [p, r(lat), r(lon), Math.floor(start/3600000), Math.floor(end/3600000)].join("|");
  }
  ...
  function putCache(key, value, ttl){
    CACHE[key] = { value: value, storedAt: scheduleHelpers.now(), ttl: ttl || DEFAULT_TTL_MS };
  }
  ...
  function heatIndexC(tempC, rh){
    ...
    var HI = -42.379 + 2.04901523*T + 10.14333127*R - 0.22475541*T*R - 6.83783e-3*T*T - 5.481717e-2*R*R + 1.22874e-3*T*T*R + 8.5282e-4*T*R*R - 1.99e-6*T*T*R*R;
    // adjustments
  ...
  function wetBulbC(tempC, rh){
    if (tempC == null || rh == null) return null;
    var T = tempC, R = rh;
    var Tw = T * Math.atan(0.151977*Math.sqrt(R+8.313659)) + Math.atan(T+R) - Math.atan(R-1.676331) + 0.00391838*Math.pow(R,1.5)*Math.atan(0.023101*R) - 4.686035;
    return Tw;
  }
  ...
  function ensureMetricsOnHourly(hourly){
    for (var i=0;i<(hourly||[]).length;i++){
      var h = hourly[i];
      if (typeof h.heatIndexC !== "number") h.heatIndexC = heatIndexC(h.tempC, h.rh);
      if (typeof h.windChillC !== "number") h.windChillC = windChillC(h.tempC, h.windKph);
  ...
  function assessRiskAt(hour){
    ...
    if (typeof hi === "number") {
      if (hi >= 32) { level = bump(level, "high"); reasons.push("Dangerous heat index"); flags.heat = true; }
      else if (hi >= 27) { level = bump(level, "medium"); reasons.push("Elevated heat index"); flags.heat = true; }
    }
    // Cold/WindChill
    var wc = hour.windChillC;
    if (typeof wc === "number") {
      if (wc <= -15) { level = bump(level, "high"); reasons.push("Severe wind chill"); flags.cold = true; }
      else if (wc <= 0) { level = bump(level, "medium"); reasons.push("Low wind chill"); flags.cold = true; }
    }
    // Wind
    if (typeof hour.gustKph === "number" && hour.gustKph >= 45) {
      level = bump(level, "high"); reasons.push("Strong gusts"); flags.wind = true;
    } else if (typeof hour.windKph === "number" && hour.windKph >= 30) {
      level = bump(level, "medium"); reasons.push("Windy"); flags.wind = true;
    }
    // Precip
    if (typeof hour.precipProb === "number") {
      if (hour.precipProb >= 0.7) { level = bump(level, "high"); reasons.push("High precipitation probability"); flags.precip = true; }
      else if (hour.precipProb >= 0.4) { level = bump(level, "medium"); reasons.push("Chance of precipitation"); flags.precip = true; }
    }
    // UV (if available)
    if (typeof hour.uv === "number") {
      if (hour.uv >= 8) { level = bump(level, "high"); reasons.push("Very high UV"); flags.uv = true; }
      else if (hour.uv >= 5) { level = bump(level, "medium"); reasons.push("Moderate UV"); flags.uv = true; }
    }
  ...
  function findSafeWindows(weather, opts){
    ...
    for (var i=0;i<(weather.hourly||[]).length;i++){
      ...
      if (ok) {
        if (!active) active = { start: h.t, end: h.t + 60*60*1000, reasons: [] };
        else active.end = h.t + 60*60*1000;
      } else {
        if (active) { slots.push(active); active = null; }
      }
    }
    ...
    for (var j=0;j<slots.length;j++){
      ...
      out.push({ start: slots[j].start, end: slots[j].end, score: score, reasons: [] });
    }
  ...
  function assessTaskWeather(task, weather, domain){
    ...
    if (!task || task.indoor === true) return { level: "low", reasons: [], suggestions: [], safeWindows: [] };
    ...
    if (w) {
      var hits = (weather.hourly||[]).filter(function(h){ return h.t >= w.start && h.t <= w.end; });
      var worst = "low";
      ...
      if (level !== "low") {
        ...
        suggestions.push({ title: "Find earliest safe window", autoApply: false, intent: "option", emit: { name: "planner.schedule.safeWindow.requested", payload: { itemId: task.id } } });
        suggestions.push({ title: "Add PPE & hydration plan", autoApply: false, intent: "option", emit: { name: "prep.tasks.requested", payload: { domain: domain, tasks: ["Hydration break every 30–45 min", "Sun/heat or cold protection"] } } });
      }
    }

    // Offer safe windows regardless
    var safe = findSafeWindows(weather, { horizonHours: 72, minBlockMinutes: 30, preferEarly: true });
    return { level: level, reasons: reasons, suggestions: suggestions, safeWindows: safe };
  }
  ...
  function getWeather(loc, range, opts){
    ...
    if (cached && !cached.isFresh && opts.allowStale !== false) {
      ...
      try {
        providers[provider](lat, lon, start, end).then(function(fresh){
    ...
    // Fetch fresh
    return providers[provider](lat, lon, start, end).then(function(raw){
      putCache(ck, raw, opts.ttlMs);
      return normalize(raw);
    }).catch(function(err){
      console.warn("[WeatherGate] provider failed:", provider, err && err.message);
      ...
      return Promise.resolve(normalize({
        location: { lat: lat, lon: lon, name: loc.name || "Unknown", tz: loc.tz || "America/Chicago" },
        range: { start: start, end: end },
        hourly: [],
        daily: [],
        meta: { provider: provider, fetchedAt: scheduleHelpers.now(), units: { temp: "C", wind: "kph", precip: "mm" } }
      }));
    });
  }
  ...
  registerProvider("devLocal", function(lat, lon, start, end){
    ...
    var out = {
      ...
      hourly: [],
      daily: [],
      meta: { provider: "devLocal", fetchedAt: Date.now(), units: { temp: "C", wind: "kph", precip: "mm" } }
    };
    ...
    // roll up daily
    var dayCursor = new Date(start); dayCursor.setHours(12,0,0,0);
    while (dayCursor.getTime() <= end) {
      var d = dayCursor.getTime();
      var dayHours = out.hourly.filter(function(h){
        var hd = new Date(h.t); hd.setHours(12,0,0,0); return hd.getTime() === d;
      });
      if (dayHours.length) {
        var temps = dayHours.map(function(h){ return h.tempC; });
        var rhAvg = avg(dayHours.map(function(h){ return h.rh; }));
        var windMax = Math.max.apply(null, dayHours.map(function(h){ return h.windKph; }));
        var precipProbMax = Math.max.apply(null, dayHours.map(function(h){ return h.precipProb; }));
        out.daily.push({
          d: d,
          ...
          uvMax: Math.max.apply(null, dayHours.map(function(h){ return h.uv || 0; }))
        });
      }
    ...
    return {
      ...
      payload: {
        ...
        conflict: {
          ...
          rationale: assess.reasons.join("; ") || "Outdoor conditions may reduce success or safety.",
          score: score,
          affected: [task.id],
          suggestions: [
            { title: "Find safe window", autoApply: false, intent: "option", emit: { name: "planner.schedule.safeWindow.requested", payload: { itemId: task.id } } },
            { title: "Add PPE & hydration plan", autoApply: false, intent: "option", emit: { name: "prep.tasks.requested", payload: { domain: domain || "garden", tasks: ["Hydration break every 30–45 min", "Sun/heat or cold protection"] } } }
          ]
        }
      }
    };
  ...
  var api = {
    ...
    // Math helpers
    cToF: cToF, fToC: fToC,
    heatIndexC: heatIndexC, windChillC: windChillC, wetBulbC: wetBulbC,
    kphToMph: kphToMph,
