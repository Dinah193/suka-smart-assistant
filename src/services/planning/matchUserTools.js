...
const STATION_FROM_TOOLS = [
  ...
  {
    key: "burners",
    includes: ["pan", "pot", "skillet", "wok", "saucepan", "stockpot"],
  },
...
function stationForStep(step = {}) {
  ...
  for (const st of STATION_FROM_TOOLS) {
    if (
      req.some((id) => st.includes.includes(id)) ||
      tags.some((t) => st.includes.includes(t))
    )
      return st.key;
  }
...
function safetyNeeds(step = {}) {
  ...
  const out = new Set();
  if (tags.has("raw_chicken") || tags.has("raw_poultry"))
    out.add("raw_poultry");
...
const matchUserTools = async (steps = [], users = []) => {
  ...
  if (!availableTools.length)
    warnings.push("Tool catalog empty or DexieDB.tools not available.");

  if (!users || !users.length) {
    users = (await safe(() => DexieDB.users?.toArray?.(), [])) || [];
    if (!users.length)
      warnings.push(
        "No users provided and DexieDB.users not available; all steps will be unassigned."
      );
  }
  ...
  function coverageFor(user, requiredToolIds = []) {
    ...
    const matchingTools = findBestMatchingTools(
      requiredToolIds.map((tid) => ({
        id: tid,
        tags: toolIndex.get(tid)?.tags || [],
      })),
    ...
    return {
      count: matchingTools.length + usedEquivalents.size,
      missing,
      usedEquivalents: Array.from(usedEquivalents),
    };
  }

  function proficiencyFor(user, requiredToolIds = []) {
    if (!profs?.length) return 0;
    const rows = profs.filter(
      (p) => p.userId === user.id && requiredToolIds.includes(p.toolId)
    );
    if (!rows.length) return 0;
    // average 0..1
    const avg =
      rows.reduce((s, r) => s + Number(r.level || 0) / 3, 0) / rows.length;
    return avg;
  }
  ...
  function experienceFor(user, requiredToolIds = []) {
    ...
    const rows = history.filter(
      (h) => h.userId === user.id && requiredToolIds.includes(h.toolId)
    );
    if (!rows.length) return 0;
  ...
  function preferenceFor(user, step) {
    ...
    const preferredStations = new Set(
      (user.preferredStations || []).map(String)
    );
  ...
  const assignments = steps.map((step) => {
    ...
    const rotated = [
      ...orderedUsers.slice(rrIndex),
      ...orderedUsers.slice(0, rrIndex),
    ];
    ...
    for (const user of rotated) {
      ...
      const reasons = [];
      if (cov.count)
        reasons.push(`Tool coverage ${cov.count}/${requiredTools.length}`);
      if (cov.usedEquivalents.length)
        reasons.push(`Using equivalents: ${cov.usedEquivalents.join(", ")}`);
    ...
    const missing =
      pick?.coverage?.missing ||
      requiredTools.filter((id) => !toolIndex.has(id));
    missing.forEach((tid) => {
    ...
    const toolNames = requiredTools.map(
      (tid) => toolIndex.get(tid)?.name || tid
    );
  ...
  const borrowList = Array.from(borrowSuggestions.entries()).map(
    ([toolId, set]) => ({
      toolId,
