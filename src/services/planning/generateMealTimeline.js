 ...
 * - This file previously used CommonJS `module.exports = { generateMealTimeline }`
...
try {
  InventoryService =
    require("../../features/inventory/services/InventoryService").default;
} catch {}
try {
  NutritionService =
    require("../../features/nutrition/services/nutritionLookupService").default;
} catch {}
try {
  PreferencesService =
    require("../../services/preferences/PreferencesService").default;
} catch {}
try {
  UnitConverter = require("../../lib/unitConverter").default;
} catch {}
...
const DEFAULT_PREFS = {
  meals: {
    ...
    batchCooking: {
      ...
      warmUpTips: true,
      daysPerBatch: 2,
      maxRecipesPerBatch: 6,
    },
    donenessHints: { vegetables: "tender-crisp", meat: "medium" },
  },
  calendar: {
    sabbathAware: true,
    sabbathSunsetOffsetMin: 30,
    provider: "local",
    exportEvents: false,
    blockQuietWindows: false,
  },
...
function sabbathWindow(
  ...
  const satSunset = saturdaySunset
    ? dayjs(saturdaySunset)
    : sat.hour(18).minute(0);
  const stopCookingBy = friSunset.subtract(
    sabbathSunsetOffsetMin || 0,
    "minute"
  );
...
function buildFallbackCatalog(prefs = DEFAULT_PREFS) {
  ...
  const dinner = proteins.slice(0, 3).map((p, i) =>
    makeFallbackRecipe({
      protein: p,
      veg: rotatePick(veggies, i),
      carb: rotatePick(breads, i),
      slot: "dinner",
      portions,
      prefs: P,
    })
  ...
  const lunch = proteins.slice(0, 2).map((p, i) =>
    makeFallbackRecipe({
      protein: `${p} salad`,
      veg: rotatePick(veggies, i + 1),
      carb: rotatePick(breads, i + 1),
      slot: "lunch",
      portions,
      prefs: P,
    })
  ...
  for (const ing of ingredients) {
    ...
    if (remaining > 0) {
      needed.push({
        ...
        qty: remaining,
        unit: wantUnit || "",
        reason: "not_in_forecast_or_inventory",
      });
    }
  }
...
function applySabbathConstraints(days, prefs, sunsetInfo = {}) {
  ...
  const window = sabbathWindow(
    days[0]?.date || dayjs().toISOString(),
    offMin,
    fridaySunset,
    saturdaySunset
  );
  ...
  for (const d of days) {
    ...
    if (isFri && d.slots?.dinner) {
      ...
      d.slots.dinner.notes = [
        ...(d.slots.dinner.notes || []),
        `Sabbath-aware: finish cooking before ${window.start.format(
          "HH:mm"
        )}. Serve reheat/cold.`,
      ];
    }
    ...
    if (isSat) {
      for (const slot of SLOT_KEYS) {
        ...
        d.slots[slot].notes = [
          ...(d.slots[slot].notes || []),
          "Sabbath-aware: cold or gentle reheat only.",
        ];
      }
    }
  }
...
function buildBatchPlan(days, prefs) {
  ...
  const sessionDayName = (
    prefs?.meals?.batchCooking?.sessionDay || "Sunday"
  ).toLowerCase();
  const sessionDayIdx = [
    "sunday",
    "monday",
  ...
  for (const d of days) {
    for (const k of ["dinner", "lunch"]) {
      const sl = d.slots[k];
      if (sl?.recipe && !sl.noCook)
        upcoming.push({ ...sl, date: d.date, slot: k });
    }
  }
  const maxRecipes = Number(
    prefs?.meals?.batchCooking?.maxRecipesPerBatch || 6
  );
  ...
  const reheatNotes = prefs?.meals?.batchCooking?.warmUpTips
    ? selected.map((s) => ({
        recipeId: s.recipe.id,
        note: `Reheat ${s.recipe.title} gently; keep veggies ${
          prefs.meals?.donenessHints?.vegetables || "tender-crisp"
        }.`,
      }))
    : [];
  ...
  const startMinute =
    (Number(prefs?.meals?.batchCooking?.sessionHour) || 14) * 60;
...
async function computeNutritionForPlan(days) {
  if (!NutritionService)
    return {
      totalsPerDay: [],
      perMeal: [],
      notes: ["NutritionService unavailable"],
    };
  const totalsPerDay = [];
  const perMeal = [];
...
function buildCalendarEventStubs(days, prefs, anchorISO) {
  if (!prefs?.calendar?.exportEvents)
    return {
      provider: prefs.calendar?.provider || "local",
      exportable: false,
      events: [],
    };
  ...
  for (let i = 0; i < days.length; i++) {
    ...
    for (const slot of SLOT_KEYS) {
      ...
      events.push({
        ...
        description: s.noCook
          ? "Sabbath-aware: cold or reheat"
          : "Cook/serve window",
        location: "Home",
...
async function generateMealTimeline(args = {}) {
  ...
  if (PreferencesService && P.calendar?.sabbathAware) {
    try {
      ...
      if (quiet?.length)
        warnings.push("Quiet windows available via PreferencesService.");
    } catch {
      warnings.push(
        "PreferencesService quiet windows unavailable; using local Sabbath rule."
      );
    }
  }
  ...
  // 3) Slot assignment
  const bySlot = {
    breakfast: catalog.filter(
      (r) => (r.slotHint || "").toLowerCase() === "breakfast"
    ),
    lunch: catalog.filter((r) => (r.slotHint || "").toLowerCase() === "lunch"),
    dinner: catalog.filter(
      (r) => (r.slotHint || "").toLowerCase() === "dinner"
    ),
  };
  ...
  const forecastMap = P.integrateGarden
    ? buildForecastAvailability(forecast)
    : null;
  const groceryList = [];
  for (const d of days) {
    for (const sk of SLOT_KEYS) {
      ...
      if (!s?.recipe) continue;
      const alloc =
        P.integrateGarden || P.integrateInventory
          ? allocateIngredientsForDay(s.ingredients, d.date, {
              forecastMap,
              inventory,
            })
          : {
              needed: s.ingredients.map((i) => ({
                ...i,
      ...
      s.allocated = {
        forecast: alloc.usedForecast,
        inventory: alloc.usedInventory,
        shopping: alloc.needed,
      };
      groceryList.push(
        ...alloc.needed.map((n) => ({
          ...n,
        ...
        sessions: [
          {
            ...
            dayIndex: days.findIndex((x) =>
              dayjs(x.date).isSame(dayjs(batchPlan.sessionDate), "day")
            ),
            recipeIds: batchPlan.recipes.map((r) => r.id),
            startMinute: batchPlan.startMinute,
            estimatedTotalMinutes: Math.min(
              240,
              (batchPlan.recipes.length || 1) * 40
            ),
  ...
  const metrics = {
    totalPortions: days.reduce(
      (sum, d) =>
        sum +
        SLOT_KEYS.reduce((acc, k) => acc + (d.slots[k]?.portions || 0), 0),
      0
    ),
    days: days.length,
  };
    ...
    try {
      ...
      for (const d of days) {
        for (const sk of SLOT_KEYS) {
          ...
          s.allocated.inventory.forEach((i) =>
            toReserve.push({ name: i.name, qty: i.qty, unit: i.unit })
          );
        }
      }
      inventoryDelta = { missing: groceryMerged, toReserve, notes: [] };
    } catch (e) {
      inventoryDelta.notes.push(
        `InventoryService error: ${String(e.message || e)}`
      );
    }
  }
...
function buildEventsAndTimers(days, prefs) {
  ...
  const timers = [];
  const makeId = (...parts) =>
    parts.map((p) => String(p).replace(/\s+/g, "_")).join("_");
  ...
  for (let i = 0; i < days.length; i++) {
    ...
    for (const slot of SLOT_KEYS) {
      ...
      const prepStart = s.noCook
        ? null
        : serveStart.subtract(prepDuration + cookDuration, "minute");
      const cookStart = s.noCook
        ? null
        : serveStart.subtract(cookDuration, "minute");
          ...
          timers.push(
            {
              ...
              voiceCue: prefs.timers?.voiceAlerts
                ? `Start prep for ${s.recipe.title}`
                : undefined,
      ...
      if (prefs.timers?.generate) {
        timers.push({
          ...
          durationMinutes: 45,
          voiceCue: prefs.timers?.voiceAlerts
            ? `Meal time: ${capitalize(slot)} — ${s.recipe.title}`
            : undefined,
...
export default generateMealTimeline;
export {
  generateMealTimeline,
  normalizeRecipes,
  buildFallbackCatalog,
