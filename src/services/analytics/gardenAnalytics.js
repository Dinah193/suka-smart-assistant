...
try {
  ({ useGardenStore: GardenStore } = await import("@/store/GardenStore"));
} catch {}
try {
  ({ useSeedStore: SeedStore } = await import("@/store/SeedStore"));
} catch {}
try {
...
try {
  ({ useWeatherStore: WeatherStore } = await import("@/store/WeatherStore"));
} catch {}
try {
  ({ useCalendarStore: CalendarStore } = await import("@/store/CalendarStore"));
} catch {}
try {
  ({ useTaskStore: TaskStore } = await import("@/store/TaskStore"));
} catch {}
try {
...
async function getMemberGarden(userId) {
  try {
    return await CoalitionStore?.getMemberGarden?.(userId);
  } catch {}
  try {
    return await GroupStore?.getMemberGarden?.(userId);
  } catch {}
...
export function computeGardenSnapshot({
  ...
  const isInScope = (x) =>
    groupId == null ? !x?.groupId : String(x?.groupId) === String(groupId);
  ...
  const plantedArea = sum(
    beds
      .filter((b) => plantedBeds.has(b.id))
      .map((b) => Number(b.areaSqFt || 0))
  );
  const bedUsagePct = totalArea ? plantedArea / totalArea : 0;

  const plannedCal = (calendar.plan || []).filter(
    (e) => e.type === "plant" && isInScope(e) && e.ts >= since30
  );
  const actualThisMonth = plantings.filter((p) => p.tsPlant >= since30);
  const onPlanHits = plannedCal.filter((e) =>
    actualThisMonth.some(
      (p) => p.crop === e.crop && roughlySameDay(p.tsPlant, e.ts)
    )
  ).length;
  ...
  const pestPressure = Math.min(
    100,
    Math.round(avg(pest30.map((p) => Number(p.severity || 1) * 20)) || 0)
  );
  ...
  return {
    ...
    beds: {
      ...
      plantedArea: round2(plantedArea),
      usagePct: round2(bedUsagePct),
    },
    plan: {
      adherence: round2(planAdherence),
      planned: plannedCal.length,
      plantedThisMonth: actualThisMonth.length,
    },
...
export async function computeCoalitionSnapshot({
  ...
  for (const m of members) {
    ...
    if (!mg) {
      memberSnaps.push({ userId: m.userId, error: "unavailable" });
      continue;
    }
...
function aggregateCoalition(memberSnaps, coalition, horizonDays) {
  ...
  for (const m of memberSnaps) {
    ...
    Object.entries(s.forecast).forEach(([crop, v]) => {
      ...
      pooledForecast[crop].harvests = pooledForecast[crop].harvests.concat(
        v.harvests || []
      );
    });
    ...
    perMember.push({
      ...
      contribution: {
        forecastLbs: sum(
          Object.values(s.forecast || {}).map((x) => Number(x.lbs || 0))
        ),
      },
    });
  }
  ...
  // Normalize rounding
  Object.keys(pooledForecast).forEach((c) => {
    pooledForecast[c].lbs = round2(pooledForecast[c].lbs);
  });
  ...
  return {
    pooled: {
      beds: {
        totalArea: round2(pooledBeds.totalArea),
        plantedArea: round2(pooledBeds.plantedArea),
        usagePct: pooledBeds.totalArea
          ? round2(pooledBeds.plantedArea / pooledBeds.totalArea)
          : 0,
      },
...
function fairnessAnalysis({ members, coalition }) {
  ...
  const perMember = members.map((m, i) => ({
    ...
    deltaToTarget:
      perMemberTarget != null ? round2(values[i] - perMemberTarget) : null,
  }));

  return { basis, total, mean: round2(mean), imbalanceIdx, members: perMember };
}

function coalitionCoordination({
  pooledForecast,
  pooledDemand,
  pooledSD,
  memberSnaps,
  ...
  const deficits = Object.entries(pooledSD || {})
    .filter(([_, v]) => (v.delta || 0) < 0)
    .map(([crop, v]) => ({ crop, need: Math.abs(v.delta) }));
  const suggestions = [];
  ...
  for (const d of deficits) {
    ...
    if (remaining > 0) {
      suggestions.push({
        ...
        amount: remaining,
        unit: "lb",
        note: "Purchase or expand plantings",
      });
    }
  }
  ...
  Object.entries(pooledForecast || {}).forEach(([crop, v]) => {
    ...
    const peaks = Object.entries(byWeek).filter(
      ([_, lbs]) => lbs > avg(Object.values(byWeek)) * 1.5
    );
    if (peaks.length >= 1) {
      schedules.push({
        crop,
        peaks: peaks.map(([w, lbs]) => ({ week: w, lbs: round2(lbs) })),
        suggestion: "Stagger sow/harvest to smooth peaks",
      });
    }
  });
...
function buildCoalitionAlerts(agg) {
  const alerts = [];
  const deficits = Object.values(agg.pooled.surplusDeficit || {}).filter(
    (v) => v.delta < 0
  ).length;
  if (deficits) {
    alerts.push({
      ...
      actions: [
        { label: "Open Coordination", topic: "garden.coalition.balance.open" },
      ],
    });
  }
      ...
      actions: [
        {
          ...
          topic: "garden.coalition.tasks.balance.open",
        },
      ],
    });
  }
  if ((agg.coord?.schedules || []).length) {
    alerts.push({
      ...
      actions: [
        { label: "View Stagger Plan", topic: "garden.coalition.stagger.open" },
      ],
    });
  }
  return alerts;
...
function forecastYield({ plantings, horizonDays }) {
  ...
  Object.keys(byCrop).forEach((c) => {
    byCrop[c].lbs = round2(byCrop[c].lbs);
  });
  return byCrop;
}
...
function demandProfile({ prefs, groups, groupId }) {
  ...
  if (groupId != null) {
    const g = (groups.groups || []).find(
      (x) => String(x.id) === String(groupId)
    );
    if (g?.cropDemand)
      Object.entries(g.cropDemand).forEach(([crop, lbs]) => {
        const k = (crop || "").toLowerCase();
        demand[k] = Number(lbs || 0) + (demand[k] || 0);
      });
  }
  return demand;
}
...
function analyzeSuccession({ plantings }) {
  ...
  const byCrop = {};
  (plantings || []).forEach((p) => {
    (byCrop[(p.crop || "").toLowerCase()] ||= []).push(p.tsPlant);
  });
  const gaps = {};
  Object.entries(byCrop).forEach(([crop, times]) => {
    ...
    for (let i = 1; i < s.length; i++) {
      ...
      if (gapDays > targetGap)
        longGaps.push({ from: s[i - 1], to: s[i], days: gapDays });
    }
...
function analyzeRotation({ plantings, beds }) {
  ...
  (plantings || []).forEach((p) => {
    ...
    (perBed[b] ||= []).push({
      year: p.rotationYear ?? new Date(p.tsPlant).getFullYear(),
      family: fam(p.crop),
    });
  });
  const issues = [];
  Object.entries(perBed).forEach(([bedId, rows]) => {
    ...
    for (let i = 1; i < sorted.length; i++) {
      if (sorted[i].family === sorted[i - 1].family) {
        const bedName =
          beds.find((b) => String(b.id) === bedId)?.name || `Bed ${bedId}`;
        issues.push({
          bedId,
          bedName,
          yearA: sorted[i - 1].year,
          yearB: sorted[i].year,
          family: sorted[i].family,
        });
      }
    }
  });
...
function estimateWaterNeed({ weather, irrigations, beds }) {
  const last7 = (weather.history || []).filter(
    (w) => w.ts >= now() - 7 * dayMs
  );
  ...
  const irrigL = sum(
    (irrigations || [])
      .filter((i) => i.ts >= now() - 7 * dayMs)
      .map((i) => Number(i.volumeL || 0))
  );
  const irrigInches =
    irrigL /
    1000 /
    Math.max(1, sum(beds.map((b) => Number(b.areaSqFt || 0))) / 43560);
...
function analyzeSeeds({ packets, upcoming }) {
  ...
  const avgViability =
    avg((packets || []).map((p) => Number(p.viabilityPct || 0))) || 0;
  const need = {};
  (upcoming || []).forEach((e) => {
  ...
  (packets || []).forEach((p) => {
    ...
    have[k] =
      (have[k] || 0) + Number(p.qtySeeds || 0) * Number(p.viabilityPct || 1);
  });
  const shortages = Object.keys(need)
    .filter((k) => (have[k] || 0) < need[k])
    .map((k) => ({
      crop: k,
      shortBySeeds: Math.max(0, Math.round(need[k] - (have[k] || 0))),
    }));
...
function estimateCostPerLb({ inventory, forecast }) {
  const items = (inventory.items || []).filter((i) =>
    /compost|fertilizer|amend/i.test(i.category || i.name || "")
  );
  const cost = sum(
    items.map((i) => Number(i.unitCost || 0) * Number(i.qty || 1))
  );
...
function analyzeWorkload({ tasks, horizonDays }) {
  ...
  const upcoming = back.filter(
    (t) => t.dueTs && t.dueTs <= now() + horizonDays * dayMs
  );
...
function buildAlerts({
  ...
  if (bedUsagePct < 0.6) {
    alerts.push({
      level: "info",
      code: "BED_UNDERUSED",
      message: "Beds underused. Want a quick succession plan?",
      actions: [
        {
          label: "Suggest Successions",
          topic: "garden.succession.suggest.open",
        },
      ],
    });
  }
  if (planAdherence < 0.7) {
    alerts.push({
      level: "warning",
      code: "PLAN_OFF_TRACK",
      message: "Planting plan is off track this month.",
  ...
  if (pestPressure >= 60) {
    alerts.push({
      level: "warning",
      code: "PEST_PRESSURE",
      message: "Pest pressure is high. Queue IPM tasks?",
  ...
  if (waterNeed.risk >= 60) {
    alerts.push({
      level: "warning",
      code: "WATER_DEFICIT",
      message: "Irrigation deficit likely. Schedule watering?",
      actions: [
        {
          label: "Schedule Watering",
          topic: "garden.irrigation.schedule.open",
        },
      ],
    });
  }
  if ((seedStatus.shortages || []).length) {
    alerts.push({
      level: "warning",
      code: "SEED_SHORTAGE",
      message: "Seed shortages detected for upcoming sowings.",
      actions: [{ label: "Build Seed Order", topic: "seeds.order.plan.open" }],
    });
  }
  const deficits = Object.entries(surplusDeficit).filter(
    ([_, v]) => (v.delta || 0) < 0
  );
  if (deficits.length) {
    alerts.push({
      level: "info",
      code: "CROP_DEFICIT",
      message: `${deficits.length} crop deficit(s) within 4 weeks.`,
      actions: [
        { label: "Swap/Scale Plan", topic: "garden.plan.balance.open" },
      ],
    });
  }
  if ((rotation.issues || []).length) {
    alerts.push({
      level: "info",
      code: "ROTATION_RISK",
      message: "Rotation risks flagged on some beds.",
  ...
  if (Object.keys(succession.gaps || {}).length) {
    alerts.push({
      level: "info",
      code: "SUCCESSION_GAPS",
      message: "Gaps in successions detected.",
      actions: [
        { label: "Fill Gaps", topic: "garden.succession.suggest.open" },
      ],
    });
  }
  return alerts;
...
export function toDashboardCards(snapshot) {
  ...
  if (snapshot.scope === "coalition") {
    const deficits = Object.values(
      snapshot.pooled?.surplusDeficit || {}
    ).filter((v) => v.delta < 0).length;
    ...
    return [
      ...
      },
      {
        ...
        intent:
          (snapshot.pooled?.beds?.usagePct || 0) >= 0.8
            ? "success"
            : (snapshot.pooled?.beds?.usagePct || 0) >= 0.6
            ? "info"
            : "warning",
      },
    ];
  ...
  const deficits = Object.values(snapshot.surplusDeficit || {}).filter(
    (v) => v.delta < 0
  ).length;
  ...
  return [
    {
      ...
      intent:
        snapshot.beds.usagePct >= 0.8
          ? "success"
          : snapshot.beds.usagePct >= 0.6
          ? "info"
          : "warning",
    },
    {
      ...
      intent:
        snapshot.plan.adherence >= 0.85
          ? "success"
          : snapshot.plan.adherence >= 0.7
          ? "info"
          : "warning",
    },
    {
...
class GardenAnalytics extends EventEmitter {
  ...
  snapshotPersonal() {
    return this._snaps.personal || this.recompute({});
  }
  ...
  recompute({ groupId = null } = {}) {
    ...
    automation?.emitEvent?.("garden.analytics.updated", {
      key,
      snapshot: snap,
    });
  ...
  async recomputeCoalition(coalitionId) {
    ...
    this.emit("updated", { key, snapshot: snap });
    automation?.emitEvent?.("garden.coalition.analytics.updated", {
      key,
      snapshot: snap,
    });
  ...
  _maybeNBA(snap) {
    ...
    const deficits = Object.values(snap.surplusDeficit || {}).filter(
      (v) => v.delta < 0
    ).length;
  ...
  _maybeNBACoalition(s) {
    if (!automation?.emitEvent) return;
    const deficits = Object.values(s.pooled?.surplusDeficit || {}).filter(
      (v) => v.delta < 0
    ).length;
        ...
        actions: [
          {
            ...
            topic: "garden.coalition.balance.open",
            payload: { coalitionId: s.coalitionId },
          },
          {
            label: "Trade Board",
            topic: "garden.coalition.tradeboard.open",
            payload: { coalitionId: s.coalitionId },
          },
        ],
        ts: now(),
    ...
    if ((s.fairness?.imbalanceIdx || 0) > 0.35) {
      automation.emitEvent("nba", {
        ...
        message:
          "Contribution imbalance detected across members. Reassign crops or tasks?",
        actions: [
          {
            label: "Reassign Plan",
            topic: "garden.coalition.tasks.balance.open",
  ...
  hookEvents() {
    ...
    const groupTopics = ["group.membership.updated", "group.demand.updated"];
    ...
    groupTopics.forEach((t) =>
      automation?.onTopic?.(t, (evt) => {
        const gid = evt?.payload?.groupId ?? null;
        if (gid != null)
          try {
            this.recompute({ groupId: gid });
          } catch {}
      })
    );
    ...
    if (eventBus?.on) {
      [...gardenTopics, ...groupTopics, ...coalitionTopics].forEach((t) =>
  ...
  automation.register([
    ...
    },
    {
      ...
      async run({ emit }) {
        ...
        for (const c of coalitions) {
          ...
          emit?.("garden.coalition.analytics.daily", {
            coalitionId: c.id,
            snapshot: snap,
          });
        }
    ...
    },
    {
      ...
      async run({ emit }) {
        ...
        for (const c of coalitions) {
          const snap =
            gardenAnalytics.snapshotCoalition(c.id) ||
            (await gardenAnalytics.recomputeCoalition(c.id));
...
export async function getCoalitionSnapshot(coalitionId) {
  return (
    ...
    (await gardenAnalytics.recomputeCoalition(coalitionId))
  );
}
export function getDashboardCards({ groupId = null, coalitionId = null } = {}) {
  if (coalitionId != null)
    return toDashboardCards(gardenAnalytics.snapshotCoalition(coalitionId));
  const snap = groupId == null ? getSnapshot() : getGroupSnapshot(groupId);
  return toDashboardCards(snap);
}
export function exportGardenAnalytics({
  groupId = null,
  coalitionId = null,
  format = "json",
} = {}) {
  ...
  if (format === "csv") {
    if (snap.scope === "coalition") {
      const deficits = Object.entries(snap.pooled?.surplusDeficit || {})
        .map(([c, v]) => `${c}:${v.delta}`)
        .join("|");
      const row = [
        ["ts", snap.ts],
      ...
      const deficits = Object.entries(snap.surplusDeficit || {})
        .map(([c, v]) => `${c}:${v.delta}`)
        .join("|");
      const row = [
        ["ts", snap.ts],
