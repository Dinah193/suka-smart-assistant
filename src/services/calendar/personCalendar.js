  ...
  eventBus.on(Events?.SESSION_APPROVED || "session/approved", async ({ data }) => {
    ...
    for (const need of humanNeeds) {
      ...
      const { slots } = await safeTry(() =>
        suggestRoleSlots({ role, startISO: start, endISO: end, durationMin: Math.max(1, Math.round((toMs(end)-toMs(start))/60000)), domain })
      ) || { slots: [] };
      const pick = slots?.[0];
      if (pick) {
        await safeTry(() =>
...
export async function registerPerson(input) {
  ...
  await exportToHubIfEnabled({ type: "person/registered", ts: now, source: "personCalendar", data: { person: p } });
  return p;
}
...
export async function updatePerson(input) {
  ...
  await exportToHubIfEnabled({ type: "person/updated", ts: now, source: "personCalendar", data: { person: merged } });
  return merged;
}
...
export async function removePerson(id) {
  ...
  const now = Date.now();
  const hasFuture = Array.from(_bookings.values()).some(b => b.personId === p.id && toMs(b.end) >= now);
  ...
  await exportToHubIfEnabled({ type: "person/removed", ts: isoNow(), source: "personCalendar", data: { id: p.id } });
  return true;
}

export function listPeople({ activeOnly = true, role, tagsInclude = [], tagsExclude = [] } = {}) {
  ...
  if (tagsInclude?.length) arr = arr.filter(p => hasAllTags(p.tags, tagsInclude));
  if (tagsExclude?.length) arr = arr.filter(p => !hasAnyTag(p.tags, tagsExclude));
  return arr.sort((a,b)=>a.name.localeCompare(b.name));
}

/** --------------------------- Weekly Availability -------------------------- */
export async function setWeeklyAvailability(personId, rules /* Array<{day,startHM,endHM}> */) {
  guardInit();
  if (!_people.has(String(personId))) throw new Error("weekly: person not found");
  ...
  for (const r of rules || []) {
    const rule = sanitizeRule({ id: genId(), personId: String(personId), day: Number(r.day), startHM: String(r.startHM), endHM: String(r.endHM) });
    _weekly.set(rule.id, rule);
  }
  await persistWeekly();

  emit("person/weekly.set", { personId: String(personId), count: (rules||[]).length });
  await exportToHubIfEnabled({ type: "person/weekly.set", ts: isoNow(), source: "personCalendar", data: { personId: String(personId) } });
  return true;
}

export function getWeeklyAvailability(personId) {
  return Array.from(_weekly.values()).filter(r => r.personId === String(personId)).sort((a,b)=>a.day-b.day);
}

/** --------------------------- Exceptions (busy/available) ------------------ */
export async function addException({ personId, type = "busy", start, end, reason }) {
  guardInit();
  if (!_people.has(String(personId))) throw new Error("exception: person not found");
  if (!isISO(start) || !isISO(end)) throw new Error("exception: invalid start/end");
  if (toMs(end) <= toMs(start)) throw new Error("exception: end must be after start");
  ...
  await exportToHubIfEnabled({ type: "person/exception.added", ts: isoNow(), source: "personCalendar", data: { exception: ex } });
  return ex;
}
...
export async function removeException(id) {
  ...
  await exportToHubIfEnabled({ type: "person/exception.removed", ts: isoNow(), source: "personCalendar", data: { id: ex.id } });
  return true;
}
...
export async function upsertBooking(input) {
  ...
  if (!person || person.active === false) throw new Error("booking: person not active/exists");
  ...
  emit("person/booking.saved", { booking: b, person, reason: input?.id ? "updated" : "created" });
  await exportToHubIfEnabled({ type: "person/booking.saved", ts: now, source: "personCalendar", data: { booking: b } });
  return b;
}
...
export async function releaseBooking(id) {
  ...
  await exportToHubIfEnabled({ type: "person/booking.removed", ts: isoNow(), source: "personCalendar", data: { id: b.id, booking: b } });
  return true;
}
...
export function getBookingsForPerson(personId, { fromISO, toISO } = {}) {
  ...
  return Array.from(_bookings.values())
    .filter(b => b.personId === String(personId) && rangesOverlap(s, e, toMs(b.start), toMs(b.end)))
...
export async function suggestRoleSlots(params) {
  ...
  const candidates = listPeople({ activeOnly: true, role, tagsInclude, tagsExclude }).map(p => p.id);
  ...
  for (const pid of candidates) {
    ...
    for (const [a, b] of windows) {
      ...
      while (x + durMs <= b) {
        const proposed = { personId: pid, start: new Date(x).toISOString(), end: new Date(x + durMs).toISOString(), role };
        // Re-check guardrails if necessary
        const person = _people.get(pid);
        if (appliesQuietSabbath(person, role)) {
          try { enforceQuietHours(proposed); } catch { x += step; continue; }
          try { enforceSabbathGuard(proposed); } catch { x += step; continue; }
        }
        // Re-check capacity
...
function emit(type, data, opts = {}) {
  ...
  eventBus.emit(type, data, { source: opts.source || "personCalendar", sticky: !!opts.sticky });
}
...
function normalizePerson(input, nowIso) {
  ...
  const p = {
    ...
    skills: Array.isArray(input?.skills) ? dedupStrings(input.skills) : undefined,
    tags: Array.isArray(input?.tags) ? dedupStrings(input.tags) : undefined,
    capacity: Number.isFinite(input?.capacity) && input.capacity > 0 ? Math.floor(input.capacity) : 1,
    preferences: isPojo(input?.preferences) ? { ...input.preferences } : undefined,
...
function sanitizeRule(r) {
  ...
  return { id: String(r.id || genId()), personId: String(r.personId), day, startHM, endHM };
}

function readHumanNeeds(data) {
  // Accept data.session?.rolesNeeded[], data?.rolesNeeded[], draft.meta.rolesNeeded[]
  const arr = data?.session?.rolesNeeded || data?.rolesNeeded || data?.draft?.meta?.rolesNeeded || [];
...
function appliesQuietSabbath(person, role) {
  ...
  const roleNoisy = /butcher|tiller|mower|grinder|chainsaw|hammer|smoker|vacuum|compressor/.test(roleStr);
...
function enforceSabbathGuard(block) {
  ...
  if (rangesOverlap(toMs(block.start), toMs(block.end), sabStart, sabEnd)) {
    ...
    if (!allowed.includes(block.domain)) throw new Error("booking violates sabbath guard");
  }
}
...
function hasPersonConflict(person, newBlock) {
  ...
  for (const [t, d] of events) {
    active += d;
    if (active > cap) {
      if (t >= s && t <= e) return { at: new Date(t).toISOString(), capacity: cap };
    }
  }
...
function computePersonFreeWindows(personId, s, e, domain) {
  ...
  for (const ex of _exceptions.values()) {
    ...
    if (ex.personId === personId && ex.type === "busy") busy.push([toMs(ex.start), toMs(ex.end)]);
  }
  ...
  const extraAvail = Array.from(_exceptions.values())
    .filter(ex => ex.personId === personId && ex.type === "available" && rangesOverlap(s,e,toMs(ex.start),toMs(ex.end)))
  ...
  if (appliesQuietSabbath(person, /*role*/null)) {
    return final.filter(([a,b]) => {
      const tmp = { start: new Date(a).toISOString(), end: new Date(b).toISOString(), domain };
      try { enforceQuietHours(tmp); } catch { return false; }
      try { enforceSabbathGuard(tmp); } catch { return false; }
      return true;
    });
  }
  return final;
...
function weeklyRulesToIntervals(personId, s, e) {
  ...
  for (let dayMs = dayStart(s); dayMs <= dayStart(e); dayMs += 24*60*60*1000) {
...
function rangeCoveredByWeeklyAndExceptions(personId, s, e) {
  const allowed = [...weeklyRulesToIntervals(personId, s, e),
...
async function hydrateAll() {
  ...
  for (const ex of exceptions) _exceptions.set(ex.id, {
...
async function persistPeople() {
  if (db?.people?.bulkPut) return db.people.bulkPut(Array.from(_people.values()));
  return saveAllLocal(LSK.people, Array.from(_people.values()));
}
async function persistWeekly() {
  if (db?.peopleWeekly?.bulkPut) return db.peopleWeekly.bulkPut(Array.from(_weekly.values()));
  return saveAllLocal(LSK.weekly, Array.from(_weekly.values()));
}
async function persistExceptions() {
  if (db?.peopleExceptions?.bulkPut) return db.peopleExceptions.bulkPut(Array.from(_exceptions.values()));
  return saveAllLocal(LSK.exceptions, Array.from(_exceptions.values()));
}
async function persistBookings() {
  if (db?.peopleBookings?.bulkPut) return db.peopleBookings.bulkPut(Array.from(_bookings.values()));
...
function ceilTo(ms, granMin) { const step = (granMin || 15) * 60_000; return Math.ceil(ms / step) * step; }
...
function approximateSabbathWindow(ts) {
  ...
  const d = new Date(ts); const day = d.getDay();
...
function normalizeIntervals(list) {
  ...
  for (let i=1;i<arr.length;i++) {
    const [a,b] = arr[i]; const last = out[out.length-1];
