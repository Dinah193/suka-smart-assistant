...
try {
  ...
  ({ usdaDefaults, setCuisineBias: setCuisineBiasFn } = await import("@/services/nutrition/usdaDefaults"));
} catch {}
...
try { ({ useHouseholdProfile: HouseholdProfile } = await import("@/store/HouseholdProfileStore")); } catch {}
...
const PASSOVER_TAGS = new Set([
  "chametz","leaven","leavening-agent","bread","pasta","beer","waffle","pancake","barley","wheat","rye","oats","spelt"
]);

const ALLERGENS = [
  "peanut","tree nut","almond","walnut","cashew","pecan","pistachio","hazelnut",
  "egg","milk","dairy","soy","wheat","gluten","fish","shellfish","sesame"
];
  ...
  try {
    ...
    for (const f of favs) {
      ...
      const aliases = Array.from(new Set([f.name, ...(f.aliases || [])]))
        .map(a => toKey(a)).filter(Boolean);
      for (const a of aliases) FAVORITE_ALIASES[a] = canonical;
    }
    logger.info?.("[ingredientResolver] favorite aliases rebuilt", Object.keys(FAVORITE_ALIASES).length);
  } catch (e) {
...
---------------------------------------------------------------------------- */
const LRU = new Map();
function lruGet(key){ if (!LRU.has(key)) return; const v=LRU.get(key); LRU.delete(key); LRU.set(key,v); return v; }
function lruSet(key,val){ LRU.set(key,val); if (LRU.size>64){ const k = LRU.keys().next().value; LRU.delete(k); }}

/* ----------------------------------------------------------------------------
   Helpers: Math & Formatting
---------------------------------------------------------------------------- */
function round(x, n = 0) { const p = 10 ** n; return Math.round((Number(x)||0)*p)/p; }
...
function toNumberOrFraction(s) {
  ...
  if (/\d+\s+\d+\/\d+/.test(str)) { const [a,b]=str.split(/\s+/); const [n,d]=b.split("/").map(Number); return Number(a)+(d?n/d:0); }
  if (/^\d+\/\d+$/.test(str)) { const [n,d]=str.split("/").map(Number); return d?n/d:0; }
  if (/^\d+\s*-\s*\d+$/.test(str)) { const [a,b]=str.split("-").map(Number); return (a+b)/2; }
  const n = Number(str); return Number.isFinite(n) ? n : 0;
}
...
function parseLine(line = "") {
  ...
  const multMatch = /(?<mult>\d+)\s*[xX]\s*(?<pack>\d+(?:\.\d+)?)\s*[- ]?\s*(?<packUnit>oz|g|ml)\b/.exec(withoutParen);
  let packQty = 0, packUnit = "", packSizeGrams = 0;
  if (multMatch?.groups) {
  ...
  const rx = /^(?<qty>\d+(?:\s+\d+\/\d+)?|\d*\/\d+)?\s*(?<unit>[a-zA-Z\.]+)?\s*(?<name>.+)$/;
  const m = rx.exec(withoutParen);
  let qty = 0, unit = "", name = withoutParen;
  if (m?.groups) { qty = toNumberOrFraction(m.groups.qty); unit = normUnit(m.groups.unit); name = (m.groups.name || "").trim(); }
  if (!qty && /^\d+$/.test(sizeText) && /egg|eggs/i.test(raw)) { qty = Number(sizeText); unit = "piece"; }
  ...
  else if (sizeText) { const g = sizeToGrams(sizeText); if (g) computedSizeGrams = g; }
  return { raw, qty, unit, name, sizeText, tags: [], computedSizeGrams };
}

function toSizeInGrams(amount, unit) {
  switch (unit) { case "g": return amount; case "ml": return amount; case "oz": return amount*28.3495; default: return 0; }
}
function sizeToGrams(txt = "") {
  const oz = /(\d+(?:\.\d+)?)\s*oz/i.exec(txt);
  const g  = /(\d+(?:\.\d+)?)\s*g\b/i.exec(txt);
  const ml = /(\d+(?:\.\d+)?)\s*ml\b/i.exec(txt);
  if (g) return Number(g[1]); if (ml) return Number(ml[1]); if (oz) return Number(oz[1]) * 28.3495; return 0;
}
...
function resolveCanonical(name = "") {
  ...
  if (low.includes("crushed tomato")) return usdaDefaults?.resolve?.("tomato, canned crushed") || { key: "tomato, canned crushed" };
  if (low.includes("onion"))          return usdaDefaults?.resolve?.("onion, chopped")           || { key: "onion, chopped" };
  if (low.includes("jollof"))         return usdaDefaults?.resolve?.("jollof rice")               || { key: "jollof rice" };
  if (low.includes("olive oil"))      return usdaDefaults?.resolve?.("olive oil")                 || { key: "olive oil" };
  if (low.includes("palm oil"))       return usdaDefaults?.resolve?.("palm oil")                  || { key: "palm oil" };
  if (low.includes("yogurt"))         return usdaDefaults?.resolve?.("yogurt, plain (whole milk)")|| { key: "yogurt, plain (whole milk)" };
  ...
  if (count === LEARN_THRESHOLD) {
    try { automation?.emit?.("favorites.learn", { alias: name }); } catch {}
  }
...
function guessDensity(name = "") {
  ...
  for (const k of Object.keys(DENSITY_HINTS)) if (n.includes(k.toLowerCase())) return DENSITY_HINTS[k];
...
function estimateGrams(parsed, canonical) {
  ...
  if (parsed.computedSizeGrams > 0) {
    const unitIsPack = parsed.unit === "can" || parsed.unit === "piece" || !parsed.unit;
    if (unitIsPack) return round((parsed.qty || 1) * parsed.computedSizeGrams, 0);
  }
  try {
  ...
  if (canonical?.servingGrams) return round((parsed.qty || 1) * canonical.servingGrams, 0);
  return 0;
}
...
function emptyMacros() { return { Calories:0,Protein:0,Carbs:0,Fat:0,Fiber:0,Sugar:0,Sodium:0,SatFat:0 }; }
...
function computeFlags(parsed, canonical, passoverMode) {
  ...
  const tags = new Set([...(canonical?.tags || []), ...(parsed.tags || [])].map(t => String(t).toLowerCase()));
  const passoverRestricted = !!passoverMode && (
    [...PASSOVER_TAGS].some(t => tags.has(t)) ||
    /(bread|pasta|barley|wheat|rye|beer|oats|spelt)/.test(name)
  );
  let additivesFlag = 0;
  try { additivesFlag = nutritionMath?.additiveRiskScore?.({ name, tags: [...tags] }) ?? 0; } catch { additivesFlag = 0; }
...
function isPassover(ing) {
  ...
  return [...PASSOVER_TAGS].some(t => tg.has(t)) || /(bread|pasta|barley|wheat|rye|beer|oats|spelt)/.test(nm);
}
...
function summarizeInvItem(i) {
  const lots = Array.isArray(i.lots) ? i.lots.map(l => ({
    qty: l.qty || 0, location: l.location || i.location || "pantry", expires: l.expires || null
...
function suggestSubs(invMatch, inventory, passoverMode, dietaryPrefs, targetMacrosPer100g) {
  ...
  for (const it of inventory.items) {
    ...
    const total = Array.isArray(it.lots) ? it.lots.reduce((a,b)=>a+(b.qty||0),0) : (it.qty || 0);
    if (total <= 0) continue;
    ...
    candidates.push({ slug: sl, name: it.name, qty: total, location: it.location || "pantry", macroDistance });
  }
  return candidates.sort((a,b)=>a.macroDistance - b.macroDistance).slice(0,6);
}

function passesDiet(name, dietaryPrefs = {}) {
  const n = (name||"").toLowerCase();
  if (dietaryPrefs?.porkFree && /(pork|bacon|ham)/i.test(n)) return false;
  if (dietaryPrefs?.dairyFree && /(milk|cheese|yogurt|butter|cream)/i.test(n)) return false;
  if (dietaryPrefs?.pescatarian && /(beef|lamb|goat|pork|chicken|turkey)/i.test(n)) return false;
...
function assignStoreAisle(name, vendors) {
  ...
  return { storeId: chosen.id, storeName: chosen.name, aisle, estimated: price };
}
...
function buildVendorCatalog(vendors = []) {
  ...
  if (!out.length) out.push({ id: DEFAULT_STORE, name: DEFAULT_STORE, priority: 9, aisles: [{ name: DEFAULT_AISLE, matchers: [] }] });
...
function estimateItemPrice(routing, grams, qty=1) {
  ...
  if (unit === "piece" || unit === "can") return round((est.pricePerUnit || 0) * (qty || 1), 2);
  if (unit === "kg") return round((est.pricePerUnit || 0) * (grams || 0) / 1000, 2);
  return null;
}
...
async function emitReserveSignal(payload) {
  try {
    if (automation?.emit) await automation.emit("inventory.reserve.requested", payload);
    else if (eventBus?.emit) eventBus.emit("inventory.reserve.requested", payload);
  } catch (e) { logger.warn?.("[ingredientResolver] emitReserveSignal failed", e); }
}
...
function primeCuisineBiasFromPrefs() {
  try {
    ...
    const normalized = (Array.isArray(tags) ? tags : []).map(t => (t?.tag || t)).filter(Boolean);
...
export const ingredientResolver = {
  ...
  async resolve(input, options = {}) {
    ...
    const cacheKey = JSON.stringify({ input, options: { ...options, vendors: undefined } }); // ignore large vendor objs in key
    ...
    const parsed = typeof input === "string" ? parseLine(input) : normalizeInput(input);
    ...
    const macroQuick = nutrit ? scalePer100g(nutrit.per100g, grams) : emptyMacros();
    ...
    const invMatch = inventory ? matchInventory(canonical?.key || parsed.name, inventory) : null;
    const substitutes = inventory
      ? suggestSubs(invMatch, inventory, passoverMode, dietaryPrefs, nutrit?.per100g)
      : [];

    const vendors = options.vendors || (await Vendors?.list?.().catch(() => [])) || getDefaultVendors();
    ...
    const out = {
      ...
      tags: Array.from(new Set([...(nutrit?.tags || []), ...(parsed.tags || [])])),
  ...
  async resolveCoalesced(list, options = {}) {
    ...
    for (const it of resolved) {
      const key = it.slug;
      if (!bySlug.has(key)) bySlug.set(key, { ...it, priceBreakdown: {}, _count: 0 });
      const acc = bySlug.get(key);
      acc.qty = (acc.qty || 0) + (it.qty || 0);
      acc.grams = (acc.grams || 0) + (it.grams || 0);
      acc.passoverRestricted = !!(acc.passoverRestricted || it.passoverRestricted);
      acc.additivesFlag = Math.max(acc.additivesFlag || 0, it.additivesFlag || 0);
      acc.allergens = Array.from(new Set([...(acc.allergens || []), ...(it.allergens || [])]));
      acc._count += 1;
      ...
      if (est && price != null) {
        ...
        acc.priceBreakdown[store] = round((acc.priceBreakdown[store] || 0) + price, 2);
      }
    }
    ...
    for (const acc of bySlug.values()) {
      ...
      for (const [store, sum] of Object.entries(acc.priceBreakdown)) {
        if (bestPrice == null || sum < bestPrice) { bestPrice = sum; bestStore = store; }
      }
      const final = {
        ...acc,
        vendor: bestStore,
      ...
      if (final.estimatedPrice != null) {
        storeTotals[final.vendor] = round((storeTotals[final.vendor] || 0) + final.estimatedPrice, 2);
      }
    }

    const totals = {
      grams: round(items.reduce((a,b)=>a+(b.grams||0),0), 0),
      price: Object.values(storeTotals).reduce((a,b)=>a+b,0)
    };
    const stores = Object.entries(storeTotals).map(([storeName, total]) => ({ storeName, total }))
  ...
  async resolveList(list, options = {}) {
    ...
    for (const it of (list || [])) {
      try { out.push(await this.resolve(it, options)); }
      catch (e) { logger.warn?.("[ingredientResolver] failed", it, e); }
    }
    return out;
  },
  ...
  addFavoriteAlias(alias, canonicalKey) {
    ...
    try { usdaDefaults?.addAlias?.(alias, canonicalKey); } catch {}
    return true;
  },
  addFavoriteAliases(pairs = []) { for (const p of pairs) this.addFavoriteAlias(p.alias, p.key); return true; },
...
function getDefaultVendors() {
  return [{
    id: DEFAULT_STORE, name: DEFAULT_STORE, priority: 9,
    aisles: [
      { name: "Produce",       match: [/onion|okra|greens|spinach|tomato/i] },
      { name: "Grains/Pasta",  match: [/rice|quinoa|oat|grit|mac(?!hine)|pasta|fufu|garri|eba|millet/i] },
      { name: "Meat/Seafood",  match: [/chicken|beef|lamb|goat|fish|catfish|turkey/i] },
...
try {
  eventBus?.on?.("cuisine.packs.changed", () => {
    ...
    rebuildFavoriteAliases();
    primeCuisineBiasFromPrefs();
    logger.info?.("[ingredientResolver] cuisine packs changed → cache cleared & cuisine bias primed");
  });

  eventBus?.on?.("household.profile.updated", ({ profile } = {}) => {
    try {
      if (setCuisineBiasFn) {
        const tags = (profile?.cuisineBias || []).map(b => b.tag || b).filter(Boolean);
        setCuisineBiasFn(tags.length ? tags : ["african-american"]);
      }
      LRU.clear?.();
      logger.info?.("[ingredientResolver] household.profile.updated → cuisine bias updated & cache cleared");
    } catch (e) {
