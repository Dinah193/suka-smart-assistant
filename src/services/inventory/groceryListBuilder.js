...
try {
  ({ MealPlans } = await import("@/store/MealPlanStore"));
} catch {}
try {
  ({ Recipes } = await import("@/store/RecipeStore"));
} catch {}
try {
...
try {
  ({ Pricing } = await import("@/services/inventory/pricingService"));
} catch {}
try {
...
export const groceryListBuilder = {
  ...
  async build(input, options = {}) {
    const plan =
      typeof input === "string" ? await MealPlans?.getById?.(input) : input;
    if (!plan) throw new Error("groceryListBuilder: missing plan");

    const prefs = PreferencesStore?.getState?.() || {};
    const passoverMode =
      options.passoverMode ?? !!prefs?.calendar?.passoverMode;
    ...
    const inventory =
      (await Inventory?.snapshot?.(
        plan.householdId || options.householdId
      ).catch(() => ({}))) || {};
    ...
    const vendors =
      (await Vendors?.list?.().catch(() => [])) || getDefaultVendors();
    const vendorStrategy = options.vendorStrategy || "fewest-stops";
    const packMap = { ...PACK_DEFAULTS, ...(options.packOverrides || {}) };
    const routed = routeToVendors(needed, vendors, {
      vendorStrategy,
      packMap,
      preferQuick: !!options.preferQuickStores,
    });
    ...
    const preservationQueue = suggestPreservationTasks(
      perishables,
      options.visitWindowISO
    );

    // Substitution suggestions (inventory-based swaps)
    const substitutions = suggestSubsFromInventory(
      needed,
      inventory,
      passoverMode
    );
  ...
  async applyReservations(list) {
    ...
    const ok = await Inventory?.reserveBatch?.(
      list.householdId,
      list.reservations
    ).catch(() => null);
  ...
  toMarkdown(list) {
    return toMarkdown(list);
  },
  formatForPrint(list) {
    return formatForPrint(list);
  },

  /** Diff two lists (simple line-up by slug) */
  diff(prevList, nextList) {
    return diffLists(prevList, nextList);
  },

  /** Merge user edits (e.g., quantity tweaks, reassigned store/aisle). */
  mergeEdits(list, edits) {
    return mergeManualEdits(list, edits);
  },

  /** Quick utility: split a flat array of items by store/aisle. */
  splitByStoreAndAisle(items, vendors) {
    return splitByStoreAndAisle(items, vendors);
  },
};
...
function isChametz(ing) {
  ...
  return (
    tags.some((t) => PASSOVER_TAGS.has(t)) ||
...
function normalizeItem(item) {
  ...
  try {
    grams =
      nutritionMath?.toGrams?.({
        qty: out.qty,
        unit: out.unit,
        name,
        densityGPerCup: out.densityGPerCup,
      }) ?? 0;
  } catch {
    grams = 0;
  }
  ...
  for (const it of items) {
    ...
    if (have >= need) {
      ...
      needed.push({
        ...it,
        normQty: short,
        prettyQty: prettyQty(short, it.normUnit),
        inventoryShort: true,
      });
...
// ---------------------------------------------------------------------------
function routeToVendors(
  items,
  vendors,
  { vendorStrategy, packMap, preferQuick }
) {
  ...
  const assigned = items.map((it) =>
    assignStoreAisle(it, catalog, { preferQuick })
  );
...
function applyPackSize(item, packMap) {
  ...
  if (!pack)
    return { ...item, packSize: null, packs: null, buyQty: item.normQty };

  const unit = item.normUnit;
  // Only apply pack for piece/serving/can-like, else skip (bulk grams usually loose)
  const packApplies =
    unit === "piece" || /can|egg|yogurt|tomato/.test(item.name || "");
  if (!packApplies)
    return { ...item, packSize: null, packs: null, buyQty: item.normQty };
...
function groupByStoreAisle(items, vendorStrategy) {
  ...
  for (const it of items) {
    ...
    ((stores[sid] ||= {
      storeId: sid,
      storeName: it.storeName || sid,
      aisles: {},
    }).aisles[aisle] ||= []).push(it);
  }
  // to arrays
  const out = Object.values(stores).map((s) => ({
    storeId: s.storeId,
    storeName: s.storeName,
    aisles: Object.entries(s.aisles)
  ...
  for (const store of itemsByStore) {
    ...
    for (const aisle of store.aisles) {
      ...
      for (const it of aisle.items) {
        ...
        const estTotal = round(
          (it.buyQty ?? it.normQty) * (q?.pricePerUnit ?? 0) +
            (q?.packFee ?? 0),
          2
        );
...
function buildPerishablesTimeline(plan, itemsByStore) {
  ...
  for (const store of itemsByStore) {
    for (const aisle of store.aisles) {
      for (const it of aisle.items) {
        const isPerishable =
          /(greens|spinach|herb|fish|meat|yogurt|milk|tomato, canned crushed)/i.test(
            it.name || ""
          );
        ...
        const useBy = new Date(
          new Date(firstUse).getTime() + useWindowDays * DAY
        );
        perish.push({
          slug: it.slug,
          name: it.name,
...
function suggestPreservationTasks(perishables, visitWindowISO) {
  ...
  for (const p of perishables) {
    let action = null;
    if (/greens|spinach|herb/i.test(p.name))
      action = "Wash, chop, and freeze in portions";
    else if (/fish/i.test(p.name)) action = "Vacuum seal and freeze portions";
    else if (/yogurt|milk/i.test(p.name))
      action = "Plan breakfast bowls to consume by use-by";
    else if (/tomato, canned crushed/i.test(p.name))
      action = "Batch cook sauce base and portion";
    if (!action) continue;
...
function suggestSubsFromInventory(needed, inventory, passoverMode) {
  ...
  for (const it of needed) {
    ...
    const candidates = [...bySlug.values()].filter((inv) =>
      isCloseMatch(inv.slug, it.slug)
    );
...
function isCloseMatch(invSlug, needSlug) {
  ...
  const a = invSlug.split(" ")[0];
  const b = needSlug.split(" ")[0];
...
function summarize(itemsByStore) {
  let est = 0,
    count = 0,
    stores = itemsByStore.length;
  for (const s of itemsByStore)
...
function diffLists(prev, next) {
  ...
  for (const [slug, it] of nMap) {
    if (!pMap.has(slug)) added.push(it);
      ...
      if (
        (prevIt.normQty || 0) !== (it.normQty || 0) ||
        prevIt.storeId !== it.storeId ||
        prevIt.aisle !== it.aisle
      )
...
function flattenItems(list) {
  ...
  for (const s of list?.itemsByStore || [])
    for (const a of s.aisles) for (const i of a.items) out.push(i);
  return out;
}
    ...
    for (const a of s.aisles) {
      a.items = a.items.map((it) => {
        ...
        return {
          ...
          prettyQty: e.normQty
            ? prettyQty(e.normQty, it.normUnit)
            : it.prettyQty,
          note: e.note ?? it.note,
...
function toMarkdown(list) {
  ...
  lines.push(
    `Estimated total: **$${(list.totals.estimatedTotal || 0).toFixed(2)}**`
  );
  if (list.passoverMode)
  ...
  for (const s of list.itemsByStore) {
    ...
    for (const a of s.aisles) {
      ...
      for (const i of a.items) {
        lines.push(
          `- [ ] ${i.name} — ${i.prettyQty} ${i.prettyUnit}${
            i.estTotal ? ` _(~$${i.estTotal.toFixed(2)})_` : ""
          }`
        );
      }
    }
  ...
  if (list.preservationQueue?.length) {
    lines.push(`\n---\n## Preservation Tasks`);
    list.preservationQueue.forEach((t) =>
      lines.push(`- ${t.action} for **${t.name}** by **${t.dueISO}**`)
    );
  }
  return lines.join("\n");
}
  ...
  return [
    {
      ...
      aisles: [
        {
          name: "Produce",
          match: ["greens", "spinach", "okra", "onion", "pepper"],
        },
    ...
    },
    {
      ...
      aisles: [
        {
          name: "Bulk Pantry",
          match: ["tomato", "rice", "oil", "oats", "beans"],
        },
...
export function splitByStoreAndAisle(items, vendors) {
  const routed = routeToVendors(items, vendors || getDefaultVendors(), {
    vendorStrategy: "fewest-stops",
    packMap: PACK_DEFAULTS,
    preferQuick: false,
  });
  return routed;
}
