...
const storage = {
  load() {
    try {
      const raw = localStorage.getItem(STORAGE_KEY) || localStorage.getItem("suka.family.ops.v2");
...
const getTZ = () => PreferencesStore?.getState?.()?.calendar?.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC";

/* ──────────────────────────────────────────────────────────────
 * Design-system glue
 * ────────────────────────────────────────────────────────────── */
function toast(variant, title, message) {
  events.emit(buildEvent(NAMES["ui.toast.shown"], { variant, title, message }, { source: "familySharing" }));
}
function nba(label, hint, route, params) {
  events.emit(buildEvent(NAMES["ui.nba.suggested"], { label, hint, route, params }, { source: "familySharing" }));
}
function empty(context, actions = []) {
  events.emit(buildEvent(NAMES["ui.empty.presented"], { context, actions }, { source: "familySharing" }));
}
...
function computeProgress(state, goalId) {
  ...
  let total = 0, done = 0;
  for (const t of tasks) { const w = isNum(t.weight) ? t.weight : 1; total += w; if (t.done) done += w; }
  for (const m of mils) { const w = isNum(m.weight) ? m.weight : 1; total += w; if (m.completedAt) done += w; }
...
function inQuietHours(settings) {
  ...
  return startM <= endM ? (curMins >= startM && curMins <= endM) : (curMins >= startM || curMins <= endM);
}

/* ──────────────────────────────────────────────────────────────
 * Family Circles
 * ────────────────────────────────────────────────────────────── */
export function createFamilyCircle(name, householdIds = [], { ownerId = "me", members = [] } = {}) {
  if (!isStr(name) || !Array.isArray(householdIds)) {
    empty("family.circle.empty", [
      { label: "Create Family Circle", eventName: NAMES["ui.modal.open"], payload: { id: "createFamilyCircle" } },
    ]);
    return null;
  }

  const { result: fam, undo } = withState((s) => {
    const fam = { id: uid(), name, householdIds, ownerId, createdAt: nowISO(), members };
  ...
  emitEvent("family.circle.created", { familyId: fam.id, householdIds, members }, {
    source: "familySharing.createFamilyCircle",
    undo: { label: "Undo", handler: () => { undo(); toast("info", "Circle removed", "Undo successful."); } },
    nextBestAction: { label: "Share Something", hint: "Start sharing with family", route: "/family", params: { familyId: fam.id, tab: "share" } },
  });

  if (findAgrarians(fam).length) {
    nba("Enable Ops Forecasts", "We’ll auto-share support plans", "/family", { familyId: fam.id, tab: "settings" });
  }
...
export function deleteFamilyCircle(familyId) {
  ...
  emitEvent("family.circle.deleted", { familyId }, {
    source: "familySharing.deleteFamilyCircle",
    undo: { label: "Undo", handler: () => { undo(); toast("info", "Family Circle restored", "Undo successful."); } },
    nextBestAction: { label: "Create New Circle", hint: "Start fresh", route: "/family#create" },
  });
...
export function updateFamilySettings(familyId, partial = {}) {
  const { undo, result } = withState((s) => {
    s.familySettings[familyId] = {
      ...
      channels: { email: true, push: true, inapp: true, sms: false, ...(s.familySettings[familyId]?.channels || {}), ...(partial.channels || {}) },
      causeIntervals: { ...(s.familySettings[familyId]?.causeIntervals || {}), ...(partial.causeIntervals || {}) },
      minIntervalMins: clamp(Number(partial.minIntervalMins ?? (s.familySettings[familyId]?.minIntervalMins ?? 180)), 15, 1440),
    };
    return s.familySettings[familyId];
  });

  emitEvent("family.settings.updated", { familyId, settings: result }, {
    source: "familySharing.updateFamilySettings",
    undo: { label: "Undo", handler: () => { undo(); toast("info", "Settings reverted", "Undo successful."); } },
  });
...
const ALLOWED_SCOPES = new Set([
  ...
  "calendar","mealplan","labels","soilWater","cooking",
]);

export function shareToFamily(familyId, { scope, refId, title, permissions = "view", meta = {} }) {
  if (!ALLOWED_SCOPES.has(scope)) { toast("warning", "Not shareable", "This category is not enabled."); return null; }
  const s = getState(); const fam = getFamily(s, familyId); if (!fam) return null;

  const { result: sh, undo } = withState((state) => {
    const sh = { id: uid(), familyId, scope, refId, title: title || refId, permissions, meta, createdAt: nowISO() };
    state.shares.unshift(sh);
    return sh;
  });

  emitEvent("family.share.created", { shareId: sh.id, familyId, scope }, {
    source: "familySharing.shareToFamily",
    undo: { label: "Undo", handler: () => { undo(); toast("info", "Share removed", "Undo successful."); } },
    nextBestAction: { label: "Create Goal", hint: "Turn this into a family plan", route: "/family", params: { familyId, tab: "goals" } },
  });

  if (["garden", "animals", "cleaning", "cooking"].includes(scope) && findAgrarians(fam).length) {
...
export function createFamilyGoal(
  familyId,
  { title, description = "", categories = [], ownerId = "me", householdIds = [], progressMode = "auto" } = {}
) {
  if (!isStr(title)) return null;

  const { result: g, undo } = withState((s) => {
    const g = {
      id: uid(), familyId, title, description,
      categories: Array.from(new Set(categories)),
      ownerId, householdIds, createdAt: nowISO(), updatedAt: nowISO(),
      progress: 0, progressMode: progressMode === "manual" ? "manual" : "auto",
  ...
  emitEvent("family.goal.created", { goalId: g.id, familyId }, {
    source: "familySharing.createFamilyGoal",
    undo: { label: "Undo", handler: () => { undo(); toast("info", "Goal removed", "Undo successful."); } },
    nextBestAction: { label: "Add Milestones", hint: "Break it into steps", route: "/family", params: { familyId, goalId: g.id, tab: "milestones" } },
  });

  const fam = getFamily(getState(), familyId);
  const cat = g.categories.map((c) => (c + "").toLowerCase());
  if (fam && findAgrarians(fam).length && cat.some((x) => ["garden","animals","cleaning","cooking","soilwater"].includes(x))) {
    autoShareOpsForecast(familyId, { cause: "goal.created", goalId: g.id, scopes: g.categories });
  }
...
export function updateFamilyGoal(goalId, partial = {}) {
  ...
  emitEvent("family.goal.updated", { goalId: g.id, familyId: g.familyId }, {
    source: "familySharing.updateFamilyGoal",
    undo: { label: "Undo", handler: () => { withState((s)=>{ const gg = s.goals.find(x=>x.id===g.id); if (gg) Object.assign(gg, prev); }); toast("info", "Goal reverted", "Undo successful."); } },
  });
...
export function addMilestone(goalId, { title, due, weight = 1 } = {}) {
  ...
  emitEvent("family.milestone.added", { goalId, milestoneId: payload.m.id, familyId: payload.famId }, {
    source: "familySharing.addMilestone",
    undo: { label: "Undo", handler: () => { undo(); toast("info", "Milestone removed", "Addition undone."); } },
    nextBestAction: { label: "Add Tasks", hint: "What steps make this real?", route: "/family", params: { tab: "goals", goalId } },
  });

  const fam = getFamily(getState(), payload.famId);
  if (fam && findAgrarians(fam).length && (payload.cats || []).some((c) => ["garden","animals","cleaning","cooking","soilWater"].includes((c + "")))) {
...
export function completeMilestone(milestoneId, done = true) {
  const { result: payload, undo } = withState((s) => {
    const m = s.milestones.find((x) => x.id === milestoneId); if (!m) return null;
  ...
  emitEvent("family.milestone.completed", { milestoneId, goalId: payload.m.goalId, familyId: payload.g?.familyId, done }, {
    source: "familySharing.completeMilestone",
    undo: { label: "Undo", handler: () => { undo(); toast("info", "Milestone reverted", "Completion undone."); } },
    nextBestAction: { label: "Plan Session", hint: "Coordinate the next push", route: "/family", params: { tab: "sessions", goalId: payload.m.goalId } },
  });

  const fam = getFamily(getState(), payload.g?.familyId);
  if (fam && findAgrarians(fam).length) autoShareOpsForecast(payload.g.familyId, { cause: "milestone.completed", goalId: payload.g.id });
...
export function addTask(goalId, { title, ownerId = null, due = null, weight = 1, milestoneId = null, notes = "" } = {}) {
  ...
  emitEvent("family.task.added", { taskId: payload.t.id, goalId, familyId: payload.famId }, {
    source: "familySharing.addTask",
    undo: { label: "Undo", handler: () => { undo(); toast("info", "Task removed", "Addition undone."); } },
  });
...
export function toggleTask(taskId, done = true) {
  const { result: payload, undo } = withState((s) => {
    const t = s.tasks.find((x) => x.id === taskId); if (!t) return null;
  ...
  emitEvent("family.task.toggled", { taskId, goalId: payload.t.goalId, familyId: payload.g?.familyId, done }, {
    source: "familySharing.toggleTask",
    undo: { label: "Undo", handler: () => { undo(); toast("info", "Task reverted", "Completion undone."); } },
    nextBestAction: done ? { label: "Review Progress", hint: "See the updated %", route: "/family", params: { tab: "goals", goalId: payload.t.goalId } } : undefined,
  });
...
export function updateTask(taskId, partial = {}) {
  ...
  emitEvent("family.task.updated", { taskId, goalId: payload.t.goalId, familyId: payload.familyId }, {
    source: "familySharing.updateTask",
    undo: { label: "Undo", handler: () => { withState((s)=>{ const tt = s.tasks.find((y)=>y.id===taskId); if (tt) Object.assign(tt, payload.prev); }); toast("info", "Task restored", "Changes undone."); } },
  });
 ...
 * Planning Sessions
 * ────────────────────────────────────────────────────────────── */
export function scheduleFamilySession(familyId, { goalId = null, title = "Family Planning Session", startISO, endISO, agenda = "" } = {}) {
  if (!isStr(startISO) || !isStr(endISO)) return null;

  const { result: sess, undo } = withState((s) => {
    const sess = { id: uid(), familyId, goalId, title, startISO, endISO, agenda, createdAt: nowISO() };
    s.sessions.push(sess);
    return sess;
  });

  emitEvent(NAMES["calendar.add"] || "calendar/add", { title, startISO, endISO, attendees: [], meta: { familyId, goalId } }, {
    source: "familySharing.scheduleFamilySession",
    nextBestAction: { label: "Open Calendar", hint: "Sync session", route: "/calendar" },
  });

  emitEvent("family.session.scheduled", { sessionId: sess.id, familyId, goalId }, {
    source: "familySharing.scheduleFamilySession",
    undo: { label: "Undo", handler: () => { undo(); toast("info", "Session removed", "Undo successful."); } },
  });

  autoShareOpsForecast(familyId, { cause: "session.scheduled", goalId });
  toast("success", "Family Session scheduled", new Date(startISO).toLocaleString());
  return sess;
}
...
function buildOpsWindows(state, familyId) {
  ...
  const goals = state.goals.filter((g) => g.familyId === familyId && !g.archived);
  const goalIds = goals.map((g) => g.id);
  const milestones = state.milestones.filter((m) => goalIds.includes(m.goalId) && m.due);
  ...
  for (const s of sessions) {
    ...
    windows.push({ startISO: asISO(ts - 1 * 24 * 3600 * 1000), days: 4, reason: "session" });
  }
  if (!windows.length && (goals.length || state.shares.some(sh => sh.familyId === familyId))) {
    windows.push({ startISO: asISO(now), days: 7, reason: "weekly" });
  }

  // Normalize for hashing (day bucket only)
  return windows.slice(0, 3).map(w => ({ startISO: new Date(w.startISO).toISOString().slice(0,10) + "T00:00:00.000Z", days: w.days, reason: w.reason }));
}
...
function composeOpsDigest(responses = [], windows = [], familyName = "Family") {
  ...
  const winTxt = windows.map(w => `${new Date(w.startISO).toLocaleDateString()} • ${w.days}d`).join(" • ") || "upcoming week";
  ...
  const renderItems = (arr=[]) => arr.slice(0, 30).map(it => {
    const date = it.date ? new Date(it.date).toLocaleDateString() : "—";
    const title = htmlEscape(it.title || "Task");
    const det = it.detail ? ` <span class="muted">• ${htmlEscape(it.detail)}</span>` : "";
    const qty = (it.qty != null) ? ` <span class="chip">${htmlEscape(String(it.qty))}${it.unit ? " " + htmlEscape(it.unit) : ""}</span>` : "";
    const pr  = it.priority ? ` <span class="chip">${htmlEscape(String(it.priority))}</span>` : "";
    const bl  = Array.isArray(it.blockers) && it.blockers.length ? ` <span class="muted"> — Blockers: ${htmlEscape(it.blockers.join(", "))}</span>` : "";
    return `<div class="item"><div class="date">${date}</div><div class="title">${title}${det}${qty}${pr}${bl}</div></div>`;
  }).join("");

  const secHtml = sectionOrder.map(sec => {
  ...
  return `<!doctype html><meta charset="utf-8"><body>
    ...
    <article class="page">
      ...
      <p class="muted">Windows: ${htmlEscape(winTxt)} • Sections: Garden, Animals, Cleaning, Cooking</p>
      ${secHtml || `<div class="sec"><div class="item"><div class="title">No forecasted items.</div></div></div>`}
...
async function shouldShareNow(familyId, cause = "auto") {
  ...
  // Global min interval
  const last = s._lastDigestAt[familyId] ? new Date(s._lastDigestAt[familyId]).getTime() : 0;
  const mins = clamp(Number(settings.minIntervalMins || 180), 15, 1440);
  if (Date.now() - last < mins * 60 * 1000) return { ok: false, reason: "rate-limit" };

  // Per-cause interval (if configured)
  const causeMins = clamp(Number(settings.causeIntervals?.[cause] || 0), 0, 10080); // up to 7 days
  if (causeMins > 0) {
    const lastHashTime = s._lastDigestAt[`${familyId}:${cause}`] ? new Date(s._lastDigestAt[`${familyId}:${cause}`]).getTime() : 0;
    if (Date.now() - lastHashTime < causeMins * 60000) return { ok: false, reason: "cause-rate-limit" };
  }
...
function sendViaChannels({ msg, html, text }) {
  const channels = msg.channels || { email: true, push: true, inapp: true, sms: false };
  const baseMeta = { familyId: msg.familyId, messageId: msg.id, kind: "ops-forecast" };
  ...
  if (channels.sms) {
    const smsTo = msg.recipients.filter(r => r.phone).map(r => ({ to: r.phone, name: r.name || r.email }));
    if (smsTo.length) {
      busEmit("notifications/send", {
        type: "sms",
        recipients: smsTo,
...
export async function autoShareOpsForecast(
  ...
  if (!gate.ok) {
    events.emit(buildEvent("family.opsForecast.skipped", { familyId, reason: gate.reason }, { source: "familySharing" }));
    return;
  }
  ...
  const payload = { familyId, householdIds: fam.householdIds || [], windows, signals, scopes, cause };
  ...
  let responses = [];
  try { responses = (await Promise.all(reqs)).filter(Boolean); } catch { responses = []; }
  if (!responses.length) { toast("info", "No operational forecasters connected", "Enable domain forecasters to power agrarian digests."); return; }

  // Normalized hash: use day-bucketed windows & simplified responses
  const compact = responses.map(r => ({ section: r.section, count: (r.items||[]).length }));
  const digestBody = { windows, signals, famName: fam.name || "Family", cause, compact };
  const digestHash = hashDigest(digestBody);

  // Per-cause de-dup
  const cHash = s._causeHash[familyId] || {};
  if (cHash[cause] === digestHash) {
    events.emit(buildEvent("family.opsForecast.duplicate", { familyId, cause }, { source: "familySharing" }));
    return;
  }
  ...
  const msg = {
    ...
    familyId,
    type: "ops-forecast",
    payload: { html, windows, signals, cause, goalId, scopes, sections: responses.map(r => r.section) },
    recipients: agrarians.map((a) => ({ email: a.email, name: a.name || a.email, phone: a.phone })),
    channels: settings.channels || { email: true, push: true, inapp: true, sms: false },
    createdAt: nowISO(),
  ...
  withState((state) => {
    ...
    state._causeHash[familyId] = { ...(state._causeHash[familyId] || {}), [cause]: digestHash };
  });

  emitEvent("family.opsForecast.shared", {
    familyId,
    messageId: msg.id,
    recipients: msg.recipients,
    ...
    undo: {
      label: "Undo",
      handler: () => {
        withState((x) => { x.messages = x.messages.filter((m) => m.id !== msg.id); });
        busEmit("notifications/cancel", { messageId: msg.id });
        toast("info", "Ops forecast removed", "Agrarian notification canceled.");
      },
    },
    nextBestAction: { label: "Open Digest", hint: "Preview and adjust", route: "/family", params: { familyId, tab: "digests", id: msg.id } },
  });

  sendViaChannels({ msg, html, text });
  toast("success", "Operational forecast shared", "Agrarian received the digest.");
}
...
export function previewLastDigest(familyId) {
  ...
  if (!m) return null;
  return { html: m.payload?.html, createdAt: m.createdAt, recipients: m.recipients, channels: m.channels, shareURL: `/share/${m.shareToken}` };
}

export function resendLastDigest(familyId, overrides = {}) {
  const m = listFamilyDigests(familyId)[0];
  if (!m) { toast("info", "No digest to resend", "Create a forecast first."); return false; }
  const html = m.payload?.html || "";
  const text = toPlainText(html);
  const msg = { ...m, id: uid(), createdAt: nowISO(), recipients: overrides.recipients || m.recipients, channels: { ...(m.channels || {}), ...(overrides.channels || {}) } };
  withState((s) => s.messages.unshift(msg));
  sendViaChannels({ msg, html, text });
  emitEvent("family.opsForecast.resent", { familyId, messageId: msg.id }, { source: "familySharing.resendLastDigest" });
...
export function getShareLink(familyId, messageId) {
  const s = getState();
  const m = s.messages.find(mm => mm.familyId === familyId && mm.id === messageId);
  if (!m) return null;
...
events.on(NAMES["meal.plan.published"] || "meal.plan.published", ({ payload }) => {
  const familyId = payload?.familyId;
  if (familyId) {
    nba("Share Operational Forecast", "Align support across households", "/family", { familyId, tab: "digests" });
    autoShareOpsForecast(familyId, { cause: "meal.plan.published", scopes: ["cooking","cleaning"] });
  }
});
...
events.on("cooking/sessionCreated", ({ payload }) => {
  const familyId = payload?.familyId;
  if (familyId) autoShareOpsForecast(familyId, { cause: "cooking.sessionCreated", scopes: ["cooking"] });
});

// Cleaning routine updates
events.on("cleaning/routineUpdated", ({ payload }) => {
  const familyId = payload?.familyId;
  if (familyId) autoShareOpsForecast(familyId, { cause: "cleaning.routineUpdated", scopes: ["cleaning"] });
});

// Garden plan or animal plan changes
events.on("garden/planUpdated", ({ payload }) => {
  const familyId = payload?.familyId;
  if (familyId) autoShareOpsForecast(familyId, { cause: "garden.planUpdated", scopes: ["garden"] });
});
events.on("animals/planUpdated", ({ payload }) => {
  const familyId = payload?.familyId;
  if (familyId) autoShareOpsForecast(familyId, { cause: "animals.planUpdated", scopes: ["animals"] });
});

// Calendar synced/created → session alignment
events.on(NAMES["calendar.events.created"] || "calendar.events.created", ({ payload }) => {
  const famId = payload?.familyId;
  if (famId) autoShareOpsForecast(famId, { cause: "calendar.events.created" });
});
 ...
 * Queries
 * ────────────────────────────────────────────────────────────── */
export function listFamilies() { return getState().families; }
export function listFamilyShares(familyId) { return getState().shares.filter(sh => sh.familyId === familyId); }
export function listFamilyGoals(familyId) { return getState().goals.filter(g => g.familyId === familyId && !g.archived); }
export function listFamilySessions(familyId) { return getState().sessions.filter(ss => ss.familyId === familyId); }
export function listFamilyDigests(familyId) { return getState().messages.filter(m => m.familyId === familyId && m.type === "ops-forecast"); }
export function listFamilySettings(familyId) { const s = getState(); return s.familySettings[familyId] || null; }
