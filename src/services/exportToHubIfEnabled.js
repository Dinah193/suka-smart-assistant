...
function _buildEnvelope(kind, payload) {
  const fmt =
    ctx.HubPacketFormatter &&
    (ctx.HubPacketFormatter.default || ctx.HubPacketFormatter);
  if (fmt && typeof fmt.format === "function") {
    try {
      return fmt.format(kind, payload);
    } catch {}
  }
...
async function _flushQueue() {
  ...
  try {
    ...
    const conn =
      ctx.FamilyFundConnector &&
      (ctx.FamilyFundConnector.default || ctx.FamilyFundConnector);
    ...
    while (_qLen() > 0 && safety-- > 0) {
      ...
      try {
        ...
        ctx.eventBus.emit("hub.export.success", {
          type: "hub.export.success",
          ts: _iso(),
          source: item.source || "exportToHubIfEnabled",
          data: { id: _id(item.payload?.id), kind: item.kind },
        });
        // Also emit custom success event if saved in queued item
        if (item.successEvent) {
          ctx.eventBus.emit(item.successEvent, {
            type: item.successEvent,
            ts: _iso(),
            source: item.source || "exportToHubIfEnabled",
            data: { id: _id(item.payload?.id), session: item.payload },
          });
        }
      } catch (err) {
        // Put it back at the front and stop—avoid hot loop on repeat failure
        _qWrite([item, ..._qRead()]);
        ctx.eventBus.emit("hub.export.failed", {
          type: "hub.export.failed",
          ts: _iso(),
          source: item.source || "exportToHubIfEnabled",
          data: {
            id: _id(item.payload?.id),
            kind: item.kind,
...
if (typeof window !== "undefined") {
  window.addEventListener("online", () => {
    _ensureCtx().then(_flushQueue).catch(_noop);
  });
 ...
 * @returns {Promise<{sent:boolean, queued?:boolean, reason?:string}>}
 */
export default async function exportToHubIfEnabled(
  payload,
  {
    kind = "generic",
  ...
  ctx.eventBus.emit("hub.export.requested", {
    type: "hub.export.requested",
    ts: _iso(),
    source,
    data: { id, kind },
  });
  ...
  if (!ctx.featureFlags?.familyFundMode) {
    if (queueIfUnavailable) {
      ...
      ctx.eventBus.emit("hub.export.queued", {
        type: "hub.export.queued",
        ts: _iso(),
        source,
        data: { id, kind, reason: "disabled" },
      });
  ...
  if (!conn || typeof conn.send !== "function") {
    if (queueIfUnavailable) {
      ...
      ctx.eventBus.emit("hub.export.queued", {
        ...
        ts: _iso(),
        source,
        data: { id, kind, reason: "connector-missing" },
      });
  ...
  try {
    ...
    if (successEvent) {
      ctx.eventBus.emit(successEvent, {
        type: successEvent,
        ts: _iso(),
        source,
        data: { id, session: payload },
      });
    }
    ...
    if (queueIfUnavailable) {
      ...
      ctx.eventBus.emit("hub.export.queued", {
        ...
        ts: _iso(),
        source,
        data: { id, kind, reason: String((error && error.message) || error) },
      });
    ...
    ctx.eventBus.emit("hub.export.failed", {
      ...
      ts: _iso(),
      source,
      data: { id, kind, error: String((error && error.message) || error) },
    });
