...
function parseIngredientLine(line = '') {
  ...
  return { line: original, qty: clean(qty), unit: clean(unit), item: clean(item), notes: clean(notes) };
}
...
export function registerNormalizer(plugin) {
  if (!plugin || !plugin.id || typeof plugin.test !== 'function' || typeof plugin.normalize !== 'function') {
...
const RecipeNormalizer = {
  ...
  normalize: (p) => {
    ...
    const ingredientsTable = makeTable('recipe.ingredients', ['order', 'line', 'qty', 'unit', 'item', 'notes'], ingredients);
    const stepsTable = makeTable('recipe.steps', ['order', 'text', 'prepMinutes', 'performMinutes', 'temperature', 'cue'], steps);
    const nutritionTable = makeTable('recipe.nutrition', ['nutrient', 'value'], nutritionRows);
    ...
    return { kind: 'recipe', id: normalized.id, normalized, tables, warnings: [] };
  },
};
...
const ProcedureNormalizer = {
  ...
  normalize: (p) => {
    ...
    const supplies = arr(h.supplies).map((s) => ({ item: clean(s) })).filter((x) => x.item);
    const tools = arr(h.tools).map((t) => ({ tool: clean(t) })).filter((x) => x.tool);
    ...
    const suppliesTable = makeTable(`${domainHint}.supplies`, ['item'], supplies);
    const toolsTable = makeTable(`${domainHint}.tools`, ['tool'], tools);
    const stepsTable = makeTable(`${domainHint}.steps`, ['order', 'text', 'prepMinutes', 'performMinutes'], steps);
    ...
    return { kind: 'procedure', id: normalized.id, normalized, tables: [suppliesTable, toolsTable, stepsTable], warnings: [] };
  },
};
...
const GardenNormalizer = {
  ...
  normalize: (p) => {
    ...
    const hintsTable = makeTable('garden.hints', ['crop', 'spacing', 'depth', 'germination', 'daysToMaturity', 'zone', 'sun', 'water', 'row'], rows);
    ...
    return { kind: 'garden', id: normalized.id, normalized, tables: [hintsTable], warnings: [] };
  },
};
...
const StoreProductNormalizer = {
  ...
  normalize: (p) => {
    ...
    const productTable = makeTable('storehouse.product', ['name', 'brand', 'sku', 'price', 'currency', 'availability', 'url'], rows);
    ...
    return { kind: 'storehouse', id: normalized.id, normalized, tables: [productTable], warnings: [] };
  },
};
...
const VideoNormalizer = {
  ...
  normalize: (p) => {
    ...
    const videoTable = makeTable('video.meta', ['title', 'author', 'durationMinutes', 'embedUrl', 'thumbnail', 'description'], rows);
    ...
    return { kind: 'video', id: normalized.id, normalized, tables: [videoTable], warnings: [] };
  },
};
...
const UnknownNormalizer = {
  ...
  normalize: (p) => {
    ...
    const linksRows = arr(p.links || []).map((l) => ({ href: clean(l.href), text: clean(l.text) }));
    ...
    return { kind: 'unknown', id: normalized.id, normalized, tables, warnings: [] };
  },
};
...
export async function normalize(payload, options = {}) {
  ...
  if (!payload || typeof payload !== 'object') {
    ...
    emit('normalize.completed', { ok: false, error, startedAt, finishedAt: nowISO() });
  ...
  if (preferKind) {
    ...
    const cand = [..._registry.values()].find((n) => n.id === candidateId && safeTest(n, payload));
  ...
  emit('import.parsed', {
    ...
    kind: result.kind,
    id: result.id,
    title: result.normalized?.name || result.normalized?.title || payload.meta?.title || '',
    startedAt,
    finishedAt,
    tables: result.tables?.map((t) => ({ name: t.name, rows: t.rows?.length || 0 })) || [],
  });
