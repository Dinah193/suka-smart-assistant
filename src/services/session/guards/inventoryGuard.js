...
(function () {
  ...
  let inventory = null;
  try {
    inventory = (require("@/stores/inventory") || {}).inventory || null;
  } catch (_e) {}
  if (!inventory) {
    try {
      inventory =
  ...
  try {
    listBuilder = require("@/services/listBuilder");
  } catch (_e) {}
  ...
  const DEFAULTS = {
    ...
    skuAliases: {
      ...
      "fresh-ground-whole-grain-flour": [
        "fresh ground whole grain flour",
  ...
  function normalizeLine(line = {}) {
    ...
    if (estimateEngine && estimateEngine.normalizeQty) {
      try {
        qty = estimateEngine.normalizeQty({ sku, qty, unit }) || qty;
      } catch (_e) {}
    }
  ...
  function summarizeShortages(lines = []) {
    ...
    lines.forEach((l) => {
      const onHand =
        inventory && typeof inventory.getQty === "function"
          ? inventory.getQty(l.sku) || 0
          : 0;
      if (onHand <= 0 && l.qty > 0)
        missing.push({ sku: l.sku, needed: l.qty, onHand });
  ...
  async function guardRequirements(e = {}) {
    ...
    if (!prefs.enabled || !prefs.domainMap[e.domain || "general"])
      return { allow: true };
    ...
    eventBus.emit(
      ...
      Object.assign({}, shortagePayload, { reason: "not-enough-on-hand" })
    );
    ...
    if (prefs.grocerylistOnShortage) {
      const glines = shortages.map((s) => ({
        sku: s.sku,
        qty: Math.max(0, s.needed - (s.onHand || 0)),
      }));
      ...
      if (listBuilder && listBuilder.addLines) {
        try {
          listBuilder.addLines({ domain: e.domain, lines: glines });
        } catch (_e) {}
      }
    }
    ...
    toast(
      "Items missing",
      "We’re short on some supplies. I added them to your list and suggested substitutions.",
      ["inventory", e.domain || "general"]
    );
  ...
  function inferLinesFromEvent(e = {}) {
    ...
    if (e.domain === "meals" && e.recipe?.ingredients?.length) {
      return e.recipe.ingredients.map((ing) => ({
        sku: ing.name,
        qty: ing.qty,
        unit: ing.unit,
        title: ing.name,
    ...
    if (e.domain === "cleaning" && e.procedure?.supplies?.length) {
      return e.procedure.supplies.map((s) => ({
        sku: s.sku || s.name,
        qty: s.qty,
        unit: s.unit,
        title: s.name || s.sku,
    ...
    if (e.domain === "animals" && e.protocol?.inputs?.length) {
      return e.protocol.inputs.map((i) => ({
        sku: i.sku,
        qty: i.qty,
        unit: i.unit,
        title: i.title || i.sku,
  ...
  async function onScheduleCreated(e = {}) {
    ...
    (e.items || []).forEach((it) => {
      const req =
        it?.payload?.requirements ||
        it?.payload?.ingredients ||
        it?.payload?.supplies ||
        [];
  ...
  function getResIdForAnchor(anchorId) {
    return `res:${anchorId}`;
  }
  ...
  const inventoryGuard = {
    init(userPrefs = {}) {
      ...
      (prefs.autoConsumeOn || []).forEach((ev) =>
        eventBus.on(ev, (e = {}) => {
          if (e.anchorId) commitReservation(e.anchorId);
        })
      );
      (prefs.autoReleaseOn || []).forEach((ev) =>
        eventBus.on(ev, (e = {}) => {
          if (e.anchorId) releaseReservation(e.anchorId);
        })
      );
        ...
        if (!verdict.allow) {
          eventBus.emit("ui.modal.converted", {
            ...
            payload: {
              ...
              subhead:
                "I’ve added missing items to your list and suggested substitutions.",
              domain,
              readOnly: true,
              ...
              actions: [
                { id: "ack", label: "OK", kind: "ack" },
                {
                  ...
                  label: "Open list",
                  kind: "view",
                  payload: { path: "/tier2/household/meals?tab=lists" },
                },
              ],
              original: {
                id: m.id,
    ...
    reserve(anchorId, domain, lines) {
      ...
      reservations[anchorId || res.id] = res;
      save(K.RESMAP, reservations);
      return res;
    },
    release(anchorId) {
      releaseReservation(anchorId);
    },
    consume(anchorId) {
      commitReservation(anchorId);
    },
    ...
    // Debug
    _getReservations() {
      return safeJSON.parse(safeJSON.stringify(reservations), {});
    },
    _reset() {
      reservations = {};
      save(K.RESMAP, reservations);
