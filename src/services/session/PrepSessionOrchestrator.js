...
(function () {
  ...
  try { prepEngine = require("@/engines/planning/prepConsolidationEngine"); } catch (_e) {}

  var scheduleHelpers = null; // optional
  try { scheduleHelpers = require("@/engines/scheduling/scheduleHelpers"); } catch (_e) {}

  var IngredientLinker = null; // optional
  try { IngredientLinker = require("@/engines/linkers/IngredientLinker"); } catch (_e) {}

  var config = { get: function(_p, fb){ return fb; }, sabbathGuard:{ enabled:false } };
  try {
    var cfg = require("@/services/config");
    if (cfg && typeof cfg.get === "function") config = cfg;
  } catch (_e) {}

  var automation = { scheduleAt:function(){}, scheduleAfter:function(){} };
  try {
  ...
  function asMinutes(v, fb) { var n = Number(v); return (isFinite(n) && n > 0) ? n : fb; }
  function addHours(dateLike, hrs) { var d = new Date(dateLike || Date.now()); d.setHours(d.getHours() + Number(hrs || 0)); return d; }
  function addDays(dateLike, days) { var d = new Date(dateLike || Date.now()); d.setDate(d.getDate() + Number(days || 0)); return d; }
  function shallowClone(obj) { var out = {}; for (var k in (obj||{})) if (Object.prototype.hasOwnProperty.call(obj, k)) out[k] = obj[k]; return out; }
  function getDomain(x, fb) { return safeStr((x && (x.domain || (x.meta && x.meta.domain))) || fb || "prep"); }
  ...
  function _cfgSabbath() {
    return (config && (config.sabbathGuard || (config.get && config.get("sabbath.guard", { enabled:false })))) || { enabled:false };
  }
  ...
  function withholdActive(domainKey) {
    var windows = (config && config.get && config.get(domainKey + ".withholdWindows", [])) || [];
    try {
      var now = new Date();
      var cur = now.getHours()*60 + now.getMinutes();
      for (var i=0;i<windows.length;i++){
        var w = windows[i] || {};
        var pf = String(w.from||"00:00").split(":"); var from = (+pf[0])*60 + (+pf[1]||0);
        var pt = String(w.to||"00:00").split(":");   var to   = (+pt[0])*60 + (+pt[1]||0);
        var spans = to < from;
  ...
  function normalizeToSuggestion(input) {
    if (!input) return { id: uid("sug"), title: "Session", domain: "prep", tasks: [] };

    var looksRunbook = Array.isArray(input.steps) && !!input.domain && !!input.title;
  ...
  function toTimersFromSuggestion(sug, options) {
    ...
    for (var r=0; r<rbSteps.length; r++){
      var st = rbSteps[r] || {};
      if (String(st.type||"").toUpperCase() === "TIMER" && st.timer && st.timer.minutes){
    ...
    if (rbSteps.length) {
      for (var i=0;i<rbSteps.length;i++){
        ...
        steps.push({
          ...
          estimatedTime: asMinutes((s.timer && s.timer.minutes) || s.minutes, 5),
          domain: safeStr(sug.domain || "prep"),
  ...
  function buildFromSuggestion(suggestion, options) {
    ...
    if (prepEngine && typeof prepEngine.toBatchSession === "function") {
      try {
        ...
        logger.warn("[PrepSessionOrchestrator] prepEngine.toBatchSession failed; falling back", e);
      }
    }
    ...
    session.meta.sabbathGuard = !!(options.sabbathGuard || (suggestion.meta && suggestion.meta.guards && suggestion.meta.guards.sabbathActive));
    ...
    session.meta.resumeState = session.meta.resumeState || (options.resumeState || null);
    session.meta.domain = session.meta.domain || session.domain;

    try {
      eventBus.emit("runbook:created", { session: session, domain: session.domain });
      eventBus.emit("batch:session:create", { session: session, domain: session.domain });
      analytics.track && analytics.track("session/created", { id: session.id, domain: session.domain, store: session.meta.store });
      eventBus.emit("nba:candidate", { kind:"session", id:session.id, title:session.title, cta:"Open Session", domain: session.domain, payload:{ sessionId: session.id } });
      eventBus.emit("session:lifecycle", { id: session.id, action: "start", domain: session.domain, atISO: nowISO() });
    } catch (_e) {}
  ...
  function detectLeadPrepNeeds(suggestion) {
    ...
    var checklistSteps = safeArr(suggestion && suggestion.checklist && suggestion.checklist.steps);
    ...
    function consider(label, meta) {
      ...
      if (!hours && !days) {
        ...
        if (/overnight|12\s*h|twelve\s*hours|24\s*h|twenty[-\s]*four/.test(l)) { hours = 12; inferred = true; }
        if (/soak|soaking|beans|legume/.test(l))        { hours = Math.max(hours, 8); inferred = true; }
        if (/brine|brining|salt\s*water/.test(l))       { hours = Math.max(hours, 8); inferred = true; }
        if (/marinat(e|ing)/.test(l))                   { hours = Math.max(hours, 4); inferred = true; }
        if (/ferment|proof|starter|levain|culture|kimchi|sauerkraut|yogurt/.test(l)) { days = Math.max(days, 1); inferred = true; }
        if (/cure|dry\s*age|salt\s*rub\s*rest/.test(l)) { days = Math.max(days, 1); inferred = true; }
        if (/defrost|thaw/.test(l))                     { hours = Math.max(hours, 12); inferred = true; }
        // Fresh-ground whole grain flour → autolyse / rest before knead
        if (/fresh(ly)?\s*(ground|milled)|whole\s*grain\s*flour|fresh\s*ground\s*flour/.test(l)) {
          hours = Math.max(hours, 0.5); // 30 min autolyse default
          inferred = true;
          m.hint = (m.hint || "Autolyse") + " — let dough rest 20–40 min to hydrate bran.";
        }
        // Frozen fruit/veg for smoothies/stock → thaw
        if (/frozen\s*(fruit|veg|vegetable|berries)/.test(l)) { hours = Math.max(hours, 1); inferred = true; }
      }
    ...
    for (var i=0;i<tasks.length;i++) consider(tasks[i] && (tasks[i].title || tasks[i].label), tasks[i]);
    for (var j=0;j<checklistSteps.length;j++) consider(checklistSteps[j] && (checklistSteps[j].description || checklistSteps[j].label), checklistSteps[j]);
    for (var k=0;k<rbSteps.length;k++) consider(rbSteps[k] && (rbSteps[k].label || rbSteps[k].description), rbSteps[k]);
  ...
  function scheduleLeadPrepSessions(suggestion, options) {
    options = options || {};
    var targetStartISO = options.targetStartISO || options.targetEventISO || options.atISO || null;
    ...
    for (var i = 0; i < needs.length; i++) {
      var n = needs[i];
      var totalHours = Number(n.leadTimeHours || 0) + (Number(n.leadTimeDays || 0) * 24);
      if (!totalHours) continue;

      var startTime = addHours(anchor, -totalHours);
      if (startTime.getHours() < 7 || startTime.getHours() > 21) startTime.setHours(9,0,0,0);
      var scheduledTime = adjustForSabbath(startTime, options);

      var withholdReason = withholdActive("sessions") || withholdActive(n.domain || "prep");
      if (withholdReason) { var bump = new Date(scheduledTime); bump.setHours(9,0,0,0); scheduledTime = bump; }

      var leadSession = {
        id: "lead:" + (suggestion.id || uid("sug")) + ":" + (n.id || uid("need")),
        ...
        steps: [{
          ...
          estimatedTime: Math.max(10, Math.min(180, totalHours >= 1 ? 30 : 15)),
          domain: n.domain || "prep",
      ...
      try {
        eventBus.emit("batch:session:create", { session: leadSession, domain: leadSession.domain });
        eventBus.emit("schedule:session:create", {
          id: leadSession.id,
          title: leadSession.title,
          start: leadSession.when,
          end: iso(addHours(scheduledTime, 1)),
          metadata: { domain: leadSession.domain, type: "lead-prep", parent: suggestion.id || null },
          domain: leadSession.domain,
          disabled: !!options.sabbathGuard && isSabbath(scheduledTime, options)
        });
        eventBus.emit("nba:candidate", { kind:"lead-prep", id:leadSession.id, title:leadSession.title, cta:"Open Lead Prep", domain: leadSession.domain, payload:{ sessionId: leadSession.id } });
        analytics.track && analytics.track("lead_prep/scheduled", { id: leadSession.id, hours: totalHours, domain: leadSession.domain });
      } catch (_e) {}
      ...
      try {
        if (scheduleHelpers) {
          ...
          if (/defrost|thaw/.test(label) && scheduleHelpers.defrostReminder) {
            var r1 = scheduleHelpers.defrostReminder(dummyItem, scheduledTime); if (r1) { r1.domain = leadSession.domain; eventBus.emit("schedule:reminder:create", shallowClone(r1)); }
          }
          if (/marinat/.test(label) && scheduleHelpers.marinateReminder) {
            var r2 = scheduleHelpers.marinateReminder(dummyItem, scheduledTime); if (r2) { r2.domain = leadSession.domain; eventBus.emit("schedule:reminder:create", shallowClone(r2)); }
          }
          if (/autolyse|fresh/.test(label) && scheduleHelpers.autolyseReminder) {
            var r3 = scheduleHelpers.autolyseReminder(dummyItem, scheduledTime); if (r3) { r3.domain = leadSession.domain; eventBus.emit("schedule:reminder:create", shallowClone(r3)); }
          }
          if (/proof|starter|levain/.test(label) && scheduleHelpers.proofingReminder) {
            var r4 = scheduleHelpers.proofingReminder(dummyItem, scheduledTime); if (r4) { r4.domain = leadSession.domain; eventBus.emit("schedule:reminder:create", shallowClone(r4)); }
          }
        }
      } catch (_e) {}
    ...
    try {
      if (created.length) {
        eventBus.emit("prep:lead:sessions", { count: created.length, anchorISO: iso(anchor), sessions: created, domain: getDomain(suggestion) });
      }
    } catch (_e) {}
  ...
  function emitRemindersFor(session, options) {
    ...
    try {
      for (var i = 0; i < session.steps.length; i++) {
        ...
        var isBread = /(dough|bread|loaf|knead|mix flour|freshly\s*milled|fresh\s*ground\s*flour)/.test(desc);

        if (isMeat && scheduleHelpers.defrostReminder)  { var df = scheduleHelpers.defrostReminder({ name: s.description }, new Date()); if (df) { df.domain = session.domain; out.push(df); } }
        if (isMeat && scheduleHelpers.marinateReminder) { var mr = scheduleHelpers.marinateReminder({ name: s.description }, new Date()); if (mr) { mr.domain = session.domain; out.push(mr); } }
        if (isOven && scheduleHelpers.preheatReminder)  { var ph = scheduleHelpers.preheatReminder({ name: s.description }, new Date()); if (ph) { ph.domain = session.domain; out.push(ph); } }
        if (isBread && scheduleHelpers.autolyseReminder) { var ar = scheduleHelpers.autolyseReminder({ name: s.description }, new Date()); if (ar) { ar.domain = session.domain; out.push(ar); } }
        if (/proof|rise/.test(desc) && scheduleHelpers.proofingReminder) { var pr = scheduleHelpers.proofingReminder({ name: s.description }, new Date()); if (pr) { pr.domain = session.domain; out.push(pr); } }
      }
    } catch (_e) {}
  ...
  function startTimersForSuggestion(suggestion, options) {
    ...
    try {
      for (var i=0;i<timers.length;i++){
        ...
        if (!tmr.disabled && tmr.minutes > 0) {
          var fireAt = new Date(Date.now() + (Number(tmr.startOffsetSec||0)*1000) + (tmr.minutes*60000));
          ...
          nba.upsert({ kind:"window", domain: getDomain(suggestion, tmr.domain), runbookId: suggestion.sessionId || suggestion.id || null, stepId: tmr.id, label:"Use wait window wisely", priority:"info" });
        }
      }
    } catch (_e) {}

    var withhold = withholdActive("sessions") || withholdActive(getDomain(suggestion));
    if (options.sabbathGuard || options.dryRun || withhold) {
      try { eventBus.emit("multitimer:preview", { groupId: suggestion.id || uid("grp"), timers: timers, domain: getDomain(suggestion) }); } catch (_e) {}
      return timers;
    }
    ...
    try {
      eventBus.emit("multitimer:start", {
        ...
        timers: timers,
        meta: { sessionHint: suggestion.sessionId || null, domain: getDomain(suggestion) },
        domain: getDomain(suggestion)
      });
      analytics.track && analytics.track("timers/started", { count: timers.length, domain: getDomain(suggestion) });
    } catch (_e) {}
    return timers;
  }
  ...
  function streamNextSteps(suggestion, options) {
    options = options || {};
    var steps = ((suggestion && suggestion.checklist) ? (suggestion.checklist.steps || []) : safeArr(suggestion && suggestion.steps)).slice();
    var cursor = 0;
    var listeners = [];
    var destroyed = false;
    ...
    function notify() {
      ...
      for (var i=0;i<listeners.length;i++){ try { listeners[i](payload); } catch (_e) {} }
      try { eventBus.emit("prepstream:update", payload); } catch (_e) {}
    }

    function onUpdate(cb) { if (typeof cb === "function") listeners.push(cb); notify(); return function off(){ for (var i=0;i<listeners.length;i++) if (listeners[i]===cb){ listeners.splice(i,1); break; } }; }
    function advance() { if (cursor < steps.length) { cursor += 1; notify(); } }
    function skip(n){ n = clamp(n||1,1,steps.length); cursor = clamp(cursor + n, 0, steps.length); notify(); }
    function back(n){ n = clamp(n||1,1,steps.length); cursor = clamp(cursor - n, 0, steps.length); notify(); }
    function completeAll(){ cursor = steps.length; notify(); }

    function handleDone(){ advance(); }
    function handleGroupPaused(){ notify(); }
    function handleGroupResumed(){ notify(); }
    ...
    try {
      ...
      eventBus.on && eventBus.on("multitimer:group:resumed", handleGroupResumed);
    } catch (_e) {}
    ...
    function destroy(){
      ...
      try {
        ...
        eventBus.off && eventBus.off("multitimer:group:paused", handleGroupPaused);
        eventBus.off && eventBus.off("multitimer:group:resumed", handleGroupResumed);
      } catch (_e) {}
      listeners = [];
    }

    return { onUpdate:onUpdate, advance:advance, skip:skip, back:back, completeAll:completeAll, destroy:destroy };
  }

  /* ------------------------------ Conflict detection ------------------------- */
  function detectAndEmitConflicts(suggestion, options) {
    try {
      if (!scheduleHelpers || typeof scheduleHelpers.detectConflicts !== "function") return [];
      var conflicts = safeArr(scheduleHelpers.detectConflicts(suggestion, options));
      var domain = getDomain(suggestion);
  ...
  function buildDrawerState(session, suggestion) {
    var timers = toTimersFromSuggestion(suggestion || {}, { overlapSec: 60, sabbathGuard: !!(session.meta && session.meta.sabbathGuard) });
    var current = (session.steps && session.steps[0]) || null;

    var aisle = null;
    try {
      if (IngredientLinker && IngredientLinker.mapAisleHint && domainUX[session.domain] && domainUX[session.domain].aisleHint) {
        var tags = (session.meta && session.meta.tags) || [];
        aisle = IngredientLinker.mapAisleHint(tags, session.title || "");
      }
    } catch (_e) {}

    var totalMin = 0; for (var i=0;i<timers.length;i++) totalMin += Number(timers[i].minutes||0);
    ...
    return {
      ...
      etaISO: eta,
      progress: { done: 0, total: (session.steps && session.steps.length) || 0 },
  ...
  function orchestrate(suggestionLike, options) {
    ...
    try { eventBus.emit("prep:drawer:state", shallowClone(drawerState)); } catch (_e) {}

    // NBA hint: “resume later” for paused windows
    try { nba.upsert({ kind:"hint", domain: domain, runbookId: session.id, label:"Session controls available (pause/resume/end)", priority:"low" }); } catch (_e) {}

    return { session:session, leadSessions:leadSessions, timers:timers, stream:stream, reminders:reminders, conflicts:conflicts, drawerState:drawerState };
  }

  /* --------------------------- Lifecycle controls ---------------------------- */
  function pause(sessionId, meta) {
    try {
      eventBus.emit("multitimer:group:pause", { groupId: sessionId, domain: getDomain(meta) });
      eventBus.emit("session:lifecycle", { id: sessionId, action: "pause", domain: getDomain(meta), atISO: nowISO() });
      nba.upsert({ kind:"window", domain: getDomain(meta), runbookId: sessionId, label:"Session paused — we’ll remind you about in-progress timers", priority:"info" });
    } catch (_e) {}
  }

  function resume(sessionId, meta) {
    try {
      eventBus.emit("multitimer:group:resume", { groupId: sessionId, domain: getDomain(meta) });
      eventBus.emit("session:lifecycle", { id: sessionId, action: "resume", domain: getDomain(meta), atISO: nowISO() });
      nba.upsert({ kind:"action", domain: getDomain(meta), runbookId: sessionId, label:"Back to it — next steps loaded", priority:"normal" });
    } catch (_e) {}
  }

  function end(sessionId, meta) {
    try {
      eventBus.emit("multitimer:group:stop", { groupId: sessionId, domain: getDomain(meta) });
      eventBus.emit("batch:session:end", { id: sessionId, domain: getDomain(meta), atISO: nowISO() });
      eventBus.emit("session:lifecycle", { id: sessionId, action: "end", domain: getDomain(meta), atISO: nowISO() });
      if (nba && typeof nba.clear === "function") { try { nba.clear({ runbookId: sessionId, domain: getDomain(meta) }); } catch (_e) {} }
      analytics.track && analytics.track("session/ended", { id: sessionId, domain: getDomain(meta) });
    } catch (_e) {}
  }
  ...
  function orchestrateAny(input, opts) {
    var looksRunbook = input && Array.isArray(input.steps) && !!input.domain && !!input.title && (input.meta || input.guards || input.hazards);
    return looksRunbook ? orchestrateRunbook(input, opts) : orchestrateSuggestion(input, opts);
  }
