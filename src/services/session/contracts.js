...
export const ResourceSchema = {
  ...
  type: "object",
  properties: {
    id:        { type: "string",  nullable: true,  desc: "Internal id (inventory/device/person)" },
    sku:       { type: "string",  nullable: true,  desc: "SKU or external code" },
    name:      { type: "string",  nullable: true,  desc: "Human-label (display name)" },
    qty:       { type: "number",  nullable: true,  desc: "Requested quantity" },
    unit:      { type: "string",  nullable: true,  desc: "Unit of measure (kg, L, pkt, ea)" },
    deviceId:  { type: "string",  nullable: true,  desc: "Concrete device id (for calendars)" },
    kind:      { type: "string",  nullable: true,  desc: "Device kind / category (e.g., 'tiller')" },
    title:     { type: "string",  nullable: true,  desc: "Equipment title override" },
    role:      { type: "string",  nullable: true,  desc: "Required role (when person resource)" },
    count:     { type: "number",  nullable: true,  desc: "How many people/devices of this role/kind" },
  },
...
export const StepSchema = {
  ...
  properties: {
    ...
    bed:     { type: "string",  nullable: true, desc: "Garden bed/plot (garden domain)" },
...
export const PlanWindowSchema = {
  ...
  type: "object",
  properties: {
    startISO: { type: "string", format: "date-time", nullable: true, desc: "Planned start (ISO8601)" },
    endISO:   { type: "string", format: "date-time", nullable: true, desc: "Deadline/end (ISO8601)" },
  },
  additionalProperties: false,
};
...
export const SessionSchema = {
  ...
  properties: {
    ...
    domain:       { type: "string",  enum: ["cooking", "cleaning", "garden", "animals", "preservation", "storehouse", "general"] },
...
export const PlanSchema = {
  ...
  properties: {
    ...
    status:   { type: "string", enum: ["draft", "approved", "scheduled", "executed", "completed", "discarded", "error"] },
...
export function validateResource(r) {
  ...
  if (r.qty != null && !isFiniteNumber(r.qty)) errors.push("resource.qty must be number");
  if (r.count != null && !isFiniteNumber(r.count)) errors.push("resource.count must be number");
  for (const k of ["id","sku","name","unit","deviceId","kind","title","role"]) {
    if (r[k] != null && typeof r[k] !== "string") errors.push(`resource.${k} must be string`);
  }
...
export function validateStep(s) {
  ...
  if (!isInteger(s.idx) || s.idx < 1) errors.push("step.idx must be integer ≥1");
  if (!isNonEmptyString(s.label)) errors.push("step.label required");
  if (s.estMin != null && !isFiniteNumber(s.estMin)) errors.push("step.estMin must be number");
  for (const k of ["zone","bed","crop","notes"]) {
    if (s[k] != null && typeof s[k] !== "string") errors.push(`step.${k} must be string`);
  }
...
export function validateWindow(w) {
  ...
  if (w.startISO != null && !isISO(w.startISO)) errors.push("window.startISO must be ISO datetime");
  if (w.endISO != null && !isISO(w.endISO)) errors.push("window.endISO must be ISO datetime");
...
export function validateSession(sess) {
  ...
  if (!isNonEmptyString(sess.title)) errors.push("session.title required (string)");
  if (!isNonEmptyString(sess.domain)) errors.push("session.domain required (string)");
  if (!isFiniteNumber(sess.durationMin) || sess.durationMin <= 0) errors.push("session.durationMin must be > 0");
  ...
  if (sess.equipment) {
    if (!Array.isArray(sess.equipment)) errors.push("session.equipment must be array");
    else for (let i = 0; i < sess.equipment.length; i++) {
      const v = validateResource(sess.equipment[i]);
      if (!v.ok) errors.push(...prefix(`session.equipment[${i}]: `, v.errors));
    }
  }
  if (sess.ingredients) {
    if (!Array.isArray(sess.ingredients)) errors.push("session.ingredients must be array");
    else for (let i = 0; i < sess.ingredients.length; i++) {
      const v = validateResource(sess.ingredients[i]);
      if (!v.ok) errors.push(...prefix(`session.ingredients[${i}]: `, v.errors));
    }
  }
  if (sess.rolesNeeded) {
    if (!Array.isArray(sess.rolesNeeded)) errors.push("session.rolesNeeded must be array");
    else for (let i = 0; i < sess.rolesNeeded.length; i++) {
      const v = validateResource(sess.rolesNeeded[i]);
      if (!v.ok) errors.push(...prefix(`session.rolesNeeded[${i}]: `, v.errors));
    }
  }
  if (sess.steps) {
    if (!Array.isArray(sess.steps)) errors.push("session.steps must be array");
    else for (let i = 0; i < sess.steps.length; i++) {
  ...
  for (const k of ["location"]) {
    if (sess[k] != null && typeof sess[k] !== "string") errors.push(`session.${k} must be string`);
  }
  for (const k of ["outdoor","noisy"]) {
    if (sess[k] != null && typeof sess[k] !== "boolean") errors.push(`session.${k} must be boolean`);
  }
  if (sess.flexibilityMin != null && (!isFiniteNumber(sess.flexibilityMin) || sess.flexibilityMin < 0)) {
    errors.push("session.flexibilityMin must be number ≥ 0");
  }

  // meta is extensible, but if present should be an object
  if (sess.meta != null && !isObject(sess.meta)) errors.push("session.meta must be object");
...
export function validatePlan(plan) {
  ...
  if (!["draft","approved","scheduled","executed","completed","discarded","error"].includes(plan.status)) {
    errors.push("plan.status invalid");
  }
  if (!isISO(plan.createdAt)) errors.push("plan.createdAt must be ISO datetime");
  if (plan.updatedAt != null && !isISO(plan.updatedAt)) errors.push("plan.updatedAt must be ISO datetime");
...
export function normalizeSessionDraft(x = {}) {
  const s = {
    ...
    window: normalizeWindow(x.window || { startISO: firstISO(x.plannedStart), endISO: firstISO(x.deadline) }),
    equipment: dedupArray((x.equipment || []).map(normalizeResource), resourceKey),
    ingredients: dedupArray((x.ingredients || []).map(normalizeResource), resourceKey),
    rolesNeeded: dedupArray((x.rolesNeeded || []).map(normalizeResource), resourceKey),
...
export function computeEstMinutes(session) {
  if (Array.isArray(session?.steps) && session.steps.some(s => isFiniteNumber(s?.estMin))) {
    return session.steps.reduce((acc, s) => acc + (isFiniteNumber(s?.estMin) ? s.estMin : 0), 0);
  }
...
export function assertPlanTransition(from, to) {
  const order = ["draft","approved","scheduled","executed","completed","discarded","error"];
...
(() => {
  try {
    eventBus.emit("schema/contractsReady", {
...
/* ------------------------------- Small helpers ------------------------------ */
function result(errors) { return errors.length ? { ok: false, errors } : { ok: true }; }
function fail(errors)   { return { ok: false, errors }; }
function prefix(p, arr) { return (arr || []).map(e => p + e); }
...
function firstISO(...vals) { return vals.find(isISO); }

function toNum(n)  { return Number.isFinite(n) ? n : Number.isFinite(+n) ? +n : undefined; }
function toInt(n)  { const v = toNum(n); return Number.isInteger(v) ? v : undefined; }
function clamp(n, lo, hi) { const x = Number.isFinite(n) ? n : lo; return Math.max(lo, Math.min(hi, x)); }
function strOrUndef(v) { return v != null ? String(v) : undefined; }
function pruneUndef(o) { const out = {}; for (const k in o) if (o[k] !== undefined) out[k] = o[k]; return out; }
...
function rejectUnknown(obj, schema, errors) {
  ...
  for (const k of Object.keys(obj)) {
    ...
    if (!allowed.has(k)) errors.push(`${schema.$id || "object"}: unknown property "${k}"`);
  }
}

function resourceKey(r) { return r?.id || r?.deviceId || r?.sku || r?.name || r?.kind || r?.role || ""; }
...
function sortByIdx(steps) {
  return (steps || []).slice().sort((a, b) => (toInt(a?.idx) || 0) - (toInt(b?.idx) || 0));
}
