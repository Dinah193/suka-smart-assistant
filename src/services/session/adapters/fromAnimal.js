...
export function mapAnimalToSession(input = {}) {
  ...
  const outdoor =
    kind !== "milking" && kind !== "collection" ? true : !!src.outdoor; // most butchery/care outdoors
  const noisy =
    hasKind(equipment, "plucker") ||
    hasKind(equipment, "compressor") ||
    hasKind(equipment, "generator");
  ...
  const draft = {
    ...
    meta: {
      ...
      refrigerationCritical:
        kind === "butchery" || /chill|cooler/i.test(JSON.stringify(steps)),
      wasteDisposalPlan: src.wasteDisposalPlan || undefined,
    },
  };

  if (draft.durationMin <= 0)
    throw new Error("Invalid duration for animal/butchery draft");
  return draft;
}
...
function normalizeAnimalInput(x = {}) {
  ...
  const animals = Array.isArray(plan.animals)
    ? plan.animals
    : Array.isArray(herd.animals)
    ? herd.animals
    ...
    ? [
        {
          id: meta.animalId,
          tag: meta.tag,
          species: plan.species || meta.species,
          count: plan.count || meta.count,
        },
      ]
    : [];
  ...
  return {
    ...
    animals,
    // Quantities
    count: Number.isFinite(plan.count)
      ? +plan.count
...
function inferWorkKind(src) {
  ...
  if (/vet|vaccin|deworm|ai\b|insemination|hoof|trim|shear|tag|band/.test(text))
    return "care";
  return "care";
}
...
function deriveDurationMin(src, kind) {
  ...
  const count =
    num(src.count) ||
    (Array.isArray(src.animals)
      ? src.animals[0]?.count || src.animals.length
      : 1);
  switch (kind) {
    case "butchery":
      // Rough heuristics per head (poultry faster, large stock longer)
      if (/chicken|turkey|duck|poultry|broiler|layer/i.test(src.species))
        return clamp(20 + count * 12, 30, 8 * 60);
      if (/rabbit/i.test(src.species))
        return clamp(30 + count * 15, 45, 8 * 60);
...
function deriveEquipment(src, kind) {
  ...
  if (kind === "butchery") {
    ...
    if (/hoof|trim/i.test(String(src.operation)))
      implied.push({ kind: "hoof-trimmer", title: "Hoof Trimmer" });
    if (/shear/i.test(String(src.operation)))
      implied.push({ kind: "shears", title: "Shears" });
...
function deriveConsumables(src, kind) {
  ...
  if (kind === "butchery") {
    ...
    if (/deworm|vaccin/i.test(String(src.operation)))
      implied.push({ name: "Syringes / meds", qty: 1, unit: "kit" });
    if (/hoof|trim/i.test(String(src.operation)))
      implied.push({ name: "Hoof blocks / glue", qty: 1, unit: "kit" });
  }
...
function deriveRoles(src, kind) {
  const base = [
    { role: kind === "butchery" ? "butcher" : "handler", count: 1 },
  ];
  const count = num(src.count) || 1;
  if (kind === "butchery") {
    base.push({ role: "helper", count: count > 5 ? 2 : 1 });
    if (/large|goat|sheep|pig|beef|cow|steer/i.test(src.species))
      base.push({ role: "adult-supervisor", count: 1 });
  } else if (kind === "care") {
    if (/vet|vaccin|ai|insemination/i.test(String(src.operation)))
      base.push({ role: "vet-tech", count: 1 });
  }
  return base;
}
...
function deriveSteps(src, kind) {
  ...
  switch (kind) {
    case "butchery":
      if (/poultry|chicken|turkey|duck|broiler|layer/i.test(src.species)) {
        steps.push({
          label: "Set up scalder/plucker & sanitize area",
          estMin: 20,
        });
    ...
    default:
      ...
      steps.push({
        label: toTitle(src.operation || "Perform care procedure"),
        estMin: 20,
      });
...
function deriveSafetyFlags(src, kind) {
  ...
  if (
    kind === "care" &&
    /vaccin|med|ai|insemination/i.test(String(src.operation))
  ) {
    hazards.push("needlesMeds");
  }
  if (hasKind(deriveEquipment(src, kind), "scalder"))
    hazards.push("hotLiquids");
  if (hasKind(deriveEquipment(src, kind), "plucker"))
    hazards.push("pinchHazard");
  return { hazards };
}
...
function buildTitle(src, kind) {
  const base =
    src.title || (kind === "butchery" ? "Butchery Session" : "Animal Session");
