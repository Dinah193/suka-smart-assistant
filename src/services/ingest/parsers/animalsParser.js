// src/services/ingest/parsers/animalsParser.js
// -----------------------------------------------------------------------------
// animalsParser
// -----------------------------------------------------------------------------
...
export default async function animalsParser(artifact) {
  ...
  const lexMatches = [
    ...LayerRegistry.scanTextWithLexicon({ text, lexiconIdOrDomain: "animals", maxMatches: 90 }),
    ...LayerRegistry.scanTextWithLexicon({ text, lexiconIdOrDomain: "animalsestimaterecommendation", maxMatches: 50 }),
    ...LayerRegistry.scanTextWithLexicon({ text, lexiconIdOrDomain: "slaughter", maxMatches: 40 }).filter(Boolean),
  ...
  for (const r of ranked) {
    ...
    methodMapIds.push(await db.method_maps.add(rec));
  }
  ...
  return { ok: true, domain, artifactId: artifact.id, candidateId, methodMapIds, ranked };
}
