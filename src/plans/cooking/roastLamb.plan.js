...
(function () {
  ...
  var parseOffset = function (s) {
    return 0;
  };
  try {
    parseOffset =
      (require("@/services/session/utils/offsetParser") || {}).parseOffset ||
      parseOffset;
  } catch (_e) {}

  // Pause policies (freeze/continue/safety windows)
  var pausePolicies = {
    shouldFreezeTask: function () {
      return false;
    },
    canContinueTask: function () {
      return true;
    },
    safetyWindowActive: function () {
      return false;
    },
  };
  try {
    pausePolicies =
      require("@/services/session/policies/pausePolicies") || pausePolicies;
  } catch (_e) {}

  // Inventory utilities (ensure items on hand, shortages event)
  var inventoryGuard = {
    ensureItemsOnHand: function () {
      return { ok: true, missing: [] };
    },
    asSKUs: function (items) {
      return items || [];
    },
  };
  try {
    inventoryGuard =
      require("@/services/session/guards/inventoryGuard") || inventoryGuard;
  } catch (_e) {}
  ...
  try {
    scheduleHelpers =
      require("@/services/session/scheduleHelpers") || scheduleHelpers;
  } catch (_e) {}
  ...
  try {
    estimateEngine =
      require("@/services/session/estimateEngine") || estimateEngine;
  } catch (_e) {}
  ...
  function calcRoastMinutes(weightLb, donenessKey, altitudeFt, ovenRunsCool) {
    var base = (DONENESS_TARGETS[donenessKey] || DONENESS_TARGETS.medium)
      .perLbMin;
  ...
  function buildIngredients(opts) {
    ...
    return [
      {
        sku: "MEAT-LAMB-" + (opts.cut || "leg").toUpperCase(),
        name: "Lamb " + (opts.cut || "leg"),
        qty: weightLb,
        uom: "lb",
      },
      { sku: "PROD-GARLIC-FRESH", name: "Garlic cloves", qty: 6, uom: "each" },
      {
        sku: "PROD-ROSEMARY-FRESH",
        name: "Fresh rosemary",
        qty: 4,
        uom: "sprigs",
      },
      ...
      {
        sku: "PAN-FLOUR-WHOLEGRAIN",
        name: "Fresh-ground whole grain flour",
        qty: 2,
        uom: "tbsp",
        optional: true,
      },
  ...
  function stepsFor(opts) {
    ...
    var roastMinutes = calcRoastMinutes(
      weightLb,
      donenessKey,
      altitudeFt,
      ovenRunsCool
      ...
      ? [
          {
            ...
            label: "Sear fat cap",
            details:
              "Heat a heavy pan until very hot. Sear lamb on fat side 2–3 min until deep golden.",
            kind: "cook",
            durationMs: 3 * 60 * 1000,
            appliance: "stovetop",
            pausePolicy: "continue", // can continue through short pauses
            ppe: scheduleHelpers.suggestPPE({
              domain: DOMAIN,
              context: "sear",
            }),
          },
        ...
        ? [
            {
              ...
              details:
                "Score fat. Mash garlic, rosemary, thyme, salt, pepper, lemon zest, Dijon with oil. Rub all over the lamb. Cover and marinate in fridge.",
              kind: "prep",
              ...
              reminders: [
                {
                  atOffset: "-30m",
                  message:
                    "Pull lamb from fridge soon so it can lose its chill.",
                },
              ],
            },
          ]
      ...
      ? [
          {
            ...
            details:
              "Cut potatoes & carrots; toss with oil, salt, pepper. Start roasting when lamb goes into oven.",
            kind: "prep",
      ...
      .concat([
        {
          ...
          label: "Roast lamb",
          details:
            "Transfer lamb to rack in roasting pan. Insert thermometer into thickest part. Roast until " +
            DONENESS_TARGETS[donenessKey].window[0] +
            "–" +
            DONENESS_TARGETS[donenessKey].window[1] +
            "°F.",
          kind: "cook",
          ...
          reminders: [
            {
              atOffset: "-20m",
              message:
                "Baste lamb (optional) and rotate pan for even browning.",
            },
            {
              atOffset: "+0",
              message: "Check internal temp now; do not overshoot.",
            },
          ],
          pausePolicy: "safety", // long cook: respect withhold windows & sabbath guard
          safetyWindows:
            scheduleHelpers.withholdWindows({ domain: DOMAIN }) || [],
        },
        {
          id: "finish-boost",
          label: "Finish at " + tplan.finishF + "°F (short)",
          details:
            "Increase heat briefly to build crust if needed; monitor closely.",
          kind: "cook",
          durationMs: 5 * 60 * 1000,
          appliance: "oven",
          optional: true,
          skipIf: function (ctx) {
            return (ctx && ctx.crustIsGood) || false;
          },
        },
        {
          id: "rest",
          ...
          reminders: [
            {
              atOffset: "+0",
              message:
                "Start gravy now (optional). Use pan drippings + stock; thicken with fresh-ground whole grain flour if desired.",
            },
          ],
          pausePolicy: "continue",
        },
        {
          ...
          label: "Carve & serve",
          details:
            "Slice against the grain. Taste for salt & lemon. Serve with roasted veg.",
          kind: "serve",
          durationMs: 5 * 60 * 1000,
        },
      ])
  ...
  function createPlan(userOpts) {
    ...
    var plan = {
      ...
      synopsis:
        "A flexible roast lamb plan that adapts to cut, size, doneness, and kitchen quirks.",
      ...
      metrics: (function () {
        ...
        for (var i = 0; i < steps.length; i++)
          cookMs += steps[i].durationMs || 0;
        var activeMs = 0;
        for (var j = 0; j < steps.length; j++) {
          var s = steps[j];
          if (s.kind === "prep" || s.kind === "serve" || s.kind === "cook")
            activeMs += Math.min(s.durationMs || 0, 10 * 60 * 1000); // crude "active" proxy
        }
        return {
    ...
    try {
      ...
      var res = inventoryGuard.ensureItemsOnHand({
        skus: skus,
        domain: DOMAIN,
      });
    ...
    try {
      var estimate = estimateEngine.estimateCost({
  ...
  function saveAsFavorite(userId, overrides) {
    ...
    var fav = {
      ...
      tags: (plan.tags || [])
        .concat((overrides && overrides.tags) || [])
        .filter(Boolean),
    ...
    try {
      ...
      eventBus.emit("plan.favorite.saved", {
        domain: DOMAIN,
        planKey: PLAN_KEY,
        userId: userId,
        ref: ref,
  ...
  var api = {
    ...
    title: "Roast Lamb",
    synopsis:
      "Dynamic roast lamb with cut, doneness, and schedule intelligence.",
    // Core
