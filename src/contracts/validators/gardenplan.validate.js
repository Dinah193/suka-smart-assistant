...
(function () {
  ...
  function daysAfter(iso, days){
    if (!iso) return null;
    return new Date(Date.parse(iso) + (Number(days)||0)*24*3600*1000).toISOString();
  }
  ...
  function loadSchema(){
    ...
    return {
      ...
      description: "Fallback schema for garden plans when canonical file is unavailable.",
      type: "object",
      additionalProperties: false,
      required: ["id","x-domain","householdId","period","beds","crops","tasks"],
      properties: {
        id: { type: "string", pattern: "^(workplan|gardenplan):[A-Za-z0-9_\\-:.]+$" },
        ...
        settings: {
          ...
          properties: {
            ...
            calendarStart: { type: "string", enum: ["full-moon","new-moon","first-crescent","meridian-rule","gregorian"], default: "full-moon" },
        ...
        crops: {
          type: "array", default: [],
          items: {
            type: "object", additionalProperties: true, required:["id","species"],
            properties: {
              id: { type: "string" },
              ...
              companions: { type: "array", items: { type: "string" }, default: [] },
              antagonists: { type: "array", items: { type: "string" }, default: [] },
              notes: { type: "string" }
            }
          }
        },
        tasks: {
          type: "array", default: [],
          items: {
            type: "object", additionalProperties: true, required:["id","title","kind"],
            properties: {
              id: { type: "string" },
              title: { type: "string" },
              kind: {
                type: "string",
                enum: [
                  "soil_prep","sow","transplant","water","fertilize","trellis","prune",
                  "pest_scout","pesticide","weed","mulch","harvest","preserve","closeout"
                ]
              },
              scheduledAt: { type: "string", format: "date-time" },
              durationMin: { type: "number", default: 20 },
              bedId: { type: "string" },
              cropIds: { type: "array", items: { type: "string" }, default: [] },
  ...
  function normalizeGardenPlan(input){
    ...
    plan.period.end =
      ...
      new Date(Date.parse(plan.period.start) + 90 * 24 * 3600 * 1000).toISOString();

    plan.settings = plan.settings || {};
    if (!plan.settings.calendarStart) plan.settings.calendarStart = "full-moon";

    plan.beds = uniqueBy(coerceArray(plan.beds), function(b){ return b && b.id ? b.id : JSON.stringify(b); })
      .map(function(b){
    ...
    var bedIds = plan.beds.map(function(b){ return b.id; });

    plan.crops = uniqueBy(coerceArray(plan.crops), function(c){ return c && c.id ? c.id : JSON.stringify(c); })
      .map(function(c){
        var cc = Object.assign({ quantity: 1, companions: [], antagonists: [] }, c || {});
    ...
    var cropIds = plan.crops.map(function(c){ return c.id; });
    ...
    plan.tasks = uniqueBy(coerceArray(plan.tasks), function(t){ return t && t.id ? t.id : (t && t.title) ? t.title : JSON.stringify(t); })
      .map(function(t){
        ...
        tt.bedId = (tt.bedId && bedIds.indexOf(tt.bedId) >= 0) ? tt.bedId : (bedIds[0] || null);
        tt.cropIds = coerceArray(tt.cropIds).filter(function(id){ return cropIds.indexOf(id) >= 0; });
        ...
        if (tt.irrigation && tt.irrigation.minutes == null) tt.irrigation.minutes = 0;
        return tt;
      });
  ...
  function computeWarnings(plan){
      ...
      if (bed && c.spacingIn && c.quantity != null) {
        ...
        if (cap && c.quantity > cap) {
          warnings.push({
            ...
            level: "warn",
            cropId: c.id,
            msg: "Planned quantity (" + c.quantity + ") exceeds bed capacity (" + cap + ") for spacing " + c.spacingIn + "in."
          });
        }
      }
    }
    ...
    for (var bedKey in cropsByBed){
      ...
      for (var a=0;a<list.length;a++){
        for (var d=a+1; d<list.length; d++){
          var A = list[a], D = list[d];
          // antagonists either direction
          if ((A.antagonists||[]).indexOf((D.species||"").toLowerCase()) >= 0 ||
              (D.antagonists||[]).indexOf((A.species||"").toLowerCase()) >= 0) {
            warnings.push({
              code: "companion.antagonist",
              level: "warn",
              cropIds: [A.id, D.id],
              msg: "Antagonist pairing detected in the same bed: " + (A.species||"A") + " vs " + (D.species||"B")
            });
          }
        }
      }
    ...
    var lastFrostISO = plan.settings && plan.settings.lastFrost ? toISO(plan.settings.lastFrost) : null;
    var firstFrostISO = plan.settings && plan.settings.firstFrost ? toISO(plan.settings.firstFrost) : null;

    for (var t=0;t<plan.tasks.length;t++){
      var task = plan.tasks[t];

      if (task.bedId && !bedMap[task.bedId]) {
        warnings.push({ code: "task.bed.missing", level: "error", taskId: task.id, msg: "Task references unknown bed: " + task.bedId });
      }
      for (var q=0;q<task.cropIds.length;q++){
        if (!cropMap[task.cropIds[q]]) {
          warnings.push({ code: "task.crop.missing", level: "error", taskId: task.id, msg: "Task references unknown crop: " + task.cropIds[q] });
        }
      }
      ...
      if (plan.settings && plan.settings.sabbathGuard) {
        try {
          ...
          if (day === 6) {
            warnings.push({ code: "sabbath.guard", level: "info", taskId: task.id, msg: "Sabbath guard enabled — consider rescheduling Saturday work." });
          }
        } catch (e) { /* no-op */ }
      }
      ...
      if (task.kind === "sow" || task.kind === "transplant") {
        if (lastFrostISO && Date.parse(task.scheduledAt) < Date.parse(lastFrostISO)) {
          warnings.push({ code: "frost.pre_last", level: "warn", taskId: task.id, msg: "Sowing/transplant scheduled before last frost date." });
        }
      }
      if (task.kind === "harvest") {
        if (firstFrostISO && Date.parse(task.scheduledAt) > Date.parse(firstFrostISO)) {
          warnings.push({ code: "frost.post_first", level: "info", taskId: task.id, msg: "Harvest scheduled after first frost — verify cold-hardiness." });
        }
      }
      ...
      if (task.kind === "pesticide" && task.phiDays > 0) {
        ...
        for (var h=0; h<plan.tasks.length; h++){
          ...
          var intersects = H.cropIds.length === 0 || task.cropIds.length === 0
            ? true // conservative: unknown crops => warn
            : task.cropIds.some(function(id){ return H.cropIds.indexOf(id) >= 0; });
          if (intersects && Date.parse(H.scheduledAt) < Date.parse(blockUntil)) {
            warnings.push({
              code: "phi.violation",
              level: "error",
              taskIds: [task.id, H.id],
              msg: "Harvest is scheduled before PHI (" + task.phiDays + " days) after pesticide."
            });
          }
        }
      }
    ...
    for (var i1=0;i1<plan.tasks.length;i1++){
      ...
      for (var i2=i1+1;i2<plan.tasks.length;i2++){
        var B = plan.tasks[i2];
        // Tool overlap
        var sharedTools = (A.tools||[]).filter(function(tn){ return (B.tools||[]).indexOf(tn) >= 0; });
        if (sharedTools.length && minutesOverlap(A.scheduledAt, A.durationMin, B.scheduledAt, B.durationMin)) {
        ...
        if (wA && wB && wA === wB && minutesOverlap(A.scheduledAt, A.durationMin, B.scheduledAt, B.durationMin)) {
          warnings.push({
  ...
  function projectedHarvests(plan){
    ...
    for (var i=0;i<plan.tasks.length;i++){
      ...
      for (var c=0;c<t.cropIds.length;c++){
        ...
        if (!prev || Date.parse(when) < Date.parse(prev)) firstActionByCrop[id] = when;
      }
    }
    for (var cropId in firstActionByCrop){
      var crop = plan.crops.find(function(x){ return x.id === cropId; });
      if (!crop) continue;
      var start = firstActionByCrop[cropId];
      var eta = daysAfter(start, crop.daysToMaturity || 0);
      result.push({ cropId: cropId, species: crop.species, variety: crop.variety, eta: eta });
    }
    return result;
  }
  ...
  function computeMetrics(plan){
    var tasksByKind = {
      soil_prep:0, sow:0, transplant:0, water:0, fertilize:0, trellis:0, prune:0,
      pest_scout:0, pesticide:0, weed:0, mulch:0, harvest:0, preserve:0, closeout:0
    };
    var totalMinutes = 0;
    ...
    for (var i=0;i<plan.crops.length;i++){
      ...
      cropsBySpecies[sp] = (cropsBySpecies[sp] || 0) + (Number(plan.crops[i].quantity) || 0);
    }
    ...
    for (var j=0;j<plan.tasks.length;j++){
      ...
      if (t.irrigation && t.irrigation.waterlineId && t.irrigation.minutes) {
        ...
        irrigationMinutesByLine[key] = (irrigationMinutesByLine[key] || 0) + Number(t.irrigation.minutes);
      }
    }
    ...
    if (listBuilder && typeof listBuilder.buildSupplies === "function") {
      try {
        supplies = listBuilder.buildSupplies({ tasks: plan.tasks, domain: "garden" }) || [];
      } catch (e) { /* no-op */ }
    } else {
  ...
  // ----------------------------- Validator -----------------------------
  var _ajv = null, _schema = null, _validateFn = null;
  ...
  function validateGardenPlan(data, opts){
    ...
    if (workPrepConsolidation && typeof workPrepConsolidation.analyze === "function") {
      try {
        var cr = workPrepConsolidation.analyze({ tasks: normalized.tasks, domain: "garden" });
      ...
      try {
        if (result.ok) {
          ...
          eventBus.emit("nba.updated", {
            ...
            planId: normalized.id,
            hints: (function(){
              if (warnings.some(function(w){ return w.code.indexOf("phi.") === 0; })) {
                return [{ code: "respect_phi", label: "Fix PHI harvest conflicts", weight: 0.9 }];
              }
              if (warnings.some(function(w){ return w.code.indexOf("frost.") === 0; })) {
                return [{ code: "resolve_frost_risks", label: "Adjust for frost dates", weight: 0.8 }];
              }
              if (warnings.some(function(w){ return w.code.indexOf("companion.antagonist") === 0; })) {
                return [{ code: "apply_companion_rules", label: "Separate antagonists", weight: 0.7 }];
              }
              if (warnings.some(function(w){ return w.code.indexOf("tools.overlap") === 0 || w.code.indexOf("waterline.overlap") === 0; })) {
                return [{ code: "reschedule_conflicts", label: "Resolve tool/waterline overlaps", weight: 0.6 }];
              }
              // harvest clustering hint: plan preservation session
              if ((metrics.harvestETAs || []).length > 0) {
                return [{ code: "plan_preservation", label: "Plan canning/drying for harvest window", weight: 0.5 }];
              }
              return [{ code: "review_plan", label: "Review garden plan", weight: 0.3 }];
            })(),
            ts: nowISO()
          });
        } else {
