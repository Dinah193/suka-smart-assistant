...
function analyzeStructure(raw, kind = "markdown") {
  ...
  const text = kind === "html" ? stripHTML(src) : (kind === "markdown" ? stripMarkdown(src) : src);

  // primitives
  const images = kind === "html"
    ? count("<img\\b", src)
    : count("!\\[[^\\]]*\\]\\([^\\)]+\\)", src);
  ...
  const lists = (count("^\\s*[-*+]\\s", src) + count("^\\s*\\d+\\.\\s", src) + count("<li\\b", src));
...
function moduleWeights(module = "meals") {
  ...
  switch (module) {
    case "meals":
      return { wpm: DEFAULT_WPM, stepWpm: 170, listWpm: SKIM_WPM, imageSecs: 1.0, codeFactor: 1.0, tableFactor: TABLE_PENALTY_FACTOR };
    case "cleaning":
      return { wpm: DEFAULT_WPM, stepWpm: 180, listWpm: SKIM_WPM, imageSecs: 0.8, codeFactor: 1.0, tableFactor: 1.05 };
    case "garden":
      return { wpm: 215,        stepWpm: 175, listWpm: 360,      imageSecs: 1.1, codeFactor: 1.0, tableFactor: 1.12 };
    case "animals":
      return { wpm: 210,        stepWpm: 170, listWpm: 350,      imageSecs: 1.1, codeFactor: 1.0, tableFactor: 1.12 };
    default:
      return { wpm: DEFAULT_WPM, stepWpm: 185, listWpm: SKIM_WPM, imageSecs: 1.0, codeFactor: 1.0, tableFactor: TABLE_PENALTY_FACTOR };
  }
}

function estimateCore(textOrHTML, {
  ...
  const tablePenaltyWords = includeTablesPenalty ? Math.ceil(words * (weights.tableFactor - 1)) : 0;
  const codePenaltyWords = includeCodePenalty ? Math.ceil((s.codeFences || 0) * 35 * (DEFAULT_WPM / CODE_WPM)) : 0;
  ...
  // base time
  const baseMinutes = (words + stepWords + ingredientWords + tablePenaltyWords + codePenaltyWords - listBonusWords) / (weights.wpm * 1.0);

  // images time (Medium-like), scaled by module
  const imageMinutes = includeImages ? (mediumImageSeconds(s.images) * weights.imageSecs) / 60 : 0;
...
function estimate(content, options = {}) {
  ...
  const kind =
    options.kind ||
    (isObj && (content.markdown ? "markdown" : content.html ? "html" : "text")) ||
    "markdown";
  const module = options.module || (isObj && content.module) || "meals";
  const src = isObj ? (content.markdown || content.html || content.text || "") : String(content || "");
  const key = hash32(module + "::" + kind + "::" + src.slice(0, 20000)); // cap hash input

  const cached = cacheGet(key);
  if (cached) return { ...cached, cached: true };

  const rt = estimateCore(src, { kind, module, includeImages: options.includeImages !== false });
...
function estimateForArticle(frontmatter = {}, bodyMarkdown = "") {
  ...
  const rt = estimate({ id, module, markdown: bodyMarkdown }, { kind: "markdown", module });
  return { id, module, ...rt };
}
...
function badgeProps(rt) {
  return {
    ...
    title: `~${Math.max(1, Math.round(rt.minutes))} minutes (${rt.words} words, ${rt.images} images)`,
    details: rt,
  };
}
 ...
 */
async function saveFavoritePlanFromReading({ contentId, title, module = "meals", tags = [] }, target = "local") {
  ...
  if (res?.ok) {
    eventBus.emit?.("toast", { kind: "success", message: "Saved as Favorite Plan", tsISO: new Date().toISOString() });
  } else {
    eventBus.emit?.("toast", { kind: "error", message: "Could not save favorite", tsISO: new Date().toISOString() });
  }
  return res;
}
