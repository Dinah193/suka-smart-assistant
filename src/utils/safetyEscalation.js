...
(function () {
  ...
  // Time helpers
  let timeUtil = null;
  try { timeUtil = require("@/utils/dateFormat"); } catch (_e) {}

  // Scheduler/global settings (quiet hours, sabbath guard, safety thresholds)
  let getSchedulerPrefs = () => ({
    quietHours: { enabled: false, start: "22:00", end: "06:00" },
    sabbathGuard: { enabled: false, start: "Friday 18:00", end: "Saturday 19:30" },
    safety: {
  ...
  const now = () => Date.now();
  const toTs = (x) => x instanceof Date ? x.getTime() : typeof x === "number" ? x : Date.parse(x);
  ...
  function isWithinQuietHours(ts, quiet) {
    ...
    const sMin = start.h*60+start.m, eMin = end.h*60+end.m;
  ...
  function slackMs(input, nowMs) {
    ...
    const soft = input.softAt ? toTs(input.softAt) : (due - (getSchedulerPrefs().safety.softLeadMs || 120000));
    const hardGrace = input.maxOverrunMs ?? getSchedulerPrefs().safety.hardGraceMs ?? 60000;

    return {
      toSoft: soft - nowMs,
      toDue: due - nowMs,
      toHard: (due + hardGrace) - nowMs,
      softAt: soft, dueAt: due, hardAt: due + hardGrace
    };
  }
  ...
  function nbasForSoft(input, delta) {
    ...
    if (input.domain === "cooking") {
      list.push({ code:"PREHEAT_OR_PREP", label:"Pre-heat/Prep now", rationale:"Avoid crunch at hard deadline" });
      list.push({ code:"SET_TIMER", label:"Set timer for "+formatRemaining(delta), rationale:"Stay on track" });
      if (inventoryGuard?.suggestSubs) {
        list.push({ code:"CHECK_SUBS", label:"Check substitutions", rationale:"If ingredient missing, adapt now", data:{suggest: true}});
      }
    } else if (input.domain === "garden") {
      list.push({ code:"STAGE_TOOLS", label:"Stage tools & water", rationale:"Be ready for action window" });
    } else if (input.domain === "animals") {
      list.push({ code:"STAGE_FEED_SHELTER", label:"Stage feed/shelter", rationale:"Reduce animal stress" });
    } else {
      list.push({ code:"REVIEW_STEP", label:"Review upcoming step", rationale:"Reduce errors under time pressure" });
    }
    return list;
  }
  ...
  function nbasForHard(input) {
    const list = [];
    const basePause = { code:"AUTO_PAUSE", label:"Auto-pause session", rationale:"Exceeded hard deadline" };
    const baseStop  = { code:"AUTO_STOP", label:"Stop step", rationale:"Exceeded hard deadline" };

    if (input.domain === "cooking") {
      list.push(basePause, { code:"SAFE_REMOVE_HEAT", label:"Remove from heat", rationale:"Prevent burning" });
      list.push({ code:"COOL_DOWN", label:"Start cool-down", rationale:"Food safety" });
    } else if (input.domain === "animals") {
      list.push(basePause, { code:"ESCALATE_ANIMAL_WELFARE", label:"Escalate care", rationale:"Animal welfare priority" });
    } else if (input.domain === "garden") {
      list.push(baseStop, { code:"WATER_OR_SHADE", label:"Water/shade plants", rationale:"Prevent stress" });
    } else {
      list.push(basePause);
    }
    return list;
  ...
  function evaluate(input, nowMs = now()) {
    ...
    const { toSoft, toDue, toHard, softAt, dueAt, hardAt } = slackMs(input, nowMs);
    ...
    if (toHard <= 0) {
      ...
      nextCheckMs = Math.min(nextCheckMs, Math.max(3000, Math.floor(toDue/3)));
    } else {
    ...
    const shouldEmit = level !== "none" && cooldownOk(input.id || input.meta?.sessionId || dueAt, cool);

    const payload = {
      id: input.id || `${input.domain || "task"}:${dueAt}`,
      domain: input.domain || "task",
      level, code, reason, risk,
    ...
    if (!guardLock && level === "hard" && pausePolicies?.autoPauseForSafety) {
      try {
        ...
        eventBus.emit("session:safety:autopause", { id: payload.id, reason: payload.code });
      } catch (e) {
  ...
  // ESM compatibility
  // @ts-ignore
  if (typeof exports !== "undefined") { exports.evaluate = evaluate; exports.evaluateMany = evaluateMany; exports.default = api; }
  // Browser fallback
  // @ts-ignore
