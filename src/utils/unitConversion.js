...
/* --------------------------------- aliases --------------------------------- */
const FRACTIONS = { "¼": 0.25, "½": 0.5, "¾": 0.75, "⅐": 1/7, "⅑": 1/9, "⅒": 0.1, "⅓": 1/3, "⅔": 2/3, "⅕": 0.2, "⅖": 0.4, "⅗": 0.6, "⅘": 0.8, "⅙": 1/6, "⅚": 5/6, "⅛": 0.125, "⅜": 0.375, "⅝": 0.625, "⅞": 0.875 };
const MASS = {
  g: 1, gram: 1, grams: 1,
...
const VOLUME = {
  ...
  "fl-oz": 29.5735295625, floz: 29.5735295625, "fl oz": 29.5735295625,
...
const AREA = {
  "m2": 1, sqm: 1, "sq m": 1, "square meter": 1, "square meters": 1,
  "cm2": 1/10000, "sq cm": 1/10000,
  "mm2": 1/1e6, "sq mm": 1/1e6,
  "ft2": 0.09290304, sqft: 0.09290304, "sq ft": 0.09290304, "square foot": 0.09290304, "square feet": 0.09290304,
  "yd2": 0.83612736, sqyd: 0.83612736, "sq yd": 0.83612736,
...
const density = new Map(Object.entries({
  water: 1.0,
  milk: 1.035,
  oil: 0.92,
  olive_oil: 0.918,
...
function registerDensity(name, gramsPerMl) {
  try { density.set(String(name).toLowerCase(), Number(gramsPerMl)); } catch (_e) {}
  return true;
}
...
function normalizeUnit(u) {
  ...
  if (["c","°c","degc","celcius","celsius"].includes(s)) return { kind: "temp", unit: "C" };
  if (["f","°f","degf","fahrenheit"].includes(s)) return { kind: "temp", unit: "F" };
  if (["k","kelvin"].includes(s)) return { kind: "temp", unit: "K" };
  // common kitchen shorthands
  if (["t","tsp."] .includes(s)) return { kind: "volume", unit: "tsp" };
  if (["T","tbsp.", "tbs"].map(x=>x.toLowerCase()).includes(s)) return { kind: "volume", unit: "tbsp" };
...
function convertMass(value, from, to) {
  ...
  const a = MASS[from]; const b = MASS[to];
...
function convertVolume(value, from, to) {
  const v = parseNumberLike(value);
  const a = VOLUME[from]; const b = VOLUME[to];
...
function convertLength(value, from, to) {
  const v = parseNumberLike(value);
  const a = LENGTH[from]; const b = LENGTH[to];
...
function convertArea(value, from, to) {
  const v = parseNumberLike(value);
  const a = AREA[from]; const b = AREA[to];
...
function convert(value, fromUnit, toUnit, opts = {}) {
  ...
  try {
    ...
    if (from.kind === to.kind) {
      ...
      else if (from.kind === "volume") result = convertVolume(value, from.unit, to.unit);
      else if (from.kind === "length") result = convertLength(value, from.unit, to.unit);
      else if (from.kind === "area") result = convertArea(value, from.unit, to.unit);
      else if (from.kind === "temp") result = convertTemp(value, from.unit, to.unit);
    } else if ((from.kind === "volume" && to.kind === "mass") || (from.kind === "mass" && to.kind === "volume")) {
...
/* ------------------------------ baker's math ------------------------------- */
function bakersHydration({ flour_g, water_g, starter_water_g = 0, starter_flour_g = 0 }) {
...
function bakersScaleByHydration({ flour_g, water_g, targetHydrationPct, keepFlour = true }) {
  const current = bakersHydration({ flour_g, water_g });
  if (keepFlour) {
    const targetWater = (targetHydrationPct/100) * flour_g;
    return { flour_g, water_g: targetWater, delta_water_g: targetWater - water_g, hydrationPct: targetHydrationPct, currentPct: current };
  }
  // keep water, adjust flour
  const targetFlour = (water_g) / (targetHydrationPct/100);
  return { flour_g: targetFlour, water_g, delta_flour_g: targetFlour - flour_g, hydrationPct: targetHydrationPct, currentPct: current };
}

/* ------------------------------ cleaning math ------------------------------ */
// Percent/ppm/ratio helpers (v/v by default)
function percentToRatio(percent) { return 1 / (percent / 100); }      // e.g., 2% → 50:1
function ratioToPercent(ratio)  { return 100 / ratio; }               // 50:1 → 2%
function percentToPPM(percent)  { return percent * 10000; }           // 1% = 10,000 ppm
function ppmToPercent(ppm)      { return ppm / 10000; }

function dilutionSolve({ stockPercent, targetPercent, finalVolume, finalUnit = "ml" }) {
  ...
  const V2 = convertVolume(parseNumberLike(finalVolume), normalizeUnit(finalUnit)?.unit || "ml", "ml"); // → mL
...
function seedSpacing({ in_row, between_rows, unit = "in" }) {
  ...
  const sIn = convertLength(parseNumberLike(in_row), normalizeUnit(unit)?.unit || "in", "cm"); // → cm
  const sRow = convertLength(parseNumberLike(between_rows), normalizeUnit(unit)?.unit || "in", "cm");
...
async function saveFavoriteConversionPreset(meta, target = "local") {
  ...
  const payload = {
    planId: meta.planId || `unit-preset:${meta.slug || meta.title || Date.now()}`,
  ...
  try {
    if (PlanStorageRouter?.savePlanFavorite) {
      ...
      eventBus.emit?.("toast", { kind: "success", message: "Saved favorite preset", tsISO: toISO() });
  ...
  try {
    if (isBrowser) {
      ...
      prev.push({ id: payload.planId, domain: payload.domain, title: payload.meta.title, meta: payload.meta });
  ...
  eventBus.emit?.("toast", { kind: "error", message: "Could not save preset", tsISO: toISO() });
  return { ok: false };
}
