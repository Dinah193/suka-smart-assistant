...
const safeJSON = { parse: (s,f=null)=>{ try{ return JSON.parse(s);}catch{ return f;} } };

function dntEnabled() {
  try {
    if (!_opts.respectDNT) return false;
    const v = (navigator.doNotTrack || window.doNotTrack || navigator.msDoNotTrack || "0");
    return v === "1" || v === "yes";
  } catch { return false; }
}
...
function withTimeout(promise, ms, onTimeout) {
  let t;
  const timeout = new Promise((res) => { t = setTimeout(() => res(onTimeout?.()), ms); });
...
export function initAdSense(options = {}) {
  ...
  eventBus.emit?.("adsense.init", { clientId: _clientId, autoAds: !!_opts.autoAds, tsISO: toISO() });
  return { ok: true };
}
...
export function mountAdSlot(container, opts = {}) {
  ...
  if ((_opts.blockForPremium && isPremiumUser()) || dntEnabled()) {
    eventBus.emit?.("adsense.slot.skipped", { slotId: options.slotId, reason: "policy", tsISO: toISO() });
  ...
  Object.keys(options.style || {}).forEach(k => { ins.style[k] = options.style[k]; });

  ins.setAttribute("data-ad-client", _clientId);
  ins.setAttribute("data-ad-format", options.format);
  if (options.responsive) ins.setAttribute("data-full-width-responsive", "true");
  if (options.layoutKey) ins.setAttribute("data-ad-layout-key", options.layoutKey);
  ...
  const doPush = () => {
    try {
      ...
      eventBus.emit?.("adsense.slot.pushed", { slotId: options.slotId, meta: options.meta, tsISO: toISO() });
    } catch (e) {
      eventBus.emit?.("adsense.slot.error", { slotId: options.slotId, error: e?.message || String(e), tsISO: toISO() });
      renderHouseAd(container, options.meta);
    }
  };
  ...
  if (_opts.lazy && "IntersectionObserver" in window) {
    const io = new IntersectionObserver((entries) => {
      if (entries.some((en) => en.isIntersecting || en.intersectionRatio > 0)) {
        io.disconnect();
        withTimeout(Promise.resolve().then(doPush), _opts.loadTimeoutMs, () => renderHouseAd(container, options.meta));
      }
...
function renderHouseAd(container, meta = {}) {
  ...
  wrap.style.cssText = "border:1px solid #e5e7eb;border-radius:12px;padding:14px;display:flex;align-items:center;gap:12px;background:#fafafa;";
  const text = document.createElement("div");
  text.style.cssText = "flex:1;";

  const title = meta.title || "Make life easier with a Favorite Plan";
  const desc = meta.desc || "Save your best runbooks to reuse—meals, cleaning, garden, and animals.";
  text.innerHTML = `<div style="font-weight:600">${escapeHTML(title)}</div><div style="opacity:.75;font-size:0.95em">${escapeHTML(desc)}</div>`;
  ...
  btnSave.style.cssText = "border:1px solid #111;border-radius:10px;padding:8px 12px;background:#111;color:#fff;cursor:pointer;";
  btnSave.onclick = () => {
    const suggested = {
    ...
    setTimeout(async () => { await saveFavoritePlanFallback(suggested); }, 300);
  };

  const btnCreate = document.createElement("button");
  btnCreate.textContent = "Create a Plan";
  btnCreate.style.cssText = "border:1px solid #e5e7eb;border-radius:10px;padding:8px 12px;background:#fff;cursor:pointer;";
  btnCreate.onclick = () => {
...
async function saveFavoritePlanFallback(meta) {
  ...
  try {
    if (typeof useFavoritePlans === "function") {
      ...
      st?.addFavorite?.({ id: meta.planId, domain: meta.domain, title: meta.title || meta.planTitle || "Favorite Plan", meta });
      eventBus.emit?.("toast", { kind: "success", message: "Saved as Favorite Plan", tsISO: toISO() });
      return true;
    }
  } catch (_e) {}
  try {
    ...
    prev.push({ id: meta.planId, domain: meta.domain, title: meta.title || meta.planTitle || "Favorite Plan", meta });
