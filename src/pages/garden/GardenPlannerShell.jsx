...
function useGlobalEventBus() {
  ...
  useEffect(() => {
    ...
    if (!w.SUKA_EVENTBUS) {
      ...
      // Expose as a very light module proxy (helps shims that dynamically import services/events/eventBus.js)
...
export default function GardenPlannerShell() {
  ...
  useEffect(() => {
    ...
    const off = bus.on("export.requested", (payload) => {
      setInbox((s) =>
        [{ id: uid("x"), at: Date.now(), ...payload }, ...s].slice(0, 200)
      );
      // Friendly nudge
      raiseToast(
        "Export received",
        `${payload?.target || "Destination"} • ${payload?.kind || "garden"}`
      );
    });
    return () => off && off();
  }, [bus]);
  ...
  const [calc, setCalc] = useState({
    widthFt: 3,
    lengthFt: 10,
    depthIn: settings.soil.bedDepthIn,
    wastePct: settings.soil.wastePct,
  });
      ...
      <main className="max-w-7xl mx-auto px-4 py-4 grid lg:grid-cols-[1fr_320px] gap-4">
        ...
        <aside className="space-y-4">
          <section className="rounded-2xl border bg-white shadow-sm p-4">
            ...
            <div className="grid grid-cols-2 gap-2 mb-2">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSettings((s) => ({ ...s, restDay: e.target.value }))
                  }
                >
                  {["Sunday", "Saturday", "None"].map((d) => (
              ...
              <div className="text-[11px] text-gray-600 flex items-end">
                {restDayHelp}
              </div>
            </div>
            ...
            <div className="text-xs uppercase tracking-wide mt-3 mb-1">
              Irrigation Zones
            </div>
            <TagEditor
              tags={settings.irrigationZones}
              onChange={(tags) =>
                setSettings((s) => ({ ...s, irrigationZones: tags }))
              }
              placeholder="Add IZ"
            />
          </section>
          ...
          <section className="rounded-2xl border bg-white shadow-sm p-4">
              ...
              <Field label="Waste / loss %">
                <input
                  ...
                  onChange={(e) =>
                    setCalc((c) => ({
                      ...c,
                      wastePct: clampPct(e.target.value),
                    }))
                  }
                />
              </Field>
            ...
            <div className="mt-2 text-sm rounded-lg border p-2 bg-gray-50">
              <div>
                Area ≈ <strong>{areaFt2.toFixed(1)}</strong> ft²
              </div>
              <div>
                Volume (adj) ≈ <strong>{volFt3Adj.toFixed(1)}</strong> ft³ (
                <strong>{volYd3Adj.toFixed(2)}</strong> yd³)
              </div>
              <div className="text-xs text-gray-600">
                Includes waste/loss buffer.
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl border bg-white shadow-sm p-4">
            ...
            <ul className="text-sm space-y-1">
              <li>
                <kbd className="kbd">Ctrl/⌘</kbd> + <kbd className="kbd">1</kbd>{" "}
                — Collect
              </li>
              <li>
                <kbd className="kbd">Ctrl/⌘</kbd> + <kbd className="kbd">2</kbd>{" "}
                — Seedlings
              </li>
              <li>
                <kbd className="kbd">Ctrl/⌘</kbd> + <kbd className="kbd">3</kbd>{" "}
                — Plan
              </li>
              <li>
                <kbd className="kbd">Ctrl/⌘</kbd> + <kbd className="kbd">4</kbd>{" "}
                — Exports
              </li>
            </ul>
          </section>
      ...
      <div className="p-4">
        {inbox.length ? (
            ...
            {inbox.map((x) => (
              <div key={x.id} className="rounded-xl border p-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-gray-600">
                    {x.target || "—"} • {new Date(x.at).toLocaleString()}
                  </div>
                </div>
                <div className="text-xs text-gray-600 mt-1">
                  Payload keys:{" "}
                  {Object.keys(x || {})
                    .filter((k) => !["id", "at"].includes(k))
                    .join(", ") || "—"}
                </div>
                {x.schedule?.length ? (
                  <div className="mt-2 text-xs">
                    <div className="font-semibold mb-1">
                      Schedule items: {x.schedule.length}
                    </div>
                    <ul className="list-disc ml-5 space-y-1">
                      {x.schedule.slice(0, 5).map((s) => (
                        <li key={s.id}>
                          {s.date} — {s.title}
                        </li>
                      ))}
                ...
                <div className="mt-2 text-right">
                  <button
                    className="text-red-600 text-xs rounded border px-2 py-0.5 hover:bg-red-50"
          ...
          <div className="text-sm text-stone-500 italic">
            No exports captured yet. Actions from Collect/Seedlings/Plan will
            appear here when they emit <code>export.requested</code>.
          </div>
        )}
      </div>
    </div>
