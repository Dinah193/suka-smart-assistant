...
function parseBulk(text) {
  ...
  return rows.map((line) => {
    ...
    const [type, name, cultivar, qty, unit, tags, status, location, threshold, purchasedOn, source, price, lot, sowBy, harvestedYear, viabilityOverridePct] =
      [...parts, "", "", "", "", "", "", "", "", "", "", "", "", "", ""].slice(0, 16);
    return {
      ...DEFAULT_ITEM(),
      type: type || "seed",
...
function yearsSince(yearOrIso) {
  ...
  const y = String(yearOrIso).length === 4 ? Number(yearOrIso) : new Date(yearOrIso).getUTCFullYear();
...
function estimateViabilityPct(item, settings) {
  ...
  const years = item.harvestedYear ? yearsSince(item.harvestedYear) : (item.sowBy ? yearsSince(item.sowBy) : 0);
...
function suggestRestock(items) {
  return items.filter((it) => Number(it.threshold) > 0 && Number(it.qty) <= Number(it.threshold));
}

function downloadCSV(filename, rows) {
  const headers = [
    "type","name","cultivar","qty","unit","tags","status","location","threshold","purchasedOn","source","price","lot","sowBy","harvestedYear","viabilityOverridePct"
  ];
  const csv = [headers.join(",")]
    .concat(
      rows.map((r) => [
        r.type, r.name, r.cultivar, r.qty, r.unit, (r.tags || []).join("; "), r.status, r.location, r.threshold, r.purchasedOn, r.source, r.price, r.lot, r.sowBy, r.harvestedYear, r.viabilityOverridePct
...
function Chip({ children, onRemove }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs">
      {children}
      {onRemove ? <button type="button" onClick={onRemove} className="opacity-70 hover:opacity-100">×</button> : null}
    </span>
  );
}
...
export default function CollectOrganize() {
  ...
  const [settings, setSettings] = useState(() => {
    try { const raw = localStorage.getItem(LS_SETTINGS); return raw ? JSON.parse(raw) : DEFAULT_SETTINGS; } catch { return DEFAULT_SETTINGS; }
  });
  ...
  const kanban = useMemo(() => {
    ...
    filtered.forEach((it) => (map[it.status || "on-hand"] || map["on-hand"]).push(it));
    return { lanes, map };
  }, [filtered]);
  ...
  const sendTo = async (target, payloadExtra = {}) => {
    ...
    try { invokeNBA?.({ reason: "garden_collect_export", context: { target, count: items.length } }); } catch {}
  };
  ...
  const planWithAI = async () => {
    ...
    try {
      const res = await automation.runTemplate("garden.collect.normalize", { items, settings });
      if (Array.isArray(res?.items)) setItems(res.items);
      raiseToast("AI normalize", "Cleaned fields & tags.", false);
    } catch {
      automation.emit?.("event", { type: "garden/collect_normalize_request", payload: { items } });
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      <header className="mb-4 md:mb-6">
        <div className="flex items-center justify-between gap-3">
          <h1 className="text-xl md:text-2xl font-semibold">🗂️ Garden — Collect & Organize</h1>
          <div className="flex items-center gap-2">
            <button className="rounded-lg border px-3 py-1.5 text-sm hover:bg-gray-50" onClick={planWithAI}>AI Normalize</button>
            ...
            <button className="rounded-lg border px-3 py-1.5 text-sm hover:bg-gray-50" onClick={exportCSV}>Export CSV</button>
        ...
        <nav className="mt-4 flex gap-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Filter name/tag/location…" className="px-3 py-1.5 text-sm rounded-lg border w-64" />
            <button className="rounded-lg border px-3 py-1.5 text-sm hover:bg-gray-50" onClick={addRow}>+ Item</button>
          </div>
        </nav>
      </header>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <SectionCard title="Seed Viability & Loss Settings">
          <div className="grid grid-cols-3 gap-3">
            <Field label="Annual decay % (base)" hint="generic viability drop">
              <input className="w-full border rounded-md px-2 py-1" inputMode="decimal" value={settings.baseDecayPctPerYear}
                onChange={(e) => setSettings(s => ({ ...s, baseDecayPctPerYear: clampPct(e.target.value) }))}/>
            </Field>
            <Field label="Storage quality" hint="affects decay rate">
              <select className="w-full border rounded-md px-2 py-1" value={settings.storageQuality}
                onChange={(e) => setSettings(s => ({ ...s, storageQuality: e.target.value }))}>
                {["cool_dark","room","hot_humid"].map(k => <option key={k} value={k}>{k.replace("_"," ")}</option>)}
              </select>
            </Field>
            <Field label="Handling loss %" hint="spillage/miscount">
              <input className="w-full border rounded-md px-2 py-1" inputMode="decimal" value={settings.handlingLossPct}
                onChange={(e) => setSettings(s => ({ ...s, handlingLossPct: clampPct(e.target.value) }))}/>
            </Field>
          </div>
          <div className="text-xs text-gray-600 mt-2">Items can override viability in the table (Viability % column).</div>
        </SectionCard>
        ...
        >
          <Field label="Paste CSV or simple lines"
        ...
        <SectionCard title="Restock Alerts"
          actions={<button className="rounded-lg border px-3 py-1.5 text-sm hover:bg-gray-50" onClick={restockToTasks}>Create Restock Tasks</button>}>
          {restock.length ? (
            <ul className="text-sm space-y-1">
              {restock.map((it) => (
                <li key={it.id}>
                  <strong>{it.name}{it.cultivar ? ` — ${it.cultivar}` : ""}</strong> • {it.qty} {it.unit} in {it.location || "—"} (≤ {it.threshold})
                </li>
              ))}
            </ul>
          ) : <div className="text-sm text-gray-600">No items below threshold.</div>}
        </SectionCard>
      </div>

      {/* TABLE VIEW */}
      {tab === "table" && (
        <SectionCard title="Items (Table)" actions={<button className="rounded-lg border px-3 py-1.5 text-sm hover:bg-gray-50" onClick={addRow}>+ Item</button>}>
          {items.length ? <InventoryTable rows={filtered} updateRow={updateRow} removeRow={removeRow} settings={settings} estimateViabilityPct={estimateViabilityPct}/> : <EmptyState onAdd={addRow} />}
        </SectionCard>
      )}
      ...
      {tab === "kanban" && (
        <SectionCard title="Status Board (Kanban)">
          <Kanban lanes={kanban.lanes} map={kanban.map} setStatus={setStatus} removeRow={removeRow} />
        </SectionCard>
      )}

      {/* LABELS */}
      {tab === "labels" && (
        <SectionCard title="Labels" actions={<button className="rounded-lg border px-3 py-1.5 text-sm hover:bg-gray-50" onClick={() => window.print()}>Print</button>}>
      ...
      {/* SUMMARY */}
      {tab === "preview" && (
        <SectionCard title="Summary" actions={<button className="rounded-lg border px-3 py-1.5 text-sm hover:bg-gray-50" onClick={() => sendTo("Inventory")}>Send → Inventory</button>}>
          <SummaryCard items={filtered} settings={settings} estimateViabilityPct={estimateViabilityPct} />
        </SectionCard>
      )}
...
// ------------------ Subcomponents ------------------
function InventoryTable({ rows, updateRow, removeRow, settings, estimateViabilityPct }) {
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full text-sm">
        <thead>
          <tr className="text-left border-b">
            {["Type","Name","Cultivar","Qty","Unit","Status","Tags","Location","Threshold","Purchased","Source","Price","Sow by","Harvested Yr","Viability %",""].map((h) => (
            ...
            return (
              <tr key={r.id} className="border-b">
                <td className="py-2 pr-3">
                  <select className="w-28 border rounded-md px-2 py-1" value={r.type} onChange={(e) => updateRow(r.id, { type: e.target.value })}>
                    {["seed","tool","amendment","other"].map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </td>
                <td className="py-2 pr-3">
                  <input value={r.name} onChange={(e) => updateRow(r.id, { name: e.target.value })} placeholder="Name" className="w-44 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <input value={r.cultivar} onChange={(e) => updateRow(r.id, { cultivar: e.target.value })} placeholder="Cultivar" className="w-40 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <input type="number" min="0" value={r.qty} onChange={(e) => updateRow(r.id, { qty: numOr(e.target.value, r.qty) })} className="w-20 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <input value={r.unit} onChange={(e) => updateRow(r.id, { unit: e.target.value })} placeholder="pkt" className="w-20 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <select className="w-28 border rounded-md px-2 py-1" value={r.status} onChange={(e) => updateRow(r.id, { status: e.target.value })}>
                    {["wishlist","ordered","on-hand","planted","used-up"].map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </td>
                <td className="py-2 pr-3">
                  <TagEditor tags={r.tags} onChange={(tags) => updateRow(r.id, { tags })}/>
                </td>
                <td className="py-2 pr-3">
                  <input value={r.location} onChange={(e) => updateRow(r.id, { location: e.target.value })} placeholder="Bin/Room" className="w-28 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <input type="number" min="0" value={r.threshold} onChange={(e) => updateRow(r.id, { threshold: numOr(e.target.value, r.threshold) })} className="w-20 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <input value={r.purchasedOn} onChange={(e) => updateRow(r.id, { purchasedOn: e.target.value })} placeholder="YYYY-MM-DD" className="w-28 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <input value={r.source} onChange={(e) => updateRow(r.id, { source: e.target.value })} placeholder="Vendor" className="w-28 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <input value={r.price} onChange={(e) => updateRow(r.id, { price: e.target.value })} placeholder="$" className="w-20 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <input value={r.sowBy} onChange={(e) => updateRow(r.id, { sowBy: e.target.value })} placeholder="YYYY-MM-DD or year" className="w-28 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <input value={r.harvestedYear} onChange={(e) => updateRow(r.id, { harvestedYear: e.target.value })} placeholder="YYYY" className="w-20 border rounded-md px-2 py-1"/>
                </td>
                <td className="py-2 pr-3">
                  <input value={r.viabilityOverridePct} onChange={(e) => updateRow(r.id, { viabilityOverridePct: e.target.value })} placeholder={String(estimateViabilityPct(r, settings).toFixed(0))} className={`w-20 border rounded-md px-2 py-1 ${atRisk ? "bg-red-50" : ""}`}/>
                </td>
                <td className="py-2 pr-3 text-right">
                  <button onClick={() => removeRow(r.id)} className="text-red-600 text-xs rounded-lg border px-2 py-1 hover:bg-red-50">Remove</button>
                </td>
              </tr>
            );
          })}
...
function TagEditor({ tags = [], onChange }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1">
      {tags.map((t, i) => <Chip key={`${t}-${i}`} onRemove={() => onChange(tags.filter((_, idx) => idx !== i))}>{t}</Chip>)}
      <input
        value={draft}
        onChange={(e) => setDraft(e.target.value)}
        onKeyDown={(e) => {
          if (e.key === "Enter" && draft.trim()) {
            onChange([...(tags || []), draft.trim()]); setDraft("");
          }
        }}
        placeholder="add tag"
    ...
    <div className="grid md:grid-cols-5 gap-3">
      {lanes.map((lane) => (
        <div key={lane} className="rounded-xl border p-3 bg-gray-50/40">
          <div className="text-xs uppercase tracking-wide mb-2 font-semibold">{lane}</div>
          <div className="space-y-2">
            {(map[lane] || []).map((it) => (
              <div key={it.id} className="rounded-lg border bg-white p-3">
                <div className="font-medium text-sm">{it.name}{it.cultivar ? ` — ${it.cultivar}` : ""}</div>
                <div className="text-xs text-gray-600">{it.qty} {it.unit} • {it.location || "—"}</div>
                <div className="mt-2 flex gap-1 flex-wrap">
                  {lanes.filter(l => l !== lane).slice(0,3).map((target) => (
                    <button key={target} className="rounded border px-2 py-0.5 text-xs hover:bg-gray-50" onClick={() => setStatus(it.id, target)}>
                      → {target}
                    </button>
                  ))}
                  <button className="ml-auto text-red-600 text-xs rounded border px-2 py-0.5 hover:bg-red-50" onClick={() => removeRow(it.id)}>Remove</button>
                </div>
              </div>
            ))}
            {!(map[lane] || []).length && <div className="text-xs text-gray-500">Empty</div>}
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="grid md:grid-cols-3 gap-3">
      {labels.map((l) => (
        <div key={l.id} className="rounded-lg border p-3 text-sm">
          ...
          <div className="text-xs">Qty: {l.qty} {l.unit}</div>
          {l.sowBy ? <div className="text-xs">Sow by: {l.sowBy}</div> : null}
          {l.harvestedYear ? <div className="text-xs">Harvested: {l.harvestedYear}</div> : null}
          <div className="text-[11px] mt-1 opacity-70">Tags: {(l.tags || []).join(", ") || "—"}</div>
          <div className="text-[11px] opacity-70">Loc: {l.location || "—"} • Lot: {l.lot || "—"}</div>
        </div>
      ))}
    </div>
  );
...
function SummaryCard({ items, settings, estimateViabilityPct }) {
  const totals = useMemo(() => {
    ...
    items.forEach((it) => {
      ...
      if (it.type === "seed" && estimateViabilityPct(it, settings) < 60) seedRisk.push(it);
    });
    return { byType: Array.from(byType, ([k, v]) => ({ type: k, count: v })), seedRisk };
    ...
    <div className="grid md:grid-cols-2 gap-4">
      <div className="rounded-xl border p-4">
        ...
        {totals.byType.length ? (
          <ul className="text-sm space-y-1">{totals.byType.map((x) => <li key={x.type}><strong className="capitalize">{x.type}</strong>: {x.count}</li>)}</ul>
      ...
      <div className="rounded-xl border p-4">
        ...
        {totals.seedRisk.length ? (
          <ul className="text-sm space-y-1">
            {totals.seedRisk.map((it) => <li key={it.id}><strong>{it.name}{it.cultivar ? ` — ${it.cultivar}` : ""}</strong> • est. {estimateViabilityPct(it, settings).toFixed(0)}%</li>)}
          </ul>
...
function toRestockTask(it) {
  return {
    ...
    description: `Qty: ${it.qty} ${it.unit} in ${it.location || "—"} (threshold ${it.threshold}). Vendor: ${it.source || "—"}.`,
    assignedRole: "Purchasing",
    tags: ["garden", "restock"],
  };
}

function EmptyState({ onAdd, label = "No items yet", sub = "Start with a quick row or paste CSV.", btn = "Add a blank item" }) {
  return (
