...
function yieldForPlanting(p, baselines, lossGlobal) {
  ...
  const loss = {
    pestLossPct: clampPct(
      p?.lossOverrides?.pestLossPct ?? lossGlobal.pestLossPct
    ),
    weatherLossPct: clampPct(
      p?.lossOverrides?.weatherLossPct ?? lossGlobal.weatherLossPct
    ),
    handlingLossPct: clampPct(
      p?.lossOverrides?.handlingLossPct ?? lossGlobal.handlingLossPct
    ),
  };
  const eff =
    (((((100 - loss.pestLossPct) / 100) * (100 - loss.weatherLossPct)) / 100) *
      (100 - loss.handlingLossPct)) /
    100;
  return {
    unit: base.unit,
    raw: qty * perPlant,
...
export default function GardenPlanView() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(LS_KEY, JSON.stringify(beds));
    } catch {}
  }, [beds]);
  useEffect(() => {
    try {
      localStorage.setItem(LS_KEY + ".loss", JSON.stringify(loss));
    } catch {}
  }, [loss]);
  ...
  const updateBed = (id, patch) =>
    setBeds((s) => s.map((b) => (b.id === id ? { ...b, ...patch } : b)));
  const addPlanting = (bedId) =>
    setBeds((s) =>
      s.map((b) =>
        b.id === bedId
          ? {
              ...
              plantings: [
                {
                  ...
                  dtStart: "",
                  daysToMaturity: "",
                  successions: { repeats: 0, everyDays: 14 },
                  interplantOf: "",
                  lossOverrides: {},
                },
                ...(b.plantings || []),
    ...
    setBeds((s) =>
      s.map((b) =>
        b.id === bedId
          ? {
              ...b,
              plantings: (b.plantings || []).filter((p) => p.id !== plantId),
            }
          : b
      )
    );
  ...
  const sendTo = async (target) => {
    ...
    try {
      invokeNBA?.({
        reason: "garden_plan_export",
        context: { target, beds: beds.length },
      });
    } catch {}
  };
  ...
  const planWithAI = async () => {
    ...
    try {
      const res = await automation.runTemplate("garden.plan.optimize", {
        beds,
        loss,
      });
      ...
      automation.emit?.("event", {
        type: "garden/plan_optimize_request",
        payload: { beds, loss },
      });
      ...
      <header className="mb-4 md:mb-6">
        <div className="flex items-center justify-between gap-3">
          <h1 className="text-xl md:text-2xl font-semibold">
            🪴 Garden Plan — Beds • Assign • Schedule
          </h1>
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          ...
          <SectionCard title="Loss Modeling">
            <div className="grid grid-cols-3 gap-3">
              <Field label="Pest %" hint="insects, critters">
                <input
                  ...
                  inputMode="decimal"
                  value={loss.pestLossPct}
                  onChange={(e) =>
                    setLoss((l) => ({
                      ...l,
                      pestLossPct: clampPct(e.target.value),
                    }))
                  }
                />
              </Field>
              <Field label="Weather %" hint="hail, heat">
                <input
                  className="w-full border rounded-md px-2 py-1"
                  inputMode="decimal"
                  value={loss.weatherLossPct}
                  onChange={(e) =>
                    setLoss((l) => ({
                      ...l,
                      weatherLossPct: clampPct(e.target.value),
                    }))
                  }
                />
              </Field>
            ...
            <div className="text-xs text-gray-600 mt-2">
              Beds may override these in Assign → Planting.
            </div>
          </SectionCard>
            ...
            <div className="text-sm">
              ...
              {rotationAlerts.length ? (
                <ul className="list-disc ml-5 space-y-1">
                  {rotationAlerts.map((a) => (
                    <li key={a.id}>
                      <strong>{a.bed}</strong>: {a.message}
                    </li>
                  ))}
                </ul>
              ) : (
          ...
          <div key={b.id} className="rounded-xl border p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Width (ft)">
                <input
                  ...
                  value={b.widthFt}
                  onChange={(e) =>
                    onUpdate(b.id, { widthFt: Number(e.target.value) || 0 })
                  }
                />
              </Field>
              <Field label="Length (ft)">
                <input
                  ...
                  value={b.lengthFt}
                  onChange={(e) =>
                    onUpdate(b.id, { lengthFt: Number(e.target.value) || 0 })
                  }
                />
              </Field>
              <Field label="Rows">
              ...
              <Field label="Zone / Sun">
                <div className="grid grid-cols-2 gap-2">
                  <input
                    className="w-full border rounded-md px-2 py-1"
                    value={b.zone}
                    onChange={(e) => onUpdate(b.id, { zone: e.target.value })}
                    placeholder="Front"
                  />
                  <select
                    className="w-full border rounded-md px-2 py-1"
                    value={b.sun}
                    onChange={(e) => onUpdate(b.id, { sun: e.target.value })}
                  >
              ...
              <Field label="Irrigation zone">
                <input
                  className="w-full border rounded-md px-2 py-1"
                  value={b.irrigationZone}
                  onChange={(e) =>
                    onUpdate(b.id, { irrigationZone: e.target.value })
                  }
                  placeholder="IZ-1"
                />
              </Field>
              <Field
                label="Per-bed loss overrides"
                hint="leave blank to inherit"
              >
                <div className="grid grid-cols-3 gap-2">
                  <input
                    className="w-full border rounded-md px-2 py-1"
                    placeholder={`Pest ${loss.pestLossPct}%`}
                    value={b.pestLossPct ?? ""}
                    onChange={(e) =>
                      onUpdate(b.id, {
                        pestLossPct:
                          e.target.value === ""
                            ? undefined
                            : clampPct(e.target.value),
                      })
                    }
                  />
                  <input
                    className="w-full border rounded-md px-2 py-1"
                    placeholder={`Weather ${loss.weatherLossPct}%`}
                    value={b.weatherLossPct ?? ""}
                    onChange={(e) =>
                      onUpdate(b.id, {
                        weatherLossPct:
                          e.target.value === ""
                  ...
                  <input
                    className="w-full border rounded-md px-2 py-1"
                    placeholder={`Handling ${loss.handlingLossPct}%`}
                    value={b.handlingLossPct ?? ""}
                    onChange={(e) =>
                      onUpdate(b.id, {
                        handlingLossPct:
                          e.target.value === ""
            ...
            <Field label="Notes">
              <input
                className="w-full border rounded-md px-2 py-1"
                value={b.notes}
                onChange={(e) => onUpdate(b.id, { notes: e.target.value })}
                placeholder="e.g., low spot—mulch heavy"
              />
            </Field>

            <div className="text-xs text-gray-600">
              Area ≈ {area.toFixed(1)} ft² • Rows:{" "}
              {rowsPerBed(b.widthFt, b.rows)}
            </div>
            <div className="mt-3">
              <button
                className="rounded-lg border px-2 py-1 text-xs hover:bg-gray-50"
                onClick={() => onAddPlanting(b.id)}
              >
                + Planting
              </button>
            </div>
            {(b.plantings || []).length ? (
              <PlantingsMiniList bed={b} />
            ) : (
  ...
  return (
    <div className="mt-2 text-xs">
      ...
      <div className="flex flex-wrap gap-1">
        {(bed.plantings || []).map((p) => (
          <Chip key={p.id}>
            {p.crop || "crop"} • row {p.row || 1} • {p.qty || 0} pcs
          </Chip>
        ))}
      </div>
    </div>
        ...
        <div key={b.id} className="rounded-xl border p-4">
          <div className="font-semibold mb-2">
            ...
            <span className="text-xs text-gray-600">
              ({b.widthFt}×{b.lengthFt} ft, rows {b.rows})
            </span>
          </div>
          <div className="mb-2">
            <button
            ...
            return (
              <div key={p.id} className="rounded-lg border p-3 mb-3">
                <div className="flex items-center justify-between">
                  <input
                    className="font-medium border rounded-md px-2 py-1 w-40"
                    value={p.crop}
                    onChange={(e) =>
                      onUpdatePlanting(b.id, p.id, {
                        crop: e.target.value.toLowerCase().trim(),
                      })
                    }
                  ...
                  <button
                    className="text-red-600 text-xs rounded-lg border px-2 py-1 hover:bg-red-50"
                    onClick={() => onRemovePlanting(b.id, p.id)}
                  >
                    Remove
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-2">
                  ...
                  <Field label="Spacing (in)">
                    <input
                      className="w-full border rounded-md px-2 py-1"
                      inputMode="decimal"
                      value={p.spacingIn ?? base.spacingIn ?? 6}
                      onChange={(e) =>
                        onUpdatePlanting(b.id, p.id, {
                          spacingIn: Number(e.target.value) || 6,
                        })
                      }
                    />
                  </Field>
                  <Field label="Qty (max recommended shown)">
                    ...
                    <div className="text-[11px] text-gray-500 mt-1">
                      Per row {perRow} • Rows {rCount} • Max ~{maxPlants}
                    </div>
                  </Field>
                  <Field label="Method">
                    <select
                      className="w-full border rounded-md px-2 py-1"
                      value={p.method || base.method || "direct"}
                      onChange={(e) =>
                        onUpdatePlanting(b.id, p.id, { method: e.target.value })
                      }
                    >
                      {["direct", "transplant"].map((m) => (
                  ...
                  <Field label="Days to maturity">
                    <input
                      className="w-full border rounded-md px-2 py-1"
                      inputMode="numeric"
                      value={p.daysToMaturity ?? base.dtm ?? 60}
                      onChange={(e) =>
                        onUpdatePlanting(b.id, p.id, {
                  ...
                  <Field label="Successions (repeats × every N days)">
                    <div className="grid grid-cols-2 gap-2">
                      <input
                        className="w-full border rounded-md px-2 py-1"
                        inputMode="numeric"
                        value={p.successions?.repeats ?? 0}
                        onChange={(e) =>
                          onUpdatePlanting(b.id, p.id, {
                            successions: {
                              ...(p.successions || {}),
                              repeats: Math.max(0, Number(e.target.value) || 0),
                            },
                          })
                        }
                        placeholder="repeats"
                      ...
                      <input
                        className="w-full border rounded-md px-2 py-1"
                        inputMode="numeric"
                        value={p.successions?.everyDays ?? 14}
                        onChange={(e) =>
                          onUpdatePlanting(b.id, p.id, {
                            successions: {
                              ...(p.successions || {}),
                  ...
                  <Field label="Interplant of" hint="optional crop id/name">
                    <input
                      className="w-full border rounded-md px-2 py-1"
                      value={p.interplantOf || ""}
                      onChange={(e) =>
                        onUpdatePlanting(b.id, p.id, {
                          interplantOf: e.target.value,
                        })
                      }
                      placeholder="e.g., carrot under tomato"
                    />
                  </Field>

                  <Field
                    label="Loss overrides (%)"
                    hint="pest | weather | handling"
                  >
                    <div className="grid grid-cols-3 gap-2">
                      <input
                        className="w-full border rounded-md px-2 py-1"
                        value={p.lossOverrides?.pestLossPct ?? ""}
                        onChange={(e) =>
                          onUpdatePlanting(b.id, p.id, {
                            lossOverrides: {
                              ...(p.lossOverrides || {}),
                      ...
                      <input
                        className="w-full border rounded-md px-2 py-1"
                        value={p.lossOverrides?.weatherLossPct ?? ""}
                        onChange={(e) =>
                          onUpdatePlanting(b.id, p.id, {
                            lossOverrides: {
                              ...(p.lossOverrides || {}),
                      ...
                      <input
                        className="w-full border rounded-md px-2 py-1"
                        value={p.lossOverrides?.handlingLossPct ?? ""}
                        onChange={(e) =>
                          onUpdatePlanting(b.id, p.id, {
                            lossOverrides: {
                              ...(p.lossOverrides || {}),
                ...
                <div className="mt-2 text-xs rounded-lg border p-2 bg-gray-50">
                  <div>
                    Yield: raw{" "}
                    <strong>
                      {y.raw.toFixed(y.unit === "heads" ? 0 : 1)} {y.unit}
                    </strong>{" "}
                    → final{" "}
                    <strong>
                      {y.final.toFixed(y.unit === "heads" ? 0 : 1)} {y.unit}
                    </strong>{" "}
                    (eff {(y.eff * 100).toFixed(0)}%)
                  </div>
                  <div>
                    Harvest window ~{" "}
                    <strong>
                      {p.dtStart ? addDaysISO(p.dtStart, y.dtm) : "—"}
                    </strong>
                  </div>
                </div>
              </div>
    ...
    <div className="grid md:grid-cols-2 gap-4">
      <div className="rounded-xl border p-4">
        ...
        {yieldSummary.length ? (
          <ul className="text-sm space-y-1">
            {yieldSummary.map((y) => (
              <li key={y.crop}>
                <strong className="capitalize">{y.crop}</strong>:{" "}
                {y.final.toFixed(y.unit === "heads" ? 0 : 1)} {y.unit}
                <span className="text-xs text-gray-500">
                  {" "}
                  (raw {y.raw.toFixed(y.unit === "heads" ? 0 : 1)})
                </span>
              </li>
            ))}
          </ul>
      ...
      <div className="rounded-xl border p-4">
        ...
        <ul className="text-sm space-y-1">
          {beds.map((b) => (
            <li key={b.id}>
              <strong>{b.name || "Bed"}</strong>: {b.widthFt}×{b.lengthFt} ft •
              rows {b.rows} • zone {b.zone} • sun {b.sun} • IZ{" "}
              {b.irrigationZone}
            </li>
          ))}
        </ul>
      ...
      {beds.map((b) => (
        <div key={b.id} className="rounded-xl border p-4">
          ...
          <div className="text-xs text-gray-600 mb-2">
            {b.widthFt}×{b.lengthFt} ft • rows {b.rows} • {b.sun} sun • {b.zone}{" "}
            • {b.irrigationZone}
          </div>
          {(b.plantings || []).length ? (
            <ul className="text-sm space-y-1">
              {b.plantings.map((p) => {
                ...
                return (
                  <li key={p.id}>
                    <strong className="capitalize">{p.crop || "crop"}</strong> —
                    row {p.row} • {p.qty || 0} pcs • harvest ~{" "}
                    {p.dtStart ? addDaysISO(p.dtStart, y.dtm) : "—"} • final{" "}
                    {y.final.toFixed(y.unit === "heads" ? 0 : 1)} {y.unit}
                  </li>
                );
              })}
  ...
  beds.forEach((b) => {
    (b.plantings || []).forEach((p) => {
      ...
      if (dtStart)
        out.push({
          id: uid("s"),
          kind: p.method || base.method || "direct",
          date: dtStart,
          title: `${p.method || base.method || "Plant"} ${p.crop} — ${b.name}`,
          notes: `Row ${p.row} • ${p.qty} pcs • Spacing ${
            p.spacingIn || base.spacingIn || 6
          }"`,
        });
      // harvest
      if (harvest)
        out.push({
          ...
          kind: "harvest",
          date: harvest,
          title: `Harvest ${p.crop} — ${b.name}`,
          notes: `Target qty ${p.qty || 0}; adjust for loss`,
        });
      ...
      for (let i = 1; i <= reps; i++) {
        ...
        out.push({
          ...
          kind: p.method || base.method || "direct",
          date: start2,
          title: `Succession ${i}: ${p.crop} — ${b.name}`,
          notes: `Row ${p.row} • ${p.qty} pcs`,
        });
        out.push({
          id: uid("s"),
          kind: "harvest",
          date: harvest2,
          title: `Harvest (succ ${i}) ${p.crop} — ${b.name}`,
          notes: `Plan storage; loss modeled downstream`,
        });
      }
    });
  });
    ...
    Object.entries(famCount).forEach(([fam, c]) => {
      if (c > 1 && fam !== "unknown") {
        alerts.push({
          id: uid("rot"),
          bed: b.name || "Bed",
          message: `Multiple ${fam} plantings — rotate next season.`,
        });
      }
    });
  });
  ...
  beds.forEach((b) => {
    ...
    COMPANION_WARN.forEach(([a, c]) => {
      if (crops.includes(a) && crops.includes(c)) {
        alerts.push({
          bed: b.name || "Bed",
          message: `${a} with ${c} is a poor pairing.`,
        });
      }
    });
  });
