...
try {
  IFTTTRuleEditorLazy = React.lazy(() =>
    import("@/components/scheduler/IFTTTRuleEditor.jsx")
  );
} catch (_) {}
...
export default function SchedulerTemplatesPage(props) {
  ...
  let favApi = null;
  try {
    favApi = useFavoriteSchedules ? useFavoriteSchedules(domain) : null;
  } catch (_) {}
  ...
  function refreshFavorites() {
    try {
      setFavSchedules(favApi && favApi.list ? favApi.list() : []);
    } catch (_) {}
  }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (automation && automation.templates && automation.templates.listMine) {
        ...
        if (m && typeof m.then === "function") {
          m.then((res) => {
            if (Array.isArray(res)) setMine(res);
          });
    ...
    const removeT = (p = {}) =>
      setTemplates((cur) => cur.filter((x) => x.id !== p.id));
    const upsertMine = (p = {}) => {
      setMine((cur) => {
        const arr = Array.isArray(cur) ? cur.slice() : [];
        const idx = arr.findIndex((x) => x && x.id === p.id);
  ...
  const filteredSystem = useMemo(
    () =>
      !q
        ? templates
        : templates.filter((t) => JSON.stringify(t).toLowerCase().includes(q)),
    [templates, q]
  );
  const filteredMine = useMemo(
    () =>
      !q
        ? mine
        : mine.filter((t) => JSON.stringify(t).toLowerCase().includes(q)),
    [mine, q]
  );
  ...
  function toggleFavoriteScheduleFromTemplate(t) {
    ...
    if (!key) {
      toastSafe("Template has no schedule.", "warn");
      return;
    }
    try {
  ...
  function openSaveScheduleFromTemplate(t) {
    ...
    setSavePayload({
      ...
      domain,
      trigger: t.trigger,
      defaultTitle: `${t.title || "Template"} — ${prettyScheduleLabel(
        t.trigger
      )}`,
    });
    setSaveOpen(true);
  ...
  function cloneToMine(t) {
    ...
    try {
      if (automation && automation.templates && automation.templates.saveMine) {
        ...
        if (maybe && typeof maybe.then === "function")
          maybe.then(() => toastSafe("Cloned to My Templates."));
      } else {
  ...
  function removeMine(tid) {
    ...
    try {
      if (
        ...
        automation.templates.removeMine
      ) {
        automation.templates.removeMine({ domain, id: tid });
      } else {
        eventBus.emit &&
          eventBus.emit("templates.mine.removed", { id: tid, domain });
      }
    } catch (_) {}
  }
    ...
    try {
      ...
      eventBus.emit &&
        eventBus.emit("automation.rule.export.requested", {
          provider,
          rule: dump,
          filename: `templates-${domain}.json`,
          source: "templates",
        });
    ...
    <main className={WRAP}>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Tab
          id="system"
          label="System"
          icon={<I.Layers className="h-4 w-4" />}
          active={tab === "system"}
        ...
        <Tab
          id="favorites"
          label="Favorite Schedules"
          icon={<I.Star className="h-4 w-4" />}
          active={tab === "favorites"}
          onClick={() => setTab("favorites")}
          count={favSchedules.length}
        />
      </div>
          ...
          <FavoritesScheduleGrid
            ...
            onApply={(fav) => {
              ...
              const rule = {
                ...
                conditions: [],
                actions: [
                  { id: "notify.toast", value: "Running from favorite" },
                ],
          ...
          <div className="mt-3 rounded-2xl border border-gray-200 bg-white p-3">
            {showJSON ? (
              ...
              >
                <IFTTTTRuleEditorLazy
                  ...
                  initialRule={{
                    ...
                    conditions: Array.isArray(editing.conditions)
                      ? editing.conditions
                      : [],
                    actions: Array.isArray(editing.actions)
                      ? editing.actions
                      : [],
                  ...
                  onSave={(rule) => {
                    ...
                    try {
                      if (
                        automation &&
                        automation.templates &&
                        automation.templates.saveMine
                      ) {
                        automation.templates.saveMine({ domain, template: t });
                      } else {
                        eventBus.emit &&
                          eventBus.emit("templates.mine.upserted", t);
                      }
                    } catch (_) {}
              ...
              <div className="text-sm text-gray-600">
                Editor unavailable (lazy import failed).
              </div>
            )}
          </div>
        </div>
        ...
        SaveScheduleModalLazy ? (
          ...
          <InlineSaveSchedule
            ...
            onSaved={(saved) => {
              try {
                eventBus.emit &&
                  eventBus.emit("schedule.saved", { from: "templates", saved });
              } catch (_) {}
              toastSafe("Schedule saved.");
              setSaveOpen(false);
              setSavePayload(null);
        ...
        return (
          <article key={t.id} className={CARD}>
            <div className="flex items-start gap-2">
              <div className="min-w-0">
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  <span className={CHIP}>
                    <I.Clock className="h-3 w-3" /> {scheduleTitle}
                  </span>
              ...
              <div className="ml-auto flex items-center gap-1">
                <button
                  ...
                  onClick={() => onFavoriteSchedule && onFavoriteSchedule(t)}
                >
                ...
                <button
                  ...
                  title="Save schedule"
                  onClick={() => onSaveSchedule && onSaveSchedule(t)}
                >
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              >
                <I.Edit className="h-4 w-4" /> Edit
              </button>
              <button
                className={cx(BTN, VAR.ghost)}
                onClick={() => onClone && onClone(t)}
              >
                <I.Copy className="h-4 w-4" /> Clone
              </button>
              {isMine ? (
                <button
                  className={cx(BTN, VAR.danger)}
                  onClick={() => onDelete && onDelete(t)}
                >
                  <I.Trash className="h-4 w-4" /> Delete
                </button>
              ) : null}
            </div>
          </article>
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <button className={cx(BTN, VAR.subtle)} onClick={onRefresh}>
          <I.Settings className="h-4 w-4" /> Refresh
        </button>
      </div>
      {!items || !items.length ? (
        <div className="grid place-items-center rounded-2xl border border-dashed border-gray-300 p-10 text-sm text-gray-600">
