...
export default function SchedulerSettingsPage(props) {
  ...
  const [favoriteSessions, setFavoriteSessions] = useState(() =>
    favSessApi && favSessApi.list ? favSessApi.list() : []
  );
  const refreshFavSessions = () => {
    try {
      setFavoriteSessions(
        favSessApi && favSessApi.list ? favSessApi.list() : []
      );
  ...
  const [favoriteSchedules, setFavoriteSchedules] = useState(() =>
    favSchedApi && favSchedApi.list ? favSchedApi.list() : []
  );
  const refreshFavSchedules = () => {
    try {
  ...
  const [saveSessionDefaults, setSaveSessionDefaults] = useState({
    id: "__pending__",
    title: "My Session",
  });
  const [saveScheduleOpen, setSaveScheduleOpen] = useState(false);
  const [saveScheduleDefaults, setSaveScheduleDefaults] = useState({
    key: "",
    title: "My Schedule",
    ...
    try {
      if (automation && automation.settings && automation.settings.get) {
        ...
        if (maybe && typeof maybe.then === "function") {
          maybe.then(hydrateFromRuntime);
        } else hydrateFromRuntime(maybe);
      }
    } catch (_) {}
    const onUpdated = (p = {}) => hydrateFromRuntime(p.settings || p);
    try {
      eventBus.on && eventBus.on("scheduler.settings.updated", onUpdated);
    } catch (_) {}
    return () => {
      try {
        eventBus.off && eventBus.off("scheduler.settings.updated", onUpdated);
      } catch (_) {}
    };
  ...
  function saveAll() {
    ...
    const done = () => {
      ...
      try {
        eventBus.emit &&
          eventBus.emit("scheduler.settings.updated", {
            domain,
            settings: payload,
            source: "settings",
          });
    ...
    try {
      if (automation && automation.settings && automation.settings.save) {
        ...
        if (maybe && typeof maybe.then === "function") {
          maybe.then(done).catch(() => toastSafe("Save failed.", "error"));
          return;
        }
        done();
      } else {
        eventBus.emit &&
          eventBus.emit("scheduler.settings.save.requested", {
            payload,
            source: "settings",
            reply: done,
          });
    ...
    try {
      ...
      eventBus.emit &&
        eventBus.emit("automation.rule.export.requested", {
          provider,
          rule: dump,
          filename: `scheduler-settings-${domain}.json`,
          source: "settings",
        });
      toastSafe(`Sent export to ${provider}.`);
    } catch (_) {
      toastSafe("Cloud export failed.", "error");
    }
  }
  function importJSON() {
    try {
  ...
  function removeQuietRange(id) {
    setQuiet((q) =>
      Object.assign({}, q, {
        ranges: (q.ranges || []).filter((x) => x.id !== id),
      })
    );
  }
  ...
  function broadcastSabbath() {
    ...
    try {
      eventBus.emit && eventBus.emit("guard.sabbath.updated", payload);
    } catch (_) {}
    try {
      eventBus.emit &&
        eventBus.emit("ui.toast", {
          message: "Sabbath guard updated.",
          variant: "success",
        });
    } catch (_) {}
  }
  ...
  function setDefaultSession(kind, idOrTitle) {
    setDefaults((d) =>
      Object.assign(
        ...
        d,
        kind === "morning"
          ? { defaultMorningSession: idOrTitle }
          : { defaultEveningSession: idOrTitle }
      )
    );
  }
  function setDefaultSchedule(kind, trigger) {
    setDefaults((d) =>
      Object.assign(
        ...
        d,
        kind === "morning"
          ? { defaultMorningSchedule: trigger }
          : { defaultEveningSchedule: trigger }
      )
    );
  }
  ...
  function toggleFavSession(idOrTitle) {
    ...
    try {
      ...
      toastSafe("Favorite update failed.", "error");
    }
  }
  function openSaveSession(idOrTitle, titleGuess) {
    setSaveSessionDefaults({
      id: idOrTitle || "__pending__",
      title: titleGuess || idOrTitle || "My Session",
    });
    ...
    try {
      eventBus.emit &&
        eventBus.emit("session.save.modal.opened", {
          domain,
          sessionId: idOrTitle || "__pending__",
          source: "settings",
        });
    } catch (_) {}
  }
  ...
  function toggleFavSchedule(trigger) {
    ...
    if (!key) {
      toastSafe("Invalid schedule.", "warn");
      return;
    }
    try {
      if (favSchedApi && favSchedApi.toggleFavorite) {
        const meta = {
          ...
          kind: "schedule",
          trigger,
          title: prettyScheduleLabel(trigger),
        };
        const next = favSchedApi.toggleFavorite(key, meta);
        if (next && typeof next.then === "function") {
          next.then(() => refreshFavSchedules());
        } else {
          refreshFavSchedules();
        }
  ...
  function openSaveSchedule(trigger, title) {
    ...
    setSaveScheduleDefaults({
      key,
      title: title || prettyScheduleLabel(trigger),
      trigger,
    });
  ...
  function applyFavoriteScheduleInRule(fav) {
    ...
    try {
      eventBus.emit &&
        eventBus.emit("automation.rule.prefill.requested", {
          rule,
          domain,
          source: "settings",
        });
        ...
        <div className="ml-auto flex items-center gap-2">
          <details className="relative">
            ...
            <div className="absolute right-0 z-10 mt-2 min-w-[16rem] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl">
              ...
              <MenuItem
                onClick={() => exportToCloud("gdrive")}
                icon={<I.Cloud className="h-4 w-4" />}
                label="Export to Google Drive"
              />
              <MenuItem
                onClick={() => exportToCloud("onedrive")}
                icon={<I.Cloud className="h-4 w-4" />}
                label="Export to OneDrive"
              />
            </div>
          </details>
          <button className={cx(BTN, VAR.primary)} onClick={saveAll}>
            <I.Save className="h-4 w-4" /> Save all
          </button>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-4">
        <section className={GRID2}>
          ...
          <div className={CARD}>
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-gray-900">
                <I.Moon className="inline h-4 w-4 mr-1" /> Quiet Hours
              </h2>
              <label className="text-xs flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setQuiet((q) =>
                      Object.assign({}, q, { enabled: !!e.target.checked })
                    )
                  }
                />
                Enabled
            ...
            <p className="mt-1 text-xs text-gray-600">
              Suppress non-critical notifications and pause schedules during
              these ranges.
            </p>
                ...
                >
                  <div className="flex flex-wrap items-center gap-2">
                    <input
                      ...
                      value={r.label || ""}
                      onChange={(e) =>
                        updateQuietRange(r.id, { label: e.target.value })
                      }
                      placeholder="Label"
                    />
                    <select
                      className={cx(FIELD, "w-48")}
                      value={JSON.stringify(r.days || DAYS)}
                      onChange={(e) =>
                        updateQuietRange(r.id, {
                          days: safeParse(e.target.value, DAYS),
                        })
                      }
                    >
                      ...
                      <option value={JSON.stringify(["Sat", "Sun"])}>
                        Weekends
                      </option>
                      {DAYS.map((d) => (
                    ...
                    <input
                      ...
                      value={r.start || ""}
                      onChange={(e) =>
                        updateQuietRange(r.id, { start: e.target.value })
                      }
                    />
                    ...
                    <input
                      ...
                      value={r.end || ""}
                      onChange={(e) =>
                        updateQuietRange(r.id, { end: e.target.value })
                      }
                    />
                    <div className="ml-auto">
                      <button
                        className={cx(BTN, VAR.ghost, "px-2")}
                        onClick={() => removeQuietRange(r.id)}
                        title="Remove"
                      >
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <button className={cx(BTN, VAR.subtle)} onClick={addQuietRange}>
                <I.Plus className="h-4 w-4" /> Add range
              </button>
              <label className="text-xs flex items-center gap-2">
                <input
                  type="checkbox"
              ...
              <label className="text-xs flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={!!quiet.snoozeSchedules}
                  onChange={(e) =>
                    setQuiet((q) =>
                      Object.assign({}, q, {
                        snoozeSchedules: !!e.target.checked,
                      })
                    )
                  }
                />
          ...
          <div className={CARD}>
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-gray-900">
                <I.Sun className="inline h-4 w-4 mr-1" /> Sabbath Guard
              </h2>
              <label className="text-xs flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={!!sabbath.enabled}
                  onChange={(e) =>
                    setSabbath((s) =>
                      Object.assign({}, s, { enabled: !!e.target.checked })
                    )
                  }
                />
                Enabled
            ...
            <p className="mt-1 text-xs text-gray-600">
              Pause/mute automations around Shabbat based on sunset windows.
            </p>

            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              <div>
                <label className="block text-xs font-medium text-gray-700">
                  Location hint (ZIP/City)
                </label>
                <input
                  className={FIELD}
                  value={sabbath.locationHint || ""}
                  onChange={(e) =>
                    setSabbath((s) =>
                      Object.assign({}, s, { locationHint: e.target.value })
                    )
                  }
                  placeholder="e.g., 10001 or Jerusalem"
                />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-medium text-gray-700">
                    Candle-lighting offset (min)
                  </label>
                  <input
                    type="number"
                    className={FIELD}
                    value={Number(sabbath.candleLightingOffsetMin || 18)}
                    onChange={(e) =>
                      setSabbath((s) =>
                        Object.assign({}, s, {
                          candleLightingOffsetMin: Number(e.target.value || 18),
                        })
                      )
                    }
                  />
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className={FIELD}
                    value={Number(sabbath.havdalahOffsetMin || 42)}
                    onChange={(e) =>
                      setSabbath((s) =>
                        Object.assign({}, s, {
                          havdalahOffsetMin: Number(e.target.value || 42),
                        })
                      )
                    }
                  />
            ...
            <div className="mt-3 flex items-center gap-2">
              <label className="text-xs flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={!!sabbath.muteAutomations}
                  onChange={(e) =>
                    setSabbath((s) =>
                      Object.assign({}, s, {
                        muteAutomations: !!e.target.checked,
                      })
                    )
                  }
                />
              ...
              <div className="ml-auto flex items-center gap-2">
                <button
                  ...
                  onClick={broadcastSabbath}
                >
                  <I.Zap className="h-4 w-4" /> Update guard
                </button>
              </div>
            </div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setSabbath((s) =>
                  Object.assign({}, s, { notes: e.target.value })
                )
              }
              placeholder="Household preferences for observance…"
            />
          </div>
        </section>
        ...
        <section className={GRID2}>
          ...
          <div className={CARD}>
            <h2 className="text-sm font-semibold text-gray-900">
              <I.Settings className="inline h-4 w-4 mr-1" /> Defaults
            </h2>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    className={FIELD}
                    value={defaults.defaultMorningSession || ""}
                    onChange={(e) =>
                      setDefaultSession("morning", e.target.value)
                    }
                    placeholder="Session id or title"
                  />
                  <button
                    className={cx(BTN, VAR.ghost, "px-2")}
                    title="Favorite"
                    onClick={() =>
                      toggleFavSession(defaults.defaultMorningSession)
                    }
                  >
                  ...
                  <button
                    ...
                    onClick={() =>
                      openSaveSession(
                        defaults.defaultMorningSession,
                        "Morning Routine"
                      )
                    }
                  >
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700">
                  Default Evening Session
                </label>
                <div className="flex items-center gap-2">
                  <input
                    className={FIELD}
                    value={defaults.defaultEveningSession || ""}
                    onChange={(e) =>
                      setDefaultSession("evening", e.target.value)
                    }
                    placeholder="Session id or title"
                  />
                  <button
                    ...
                    title="Favorite"
                    onClick={() =>
                      toggleFavSession(defaults.defaultEveningSession)
                    }
                  >
                    <I.Star className="h-4 w-4 text-amber-500" />
                  </button>
                  <button
                    ...
                    onClick={() =>
                      openSaveSession(
                        defaults.defaultEveningSession,
                        "Evening Routine"
                      )
                    }
                  >
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-3">
              <div>
                ...
                <select
                  className={FIELD}
                  value={defaults.defaultNotificationLevel}
                  onChange={(e) =>
                    setDefaults((d) =>
                      Object.assign({}, d, {
                        defaultNotificationLevel: e.target.value,
                      })
                    )
                  }
                >
              ...
              <div className="sm:col-span-2 flex items-end">
                <label className="text-xs flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={!!defaults.inventoryAutoReorder}
                    onChange={(e) =>
                      setDefaults((d) =>
                        Object.assign({}, d, {
                          inventoryAutoReorder: !!e.target.checked,
                        })
                      )
                    }
                  />
          ...
          <div className={CARD}>
            <h2 className="text-sm font-semibold text-gray-900">
              <I.User className="inline h-4 w-4 mr-1" /> Household
            </h2>
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <div>
                <label className="block text-xs font-medium text-gray-700">
                ...
                <input
                  type="number"
                  className={FIELD}
                  value={Number(household.members || 1)}
                  onChange={(e) =>
                    setHousehold((h) =>
                      Object.assign({}, h, {
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setHousehold((h) =>
                      Object.assign({}, h, { timezone: e.target.value })
                    )
                  }
                />
              </div>
              <div>
                ...
                <input
                  type="time"
                  className={FIELD}
                  value={household.wakeTime || "07:00"}
                  onChange={(e) =>
                    setHousehold((h) =>
                      Object.assign({}, h, { wakeTime: e.target.value })
                    )
                  }
                />
              </div>
              <div>
                ...
                <input
                  type="time"
                  className={FIELD}
                  value={household.bedTime || "22:30"}
                  onChange={(e) =>
                    setHousehold((h) =>
                      Object.assign({}, h, { bedTime: e.target.value })
                    )
                  }
                />
              </div>
              <div>
                ...
                <select
                  className={FIELD}
                  value={household.choreRotation}
                  onChange={(e) =>
                    setHousehold((h) =>
                      Object.assign({}, h, { choreRotation: e.target.value })
                    )
                  }
                >
        ...
        <section className={CARD}>
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-900">
              <I.Repeat className="inline h-4 w-4 mr-1" /> Household Rhythms
            </h2>
            <div className="flex items-center gap-2">
              <button
                className={cx(BTN, VAR.subtle)}
                onClick={() =>
                  setRhythms((r) => [
                    { id: uid("rh"), day: "Mon", time: "08:00", session: "" },
                    ...r,
                  ])
                }
              >
                <I.Plus className="h-4 w-4" /> Add
              </button>
              <button className={cx(BTN, VAR.primary)} onClick={publishRhythms}>
                <I.Zap className="h-4 w-4" /> Apply
              </button>
            </div>
          </div>
          <p className="mt-1 text-xs text-gray-600">
            Weekly slots that suggest or auto-start sessions (respecting quiet
            hours & guards).
          </p>
          ...
          <div className="mt-3 space-y-3">
            {!rhythms || !rhythms.length ? (
              <div className="text-xs text-gray-600">
                No rhythms yet. Add a few to seed your week.
              </div>
            ) : (
              rhythms.map((r) => (
                <div
                ...
                >
                  <div className="grid gap-2 sm:grid-cols-[100px_120px_1fr_auto]">
                    <select
                      ...
                      value={r.day}
                      onChange={(e) =>
                        updateRhythm(r.id, { day: e.target.value })
                      }
                    >
                      {DAYS.map((d) => (
                    ...
                    <input
                      ...
                      value={r.time || "08:00"}
                      onChange={(e) =>
                        updateRhythm(r.id, { time: e.target.value })
                      }
                    />
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        value={r.session || ""}
                        onChange={(e) =>
                          updateRhythm(r.id, { session: e.target.value })
                        }
                        placeholder="Session id or title"
                      />
                      <button
                        ...
                        onClick={() => toggleFavSession(r.session)}
                      >
                        <I.Star className="h-4 w-4 text-amber-500" />
                      </button>
                      <button
                        className={cx(BTN, VAR.subtle, "px-2")}
                        title="Save session"
                        onClick={() => openSaveSession(r.session, r.session)}
                      >
                        <I.Save className="h-4 w-4" />
                      </button>
                    </div>
          ...
          <Suspense fallback={null}>
            <SaveSessionModalLazy
              ...
              onSaved={(saved) => {
                try {
                  eventBus.emit &&
                    eventBus.emit("session.saved", { from: "settings", saved });
                } catch (_) {}
        ...
        SaveScheduleModalLazy ? (
          ...
          <InlineSaveSchedule
            ...
            onSaved={(saved) => {
              try {
                eventBus.emit &&
                  eventBus.emit("schedule.saved", { from: "settings", saved });
              } catch (_) {}
              toastSafe("Schedule saved.");
              setSaveScheduleOpen(false);
              refreshFavSchedules();
  ...
  function publishRhythms() {
    ...
    try {
      if (automation && automation.rhythms && automation.rhythms.apply) {
        ...
        if (maybe && typeof maybe.then === "function") {
          maybe.then(() => toastSafe("Rhythms applied."));
          return;
        }
      }
      eventBus.emit &&
        eventBus.emit("scheduler.rhythms.apply.requested", payload);
      toastSafe("Rhythms apply requested.");
    } catch (_) {
      toastSafe("Could not apply rhythms.", "error");
    }
  }
}
    ...
    <div>
      ...
      <div className="mt-1 grid gap-2 sm:grid-cols-[140px_1fr_auto_auto]">
        <select
          ...
          value={t.type}
          onChange={(e) =>
            onChange && onChange({ type: e.target.value, value: "" })
          }
        >
        ...
        {t.type === "time.at" ? (
          <input
            ...
            onChange={(e) =>
              onChange && onChange({ ...t, value: e.target.value })
            }
          />
        ) : (
          <input
        ...
        <button
          ...
          onClick={() => onFavorite && onFavorite(t)}
        >
          <I.Star className="h-4 w-4 text-amber-500" />
        </button>
        <button
          ...
          onClick={() =>
            ...
            onSave(
              t,
              `${label} — ${t.type === "time.at" ? t.value : t.value || ""}`
            )
          }
        >
          <I.Save className="h-4 w-4" />
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        <button className={cx(BTN, VAR.subtle)} onClick={onRefresh}>
          <I.Sparkles className="h-4 w-4" /> Refresh
        </button>
      </div>
      {!items || !items.length ? (
