...
export default function StorehouseInventoryTable({
  ...
  const rowState = (row) => rowActionStates?.[rowKey(row)] || null;
            ...
            {rowState(row)?.status ? (
              <div className="mb-2">
                <span
                  ...
                  role="status"
                >
                  {rowState(row)?.status === "pending"
                    ? "Saving"
                    : rowState(row)?.status === "error"
                      ? "Save failed"
                      : "Saved"}
                </span>
              </div>
            ...
            {editable && (rowState(row)?.status === "error" || rowState(row)?.undo) ? (
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                {rowState(row)?.undo ? (
                  <button
                    type="button"
                    className="rounded border border-sky-300 px-2 py-1 text-xs text-sky-800 hover:bg-sky-50"
                    aria-label={`Undo ${row.itemName}`}
