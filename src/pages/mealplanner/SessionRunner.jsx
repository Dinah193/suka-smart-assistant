...
try {
  InlineToastAnchorLazy = React.lazy(() => import("@/components/toasts/InlineToastAnchor.jsx"));
} catch(_){}
...
try {
  ...
  I = {
    Play: L.Play, Pause: L.Pause, Square: L.Square, Check: L.Check, X: L.X, Clock: L.Clock3,
    Star: L.Star, StarOff: L.StarOff, Save: L.Save, ChevronRight: L.ChevronRight, ChevronDown: L.ChevronDown,
    External: L.ExternalLink, Alert: L.AlertTriangle, Timer: L.Timer, ListChecks: L.ListChecks, Sparkles: L.Sparkles,
    More: L.MoreHorizontal, Edit: L.Pencil, Grip: L.GripVertical, Download: L.Download, Calendar: L.Calendar,
  };
...
export default function SessionRunner(props){
  ...
  const [session, setSession] = useState({
    ...
    pausedAt: null,
    steps: [
      { id: "s1", title: "Check pantry/fridge inventory", done: false, notes: "" },
      { id: "s2", title: "Pick 4–5 mains for the week", done: false, notes: "" },
  ...
  const [isFavSession, setIsFavSession] = useState(!!(favSessApi && favSessApi.isFavorite && favSessApi.isFavorite(initialId)));
  ...
  useEffect(() => {
    ...
    try {
      if (automation && automation.sessions && automation.sessions.get){
        ...
        if (maybe && typeof maybe.then === "function"){
          maybe.then((res)=>{ if (res) setSession((s)=>Object.assign({}, s, res)); });
        } else if (maybe) {
    ...
    return () => {
      try {
        ...
        eventBus.off && eventBus.off("session.control.requested", onSessionControl);
      } catch(_){}
    };
  ...
  function doStart(){
    ...
    setBusy(true);
    const next = Object.assign({}, session, { status:"running", startedAt: session.startedAt || nowISO(), pausedAt:null });
    setSession(next);
    try {
      if (automation && automation.sessions && automation.sessions.start){
        const maybe = automation.sessions.start({ idOrTitle: session.id, domain });
        if (maybe && typeof maybe.then === "function"){ maybe.then(()=>{}); }
      } else {
        eventBus.emit && eventBus.emit("session.start.requested", { idOrTitle: session.id, domain, source:"SessionRunner" });
      }
      eventBus.emit && eventBus.emit("ui.toast", { message:"Session started.", variant:"success", ttl: 2000, session:{ idOrTitle: session.id, domain } });
  ...
  function doConfirmPause(reason){
    setShowPause(false);
    setSession((s)=>Object.assign({}, s, { status:"paused", pausedAt: nowISO(), pauseReason: reason||"" }));
    try {
      if (automation && automation.sessions && automation.sessions.pause){
        const maybe = automation.sessions.pause({ idOrTitle: session.id, domain, reason });
        if (maybe && typeof maybe.then === "function"){ maybe.then(()=>{}); }
      } else {
        eventBus.emit && eventBus.emit("session.pause.requested", { idOrTitle: session.id, domain, reason, source:"SessionRunner" });
      }
  ...
  function doResume(){
    ...
    setSession((s)=>Object.assign({}, s, { status:"running", pausedAt: null }));
    try {
      if (automation && automation.sessions && automation.sessions.resume){
        const maybe = automation.sessions.resume({ idOrTitle: session.id, domain });
        if (maybe && typeof maybe.then === "function"){ maybe.then(()=>{}); }
      } else {
        eventBus.emit && eventBus.emit("session.resume.requested", { idOrTitle: session.id, domain, source:"SessionRunner" });
      }
  ...
  function doStop(){
    ...
    try {
      if (automation && automation.sessions && automation.sessions.stop){
        const maybe = automation.sessions.stop({ idOrTitle: session.id, domain });
        if (maybe && typeof maybe.then === "function"){ maybe.then(()=>{}); }
      } else {
        eventBus.emit && eventBus.emit("session.stop.requested", { idOrTitle: session.id, domain, source:"SessionRunner" });
      }
    } catch(_){}
  ...
  function toggleStepDone(step){
    ...
    try {
      if (automation && automation.steps && automation.steps.setDone){
        automation.steps.setDone({ stepId: step.id, done, domain, sessionIdOrTitle: session.id });
      } else {
        eventBus.emit && eventBus.emit("step.status.updated", { stepId: step.id, done, domain, sessionIdOrTitle: session.id, source:"SessionRunner" });
      }
    } catch(_){}
  }
  ...
  function toggleFavoriteSession(){
    try {
      if (useFavoriteSessions && favSessApi && favSessApi.toggleFavorite){
        const next = favSessApi.toggleFavorite(session.id, { id: session.id, title: session.title, domain });
        if (next && typeof next.then === "function"){
          next.then((v)=> setIsFavSession(!!v));
        } else setIsFavSession(!!next);
      } else {
        setIsFavSession(v=>!v);
        eventBus.emit && eventBus.emit("session.favorite.toggled", { domain, sessionId: session.id, next: !isFavSession, meta:{ title: session.title }, source:"SessionRunner" });
      }
      toastSafe(isFavSession ? "Removed from favorite sessions." : "Added to favorite sessions.");
    } catch(_){ toastSafe("Could not update favorite sessions.", "error"); }
  }
  function openSaveSession(){
    setSaveSessionOpen(true);
    try { eventBus.emit && eventBus.emit("session.save.modal.opened", { domain, sessionId: session.id, source:"SessionRunner" }); } catch(_){}
  }
  ...
  function toggleFavoriteSchedule(){
    ...
    try {
      if (useFavoriteSchedules && favSchedApi && favSchedApi.toggleFavorite){
        const meta = { id: schedule.key, domain, kind:"schedule", trigger: schedule.trigger, title: `${session.title} — ${schedule.label}` };
        ...
        setIsFavSched(v=>!v);
        eventBus.emit && eventBus.emit("schedule.favorite.toggled", { domain, scheduleKey: schedule.key, trigger: schedule.trigger, next: !isFavSched, source:"SessionRunner" });
      }
      toastSafe(isFavSched ? "Removed from favorite schedules." : "Added to favorite schedules.");
    } catch(_){ toastSafe("Could not update favorite schedules.", "error"); }
  }
  function openSaveSchedule(){
    if (!schedule) return;
    setSaveScheduleOpen(true);
    try { eventBus.emit && eventBus.emit("schedule.save.modal.opened", { domain, scheduleKey: schedule.key, trigger: schedule.trigger, source:"SessionRunner" }); } catch(_){}
  }
  ...
  return (
    <main className={WRAP}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              <span className={CHIP}><I.ListChecks className="h-3.5 w-3.5" /> {progress}% complete</span>
              <span className={CHIP}><I.Timer className="h-3.5 w-3.5" /> {session.status}</span>
              {schedule ? <span className={CHIP}><I.Clock className="h-3.5 w-3.5" /> {schedule.label}</span> : null}
            </div>
          </div>

          <div className="ml-auto flex items-center gap-2">
            {/* Session favorites & save */}
            <button className={cx(BTN, VAR.ghost, "px-2")} onClick={toggleFavoriteSession} title={isFavSession ? "Unfavorite session" : "Favorite session"}>
              <I.Star className={cx("h-4 w-4", isFavSession ? "text-amber-500" : "")} />
            </button>
            <button className={cx(BTN, VAR.subtle, "px-2")} onClick={openSaveSession} title="Save session">
            ...
            {schedule ? (
              <>
                ...
                <button className={cx(BTN, VAR.ghost, "px-2")} onClick={toggleFavoriteSchedule} title={isFavSched ? "Unfavorite schedule" : "Favorite schedule"}>
                  <I.Star className={cx("h-4 w-4", isFavSched ? "text-amber-500" : "")} />
                </button>
                <button className={cx(BTN, VAR.subtle, "px-2")} onClick={openSaveSchedule} title="Save schedule">
                  <I.Save className="h-4 w-4" />
                </button>
              </>
            ) : null}
            ...
            {session.status !== "running" ? (
              <button className={cx(BTN, VAR.primary)} onClick={doStart} disabled={busy}><I.Play className="h-4 w-4" /> Start</button>
            ) : (
              <>
                <button className={cx(BTN, VAR.warn)} onClick={doPause}><I.Pause className="h-4 w-4" /> Pause</button>
                <button className={cx(BTN, VAR.danger)} onClick={doStop}><I.Square className="h-4 w-4" /> Stop</button>
              </>
            )}
            {session.status === "paused" ? (
              <button className={cx(BTN, VAR.primary)} onClick={doResume}><I.Play className="h-4 w-4" /> Resume</button>
            ) : null}
          </div>
        </div>
        ...
        <div className={cx(CARD, "p-0")}>
          <header className="flex items-center justify-between border-b border-gray-200 px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              <button className={cx(BTN, VAR.ghost)} onClick={()=>setExpanded((v)=>!v)}>
                {expanded ? <I.ChevronDown className="h-4 w-4" /> : <I.ChevronRight className="h-4 w-4" />} {expanded ? "Collapse" : "Expand"}
              </button>
            </div>
          </header>
          ...
          {expanded ? (
            <ul className="divide-y divide-gray-100">
              {(session.steps||[]).map((step, idx) => (
                <li key={step.id} className="px-4 py-3 hover:bg-gray-50 transition">
                  <div className="flex items-start gap-3">
                    <button
                      className={cx(BTN, step.done ? VAR.success : VAR.subtle, "px-2")}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <div className={cx("text-sm font-medium", step.done ? "line-through text-gray-500" : "text-gray-900")}>
                          {idx+1}. {step.title}
                        </div>
                        <span className={CHIP}>id:{step.id}</span>
                      </div>
                      {step.notes ? <p className="mt-1 text-[12px] text-gray-600">{step.notes}</p> : null}
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        <button
                          className={cx(BTN, VAR.ghost)}
                          onClick={()=>{
                            // example “nudge” toast for the step
                            eventBus.emit && eventBus.emit("ui.toast", {
                              message: step.done ? "You already completed this step." : `Ready for: ${step.title}`,
                              variant: step.done ? "warn" : "info",
                              ttl: "+6s",
                              step: { id: step.id, canUndo: true, canComplete: true },
                              session: { idOrTitle: session.id, domain },
                              actions: [
                                { label:"Mark done", event:"step.complete.requested", variant:"primary" },
                                { label:"Undo", event:"step.undo.requested" },
                              ]
                            });
                          }}
                        >
                        ...
                        <button
                          ...
                          onClick={()=>{
                            const txt = prompt("Add / edit note", step.notes || "");
                            if (txt===null) return;
                            setSession((s)=>{
                              const arr = s.steps.slice();
                              const i = arr.findIndex(x=>x.id===step.id);
                              if (i>=0) arr[i] = Object.assign({}, arr[i], { notes: txt });
                              return Object.assign({}, s, { steps: arr });
                            });
                          }}
                        >
      ...
      {/* Pause Modal */}
      {showPause ? <PauseModal onClose={()=>setShowPause(false)} onConfirm={doConfirmPause} /> : null}
        ...
        SaveSessionModalLazy ? (
          <Suspense fallback={null}>
            <SaveSessionModalLazy
              ...
              onSaved={(saved)=>{
                try { eventBus.emit && eventBus.emit("session.saved", { from:"SessionRunner", saved }); } catch(_){}
        ...
        SaveScheduleModalLazy ? (
          <Suspense fallback={null}>
            <SaveScheduleModalLazy
              ...
              onSaved={(saved)=>{
                try { eventBus.emit && eventBus.emit("schedule.saved", { from:"SessionRunner", saved }); } catch(_){}
    ...
    >
      <div className="w-[95vw] max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <div className="text-base font-semibold text-gray-900"><I.Pause className="inline h-4 w-4 mr-1" /> Pause session</div>
          <button className={cx(BTN, VAR.ghost, "px-2")} onClick={onClose} aria-label="Close"><I.X className="h-4 w-4" /></button>
        </div>
        <p className="mt-1 text-sm text-gray-600">Optionally add a short note so we can resume smoothly.</p>

        <label className="mt-4 block text-sm font-medium text-gray-700">Reason (optional)</label>
        <input className={FIELD} value={reason} onChange={(e)=>setReason(e.target.value)} placeholder="e.g., got a call, oven preheating" />

        <div className="mt-6 flex justify-end gap-2">
          <button className={cx(BTN, VAR.ghost)} onClick={onClose}>Cancel</button>
          <button className={cx(BTN, VAR.warn)} onClick={()=>onConfirm && onConfirm(reason)}>
...
function InlineSaveSession(props){
  ...
  function submit(){
    ...
    try {
      ...
      try { eventBus.emit && eventBus.emit("session.save.requested", { payload, source: "SessionRunner" }); } catch(_){}
...
function InlineSaveSchedule(props){
  ...
  function submit(){
    ...
    try {
      ...
      try { eventBus.emit && eventBus.emit("schedule.save.requested", { payload, source: "SessionRunner" }); } catch(_){}
      props.onSaved && props.onSaved(payload);
    } finally { setBusy(false); }
  }
  return (
