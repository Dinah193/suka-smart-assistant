...
let CollectInboxStore = {};
try {
  CollectInboxStore = require("@/store/CollectInboxStore");
} catch {}
...
let ProjectStore = {};
try {
  ProjectStore = require("@/store/ProjectStore");
} catch {}
let TripStore = {};
try {
  TripStore = require("@/store/TripStore");
} catch {}
let WorkoutStore = {};
try {
  WorkoutStore = require("@/store/WorkoutStore");
} catch {}
let ClosetStore = {};
try {
  ClosetStore = require("@/store/ClosetStore");
} catch {}
let ReadingListStore = {};
try {
  ReadingListStore = require("@/store/ReadingListStore");
} catch {}
let DealsStore = {};
try {
  DealsStore = require("@/store/DealsStore");
} catch {}
let BudgetStore = {};
try {
  BudgetStore = require("@/store/BudgetStore");
} catch {}
let BookmarkStore = {};
try {
  BookmarkStore = require("@/store/BookmarkStore");
} catch {}
...
const shellfishHint = (text = "") =>
  /(shrimp|prawn|crab|lobster|clam|oyster|mussel|shellfish)/i.test(
    `${text}`.toLowerCase()
  );
...
const dedupeByUrlOrTitle = (items) => {
  ...
  return (items || []).filter((x) => {
    const key =
      (x.link || x.url || "").toLowerCase() || (x.title || "").toLowerCase();
    if (!key) return true;
...
const classifyPin = (pin, board) => {
  const blob = `${pin.title || ""} ${pin.description || ""} ${pin.link || ""} ${
    board?.name || ""
  }`.toLowerCase();
  ...
  if (
    has(
      /recipe|dinner|lunch|breakfast|snack|meal|cook|bake|kitchen|instant pot|air fryer|sous vide/
    )
  )
    return "recipe";
  if (has(/workout|fitness|yoga|hiit|strength|run|gym|routine/))
    return "fitness";
  if (
    has(/travel|itinerary|trip|packing|hotel|airbnb|europe|vacation|wanderlust/)
  )
    return "travel";
  if (
    has(
      /project|diy|hack|build|how to|organize|closet system|garage|laundry|pantry/
    )
  )
    return "project";
  if (
    has(
      /decor|interior|home office|living room|bedroom|nursery|garden|patio|craft/
    )
  )
    return "project";
  if (has(/style|outfit|capsule|closet|wardrobe|fashion|hairstyle|makeup/))
    return "style";
  if (has(/book|reading list|read|novel|library|study|notes|lesson|curriculum/))
    return "reading";
  if (has(/deal|sale|coupon|promo|wishlist|shopping|buy|gift guide/))
    return "shopping";
  ...
  if (
    (pin.link || "").match(
      /allrecipes|bonappetit|seriouseats|thekitchn|epicurious|foodnetwork|tasty/
    )
  )
    return "recipe";
...
const Pinterest = {
  ...
  async auth() {
    try {
      return await PinterestAPI.auth();
    } catch {
  ...
  async listBoards() {
    try {
      return await PinterestAPI.listBoards();
    } catch {
      return [
  ...
  async listPins(boardId, cursor = null) {
    try {
      return await PinterestAPI.listPins(boardId, cursor);
    } catch {
      ...
      return {
        items: demo,
        nextCursor: (cursor || 0) + 1 < 4 ? (cursor || 0) + 1 : null,
      };
    }
  },
  async resolvePin(url) {
    try {
      return await PinterestAPI.resolvePin(url);
    } catch {
      return {
        id: uid("pin"),
...
export default function PinterestImportWizard() {
  ...
  const connectPinterest = async () => {
    ...
    try {
      ...
      setToast({
        type: "info",
        msg: "Using demo boards. Connect later in Settings.",
      });
    } finally {
      setBusy(false);
    }
  ...
  const primeBoard = async (board) => {
    ...
    try {
      ...
      const typed = (items || []).map((p) => ({
        ...p,
        __type: classifyPin(p, board),
      }));
  ...
  const loadMore = async () => {
    ...
    setBusy(true);
    try {
      const { items, nextCursor } = await Pinterest.listPins(
        activeBoard.id,
        cursor
      );
      const typed = (items || []).map((p) => ({
        ...p,
        __type: classifyPin(p, activeBoard),
      }));
  ...
  /* ------------------------------ Routing helpers ------------------------------ */
  const resolveDestForPin = (pin) =>
    pinDest[pin.id] || bulkDest[pin.__type] || "bookmarks";
  ...
  const importToModule = async (pin, destKey) => {
    ...
    eventBus.emit("recipe.scrape.requested", {
      at: nowIso(),
      url: pin.link,
      source: "pinterest",
    });
    ...
    switch (destKey) {
      case "vault":
        try {
          RecipeStore?.upsert?.(baseCard);
          eventBus.emit("recipe.vault.updated", { id: baseCard.id });
          return "saved";
        } catch {
          return "error";
        }
      ...
      case "batch":
        try {
          addToBatch?.({ ...baseCard, from: "PinterestImport" });
          eventBus.emit("batch.queue.added", { recipe: baseCard });
          return "batched";
        } catch {
          return "error";
        }
      case "projects":
        try {
          ProjectStore?.upsert?.({ ...baseCard, status: "idea" });
          eventBus.emit("projects.updated", { id: baseCard.id });
          return "projected";
        } catch {
          return "error";
        }
      case "trips":
        try {
          TripStore?.stageLink?.(baseCard) || TripStore?.upsert?.(baseCard);
          eventBus.emit("trips.updated", { id: baseCard.id });
          return "trip";
        } catch {
          return "error";
        }
      case "workouts":
        try {
          WorkoutStore?.upsert?.(baseCard);
          eventBus.emit("workouts.updated", { id: baseCard.id });
          return "workout";
        } catch {
          return "error";
        }
      case "closet":
        try {
          ClosetStore?.upsert?.(baseCard);
          eventBus.emit("closet.updated", { id: baseCard.id });
          return "closet";
        } catch {
          return "error";
        }
      case "reading":
        try {
          ReadingListStore?.upsert?.(baseCard);
          eventBus.emit("reading.updated", { id: baseCard.id });
          return "reading";
        } catch {
          return "error";
        }
      case "deals":
        try {
          DealsStore?.upsert?.(baseCard);
          eventBus.emit("deals.updated", { id: baseCard.id });
          return "deal";
        } catch {
          return "error";
        }
      case "budget":
        try {
          BudgetStore?.upsertTemplate?.(baseCard) ||
            BudgetStore?.upsert?.(baseCard);
          eventBus.emit("budget.updated", { id: baseCard.id });
          return "budget";
        } catch {
          return "error";
        }
      ...
      default:
        try {
          BookmarkStore?.upsert?.(baseCard);
          eventBus.emit("bookmarks.updated", { id: baseCard.id });
          return "bookmarked";
        } catch {
          return "error";
        }
  ...
  const importSelected = async () => {
    ...
    if (isSabbath) {
      setToast({
        type: "warning",
        msg: "Sabbath hands-off is active. You can stage items but not import.",
      });
      return;
    }
    ...
    try {
      for (const pinId of selected) {
        ...
        emitProgress?.({
          id: "pinterest.import",
          at: nowIso(),
          message: `Importing ${p.title || p.link}…`,
        });
      ...
      setToast({
        type: "success",
        msg: `Imported ${ok}/${results.length} item(s).`,
        actionLabel: "Undo",
        onAction: () => undo(),
      });
      setStep(3);
  ...
  const undo = () => {
    ...
    (last.payload || []).forEach((r) => {
      ...
      switch (r.destKey) {
        case "vault":
          emit("recipe.vault.remove.requested");
          break;
        case "inbox":
          emit("collect.inbox.remove.requested");
          break;
        case "batch":
          emit("batch.queue.remove.requested");
          break;
        case "projects":
          emit("projects.remove.requested");
          break;
        case "trips":
          emit("trips.remove.requested");
          break;
        case "workouts":
          emit("workouts.remove.requested");
          break;
        case "closet":
          emit("closet.remove.requested");
          break;
        case "reading":
          emit("reading.remove.requested");
          break;
        case "deals":
          emit("deals.remove.requested");
          break;
        case "budget":
          emit("budget.remove.requested");
          break;
        default:
          emit("bookmarks.remove.requested");
          break;
      }
    });
  ...
  const PinCard = ({ pin }) => {
    ...
    const hint =
      pin.__type === "recipe" && shellfishHint(`${pin.title} ${pin.link}`);
    const destKey = resolveDestForPin(pin);

    const perPinDestChange = (val) =>
      setPinDest((m) => ({ ...m, [pin.id]: val }));
    ...
    return (
      ...
      >
        ...
        <div className="absolute left-2 top-2 flex items-center gap-2">
          ...
          {hint && (
            <span className="rounded bg-amber-400/90 px-1.5 py-0.5 text-[10px] text-black">
              shellfish?
            </span>
          )}
        </div>
        <div className="absolute bottom-0 w-full bg-gradient-to-t from-black/60 to-transparent p-2 text-xs text-white">
          ...
          <div className="mt-1 flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) => {
                e.stopPropagation();
                perPinDestChange(e.target.value);
              }}
            >
      ...
      <header className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            size="sm"
            onClick={() =>
              eventBus.emit("ui.open", { panel: "CollectOrganize" })
            }
          >
            Collect & Organize
          </Button>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <p className="mt-1 text-xs text-zinc-600">
                  Sign in to browse boards and bulk-import pins. Or paste any
                  pin/board URL to start without signing in.
                </p>
                <div className="mt-3 flex items-center gap-2">
                  <Button onClick={connectPinterest} disabled={busy}>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-zinc-600">
                  Paste a pin or board URL anywhere on this page to stage it.
                  We’ll resolve and classify it.
                </p>
                ...
                {typeOptions.map((t) => (
                  <div key={t.key} className="flex items-center gap-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setBulkDest((d) => ({ ...d, [t.key]: e.target.value }))
                      }
                      title={`Route ${t.label}`}
                    >
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-sm">
              Imported <span className="font-semibold">{selected.length}</span>{" "}
              item(s).
            </div>
...
(function runPinterestImportWizardAllBoardsTestsOnce() {
  ...
  const expect = (cond, msg) =>
    cond
      ? console.log("[PinterestImport ALL TEST PASS]", msg)
      : console.error("[PinterestImport ALL TEST FAIL]", msg);

  // Classifier smoke tests
  expect(
    classifyPin({ title: "Paris Itinerary" }, { name: "Euro Trip" }) ===
      "travel",
    "classify travel"
  );
  expect(
    classifyPin({ title: "30-min Dumbbell Workout" }, { name: "" }) ===
      "fitness",
    "classify fitness"
  );
  expect(
    classifyPin({ title: "DIY Closet System" }, { name: "" }) === "project",
    "classify project"
  );
  expect(
    ["recipe", "bookmark"].includes(
      classifyPin({ link: "https://bonappetit.com/x" }, { name: "" })
    ),
    "recipe domain bias"
  );
})();
