...
export function analyzeCouponHistory(historyRows, opts = {}) {
  ...
  if (!Array.isArray(historyRows) || !historyRows.length) {
    return {
      samples: 0,
      status: "insufficient",
      tips: ["No local history yet. Scan/compare to learn cycles."],
    };
  }
  ...
  if (samples < 2) {
    return {
      samples,
      status: "insufficient",
      tips: ["Need more than one observed window to infer cadence."],
    };
  }
  ...
  for (let i = 1; i < rows.length; i++)
    gaps.push(rows[i].tEnd - rows[i - 1].tEnd);
  ...
  const weightedMedian = median(
    gaps.flatMap((g, i) => Array(Math.max(1, Math.round(weights[i]))).fill(g))
  );
  ...
  const iqr = (q3 ?? 0) - (q1 ?? 0);
  const jitterRatio =
    iqr && weightedMedian ? clamp(iqr / weightedMedian, 0, 2) : 1;
  ...
  tips.push(
    `Deals tend to cluster on ${weekdayName(
      bestWeekday
    )}s; strongest in ${monthName(bestMonth)}${
      bestWOM > 0 ? ` (week ${bestWOM})` : ""
    }.`
  );
...
async function readHistory({ store, brand, category }, limit = 200) {
  ...
  try {
    ...
    if (
      DexieDB.couponHistory.schema.indexes?.includes("[store+brand+category]")
    ) {
...
async function removeInsightFavorite(userId, key) {
  ...
  try {
    const row = await DexieDB.favorites
      .where({ userId: userId || "anon", type: "insight.cycle", key })
      .first();
...
export default function useCycleInsights(query, opts = {}) {
  ...
  const compute = useCallback(
    async (reason = "auto") => {
      ...
      const fresh =
        cached && Date.now() - (cached.when || 0) < ttlMs && !forceFresh;
      if (fresh) {
        setInsights(cached.payload);
        setStatus(cached.payload?.status || "ok");
        setError(null);
        eventBus.emit("cycle:insights:cache:hit", {
          key,
          query: stable,
          reason,
        });
      ...
      setStatus("loading");
      setError(null);
      eventBus.emit("cycle:insights:calc:start", {
        key,
        query: stable,
        reason,
      });
      ...
      try {
        ...
        eventBus.emit("cycle:insights:calc:success", {
          key,
          query: stable,
          payload,
          reason,
        ...
        if (emitSchedulerHint && payload?.nextWindow?.expectedStartISO) {
          ...
          if (soon) {
            eventBus.emit("nba:hint", {
              ...
              message: `Upcoming ${stable.brand || "brand"} window at ${
                stable.store || "store"
              } expected soon.`,
        ...
        setStatus("error");
        setError(e);
        eventBus.emit("cycle:insights:calc:error", {
          key,
          query: stable,
          error: String(e),
        });
  ...
  useEffect(() => {
    const onSuccess = (payload) => {
      ...
      const sameStore =
        !stable.store || toStr(payload.query.store) === stable.store;
      const sameBrand =
        !stable.brand || toStr(payload.query.brand) === stable.brand;
