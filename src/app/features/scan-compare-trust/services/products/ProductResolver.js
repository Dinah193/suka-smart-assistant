...
(function () {
  ...
  try { var eb = require("@/services/eventBus"); eventBus = (eb?.default||eb?.eventBus||eb)||eventBus; } catch(_e){}

  var DexieDB = null;
  try { DexieDB = require("@/db")?.default || require("@/db"); } catch(_e){}

  var useStoresDirectory = null;
  try { useStoresDirectory = require("@/features/scan-compare-trust/stores/useStoresDirectory").default || null; } catch(_e){}

  var safetyEval = null; // named export from useSafetyPrefs
  try { safetyEval = require("@/features/scan-compare-trust/stores/useSafetyPrefs").evaluateProduct || null; } catch(_e){}

  var priceBookRecord = null;
  try { priceBookRecord = require("@/features/scan-compare-trust/stores/usePriceBook").priceBookRecord || null; } catch(_e){}

  var nanoid = (len=6)=>Math.random().toString(36).slice(2, 2+len);
  ...
  function timeout(promise, ms, label="timeout") {
    return new Promise((resolve) => {
      ...
      promise.then(v=>{ clearTimeout(t); resolve(v); }).catch(e=>{ clearTimeout(t); resolve({ ok:false, error:String(e) }); });
    });
  }
  ...
  function lsReadAll(){ try{ const v = localStorage.getItem(LS_CACHE_KEY); return v? JSON.parse(v):{}; }catch{ return {}; } }
  function lsWriteAll(obj){ try{ localStorage.setItem(LS_CACHE_KEY, JSON.stringify(obj)); }catch{} }
  ...
  async function cachePut(key, value, ttlMs) {
    if (DexieDB?.productsCache) {
      try { await DexieDB.productsCache.put({ key, value, ttlMs: ttlMs||0, updatedAt: Date.now() }); } catch {}
      return;
    }
    const all = lsReadAll();
  ...
  function registerProviderAdapter(key, fn, meta) {
    ...
    registry.set(key, {
      fn,
      trust: (meta && typeof meta.trust === "number") ? clamp(meta.trust, 0, 1) : 0.6,
      timeoutMs: (meta && meta.timeoutMs) || 6000,
      rateLimitMs: (meta && meta.rateLimitMs) || 300, // minimal throttling
      label: (meta && meta.label) || key,
    });
    eventBus.emit?.("product:provider:registered", { key, label: meta?.label || key });
    return true;
  }

  function listProviders(){ return Array.from(registry.keys()); }
  ...
  async function callProvider(key, query, ctx) {
    ...
    if (delta < meta.rateLimitMs) await new Promise(r => setTimeout(r, meta.rateLimitMs - delta));
    ...
    if (res && res.ok && res.product) {
      res.product.source = res.product.source || { key, label: meta.label, trust: meta.trust };
    }
    return res;
  }
  ...
  function cleanArr(arr){ return uniq((arr||[]).map(x=>toStr(x)).filter(Boolean)); }
  ...
  function mergeProducts(products) {
    ...
    for (const p of products) {
      ...
      const src = p.product.source || { key:"unknown", label:"Unknown", trust:0.5 };
      const meta = { key: src.key, label: src.label, trust: num(src.trust) ?? 0.6 };
      const fieldsUsed = {};

      // UPC / GTIN
      const upc = canonicalUPC(p.product.upc || p.product.gtin || p.product.ean);
      if (upc && !merged.upc) { merged.upc = upc; fieldsUsed.upc = true; }
      if (!merged.gtin && p.product.gtin) { merged.gtin = toStr(p.product.gtin); fieldsUsed.gtin = true; }
      if (!merged.ean && p.product.ean) { merged.ean = toStr(p.product.ean); fieldsUsed.ean = true; }

      // brand/name/variant
      [["brand","brand"],["name","name"],["variant","variant"]].forEach(([f,k])=>{
      ...
      if (qty && !merged.sizeQty) { merged.sizeQty = qty; fieldsUsed.sizeQty = true; }
      if (unit && !merged.sizeUnit) { merged.sizeUnit = unit; fieldsUsed.sizeUnit = true; }

      // arrays
      merged.ingredients = uniq([...(merged.ingredients||[]), ...cleanArr(p.product.ingredients)]);
      merged.categories  = uniq([...(merged.categories||[]),  ...cleanArr(p.product.categories)]);
      merged.images      = uniq([...(merged.images||[]),      ...cleanArr(p.product.images)]);
      merged.warnings    = uniq([...(merged.warnings||[]),    ...cleanArr(p.product.warnings)]);

      // offers/price
      const offers = Array.isArray(p.product.offers) ? p.product.offers : (p.product.price ? [ { store: p.product.price.store || null, price: num(p.product.price.amount), currency: p.product.price.currency || "USD", sizeQty: num(p.product.price.sizeQty)||qty||null, sizeUnit: normUnit(p.product.price.sizeUnit||unit), promo: !!p.product.price.promo } ] : []);
      ...
      attrib.push({ key: meta.key, label: meta.label, trust: meta.trust, fields: fieldsUsed, raw: p.raw || null });
    }
    ...
    if (merged.offers.length) {
      const best = merged.offers.filter(o=>Number.isFinite(o.price)).sort((a,b)=>a.price-b.price)[0];
      merged.price = best ? { amount: best.price, currency: best.currency, store: best.store, sizeQty: best.sizeQty, sizeUnit: best.sizeUnit, promo: best.promo } : null;
    }
  ...
  async function resolveInternal(input, ctx, providers) {
    ...
    const chosen = providers && providers.length ? providers : Array.from(registry.keys());
    const calls = chosen.map(k => callProvider(k, input, ctx));
    const results = await Promise.all(calls);

    const ok = results.filter(r => r && r.ok && r.product);
    if (!ok.length) {
      const fail = results.map((r,i)=>({ provider: chosen[i], err: r?.error||"no-result" }));
      eventBus.emit?.("product:resolve:none", { input, failures: fail });
      return { ok:false, error:"no-provider-returned-product", failures: fail };
    }
    ...
    eventBus.emit?.("source:attribution", {
      ...
      items: attribution.map(a => ({
        providerKey: a.key, label: a.label, trust: a.trust, fields: a.fields
      })),
    });
    ...
    try {
      if (safetyEval) {
        const safe = safetyEval({
          upc: merged.upc,
          brand: merged.brand, store: input.store || merged.price?.store || null,
          ingredients: merged.ingredients,
          rawLabelText: null,
          categories: merged.categories,
        }, null, {});
        eventBus.emit?.("product:safety:precheck", { upc: merged.upc, ok: safe.ok, score: safe.score, violations: safe.violations });
      }
    } catch(_e){}

    // Invite coupons step to fetch options — the listener builds provider queries
    eventBus.emit?.("coupons:fetch:requested", {
      filters: { upc: merged.upc, brand: merged.brand, store: input.store || null, adapterKey: input.adapterKey || null },
  ...
  function createProductResolver(deps) {
    ...
    return {
      ...
      resolve: async function(input, ctx) {
        ...
        const providers = Array.isArray(context.providers) && context.providers.length ? context.providers : cfg.providers;
        return await resolveInternal(input, context, providers);
      },

      /** Register a provider adapter at runtime. */
      registerProviderAdapter: function(key, fn, meta){ return registerProviderAdapter(key, fn, meta); },

      /** List registered providers. */
      listProviders: function(){ return listProviders(); },

      /** Evict a cache entry for a given input (e.g., force refresh). */
      evictCache: async function(input){
        const key = keyFor(input);
        if (DexieDB?.productsCache) {
          try { await DexieDB.productsCache.delete(key); } catch {}
        } else {
          const all = lsReadAll(); delete all[key]; lsWriteAll(all);
        }
  ...
  function __exampleLocalProvider(query) {
    ...
    return Promise.resolve({
      ...
      product: {
        ...
        categories: ["unknown"],
        images: [],
        offers: query.store ? [{ store: query.store, price: 1.99, currency: "USD", sizeQty: 1, sizeUnit: "ea", promo: false }] : [],
  ...
  try { (globalThis || window).ProductResolver = api; } catch(_e){}
})();
