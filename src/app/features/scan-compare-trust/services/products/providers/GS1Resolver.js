...
(function () {
  ...
  function buildGs1Categories(meta) {
    ...
    if (meta.packagingLevel != null)
      out.push(`gtin-pack:${meta.packagingLevel}`);
    return out;
  }
  ...
  async function lookup(query = {}, ctx = {}) {
    ...
    const product = toNormalizedGs1(code, {
      check,
      issuerCountry,
      symbology,
      packagingLevel,
  ...
  function annotateProduct(product) {
    ...
    const gtin14 =
      product.gtin && product.gtin.length === 14 ? product.gtin : null;
    const rawCode =
      gtin14 ||
      (product.ean && product.ean.length === 13 ? product.ean : null) ||
      upc;
    ...
    const gs1Cats = buildGs1Categories({
      issuerCountry,
      symbology,
      packagingLevel,
    });
    ...
    if (check.valid === false) {
      product.warnings = Array.from(
        new Set([
          ...(product.warnings || []),
          `bad-check-digit: expected ${check.expected}, got ${check.actual}`,
        ])
      );
    }
    return product;
  ...
  try {
    (globalThis || window).GS1Resolver = out;
  } catch (_e) {}
})();
