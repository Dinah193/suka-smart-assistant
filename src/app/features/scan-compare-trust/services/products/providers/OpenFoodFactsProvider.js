...
(function () {
  ...
  function canonicalBarcode(input) {
    ...
    if (s.length === 12 || s.length === 13 || s.length === 8 || s.length === 14)
      return s;
    return null;
  }
  ...
  function bestImage(p) {
    const main =
      p?.image_url || p?.image_front_url || p?.image_small_url || null;
    const arr = [];
  ...
  function parseIngredients(p) {
    if (Array.isArray(p?.ingredients) && p.ingredients.length) {
      return p.ingredients
        .map((i) => lc(i.text || i.id || i.origin || i.percent_estimate || i))
        .filter(Boolean);
    }
  ...
  function parseWarnings(p) {
    ...
    if (Array.isArray(p?.additives_tags)) {
      for (const t of p.additives_tags)
        out.push("additive: " + lc(t).replace(/^en:/, ""));
    }
  ...
  function parseNutrition(p) {
    const label = p?.nutriscore_grade
      ? "nutri-score: " + String(p.nutriscore_grade).toUpperCase()
      : null;
  ...
  try {
    (globalThis || window).OpenFoodFactsProvider = out;
  } catch (_e) {}
})();
