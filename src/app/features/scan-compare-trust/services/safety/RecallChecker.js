...
try {
  const eb =
    (await import("@/services/events/eventBus")).default ??
    (await import("@/services/events/eventBus")).eventBus;
...
try {
  const sp = await import(
    "@/app/features/scan-compare-trust/stores/useSafetyPrefs"
  );
...
function lsReadAll() {
  try {
    const v = localStorage.getItem(LS_ALL);
    return v ? JSON.parse(v) : { items: {}, meta: {} };
  } catch {
    return { items: {}, meta: {} };
  }
}
function lsWriteAll(obj) {
  try {
    localStorage.setItem(LS_ALL, JSON.stringify(obj));
  } catch {}
}
...
async function dbBulkPutRecalls(list) {
  ...
  try {
    await DexieDB.recalls.bulkPut(
      list.map((r) => ({
        id: r.id,
        sourceKey: r.source?.key || "unknown",
        value: r,
...
async function dbMetaGet(key) {
  if (DexieDB?.kv) {
    try {
      const row = await DexieDB.kv.get({ space: "recalls:meta", key });
...
async function dbReplaceAll(list) {
  if (DexieDB?.recalls) {
    try {
      await DexieDB.recalls.clear();
      await dbBulkPutRecalls(list);
      return;
    } catch {}
  }
...
export function registerSource(key, fn, meta) {
  registry.set(key, {
    ...
    ttlMs: meta?.ttlMs || 24 * 60 * 60 * 1000,
  });
  eventBus.emit?.("safety:recalls:source:registered", {
    key,
    label: meta?.label || key,
  });
  return true;
...
export function normalizeRecall(sourceKey, raw) {
  ...
  const upcs = uniq(
    [...(raw.upcs || []), ...(raw.UPCs || [])].map(toUpc12)
  ).filter(Boolean);
  const gtins = uniq(
    [...(raw.gtins || []), ...(raw.GTINs || []), ...(raw.upcs || [])]
      .map((s) => String(s || "").replace(/[^0-9]/g, ""))
      .filter((s) => s.length === 14)
  );
  const brands = uniq(
    [...(raw.brands || []), ...(raw.brand_names || []), raw.brand]
      .flat()
      .map(lc)
      .filter(Boolean)
  );
  const products = uniq(
    [...(raw.productNames || []), raw.product_description, raw.product, title]
      .flat()
      .map((v) => lc(toStr(v)))
      .filter(Boolean)
  );
  ...
  return {
    ...
    source: {
      key: sourceKey,
      label: raw._sourceLabel || sourceKey,
      url: raw.url || null,
    },
    title,
    hazard: hazard || null,
    classes: uniq(
      [...(raw.classes || []), raw.classification, raw.category, raw.class]
        .map(lc)
        .filter(Boolean)
    ),
    upcs,
    ...
    lots: uniq(
      (raw.lots || raw.codes || []).map(toStr).map(lc).filter(Boolean)
    ),
    states: uniq(
      (raw.states || raw.distribution || []).map(lc).filter(Boolean)
    ),
    startISO: startISO || null,
    endISO: endISO || null,
    lastUpdatedISO: raw.lastUpdatedISO || nowISO(),
    raw: {
      id: raw.id || raw.recall_number || raw.recallID || null,
      url: raw.url || null,
    },
  };
}
...
export async function refreshSource(key, opts = {}) {
  ...
  try {
    const res = await reg.fn({
      sinceISO: meta.sinceISO,
      etag: meta.etag,
      lastModified: meta.lastModified,
      ttlMs: reg.ttlMs,
    });
    if (!res || res.ok === false) {
      eventBus.emit?.("safety:recalls:refresh:error", {
        sourceKey: key,
    ...
    if (opts.replaceAll) {
      await dbReplaceAll(mapped);
    } else {
      await dbMerge(mapped);
    }
    ...
    eventBus.emit?.("safety:recalls:refresh:done", {
      sourceKey: key,
      added: mapped.length,
    });
    eventBus.emit?.("safety:recalls:updated", {
      count: mapped.length,
      sourceKey: key,
    });
...
export async function checkProductAgainstRecalls(product, options = {}) {
  ...
  for (const r of all) {
    ...
    if (upc && r.upcs?.includes(upc)) {
      score += 0.65;
      reasons.push("upc-exact");
    }
    if (upc && r.gtins?.some((g) => g.endsWith(upc))) {
      score += 0.35;
      reasons.push("gtin-suffix");
    }

    if (brandTokens.length && r.brands?.length) {
      const bscore = jaccard(brandTokens, r.brands.flatMap(tokenize));
      if (bscore > 0.1) {
        score += clamp(bscore, 0, 0.3);
        reasons.push("brand-sim");
      }
    }
    ...
    if (nameTokens.length && r.productNames?.length) {
      ...
      if (ps > 0.1) {
        score += clamp(ps, 0, 0.35);
        reasons.push("title-sim");
      }
    }

    if (r.classes?.some((c) => c.includes("food")) || r.source?.key === "fda") {
      if (cats.some((c) => c.includes("garden:seed"))) score -= 0.25;
      else score += 0.05;
    }
    ...
    if (
      allergenBoost &&
      r.hazard &&
      /allergen|contains|undeclared/i.test(r.hazard)
    ) {
      score += allergenBoost;
      reasons.push("user-allergen-risk");
    }
  ...
  if (maxScore >= (options.emitThreshold ?? 0.6)) {
    eventBus.emit?.("safety:recall:flagged", {
      ...
      matches: matches.slice(0, 5).map((x) => ({
        id: x.recall.id,
        score: x.score,
        source: x.recall.source,
      })),
...
export async function getByUPC(upc) {
  ...
  const all = await dbAllRecalls();
  return all.filter(
    (r) => r.upcs?.includes(target) || r.gtins?.some((g) => g.endsWith(target))
  );
}
...
registerSource("fsis", FSISAdapter, {
  label: "USDA FSIS",
  ttlMs: 12 * 60 * 60 * 1000,
});
registerSource("cpsc", CPSCAdapter, {
  label: "CPSC",
  ttlMs: 24 * 60 * 60 * 1000,
});
