...
async function favRemoveHunt(userId, key) {
  ...
  try {
    const row = await DexieDB.favorites
      .where({ userId: userId || "anon", type: "coupon.hunt", key })
      .first();
...
const DEFAULT_PROVIDERS = {
  ...
  samsclub: {
    key: "samsclub",
    name: "Sam's Club",
    features: ["fetch", "clip", "apply", "loyalty"],
    oauth: true,
    version: 1,
  },
  walmart: {
...
export function listCouponProviders() {
  return Array.from(memProviders.values()).sort((a, b) =>
...
export default function useCouponPrefs(opts = {}) {
    ...
    async (patch = {}) => {
      await persist({
        ...
        optIns: {
          ...
          ...patch,
          dataConsentAtISO:
            patch?.dataConsentAtISO ||
            prefs.optIns?.dataConsentAtISO ||
            nowISO(),
        },
      });
    },
  ...
  const linkProvider = useCallback(
    async (key, accountMeta = {}) => {
      ...
      eventBus.emit("coupon:provider:linked", {
        ownerKey,
        key,
        account: providers[key],
      });
  ...
  const setLoyalty = useCallback(
    async (forKey, payload = {}) => {
      ...
      eventBus.emit("coupon:loyalty:updated", {
        ownerKey,
        forKey,
        loyalty: loyalty[forKey],
      });
  ...
  const getLoyalty = useCallback(
    (forKey) => (prefs.loyalty || {})[forKey] || null,
    [prefs.loyalty]
  );

  /* ------------------------ scheduler / hunts integration ------------------- */
  const suggestCouponSession = useCallback(
    (query = {}) => {
  ...
  const hunts = useMemo(
    () => ({
      ...
      save: async (label, query) => {
        const key = huntKey({
          ...(query || {}),
          providers: pickLinkedProviders(prefs),
        });
        const row = {
          key,
          label: String(label || "My Coupon Hunt"),
          query,
          providers: pickLinkedProviders(prefs),
          createdISO: nowISO(),
        };
      ...
      favorite: async (key) => {
        await favSaveHunt(user?.id, key, { favored: true });
        eventBus.emit("coupon:hunt:favorited", { key });
      },
      unfavorite: async (key) => {
        await favRemoveHunt(user?.id, key);
        eventBus.emit("coupon:hunt:unfavorited", { key });
      },
  ...
  const buildCouponQuery = useCallback(
    (context = {}) => {
