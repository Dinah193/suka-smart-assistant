...
function canonicalUnit(unitRaw) {
  ...
  if (["floz", "fl oz", "fluid ounce", "fluid ounces"].includes(u))
    return "floz";
...
function toCanonicalQuantity(qty, unit) {
  ...
  if (["g", "kg", "oz", "lb"].includes(u))
    return { amount: q * (U[u] / U.g), unit: "g" };
  if (["ml", "l", "floz"].includes(u))
    return { amount: q * (U[u] / U.ml), unit: "ml" };
...
function perUnit(price, qty, unit) {
  ...
  const amt = amount || 1;
  if (!Number.isFinite(price) || !Number.isFinite(amt) || amt <= 0)
    return { ppu: null, unit: u };
...
async function dbAddRow(row) {
  ...
  try {
    await DexieDB.priceBook.put(row);
    return row.id;
  } catch {
    return null;
  }
...
async function dbGetRowsBy({ upc, store }, limit = 1000) {
  ...
  try {
    if (
      upc &&
      store &&
      DexieDB.priceBook.schema.indexes?.includes("[upc+store]")
    ) {
      return await DexieDB.priceBook
        .where("[upc+store]")
        .equals([upc, store])
        .reverse()
        .limit(limit)
...
/* -------------------------------- favorites -------------------------------- */
function favKey({ upc, store }) {
  return `${toStr(upc)}::${toStr(store)}`;
}
...
async function addFavoriteWatch(userId, query, payload) {
  ...
  ls.add(key);
  lsSet(LS_KEY_FAVS, Array.from(ls));
  ...
  if (DexieDB?.favorites) {
    try {
      const row = await DexieDB.favorites
        .where({ userId: userId || "anon", type: "price.watch", key })
        .first();
...
function rollingLinearTrend(points) {
  ...
  const meanY = sumY / n;
  const ssTot =
    points.reduce((s, p) => s + Math.pow(p.y - meanY, 2), 0) || 1e-6;
  const intercept = (sumY - slope * sumX) / n;
  const ssRes = points.reduce(
    (s, p) => s + Math.pow(p.y - (slope * p.t + intercept), 2),
    0
  );
...
function seriesStats(rows) {
  ...
  const sorted = [...rows].sort(
    (a, b) => Date.parse(a.observedISO) - Date.parse(b.observedISO)
  );
  ...
  const minPPU = normalized.reduce(
    (m, r) => (r.ppu < m.ppu ? r : m),
    normalized[0]
  );
  const maxPPU = normalized.reduce(
    (m, r) => (r.ppu > m.ppu ? r : m),
    normalized[0]
  );
  ...
  const lastYear = normalized.filter(
    (r) => Date.parse(r.observedISO) >= cutoff
  );
  const low52 = lastYear.length
    ? lastYear.reduce((m, r) => (r.ppu < m.ppu ? r : m), lastYear[0])
    : minPPU;

  // Trend (linear regression) over per-unit values
  const pts = normalized.map((r) => ({
    t: Date.parse(r.observedISO) / days(1),
    y: r.ppu,
  }));
  ...
  const mean = avgPPU;
  const variance =
    normalized.reduce((s, r) => s + Math.pow(r.ppu - mean, 2), 0) /
      normalized.length || 0;
...
function toRow(obs) {
  ...
  return {
    id:
      obs.id ||
      `${obs.upc || "unknown"}-${obs.store || "any"}-${Date.now()}-${nanoid(
        4
      )}`,
    upc: toStr(obs.upc),
...
export default function usePriceBook(query = {}, opts = {}) {
  ...
  const loadSeries = useCallback(
    async (reason = "auto") => {
      ...
      if (fresh && cacheRef.current.rows?.length) {
        ...
        setStatus("ok");
        setError(null);
        eventBus.emit("pricebook:cache:hit", {
          reason,
          query: stable,
      ...
      try {
        ...
        if (!rows) {
          ...
          rows = Array.from(memRows.values())
            .sort(
              (a, b) => Date.parse(b.observedISO) - Date.parse(a.observedISO)
            )
            .slice(0, limit);
        }
        ...
        setStatus("ok");
        setError(null);
        eventBus.emit("pricebook:load:success", {
          reason,
          query: stable,
          count: narrowed.length,
        });
        ...
        setStatus("error");
        setError(e);
        eventBus.emit("pricebook:load:error", {
          reason,
          query: stable,
          error: String(e),
        });
  ...
  useEffect(() => {
    ...
    debRef.current = setTimeout(
      () => loadSeries("debounced"),
      clamp(debounceMs, 0, 2000)
    );
    return () => {
      if (debRef.current) clearTimeout(debRef.current);
    };
  ...
  const persistRow = useCallback(async (row) => {
    ...
    // LS
    const ls = Array.from(memRows.values()).sort(
      (a, b) => Date.parse(b.observedISO) - Date.parse(a.observedISO)
    );
  ...
  const record = useCallback(
    async (observation, overrides = {}) => {
      ...
      try {
        const viewRows = narrowRows(
          Array.from(memRows.values()),
          { upc: row.upc, store: row.store },
          9999
        );
        ...
        if (emitNBA && viewStats?.avgPPU) {
          ...
          if (
            Number.isFinite(ppu) &&
            ppu <= viewStats.avgPPU * nbaThresholdPct
          ) {
            eventBus.emit("nba:hint", {
              ...
              upc: row.upc,
              store: row.store,
              message: `Great price for ${row.upc} at ${
                row.store
  ...
  const perUnitFor = useCallback(
    (row) => perUnit(row?.price, row?.sizeQty, row?.sizeUnit),
    []
  );
  ...
  useEffect(() => {
    setIsFavorited(isFavoritedWatch(stable));
  }, [stable]);

  /* ------------------------------- export/import ---------------------------- */
  const exportAll = useCallback(() => {
    const rows = Array.from(memRows.values()).sort(
      (a, b) => Date.parse(b.observedISO) - Date.parse(a.observedISO)
    );
...
function narrowRows(allRows, query, limit) {
  ...
  if (query?.store)
    rows = rows.filter((r) => toStr(r.store) === toStr(query.store));
  return rows
    .sort((a, b) => Date.parse(b.observedISO) - Date.parse(a.observedISO))
    .slice(0, limit);
}
...
export function get52WeekLow(upc, store) {
  const rows = Array.from(memRows.values()).filter(
    (r) => (!upc || r.upc === upc) && (!store || r.store === store)
  );
  const cutoff = Date.now() - days(365);
  const lastYear = rows
