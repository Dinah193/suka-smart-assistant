...
function defaultPrefs() {
  return {
    ...
    avoid: {
      ingredients: [/* "sorbitol", "aspartame", ... */],
      additives:   [/* "bht", "bha", "tbhq", "red 40", "e129" */],
      categories:  [/* "ultra-processed", "energy drink" */],
      eNumbers:    [/* "e120", "e621" */],
    },
    diet: {
      vegetarian: false,
    ...
    crossContact: {
      warnOn: ["may contain", "manufactured in", "processed on shared equipment"],
    },
    recalls: {
...
async function dbSavePrefs(ownerKey, prefs) {
  ...
  try {
    await DexieDB.kv.put({ space: "safety", key: `prefs:${ownerKey}`, value: prefs, updatedAt: Date.now() });
  } catch {}
}
...
async function dbListProfiles(ownerId) {
  ...
  try {
    return await DexieDB.safetyProfiles.where("ownerId").equals(ownerId).reverse().limit(200).toArray();
  } catch { return null; }
}
async function dbSaveProfile(row) {
  if (!DexieDB?.safetyProfiles) return null;
  try { await DexieDB.safetyProfiles.put(row); return row.id; } catch { return null; }
}
async function dbDeleteProfile(id) {
  if (!DexieDB?.safetyProfiles) return;
  try { await DexieDB.safetyProfiles.delete(id); } catch {}
}
async function favSaveProfile(userId, key, payload) {
  if (!DexieDB?.favorites) return;
  try { await DexieDB.favorites.put({ userId: userId||"anon", type:"safety.profile", key, payload, createdAt: Date.now() }); } catch {}
}
async function favRemoveProfile(userId, key) {
  if (!DexieDB?.favorites) return;
  try {
    const row = await DexieDB.favorites.where({ userId: userId||"anon", type:"safety.profile", key }).first();
...
const SYN = {
  ...
  tree_nut: ["almond", "walnut", "cashew", "pecan", "hazelnut", "pistachio", "macadamia", "brazil nut", "pine nut"],
  ...
  gluten: ["wheat", "barley", "rye", "triticale", "spelt", "malt", "semolina", "farina", "seitan", "gluten"],
  ...
  sulfite: ["sulfite", "sulphite", "e220", "e221", "e222", "e223", "e224", "e226", "e227", "e228"],
  pork: ["pork", "bacon", "ham", "prosciutto", "gelatin (pork)", "lard"],
  alcohol: ["alcohol", "ethanol", "rum", "brandy", "wine", "beer", "cooking wine", "vanilla extract (alcohol)"],
  ...
  // leavening detection
  leaven: ["yeast", "sourdough", "baking powder", "baking soda", "ammonium bicarbonate"],
...
const CATEGORY_HINTS = {
  "ultra-processed": ["maltodextrin","artificial flavor","artificial color","modified starch","emulsifier","stabilizer"],
  "energy drink": ["taurine","caffeine","guarana","niacin (b3)"],
};

/* ------------------------------ normalization ------------------------------ */
function normalizeList(arr = []) { return Array.from(new Set(arr.map(toStr).filter(Boolean))); }
...
function pushViolation(res, type, tag, reason) {
  res.ok = false; res.score = clamp(res.score - 25, 0, 100);
 ...
 */
export function evaluateProduct(product = {}, prefs = defaultPrefs(), opts = {}) {
  ...
  if (prefs.recalls?.autoCheck && opts?.recalls?.flagged) {
    pushViolation(res, "recall", "recall-flag", "Product is under an active recall.");
  }
  ...
  for (const a of (prefs.allergens || [])) {
    ...
    const hit = ingredients.some(i => toStr(i).includes(tag)) ||
                Object.values(SYN).some(list => list.includes(tag) && ingredients.some(i => toStr(i).includes(list[0])));
    if (hit) {
      if (a.severity === "avoid") pushViolation(res, "allergen", tag, "Declared allergen on label.");
  ...
  for (const i of ingredients) {
    if (avoidIng.includes(toStr(i))) pushViolation(res, "avoid:ingredient", i, "In personal avoid list.");
    if (avoidAdd.some(a => toStr(i).includes(a))) pushViolation(res, "avoid:additive", i, "Avoided additive.");
    if (avoidEN.some(en => toStr(i).includes(en))) pushViolation(res, "avoid:e-number", i, "Avoided E-number.");
  }

  // category hints
  for (const [cat, needles] of Object.entries(CATEGORY_HINTS)) {
    if (anyIncludes(label, needles)) pushWarning(res, "category", cat, "Heuristic suggests this category.");
  }
  // explicit category avoid
  const avoidCats = normalizeList(prefs.avoid?.categories);
  for (const c of cats) if (avoidCats.includes(toStr(c))) pushViolation(res, "avoid:category", c, "Avoided category.");

  // 4) Diet flags
  const d = prefs.diet || {};
  const containsMeat = ["beef","pork","chicken","turkey","lamb","goat","duck","venison"].some(m => label.includes(m));
  ...
  if (d.vegan && (containsMeat || containsFish || containsShellfish || containsDairy || containsEgg || hasGelatin)) {
    pushViolation(res, "diet", "vegan", "Contains animal-derived ingredients.");
  } else if (d.vegetarian && (containsMeat || containsShellfish || hasGelatin)) {
    pushViolation(res, "diet", "vegetarian", "Contains meat/shellfish/gelatin.");
  ...
  if (d.keto && (hasSugar || (highCarb && !containsMeat && !containsFish))) {
    pushWarning(res, "diet", "keto", "Likely higher net carbs or added sugars.");
  }
  if (d.glutenFree && flatSynMatch("gluten", label)) pushViolation(res, "diet", "glutenFree", "Contains gluten sources.");
  if (d.dairyFree && containsDairy) pushViolation(res, "diet", "dairyFree", "Contains dairy.");
  if (d.nutFree && (flatSynMatch("peanut", label) || anyIncludes(label, SYN.tree_nut))) pushViolation(res, "diet", "nutFree", "Contains nuts/peanuts.");
  if (d.eggFree && containsEgg) pushViolation(res, "diet", "eggFree", "Contains egg.");
  if (d.lowFODMAP && anyIncludes(label, ["inulin","chicory root","fructo-oligosaccharide"])) {
    pushWarning(res, "diet", "lowFODMAP", "May be high-FODMAP.");
  }
  if (d.noAddedSugar && hasSugar) pushWarning(res, "diet", "noAddedSugar", "Added sugars detected.");

  // 5) Faith flags
  const f = prefs.faith || {};
  if (f.torahClean) {
    if (flatSynMatch("pork", label)) pushViolation(res, "faith", "torahClean", "Pork-derived ingredient.");
    if (containsShellfish) pushViolation(res, "faith", "torahClean", "Shellfish detected.");
    if (anyIncludes(label, ["blood","blood meal","blood plasma"])) pushViolation(res, "faith", "torahClean", "Blood-derived ingredient.");
  }
  if (f.halal) {
    if (flatSynMatch("alcohol", label)) pushViolation(res, "faith", "halal", "Alcohol present.");
    if (hasGelatin && !/halal gelatin/.test(label)) pushWarning(res, "faith", "halal", "Gelatin likely non-halal.");
  }
  if (f.kosher) {
    if (hasGelatin && !/kosher gelatin/.test(label)) pushWarning(res, "faith", "kosher", "Gelatin likely non-kosher.");
    // Full kosher logic is complex; we keep a light heuristic.
  }
  if (f.passoverUnleavenedMode) {
    if (flatSynMatch("leaven", label)) pushViolation(res, "faith", "passover", "Leavening detected during unleavened mode.");
  }

  // 6) Cross-contact warnings
  for (const phrase of (prefs.crossContact?.warnOn || [])) {
    if (label.includes(toStr(phrase))) pushWarning(res, "cross-contact", phrase, "Label indicates potential cross-contact.");
  }
...
export default function useSafetyPrefs(opts = {}) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setProfileList(rows.map(r => ({ id: r.id, name: r.name, updatedAt: r.updatedAt || r.createdAt })));
    })();
  ...
  const evaluate = useCallback((product, extra = {}) => {
  ...
  /* ------------------------------ overrides API --------------------------- */
  const setSessionOverride = useCallback(async (patch = {}) => {
    if (!sessionId) return;
    const next = {
      ...prefs,
      sessionOverrides: {
        ...(prefs.sessionOverrides || {}),
        [sessionId]: deepMerge(prefs.sessionOverrides?.[sessionId] || {}, patch),
      },
    };
    await persist(next);
  ...
  const clearSessionOverride = useCallback(async () => {
    if (!sessionId) return;
    const next = { ...prefs, sessionOverrides: { ...(prefs.sessionOverrides || {}) } };
  ...
  const saveProfile = useCallback(async (name) => {
    ...
    eventBus.emit("safety:profile:saved", { ownerKey, id: row.id, name: row.name });
    if (!useQuietHours().enabled && toast) toast("Safety profile saved.");
    return row.id;
  }, [ownerKey, prefs, listProfiles, user?.id]);

  const loadProfile = useCallback(async (idOrName) => {
    let row = null;
    if (DexieDB?.safetyProfiles) {
      const rows = await dbListProfiles(ownerKey) || [];
      row = rows.find(r => r.id === idOrName || r.name === idOrName) || null;
    } else {
    ...
    setActiveProfile({ id: row.id, name: row.name });
    eventBus.emit("safety:profile:loaded", { ownerKey, id: row.id, name: row.name });
    if (!useQuietHours().enabled && toast) toast(`Loaded profile: ${row.name}`);
    return true;
  }, [ownerKey, persist]);
  ...
  const favoriteProfile = useCallback(async (id) => {
    await favSaveProfile(user?.id, id, { name: profileList.find(p => p.id === id)?.name || "Profile" });
  ...
  const unfavoriteProfile = useCallback(async (id) => {
    await favRemoveProfile(user?.id, id);
  ...
  const importPrefs = useCallback(async (payload) => {
    ...
    if (!DexieDB?.safetyProfiles && Array.isArray(payload.profiles)) {
      ...
      setProfileList(payload.profiles.map(r => ({ id: r.id, name: r.name, updatedAt: r.updatedAt || r.createdAt })));
    }
  ...
  useEffect(() => {
    const onProcessed = ({ result }) => {
      ...
      if (!result) return;
      const r = evaluate(result, { recalls: { flagged: result?.recallFlag || false } });
      eventBus.emit("scan:safety:evaluated", { upc: result.upc, ok: r.ok, score: r.score, violations: r.violations });
