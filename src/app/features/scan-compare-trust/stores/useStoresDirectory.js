...
async function dbPutStore(row) {
  ...
  try {
    await DexieDB.storesDirectory.put(row);
    return row.id;
  } catch {
    return null;
  }
...
async function dbDeleteStore(id) {
  ...
  try {
    await DexieDB.storesDirectory.delete(id);
  } catch {}
}
async function dbListStores(ownerKey, limit = 500) {
  if (!DexieDB?.storesDirectory) return null;
  try {
    return await DexieDB.storesDirectory
      .where("ownerKey")
      .equals(ownerKey)
      .reverse()
      .limit(limit)
...
async function dbLoadAdapter(key) {
  ...
  try {
    return await DexieDB.kv.get({ space: "stores:adapters", key });
  } catch {
    return null;
  }
}
...
const DEFAULT_ADAPTERS = {
  generic: {
    ...
    features: ["pricing", "coupons"],
    version: 1,
  },
  samsclub: {
    key: "samsclub",
    name: "Sam's Club",
    features: ["pricing", "coupons", "loyalty"],
    version: 1,
  },
  costco: {
    key: "costco",
...
function listAdapters() {
  return Array.from(memAdapters.values()).sort((a, b) =>
...
function validateRow(row) {
  ...
  if (row.zip && !US_ZIP_RX.test(row.zip))
    errs.push("ZIP should be US 5-digit or 5+4.");
  return errs;
}
...
export default function useStoresDirectory(opts = {}) {
  ...
  const ownerKey = useMemo(
    () =>
      scope === "user"
        ? `user:${user?.id || "anon"}`
        : `house:${householdId || "default"}`,
    [scope, user?.id, householdId]
  );
  ...
  const updateStore = useCallback(
    async (id, patch = {}) => {
      ...
      setStores((prev) =>
        prev.map((s) => (s.id === id ? next : s)).sort(sorter)
      );
  ...
  const setPrimary = useCallback(
    async (id) => {
      ...
      for (const row of memRows.values()) {
        ...
        const next = isTarget
          ? { ...row, primary: true, updatedISO: nowISO() }
          : { ...row, primary: false, updatedISO: row.updatedISO };
  ...
  const updateSchedule = useCallback(
    async (id, winId, patch = {}) => {
      const cur = memRows.get(id);
      if (!cur) return false;
      const nextWins = (cur.schedule || []).map((w) =>
        w.id === winId ? { ...w, ...patch } : w
      );
  ...
  const removeSchedule = useCallback(
    async (id, winId) => {
      ...
      const next = {
        ...cur,
        schedule: (cur.schedule || []).filter((w) => w.id !== winId),
        updatedISO: nowISO(),
      };
      await persistSnapshot(next);
      setStores((prev) => prev.map((s) => (s.id === id ? next : s)));
  ...
  const suggestSessionTemplate = useCallback(
    (id) => {
      const s = memRows.get(id);
      if (!s) return false;
...
export function makeShoppingWindow(p) {
  return makeWindow(p || {});
}
