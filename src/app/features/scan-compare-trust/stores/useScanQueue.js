...
async function idbPutQueue(row) {
  ...
  try {
    await DexieDB.scanQueue.put(row);
    return row.id;
  } catch {
    return null;
  }
...
async function idbGetQueueAll(limit = 500) {
  if (!DexieDB?.scanQueue) return null;
  try {
    return await DexieDB.scanQueue
      .orderBy("createdISO")
      .reverse()
      .limit(limit)
      .toArray();
...
async function idbDeleteQueue(id) {
  ...
  try {
    await DexieDB.scanQueue.delete(id);
  } catch {}
}
async function idbPutBlob(id, blob) {
  if (!DexieDB?.scanBlobs) return false;
  try {
    await DexieDB.scanBlobs.put({ id, blob });
    return true;
  } catch {
    return false;
  }
...
async function idbGetBlob(id) {
  ...
  try {
    const row = await DexieDB.scanBlobs.get(id);
...
export default function useScanQueue(opts = {}) {
  ...
  // keep mem in sync
  useEffect(() => {
    for (const it of items) mem.set(it.id, it);
  }, [items]);
  ...
  const pendingCount = useMemo(
    () =>
      items.filter((x) => x.status === "queued" || x.status === "error").length,
    [items]
  );

  /** enqueue(image: Blob|File|dataURL|string, meta: object, {sessionId, favoriteSession}) */
  const enqueue = useCallback(
    async (image, meta = {}, extras = {}) => {
      const row = createQueueRow({
        image,
        meta,
        sessionId: extras.sessionId,
        favoriteSession: extras.favoriteSession,
      });
      ...
      eventBus.emit("scanqueue:enqueue", {
        id: saved.id,
        meta: saved.meta,
  ...
  const updateMeta = useCallback(
    async (id, patch = {}) => {
      const next = items.map((it) =>
        it.id === id
          ? { ...it, meta: { ...it.meta, ...patch }, updatedISO: nowISO() }
          : it
      );
      setItems(next);
      lsSave(next);
  ...
  const markFavoriteSession = useCallback(
    async (sessionId, flag = true) => {
      const next = items.map((it) =>
        it.sessionId === sessionId
          ? { ...it, favoriteSession: !!flag, updatedISO: nowISO() }
          : it
      );
      setItems(next);
      lsSave(next);
      if (DexieDB?.scanQueue) {
        for (const it of next.filter((i) => i.sessionId === sessionId))
          await idbPutQueue(it);
      }
      eventBus.emit("scanqueue:session:favorite", {
        sessionId,
        favorite: !!flag,
      });
      if (!quietHours && toast)
  ...
  const unmarkFavoriteSession = useCallback(
    async (sessionId) => markFavoriteSession(sessionId, false),
    [markFavoriteSession]
  );
      ...
      try {
        ...
        const work = items
          .filter(
            (x) =>
              x.status === "queued" ||
              (x.status === "error" && x.retryCount < maxRetries)
          )
        ...
        const runOne = async (row) => {
          ...
          try {
            ...
            if (out?.ok) {
              ...
              eventBus.emit("scanqueue:item:success", {
                id: local.id,
                result: out.result,
              });
            ...
            local.updatedISO = nowISO();
            await persistRow(local);
            eventBus.emit("scanqueue:item:error", {
              id: local.id,
            ...
            // backoff
            const wait = clamp(
              Math.round(backoffMin * Math.pow(1.6, local.retryCount)),
              backoffMin,
              backoffMax
            );
            await sleep(wait);
...
async function persistRow(row) {
  ...
  if (idx >= 0) snapshot[idx] = row;
  else snapshot.unshift(row);
  lsSave(snapshot);
}
  ...
  eventBus.on?.("scanqueue:bootstrap", ({ rows }) => {
    ...
    try {
      ...
      for (const r of [...ls, ...rows]) {
        ...
        if (
          !prev ||
          Date.parse(r.updatedISO || 0) < Date.parse(r.updatedISO || 0)
        ) {
          map.set(r.id, r);
        }
      }
      const merged = Array.from(map.values()).sort(
        (a, b) => Date.parse(b.createdISO) - Date.parse(a.createdISO)
      );
      lsSave(merged);
...
async function importQueue(payload, existing = []) {
  ...
  for (const r of [...existing, ...incoming]) {
    ...
    if (
      !prev ||
      Date.parse(prev.updatedISO || 0) < Date.parse(r.updatedISO || 0)
    )
      byId.set(r.id, r);
  }
