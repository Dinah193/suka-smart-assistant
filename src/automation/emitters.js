...
export async function emitMealRefresh({ userId, reason = "recipes-updated" }) {
  ...
  return sendToN8n(N8N_PATHS.MEAL_REFRESH, payload, { userId, source: "app.mealplanner" });
}
...
export async function emitCookingEvent(eventType, payload) {
  ...
  return sendToN8n(N8N_PATHS.COOKING_DISPATCH, full, { userId: payload.userId, source: "app.cooking" });
}

/** Garden harvest window / preservation sync */
export async function emitHarvestWindow(payload) {
  validate(harvestSchema, payload);
  return sendToN8n(N8N_PATHS.GARDEN_HARVEST, payload, { userId: payload.userId, source: "app.garden" });
}

/** Soil & water keeper (irrigation + amendments) */
export async function emitSoilWater(payload) {
  validate(soilWaterSchema, payload);
  return sendToN8n(N8N_PATHS.SOIL_WATER, payload, { userId: payload.userId, source: "app.garden" });
}

/** Animal care planner */
export async function emitAnimalPlan(payload) {
  validate(animalPlanSchema, payload);
  return sendToN8n(N8N_PATHS.ANIMAL_PLAN, payload, { userId: payload.userId, source: "app.animals" });
}
