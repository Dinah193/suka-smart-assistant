...
import { emit } from "@/services/events/eventBus"; // optional analytics; swallow if missing
...
registerSupplyProfile("glass cleaner", {
  unit: "ml",
  defaultUse: {
    generic: { qty: 10, unit: "ml" },
    perTaskType: {
      windows: { qty: 12, unit: "ml" },
      wipe: { qty: 8, unit: "ml" },
    },
  },
  density: 1.0,
...
registerSupplyProfile("all-purpose cleaner", {
  unit: "ml",
  defaultUse: {
    generic: { qty: 15, unit: "ml" },
    perTaskType: {
      wipe: { qty: 15, unit: "ml" },
      scrub: { qty: 25, unit: "ml" },
    },
  },
  density: 1.0,
  min: 250,
...
registerSupplyProfile("disinfectant", {
  unit: "ml",
  defaultUse: {
    generic: { qty: 20, unit: "ml" },
    perTaskType: {
      disinfect: { qty: 25, unit: "ml" },
      sanitize: { qty: 20, unit: "ml" },
    },
  },
  density: 1.0,
  min: 250,
...
registerSupplyProfile("degreaser", {
  ...
  defaultUse: {
    generic: { qty: 20, unit: "ml" },
    perTaskType: { scrub: { qty: 30, unit: "ml" } },
  },
  density: 1.02,
  min: 200,
  max: 1500,
...
registerSupplyProfile("dish soap", {
  unit: "ml",
  defaultUse: {
    generic: { qty: 8, unit: "ml" },
    perTaskType: { wash: { qty: 10, unit: "ml" } },
  },
  density: 1.03,
  min: 150,
  max: 1000,
...
registerSupplyProfile("bleach", {
  ...
  defaultUse: {
    perTaskType: {
      sanitize: { qty: 15, unit: "ml" },
      disinfect: { qty: 20, unit: "ml" },
    },
  },
  density: 1.2,
  min: 250,
...
registerSupplyProfile("trash bags", {
  unit: "each",
  defaultUse: {
    perTaskType: { trash: { qty: 1, unit: "each" } },
    generic: { qty: 1, unit: "each" },
  },
  isCountBased: true,
  min: 5,
  max: 40,
...
/* Built-ins */
mapTaskTypeSupply("windows", [
  { name: "glass cleaner", curve: "spray.default" },
  { name: "microfiber cloth", curve: "wipe.default" },
]);
mapTaskTypeSupply("wipe", [
  { name: "all-purpose cleaner", curve: "spray.default" },
]);
mapTaskTypeSupply("scrub", [{ name: "degreaser", curve: "spray.heavy" }]);
mapTaskTypeSupply("sanitize", [
  { name: "disinfectant", curve: "spray.default" },
]);
mapTaskTypeSupply("disinfect", [
  { name: "disinfectant", curve: "spray.heavy" },
]);
mapTaskTypeSupply("mop", [{ name: "floor cleaner", curve: "bucket.default" }]);
mapTaskTypeSupply("steam-mop", [
  { name: "floor cleaner", curve: "steam.micro" },
]);
mapTaskTypeSupply("laundry", [
  { name: "laundry detergent", curve: "dose.default" },
]);
...
registerUsageCurve("spray.default", ({ baseMl, task }) => {
  ...
  const mul =
    task.soilLevel === "high" ? 1.25 : task.soilLevel === "low" ? 0.9 : 1.0;
  return baseMl * mul;
});
registerUsageCurve(
  "spray.heavy",
  ({ baseMl, task }) => baseMl * (task.soilLevel === "high" ? 1.6 : 1.2)
);
registerUsageCurve(
  "bucket.default",
  ({ baseMl, task }) => baseMl * (task.soilLevel === "high" ? 1.3 : 1.0)
);
...
export function estimatePlanConsumption(plan, options = {}) {
  ...
  for (const zone of plan?.zones || []) {
    for (const task of zone?.tasks || []) {
      ...
      const mappings =
        TASK_SUPPLY_MAP.get(String(task.type || "").toLowerCase()) || [];
      const explicitSupplies = Array.isArray(task.supplies)
        ? task.supplies
        : [];
      ...
      for (const item of merged) {
        ...
        const curveName =
          item.curve || (prof.isCountBased ? "count.one" : "spray.default");
        ...
        if (price && Number.isFinite(price.unitPrice)) {
          const useQty = prof.isCountBased
            ? usedEach
            : fromMl(usedMl, price.unit).qty;
          const lineCost = (useQty / 1) * price.unitPrice;
          totals[item.name].cost = (totals[item.name].cost || 0) + lineCost;
          totals[item.name].currency =
            price.currency || totals[item.name].currency || "USD";
        }

        out.supplies.push({
          name: item.name,
          ml: usedMl || undefined,
          each: usedEach || undefined,
        });
      }
...
export function computeReorderRecommendation(name, opts = {}) {
  ...
  const safetyDays = Math.max(
    0,
    Number(opts.safetyDays ?? DEFAULTS.safetyDays)
  );
  ...
  const min = Number.isFinite(opts.min)
    ? opts.min
    : prof.min || (isCount ? 5 : 250);
  const max = Number.isFinite(opts.max)
    ? opts.max
    : prof.max || (isCount ? 40 : 2000);
  const roundTo = Number.isFinite(opts.roundTo)
    ? Math.max(1, opts.roundTo)
    : isCount
    ? 1
    : 50;
  ...
  const daysUntil =
    dailyUseOut > 0 ? Math.floor(available / dailyUseOut) : Infinity;
  const runoutDate = new Date(
    Date.now() + daysUntil * 24 * 3600 * 1000
  ).toISOString();
...
export function recommendForTotals(totals, options = {}) {
  const horizon = Math.max(
    1,
    Number(options.horizonDays ?? DEFAULTS.horizonDays)
  );
  const out = {};
...
function baseUsageForTask(task, profile = {}) {
  ...
  const spec = per[t] ||
    generic || {
      qty: profile.isCountBased ? 1 : 10,
      unit: profile.isCountBased ? "each" : profile.unit || "ml",
    };
  ...
  if (
    Number.isFinite(profile.dilution) &&
    profile.dilution > 0 &&
    profile.dilution < 1
  ) {
