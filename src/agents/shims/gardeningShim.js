...
function buildCuisineSignals(input = {}, context = {}) {
  ...
  const cuisineProfile = {
    regionalCuisine: explicitProfile.regionalCuisine || contextProfile.regionalCuisine || null,
    preferredDishes: explicitProfile.preferredDishes || contextProfile.preferredDishes || [],
    weeklyMealPatterns: explicitProfile.weeklyMealPatterns || contextProfile.weeklyMealPatterns || [],
    spiceProfile: explicitProfile.spiceProfile || contextProfile.spiceProfile || [],
    heatPreference: explicitProfile.heatPreference || contextProfile.heatPreference || null,
  ...
  if (cuisineProfile.preferredDishes?.length) {
    notes.push(`Preferred dishes: ${cuisineProfile.preferredDishes.join(', ')}`);
  }
...
function normalizeGardenOutput(intent, raw, cuisineSignals) {
  ...
  if (!raw || typeof raw !== 'object') {
    return {
      ...
      warnings: [{ type: 'emptyResult', message: 'Reasoner returned no structured payload.' }],
      debug,
    };
  }
...
export async function invokeShim(req) {
  ...
  try {
    ...
    // Mode selection
    const mode = selectModeForIntent({ domain, intent, input }) || 'garden.estimatePlan.v1';
    ...
    if (cached) {
      ...
      const { data, warnings: w2, debug: d2 } = normalizeGardenOutput(intent, cached, cuisineSignals);
    ...
    if (!confidence.ok) {
      warnings.push({
        type: 'lowConfidence',
        message: confidence.message || 'Reasoner confidence below threshold for gardening intent.',
      });
      emit({
    ...
    const { data, warnings: w3, debug: d3 } = normalizeGardenOutput(intent, rawResult, cuisineSignals);
