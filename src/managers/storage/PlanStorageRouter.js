...
(function () {
  ...
  var nowEpoch = function(){ return Date.now(); };
      ...
      if (!valid) {
        return {
          ...
          errors:(contractRegistry[domain].errors || []).map(function(e){
            return (e.instancePath || e.schemaPath) + " " + (e.message || "invalid");
          })
        };
      }
    }
  ...
  function MemoryAdapter(){
    ...
    return {
      name: "memory",
      ready: false,
      init: function(){ this.ready = true; return Promise.resolve(); },
      get: function(key){ return Promise.resolve(map.get(key)); },
      set: function(key, val){ map.set(key, val); return Promise.resolve(); },
      del: function(key){ map.delete(key); return Promise.resolve(); },
      keys: function(prefix){
        var out = [];
        map.forEach(function(_v, k){ if (!prefix || k.indexOf(prefix) === 0) out.push(k); });
        return Promise.resolve(out);
      },
      bulkGet: function(keys){
        return Promise.resolve(keys.map(function(k){ return map.get(k); }));
      },
      bulkSet: function(entries){
        entries.forEach(function(e){ map.set(e.key, e.value); });
  ...
  function LocalStorageAdapter(){
    var hasLS = false;
    try { hasLS = typeof window !== "undefined" && !!window.localStorage; } catch(_e){}
    return {
      name: "localStorage",
      ready: false,
      init: function(){ this.ready = hasLS; return Promise.resolve(); },
      get: function(key){
        if (!hasLS) return Promise.resolve(undefined);
        var raw = window.localStorage.getItem(key);
        if (raw == null) return Promise.resolve(undefined);
        try { return Promise.resolve(JSON.parse(raw)); } catch(_e){ return Promise.resolve(undefined); }
      },
      set: function(key, val){
        if (!hasLS) return Promise.resolve();
        try { window.localStorage.setItem(key, JSON.stringify(val)); } catch(_e){}
        return Promise.resolve();
      },
      del: function(key){
        if (!hasLS) return Promise.resolve();
      ...
      bulkSet: function(entries){
        var self = this;
        return Promise.all(entries.map(function(e){ return self.set(e.key, e.value); })).then(function(){});
      }
    };
  }
  ...
  function DexieAdapter(dbName){
    ...
    return {
      ...
      ready: false,
      init: function(){
        if (!Dexie) { this.ready = false; return Promise.resolve(); }
      ...
      get: function(key){ if (!this.ready) return Promise.resolve(undefined); return db.kv.get(key).then(function(row){ return row ? row.value : undefined; }); },
      set: function(key, val){ if (!this.ready) return Promise.resolve(); return db.kv.put({ key: key, value: val }).then(function(){}); },
      del: function(key){ if (!this.ready) return Promise.resolve(); return db.kv.delete(key).then(function(){}); },
      keys: function(prefix){
        if (!this.ready) return Promise.resolve([]);
        // Dexie doesn’t index prefixes for us; we’ll scan.
        return db.kv.toArray().then(function(rows){
          var out = [];
          rows.forEach(function(r){ if (!prefix || r.key.indexOf(prefix) === 0) out.push(r.key); });
          return out;
        });
      },
      bulkGet: function(keys){
        if (!this.ready) return Promise.resolve(keys.map(function(){ return undefined; }));
        return Promise.all(keys.map(function(k){ return db.kv.get(k).then(function(r){ return r ? r.value : undefined; }); }));
      },
      bulkSet: function(entries){
        if (!this.ready) return Promise.resolve();
        return db.transaction("rw", db.kv, function () {
          entries.forEach(function(e){ db.kv.put({ key: e.key, value: e.value }); });
        }).then(function(){});
      }
    };
  }
  ...
  function chooseAdapter(){
    ...
    return dx.init().then(function(){
      ...
      return ls.init().then(function(){
        ...
        return mem.init().then(function(){ return mem; });
      });
    });
  }
  ...
  PlanStorageRouter.prototype.init = function(){
    ...
    return chooseAdapter().then(function(adapter){
  ...
  PlanStorageRouter.prototype._ensureIndexes = function(){ return Promise.resolve(); };
  ...
  PlanStorageRouter.prototype.toggleFavorite = function(opts){
    ...
    if (!uid) return Promise.reject(new Error("toggleFavorite requires a userId"));
    var domain = opts.domain || null;
    var planId = opts.planId;
    if (!planId) return Promise.reject(new Error("toggleFavorite requires planId"));

    return self._loadFavorites(uid).then(function(fav){
      var existing = !!fav.byId[planId];
      var shouldFav = (typeof opts.favorite === "boolean") ? opts.favorite : !existing;
      if (shouldFav) {
  ...
  PlanStorageRouter.prototype.listFavorites = function(opts){
    ...
    return self._loadFavorites(uid).then(function(fav){
      ...
      if (domain) {
        ids = ids.filter(function(id){ return (fav.byId[id] && fav.byId[id].domain === domain); });
      }
      ids = ids.slice(0, limit);
      var keys = ids.map(function(id){ return self._keyPlan("global", id); }) // favorites can point to global…
        .concat(ids.map(function(id){ return self._keyPlan("user:"+uid, id); })); // …or user scope; we’ll resolve.
      return self.adapter.bulkGet(keys).then(function(values){
        ...
        values.forEach(function(val){
          ...
          if (!existing || (existing.scope === "global" && val.scope && val.scope.indexOf("user:") === 0)) {
            out[val.id] = val;
          }
        });
  ...
  PlanStorageRouter.prototype.savePlan = function(plan, opts){
    ...
    return self.adapter.get(key).then(function(existing){
      ...
      return self.adapter.set(key, normalized).then(function(){
        if (opts.favorite === true && (opts.userId || self.userId)) {
          return self.toggleFavorite({ planId: normalized.id, favorite: true, userId: (opts.userId || self.userId), domain: normalized.domain })
  ...
  PlanStorageRouter.prototype.getPlan = function(planId, opts){
    ...
    return self.adapter.get(self._keyPlan(userScope, planId)).then(function(val){
      if (val) return val;
  ...
  PlanStorageRouter.prototype.listPlans = function(opts){
    ...
    else if (opts.scope === "user") scopes = [self._resolveScope("user", opts.userId)];
    else scopes = [self._resolveScope("user", opts.userId), "global"]; // "all" or undefined

    var prefixKeys = scopes.map(function(sc){ return self._keyAllPrefix(sc); });

    return Promise.all(prefixKeys.map(function(prefix){ return self.adapter.keys(prefix); }))
      .then(function(allKeyLists){
        var keys = [];
        allKeyLists.forEach(function(arr){ keys = keys.concat(arr); });
        ...
        return self.adapter.bulkGet(keys).then(function(values){
          ...
          if (opts.favoritesOnly) {
            ...
            return self._loadFavorites(uid).then(function(fav){
              var set = fav.byId || {};
              var favd = out.filter(function(p){ return !!set[p.id]; });
  ...
  PlanStorageRouter.prototype.deletePlan = function(planId, opts){
    ...
    if (opts.hard) {
      return self.adapter.del(key).then(function(){
        self.eventBus.emit("plan.deleted", { id: planId, scope: scope, domain: opts.domain });
      });
    }
    return self.adapter.get(key).then(function(val){
      ...
      return self.adapter.set(key, val).then(function(){
        self.eventBus.emit("plan.deleted", { id: planId, scope: scope, domain: val.domain, soft: true });
      });
    });
  };
  ...
  PlanStorageRouter.prototype.restorePlan = function(planId, opts){
    ...
    return self.adapter.get(key).then(function(val){
      ...
      val.meta.version = (val.meta.version || 0) + 1;
      return self.adapter.set(key, val).then(function(){
        self.eventBus.emit("plan.restored", { id: planId, scope: scope, domain: val.domain });
        return val;
      });
    });
  };
  ...
  PlanStorageRouter.prototype.importPlans = function(payload, opts){
    ...
    var self = this;
    opts = opts || {};
    if (!payload || !Array.isArray(payload.items)) return Promise.reject(new Error("Invalid import payload"));
    var scope = self._resolveScope(opts.scope, opts.userId);
    var entries = [];
    var okCount = 0;
    ...
    payload.items.forEach(function(plan){
      ...
      if (val.ok) {
        normalized.meta.version = (normalized.meta.version || 0) + 1;
        entries.push({ key: self._keyPlan(scope, normalized.id), value: normalized });
        okCount++;
      } else {
        logger.warn("[PlanStorageRouter] import validation failed for", plan && plan.id, val.errors);
      }
    });
  ...
  PlanStorageRouter.prototype.migrate = function(handler){
    ...
    return self.listPlans({ scope: "all", limit: self.maxList, includeDeleted: true }).then(function(plans){
      var mutated = [];
      plans.forEach(function(p){
        ...
        if (np && np !== p) {
          ...
          mutated.push({ key: self._keyPlan(np.scope || "global", np.id), value: np });
        }
      });
      if (!mutated.length) return { migrated: 0 };
      return self.adapter.bulkSet(mutated).then(function(){
        self.eventBus.emit("plan.migrated", { count: mutated.length, at: nowEpoch() });
  ...
  PlanStorageRouter.prototype.afterSaveOrchestrate = function(plan){
    ...
    try {
      ...
      self.eventBus.emit("mealplan.draft.requested", { id: plan.id, params: { domain: (domain === "meals" ? "meals" : undefined) } });
      self.eventBus.emit("grocerylist.requested", { planId: plan.id, domain: (domain === "meals" ? "meals" : undefined) });
      self.eventBus.emit("prep.tasks.requested", { planId: plan.id, params: { domain: domain } });

      if (self.automation && self.automation.emit) {
        self.automation.emit("planner.conflict.detected", { // This is a sample; real detection occurs elsewhere.
          kind: "time",
          domain: domain,
          planId: plan.id,
          at: nowEpoch(),
