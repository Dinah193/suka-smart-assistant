...
(function () {
  ...
  function createOutbox(lsKey) {
    var ok = false;
    try {
      ok = isBrowser && !!window.localStorage;
    } catch (_e) {}
    ...
    function write(a) {
      if (!ok) return;
      try {
        window.localStorage.setItem(KEY, toJSON(a || []));
      } catch (_e) {}
    }
    ...
    return {
      push: function (job) {
        var a = read();
        a.push(Object.assign({ ts: now() }, job));
        write(a);
      },
      drain: function () {
        var a = read();
  ...
  function createDrive(api) {
    ...
    return {
      ...
      listByNamePrefix: async function (
        prefix,
        useVisible,
        visibleFolderId,
        pageSize
        ...
        do {
          var params = {
            fields:
              "nextPageToken, files(id,name,modifiedTime,version,md5Checksum)",
  ...
  function GoogleDriveAdapter(options) {
    ...
    this.visibleFolderName =
      options.visibleFolderName || DEFAULTS.visibleFolderName;
  ...
  GoogleDriveAdapter.prototype.isOnline = function () {
    return !!this._online;
  };
  ...
  GoogleDriveAdapter.prototype.useVisibleFolder = function (flag) {
    this._useVisibleFolder = !!flag;
  };
  ...
  GoogleDriveAdapter.prototype.init = async function () {
    try {
      ...
      if (this._useVisibleFolder) {
        try {
          var folder = await this._drive.findOrCreateVisibleFolder(
            this.visibleFolderName
          );
  ...
  GoogleDriveAdapter.prototype._ensureManifest = async function () {
    try {
      var f = await this._drive.findManifest(
        this.manifestName,
        this._useVisibleFolder,
        this._visibleFolderId
      );
      if (f && f.id) {
        ...
        this._manifest = {
          id: f.id,
          map: map,
          mtime: Date.parse(f.modifiedTime || new Date().toISOString()),
          version: f.version || 0,
        };
        ...
        var parents =
          this._useVisibleFolder && this._visibleFolderId
            ? [this._visibleFolderId]
            : ["appDataFolder"];
        var created = await this._drive.writeFile({
      ...
      logger.warn(
        "[GoogleDriveAdapter] manifest unavailable, will degrade to Drive queries",
        e
      );
      this._manifest = { id: null, map: {}, mtime: 0, version: 0 };
      this._index = {};
  ...
  GoogleDriveAdapter.prototype._updateManifest = async function (mutateFn) {
    ...
    try {
      ...
      this._manifest.version =
        (res && res.version) || this._manifest.version + 1;
  ...
  GoogleDriveAdapter.prototype.get = async function (key) {
    ...
    try {
      ...
      if (!fileId) {
        ...
        var files = await this._drive.listByNamePrefix(
          guessName,
          this._useVisibleFolder,
          this._visibleFolderId,
          50
        ...
        var found = (files || []).find(function (f) {
          return f.name === guessName;
        });
        if (found) {
          fileId = found.id;
          // opportunistically repair manifest/index
          this._index[key] = fileId;
          if (this._manifest.id)
            await this._updateManifest(function (map) {
              map[key] = fileId;
            });
        }
      }
  ...
  GoogleDriveAdapter.prototype.bulkSet = async function (entries) {
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      try {
        await this.set(e.key, e.value);
      } catch (_e) {
        /* queued in outbox if needed */
      }
    }
    return;
  };
