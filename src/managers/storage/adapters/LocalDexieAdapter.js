...
(function () {
  ...
  LocalDexieAdapter.prototype.init = async function () {
    try {
      ...
      try {
        await this._db[this._metaTable].put({
          name: "adapter",
          value: { kind: "LocalDexieAdapter", at: now() },
        });
      } catch (_e) {}
          ...
          if (
            ...
            automation &&
              ...
              automation.emit("nba.signal", {
                ...
                planId: ent.value.id,
                userId:
                  (ent.value.meta && ent.value.meta.createdBy) || undefined,
                ts: now(),
              });
          }
        }
