...
); // canMeetDeadline({session,...})
const prioritiesPolicy = requireSafe(
  "@/engines/scheduling/policies/priorities.js"
); // (optional) scoreSessions
const constraintsPolicy = requireSafe(
  "@/engines/scheduling/policies/constraints.js"
);
...
const SessionStore =
  ...
  requireSafe("@/engines/scheduling/sessionStore.js"); // CRUD on sessions
...
module.exports = {
  ...
  async sweep(args) {
    ...
    try {
      ...
      emit("gatekeeper.sweep.completed", {
        ok: !!(res && res.ok),
        meta: shrink(res),
      });
  ...
  async resolveBlockers(args) {
    ...
    let sessions = [];
    try {
      sessions = await SessionStore.getByIds(cfg.sessionIds);
    } catch (_) {}
    ...
    emit("gatekeeper.resolve.completed", {
      resolved,
      mutated,
  ...
  async contingencyPlanForWindow(args) {
    ...
    if (
      !SessionStore ||
      typeof SessionStore.listUpcomingInWindow !== "function"
    ) {
    ...
    const limit =
      isFinite(a.limit) && a.limit > 0 ? Math.min(a.limit, 100) : 20;
    emit("gatekeeper.plan.started", {
      windowStartISO: win.startISO,
      windowEndISO: win.endISO,
      limit,
    });

    let upcoming = [];
    try {
      upcoming = await SessionStore.listUpcomingInWindow(
        new Date(win.startISO),
        new Date(win.endISO)
      );
    } catch (_) {}
  ...
  async rescheduleOnBlocker(args) {
    ...
    if (
      !Rescheduler ||
      typeof Rescheduler.move !== "function" ||
      !SessionStore
    ) {
    ...
    let s = null;
    try {
      s = await (SessionStore.getByIds
        ? SessionStore.getByIds([id])
        : Promise.resolve([]));
    } catch (_) {}
    s = Array.isArray(s) && s.length ? s[0] : null;
    if (!s) {
      emit("gatekeeper.reschedule.skipped", {
    ...
    if (!attempt) {
      emit("gatekeeper.reschedule.failed", {
        sessionId: id,
    ...
    try {
      ...
      if (ok) {
        emit("gatekeeper.reschedule.moved", {
          sessionId: id,
          startISO: attempt.startISO,
          endISO: attempt.endISO,
        });
  ...
  async escalate(args) {
    ...
    try {
      ...
      emit("gatekeeper.escalate.failed", {
...
async function attemptResolve(session, cfg) {
  ...
  if (cfg.allowAltEquipment) {
    ...
    if (!equipOk) {
      ...
      if (alt && alt.accepted) {
        ...
        emit("gatekeeper.resolve.altEquipment", {
          sessionId: session.id || session.sessionId,
  ...
  if (!inv.ok) {
    if (cfg.allowSubstitutions) {
      ...
      if (sub && sub.accepted) {
        ...
        emit("gatekeeper.resolve.substitutions", {
          sessionId: session.id || session.sessionId,
          items: sub.items,
        });
      }
      ...
      try {
        if (Inventory && typeof Inventory.reserve === "function") {
          await Inventory.reserve(inv.itemsNeeded || [], {
            sessionId: session.id || session.sessionId,
            soft: true,
            expiresAtISO: minusMinutesISO(
              session.plannedStartISO || session.suggestedStartISO,
              5
            ),
          });
          result.steps.push({ action: "reserve", info: "soft" });
          emit("gatekeeper.resolve.reserve", {
            sessionId: session.id || session.sessionId,
            soft: true,
          });
        }
  ...
  if (cfg.allowSimplifyVariant) {
    ...
    if (risky) {
      ...
      if (simp && simp.accepted) {
        ...
        emit("gatekeeper.resolve.simplified", {
          sessionId: session.id || session.sessionId,
          variant: simp.variant,
        });
      }
    }
  }
  ...
  const endISO = startISO
    ? addMinutesISO(startISO, Number(session.estimatedMinutes || 0))
    : null;
  let feasible = true;
  if (
    feasibility &&
    typeof feasibility.canMeetDeadline === "function" &&
    startISO &&
    endISO
  ) {
    try {
...
async function computeContingencies(session) {
  ...
  if (!allowed) {
    out.suggestions.push({
      kind: "nudgeWindow",
      items: ["Shift outside quiet hours/holy day window"],
    });
    out.feasible = false;
  }
...
async function findFeasibleShift(session, window) {
  ...
  for (let t = startT; t + durMin * 60000 <= endT; t += step * 60000) {
    ...
    if (await quickFeasible(session, sISO, eISO))
...
async function isAllowedByConstraints(session) {
  ...
  try {
    if (typeof constraintsPolicy.isAllowed === "function") {
      return !!(await constraintsPolicy.isAllowed(session, {
        nowISO: new Date().toISOString(),
      }));
    }
    if (typeof constraintsPolicy.filterSessions === "function") {
      const res = await constraintsPolicy.filterSessions([session], {
        nowISO: new Date().toISOString(),
      });
...
async function proposeAltEquipment(session) {
  ...
  try {
    const alt = await Storehouse.altEquipment(session.equipment || [], {
      domain: session.domain,
    });
    if (alt && alt.accepted) {
      if (SessionStore && typeof SessionStore.updateEquipment === "function") {
        await SessionStore.updateEquipment(
...
async function proposeSubstitutions(inv, session) {
  if (!Inventory || typeof Inventory.proposeSubstitutions !== "function")
    return null;
  try {
    const proposal = await Inventory.proposeSubstitutions(
      inv.itemsNeeded || [],
      { domain: session.domain }
    );
    if (proposal && proposal.items && proposal.items.length) {
      if (
        SessionStore &&
        typeof SessionStore.applySubstitutions === "function"
      ) {
        await SessionStore.applySubstitutions(
          session.id || session.sessionId,
          proposal.items
        );
      }
...
async function previewSubstitutions(inv, session) {
  ...
  try {
    const proposal = await Inventory.proposeSubstitutions(
      inv.itemsNeeded || [],
      { domain: session.domain, dryRun: true }
    );
...
async function simplifyVariant(session) {
  ...
  try {
    ...
    if (
      ...
      res.accepted &&
      SessionStore &&
      typeof SessionStore.applyVariant === "function"
    ) {
      await SessionStore.applyVariant(
        session.id || session.sessionId,
        res.variant
      );
...
function boundShiftWindow(session, earliestISO, latestISO, maxShiftMinutes) {
  ...
  const maxShift =
    isFinite(maxShiftMinutes) && maxShiftMinutes > 0 ? maxShiftMinutes : 45;
  const sMin = isFinite(Date.parse(earliestISO || ""))
    ? Date.parse(earliestISO)
    : baseS - maxShift * 60000;
  const sMax = isFinite(Date.parse(latestISO || ""))
    ? Date.parse(latestISO)
    : baseS + maxShift * 60000;
  return {
    startISO: new Date(sMin).toISOString(),
    endISO: new Date(sMax + durMin * 60000).toISOString(),
  };
}
