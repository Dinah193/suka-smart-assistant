...
(mod ? describe : describe.skip)('Learning Loop — calibration updates & convergence', () => {
  let eventBus;
  beforeEach(async () => {
  ...
  it('reduces loss and emits calibration events on a learnable linear dataset', async () => {
    ...
    const result = await runCalibration(observations, model, {
      learningRate: 0.05,
      tolerance: 1e-4,
      maxSteps: 200,
    ...
    const calls = eventBus.emit.mock.calls.filter(([evt]) => evt === 'automation.event');
    ...
    expect(types.some((t) => t === 'learning.calibration.step' || t === 'learning.convergence.reached')).toBe(true);
  ...
  it('converges within tolerance when signal is strong and steps are sufficient', async () => {
    ...
    const calls = eventBus.emit.mock.calls.filter((c) => c[0] === 'automation.event');
    const convergenceEvent = calls.map((c) => c[1]).find((p) => p.type === 'learning.convergence.reached');
  ...
  it('supports per-domain calibration (params updated with domain qualifiers)', async () => {
    ...
    const hasDomainKey = result.updates.some((u) => u.key.includes(':') || u.domain);
  ...
  it('detects data drift when recent window diverges significantly from current params', async () => {
    ...
    }).map((o) => ({ ...o, ts: isoNowPlus(86_400_000 + Math.random() * 5_000) })); // one day later
    ...
    const calls = eventBus.emit.mock.calls.filter((c) => c[0] === 'automation.event');
    const driftEvt = calls.map((c) => c[1]).find((p) => p.type === 'learning.data.drift.detected');
    expect(driftEvt || result.metrics.lossAfter < result.metrics.lossBefore * 0.7).toBeTruthy();
  });

  it('is idempotent on empty/noisy input and remains defensive', async () => {
    const model = makeInitialModel({ a: 1.0, b: 0.5 });

    const empty = await runCalibration([], model, { tolerance: 1e-6 }, basicCtx());
    expect(empty.ok).toBe(true);
    expect(empty.updates.length === 0 || Math.abs(empty.metrics.lossAfter - empty.metrics.lossBefore) <= 1e-12).toBe(
      true
    );
  ...
  it('scales to large batches without timing out (basic perf sanity)', async () => {
    ...
    const result = await runCalibration(big, model, {
      learningRate: 0.02,
      tolerance: 5e-5,
      maxSteps: 120,
      regularization: 0.0001,
  ...
  it('emits a single "updated" envelope summarizing param deltas after calibration', async () => {
    ...
    const calls = eventBus.emit.mock.calls.filter((c) => c[0] === 'automation.event');
    const updates = calls.map((c) => c[1]).filter((p) => p.type === 'learning.calibration.updated');
    ...
    const sortedA = (arr) => arr.slice().sort((x, y) => x.key.localeCompare(y.key));
    expect(sortedA(last.data.updates).map((u) => ({ key: u.key, delta: round4(u.delta) }))).toEqual(
      sortedA(result.updates).map((u) => ({ key: u.key, delta: round4(u.delta) }))
    );
  });
});
