...
(mod ? describe : describe.skip)(
  "Automation Feasibility Checks (admission)",
  () => {
    let eventBus;
    beforeEach(async () => {
    ...
    it("detects inventory shortages as a blocker", async () => {
      ...
      expect(result.blockers.join(" | ").toLowerCase()).toMatch(
    ...
    it("respects quiet hours guard when enabled", async () => {
      ...
      const ctx = buildDefaultCtx({
        guards: {
          quietHours: { enabled: true, start: qStart, end: qEnd },
          sabbath: { enabled: false },
    ...
    it("respects sabbath guard when enabled and now is sabbath", async () => {
      ...
      plan.tasks.forEach((t) => {
        t.timeWindow = {
          ...
          end: new Date(saturday.getTime() + 60 * 60_000).toISOString(),
        };
      });
        ...
        tasks: [
          {
            ...
            durationMins: 20,
            requires: [],
            timeWindow: { start: isoNow(), end: isoNow() },
          },
        ],
      };
      const ctx = buildDefaultCtx({
        guards: {
          weather: {
            enabled: true,
            allowSevere: false,
            current: { severity: "severe", code: "storm" },
          },
          sabbath: { enabled: false },
          quietHours: { enabled: false },
        },
