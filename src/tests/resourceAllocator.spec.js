...
function buildCtx(overrides = {}) {
  return {
    ...
    resources: {
      devices: {
        'oven-1': { kind: 'device', id: 'oven-1', capacity: 1, label: 'Main Oven' },
...
(mod ? describe : describe.skip)('Resource Allocator — device/person conflict prevention', () => {
  let eventBus;
  beforeEach(async () => {
  ...
  it('detects device double-booking conflicts (same device, overlapping windows)', async () => {
    ...
    const hasOvenConflict = result.conflicts.some(
      (c) => c.kind === 'device' && c.id === 'oven-1' && ['s1', 's2'].every((id) => c.sessionIds.includes(id))
    );
  ...
  it('detects person double-booking conflicts (same person, overlapping windows)', async () => {
    ...
    const hasAliceConflict = result.conflicts.some(
      (c) => c.kind === 'person' && c.id === 'alice' && ['s1', 's3'].every((id) => c.sessionIds.includes(id))
    );
  ...
  it('enforces capacity pools (e.g., stovetop has capacity 4 units across sessions)', async () => {
    ...
    const capConflict = result.conflicts.find((c) => c.kind === 'capacity' && c.id === 'stovetop');
  ...
  it('can auto-stagger overlapping sessions when allowed, resolving conflicts', async () => {
    ...
    const result = await allocateResources(sessions, ctx, { dryRun: true, autoStagger: true });
    ...
    if (result.ok) {
      ...
      const r1 = result.reservations.find((r) => r.sessionId === 's1' && r.resource.id === 'oven-1');
      const r2 = result.reservations.find((r) => r.sessionId === 's2' && r.resource.id === 'oven-1');
      expect(r1 && r2).toBeTruthy();
      expect(overlaps(r1, r2)).toBe(false);
      // Should report 'allocated' event when it finds a conflict-free plan
      const calls = eventBus.emit.mock.calls.filter((c) => c[0] === 'automation.event');
      const allocatedEvt = calls.map((c) => c[1]).find((p) => p.type === 'automation.resource.allocated');
  ...
  it('commits allocations when options.commit === true and emits committed event', async () => {
    ...
    const calls = eventBus.emit.mock.calls.filter((c) => c[0] === 'automation.event');
    const committedEvt = calls.map((c) => c[1]).find((p) => p.type === 'automation.resource.allocations.committed');
