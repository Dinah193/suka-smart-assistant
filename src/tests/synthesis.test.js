...
// ----------------------------------------------------------------------------
vi.mock("@/config/featureFlags.json", () => ({
...
try {
  ...
  PrepSynthesizer = (await import("../intelligence/PrepSynthesizer.js"))
    .default;
        ...
        recipe: [
          {
            id: "preheat-oven",
            kind: "preheat",
            details: { tempC: 180, leadMinutes: 15 },
          },
        ...
        cleaning: [
          {
            id: "mix-solution",
            kind: "staging",
            details: { ratio: "1:9", agent: "bleach" },
          },
        ...
        garden: [
          {
            id: "sanitize-tools",
            kind: "sanitize",
            details: { method: "alcohol" },
          },
          {
            id: "weather-check",
            kind: "guard",
            details: { requires: "dry-window" },
          },
        ],
        animal: [
        ...
        preservation: [
          {
            id: "sterilize-jars",
            kind: "sanitize",
            details: { method: "boil", minutes: 10 },
          },
          {
            id: "pressure-check",
            kind: "hazard",
            details: { regulator: "ok" },
          },
        ],
      };
...
function readinessCoverage(addedSteps) {
  ...
  for (const s of addedSteps || []) {
    ...
    if (kind.startsWith("timing") || kind === "preheat" || kind === "boil")
      have.add("timing");
  ...
  it.each(DOMAINS)(
    ...
    ({ name: domain, id }) => {
      ...
      const synth = events.find(
        (e) => e.type === "prep.synthesized" && e.data.domain === domain
      );
      ...
      if (ratio !== 1) {
        ...
        throw new Error(
          `Readiness coverage for "${domain}" below 100%: ${(
            ratio * 100
          ).toFixed(0)}% — missing [${missing.join(", ")}]`
        );
      }
      expect(ratio).toBe(1);
    }
