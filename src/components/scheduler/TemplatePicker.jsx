...
var CARD =
  "group relative rounded-2xl border border-gray-200 bg-white p-3 shadow-xs hover:shadow-sm transition";
...
try {
  ...
  useFavoriteTemplates =
    (favMod && (favMod.useFavoriteTemplates || favMod.default)) || null;
} catch (_) {}

var SaveTemplateModalLazy = null;
try {
  SaveTemplateModalLazy = React.lazy(function () {
    return import("@/components/scheduler/SaveTemplateModal.jsx");
  });
} catch (_) {}
...
var DEFAULT_TEMPLATES = [
  {
    ...
    tags: ["daily", "short"],
    estMin: 25,
    description: "Quick morning flow with prep and cleanup.",
  },
  {
    id: "sys:evening-reset",
    title: "Evening Reset",
    domain: "cleaning",
    tags: ["daily"],
    estMin: 40,
    description: "Surface wipe, quick tidy, dish cycle.",
  },
  {
    id: "sys:pet-care",
    title: "Pet Care",
    ...
    tags: ["weekly"],
    estMin: 30,
    description: "Feed, brush, litter refresh or walk.",
  },
];
...
export default function TemplatePicker(props) {
  ...
  /* Catalog state */
  var _list = useState(
      Array.isArray(props.templates) && props.templates.length
        ? props.templates
        : DEFAULT_TEMPLATES
    ),
    list = _list[0],
  ...
  var favApi = null;
  try {
    favApi = useFavoriteTemplates ? useFavoriteTemplates(domain) : null;
  } catch (_) {}
  var _favSet = useState(function () {
      return new Set();
    }),
    favSet = _favSet[0],
    setFavSet = _favSet[1];
  ...
  /* Selection */
  var _sel = useState(props.value || null),
    selected = _sel[0],
    setSelected = _sel[1];
  ...
  useEffect(
    function () {
      ...
      try {
        ...
        eventBus.emit &&
          eventBus.emit("templates.catalog.requested", {
            domain: domain,
  ...
  function selectTemplate(t) {
    ...
    try {
      eventBus.emit &&
        eventBus.emit("scheduler.template.selected", {
          domain: domain,
          template: t,
          source: "TemplatePicker",
        });
  ...
  function previewTemplate(t) {
    ...
    try {
      eventBus.emit &&
        eventBus.emit("template.open.preview", {
          domain: domain,
          template: t,
          source: "TemplatePicker",
        });
    ...
    try {
      if (favApi && favApi.toggleFavorite) {
        ...
        setFavSet(function (prev) {
          ...
          try {
            eventBus.emit &&
              eventBus.emit("template.favorite.toggled", {
                domain: domain,
                templateId: t.id,
                next: add,
                meta: { title: t.title },
          ...
          toastSafe(
            add
              ? "Added to favorite templates."
              : "Removed from favorite templates."
          );
          return s;
        });
      }
  ...
  function openSaveTemplate(t) {
    ...
    try {
      eventBus.emit &&
        eventBus.emit("template.save.modal.opened", {
          domain: domain,
          template: t,
          source: "TemplatePicker",
        });
  ...
  function exportTemplate(t) {
    try {
      var blob = new Blob([JSON.stringify(t, null, 2)], {
    ...
    try {
      ...
      el.onchange = function (e) {
        ...
        fr.onload = function () {
          ...
          if (!tpl || !tpl.title) {
            toastSafe("Invalid template file.", "error");
            return;
          }
          tpl.id = tpl.id || uid("tpl");
          setList(function (prev) {
            return [tpl].concat(prev || []);
          });
              ...
              <div className="absolute right-0 z-10 mt-2 min-w-[14rem] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl">
                ...
                <MenuItem
                  onClick={function () {
                    ...
                    setList(function (prev) {
                      return [t].concat(prev || []);
                    });
                    setQ("My custom template");
                    toastSafe(
                      "Added a new custom template. Edit and Save as needed."
                    );
                  }}
      ...
      <div className="px-4 py-4">
        {loading ? (
          ...
          <div className="text-sm text-gray-500">
            No templates. Try clearing filters or importing one.
          </div>
        ) : view === "list" ? (
          <ListView
            items={filtered}
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {props.items.map(function (t) {
        ...
        var selected =
          props.selected && (props.selected === t.id || props.selected === t);
        return (
          <article
            key={t.id || t.title}
            className={cx(CARD, selected && "ring-2 ring-indigo-500")}
          >
            <header className="flex items-start gap-2">
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-0.5 flex items-center gap-2 text-[11px] text-gray-600">
                  ...
                  {typeof t.estMin === "number" ? (
                    <span className={CHIP}>
                      <I.Clock className="h-3.5 w-3.5" /> ~{t.estMin}m
                    </span>
                  ) : null}
                </div>
              </div>
            ...
            <div className="mt-2 flex flex-wrap gap-1">
              ...
              {(t.tags || []).length > 4 ? (
                <span className={CHIP}>+{(t.tags || []).length - 4}</span>
              ) : null}
            </div>
                ...
                <details className="relative">
                  <summary
                    className={cx(
                      BTN,
                      VAR.ghost,
                      "cursor-pointer list-none [&::-webkit-details-marker]:hidden px-2"
                    )}
                  >
                    <I.More className="h-4 w-4" />
                  </summary>
                  <div className="absolute right-0 z-10 mt-2 min-w-[12rem] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl">
                    <MenuItem
                      ...
                      icon={<I.Check className="h-4 w-4" />}
                      label="Select"
                    />
                    <MenuItem
                      onClick={function () {
          ...
          >
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                <span className={CHIP}>
                  <I.Calendar className="h-3.5 w-3.5" /> {t.domain || "session"}
                </span>
                {typeof t.estMin === "number" ? (
                  <span className={CHIP}>
                    <I.Clock className="h-3.5 w-3.5" /> ~{t.estMin}m
              ...
              <div className="mt-1 flex flex-wrap gap-1">
                {(t.tags || []).slice(0, 6).map(function (tag) {
                  return (
                    <span key={tag} className={CHIP}>
                      <I.Tag className="h-3.5 w-3.5" />
                      {tag}
...
function InlineSaveTemplate(props) {
  var _name = useState("My template"),
    name = _name[0],
    setName = _name[1];
  var _desc = useState(""),
    desc = _desc[0],
    setDesc = _desc[1];
  var _tags = useState("custom"),
    tags = _tags[0],
    setTags = _tags[1];
  ...
  function submit() {
    ...
    try {
      ...
      try {
        eventBus.emit &&
          eventBus.emit("template.save.requested", {
            payload: payload,
            source: "TemplatePicker",
          });
      } catch (_) {}
