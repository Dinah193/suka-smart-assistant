...
export default function SchedulerDevPanel(props) {
  ...
  let favApi = null;
  try {
    favApi = useFavoriteSessions ? useFavoriteSessions(domain) : null;
  } catch (_) {}

  const [favList, setFavList] = useState(() =>
    favApi && favApi.list ? favApi.list() : []
  );
  ...
  function refreshFavorites() {
    try {
      setFavList(favApi && favApi.list ? favApi.list() : []);
    } catch (_) {}
  }
  ...
  function toggleFav(id, meta) {
    try {
      ...
      const next = favApi.toggleFavorite(
        id,
        meta || { id, title: String(id), domain }
      );
      ...
      toastSafe("Favorite toggle failed.", "error");
    }
  }
  ...
  function startSession(idOrTitle) {
    try {
      ...
      toastSafe("Could not start session.", "error");
    }
  }
  ...
  function pauseAll() {
    try {
      if (automation && automation.scheduler && automation.scheduler.pauseAll) {
        const maybe = automation.scheduler.pauseAll({
          domain,
          source: "SchedulerDevPanel",
        });
        maybe && typeof maybe.then === "function"
          ? maybe.then(() => toastSafe("Pause requested."))
          : toastSafe("Pause requested.");
      } else {
        eventBus.emit &&
          eventBus.emit("scheduler.pause.requested", {
            domain,
            source: "SchedulerDevPanel",
          });
        toastSafe("Pause requested.");
  ...
  useEffect(() => {
    ...
    const onTimerRemoved = (p = {}) => {
      setTimers((cur) =>
        Array.isArray(cur) ? cur.filter((x) => x.id !== p.id) : []
      );
    ...
    const onGuardHit = (p = {}) => {
      setGuards((cur) =>
        [
          {
            id: p.id || p.kind || Math.random().toString(36).slice(2, 7),
            kind: p.kind || "guard",
            ok: !!p.ok,
            notes: p.notes || "",
            atISO: p.atISO || nowISO(),
          },
          ...cur,
        ].slice(0, 200)
      );
    ...
    const onConflict = (p = {}) => {
      pushLog("planner.conflict.detected", p);
    };
    const onShortage = (p = {}) => {
      pushLog("inventory.shortage.detected", p);
    };
    const onScheduleSaved = (p = {}) => {
      pushLog("schedule.saved", p);
    };
    const onRuleSaved = (p = {}) => {
      pushLog("automation.rule.saved", p);
    };
    ...
    return () => {
      try {
        eventBus.off &&
          eventBus.off("automation.timer.upserted", onTimerUpsert);
        eventBus.off &&
          eventBus.off("automation.timer.removed", onTimerRemoved);
        eventBus.off &&
          eventBus.off("schedule.anchor.upserted", onAnchorUpsert);
        eventBus.off &&
          eventBus.off("schedule.anchor.removed", onAnchorRemoved);
  ...
  const filteredTimers = useMemo(
    () =>
      !q
        ? timers
        : timers.filter((t) => JSON.stringify(t).toLowerCase().includes(q)),
    [timers, q]
  );
  const filteredAnchors = useMemo(
    () =>
      !q
        ? anchors
        : anchors.filter((a) => JSON.stringify(a).toLowerCase().includes(q)),
    [anchors, q]
  );
  const filteredGuards = useMemo(
    () =>
      !q
        ? guards
        : guards.filter((g) => JSON.stringify(g).toLowerCase().includes(q)),
    [guards, q]
  );
  const filteredFavs = useMemo(
    () =>
      !q
        ? favList
        : favList.filter((f) => JSON.stringify(f).toLowerCase().includes(q)),
    [favList, q]
  );
  const filteredLogs = useMemo(
    () =>
      !q
        ? logs
        : logs.filter((l) => JSON.stringify(l).toLowerCase().includes(q)),
    [logs, q]
  );
    ...
    try {
      ...
      eventBus.emit &&
        eventBus.emit("automation.rule.export.requested", {
          provider,
          rule: dump,
          filename: `scheduler-dev-${domain}.json`,
          source: "SchedulerDevPanel",
        });
    ...
    try {
      ...
      inp.onchange = (ev) => {
        ...
        fr.onload = () => {
          ...
          if (
            Array.isArray(data.favList) &&
            data.favList.length &&
            favApi &&
            favApi.toggleFavorite
          ) {
            // Merge imported favorites into local
            data.favList.forEach((f) => {
              try {
                favApi.toggleFavorite(f.id, f);
              } catch (_) {}
            });
            refreshFavorites();
          }
    ...
    <section className={WRAP} aria-label="Scheduler Dev Panel">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-gray-200 bg-gradient-to-r from-white to-gray-50 px-4 py-3 rounded-t-3xl">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button className={cx(BTN, VAR.subtle)} onClick={pauseAll}>
      ...
      <div className="px-4 pt-3">
        <div className="flex flex-wrap gap-2">
          <Tab
            label="Timers"
            id="timers"
            icon={<I.Timer className="h-4 w-4" />}
            active={tab === "timers"}
      ...
      <div className="px-4 py-4">
        {tab === "timers" && (
          <TimersTable items={filteredTimers} onStart={startSession} />
        )}
...
function Tab({ label, id, icon, active, onClick, count }) {
  return (
    <button
      className={cx(BTN, active ? VAR.primary : VAR.subtle)}
      onClick={onClick}
    >
      {icon}
      {label}
...
function TimersTable({ items, onStart }) {
  if (!items || !items.length)
    return (
      <EmptyState
        icon={<I.Timer className="h-5 w-5" />}
        text="No timers yet."
      />
    );
  return (
        ...
        <tbody className="divide-y divide-gray-100">
          {items.map((t) => (
            <tr key={t.id}>
              ...
              <Td>{t.title || "—"}</Td>
              <Td>
                <span className={CHIP}>{t.etaISO || "—"}</span>
              </Td>
              <Td>
                <span className={CHIP}>{t.kind || "timeout"}</span>
              </Td>
              <Td>
              ...
              <Td>
                {!!(t.meta && t.meta.sessionIdOrTitle) ? (
                  <button
                    ...
                    onClick={() => onStart && onStart(t.meta.sessionIdOrTitle)}
                  >
        ...
        <tbody className="divide-y divide-gray-100">
          {items.map((a) => (
            <tr key={a.id}>
              ...
              <Td>
                <span className={CHIP}>
                  {formatTriggerValue(a.type, a.value)}
                </span>
              </Td>
              <Td>
                <span className={CHIP}>{a.nextISO || "—"}</span>
              </Td>
              <Td>
                <pre className="text-[11px] whitespace-pre-wrap">
...
function GuardsTable({ items }) {
  if (!items || !items.length)
    return (
      <EmptyState
        icon={<I.Shield className="h-5 w-5" />}
        text="No guard hits yet."
      />
    );
  return (
    <div className="overflow-x-auto rounded-2xl border border-gray-200">
      <table className="min-w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-100">
          {items.map((g, idx) => (
            <tr key={g.id || idx}>
              <Td>
                <span className={CHIP}>{g.atISO || "—"}</span>
              </Td>
              <Td>
                <span className={CHIP}>{g.kind || "guard"}</span>
              </Td>
              <Td>
                {g.ok ? (
                  <span className={cx(CHIP, "border-green-300 text-green-700")}>
                    yes
                  </span>
                ) : (
                  <span className={cx(CHIP, "border-rose-300 text-rose-700")}>
                    no
                  </span>
                )}
              </Td>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <button className={cx(BTN, VAR.primary)} onClick={onOpenSave}>
      ...
      {!items || !items.length ? (
        <EmptyState
          icon={<I.Star className="h-5 w-5" />}
          text="No favorite sessions yet. Save one to get started."
        />
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((f) => (
            <div key={f.id} className="rounded-2xl border border-gray-200 p-3">
              ...
              {f.notes ? (
                <div className="mt-1 text-[12px] text-gray-600 line-clamp-2">
                  {f.notes}
                </div>
              ) : null}
              <div className="mt-3 flex items-center gap-2">
                <button
                  className={cx(BTN, VAR.subtle)}
                  onClick={() => onStart && onStart(f.id || f.title)}
                >
                ...
                <button
                  ...
                  onClick={() => onToggleFav && onToggleFav(f.id, f)}
                >
...
function LogPanel({ items }) {
  if (!items || !items.length)
    return (
      <EmptyState icon={<I.Bug className="h-5 w-5" />} text="No events yet." />
    );
  return (
