...
export default function MealSlot({
  ...
  const totals = useMemo(
    () => sumNutrition(local, recipesStore),
    [local, recipesStore]
  );
  ...
  const autoFill = async () => {
    setBusy(true);
    const fn = async () => {
      const result = await automation?.("meal.autofillSlot", {
        dayKey,
        slotName,
      });
  ...
  const nbaActions = useMemo(
    () => [
      ...
      },
      {
        ...
        label: "Balance",
        onClick: balanceSlot,
        disabled: busy || !editable || !local.length,
      },
      {
        key: "clear",
        label: "Clear",
        onClick: clearSlot,
        disabled: !local.length || !editable,
      },
    ],
    [busy, editable, local]
  );
    ...
    () => [
      {
        ...
        goal: Math.max(1, Math.round(goals.calories || 0)),
      },
      {
        key: "protein",
        label: "P",
        value: Math.round(totals.protein || 0),
        goal: Math.max(1, Math.round(goals.protein || 0)),
      },
      {
        key: "carbs",
        label: "C",
        value: Math.round(totals.carbs || 0),
        goal: Math.max(1, Math.round(goals.carbs || 0)),
      },
      {
        key: "fat",
        label: "F",
        value: Math.round(totals.fat || 0),
        goal: Math.max(1, Math.round(goals.fat || 0)),
      },
    ],
    [totals, goals]
  );
    ...
    >
            ...
            <div className="relative" ref={menuRef}>
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-1 w-40 rounded-lg border border-base-200 bg-base-100 shadow-lg z-20">
                  ...
                  <button
                    className="w-full text-left px-3 py-2 text-error hover:bg-base-200"
                    onClick={() => {
                      clearSlot();
                      setMenuOpen(false);
                    }}
      ...
      {!local.length && (
        <div className="mt-2 text-sm text-base-content/60 italic">
          No items. Click <span className="not-italic font-medium">Add</span>,
          use <span className="not-italic font-medium">Auto</span>, or drop a
          recipe here.
        </div>
      )}
...
function MealItemRow({
  ...
  const full = useMemo(
    () => resolveRecipe(item, recipesStore),
    [item, recipesStore]
  );
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          {draggable && (
            <span className="select-none text-base-content/60">⋮⋮</span>
          )}
          <div className="font-medium truncate">
            {full?.title || full?.name || "Recipe"}
          </div>
          {full?.servings ? (
            <span className="badge badge-ghost">{full.servings} sv</span>
          ) : null}
        </div>
        {showMacros && n && (
          <div
          ...
          >
            {Math.round(n.calories || 0)} kcal • P{Math.round(n.protein || 0)} /
            C{Math.round(n.carbs || 0)} / F{Math.round(n.fat || 0)}
          </div>
        )}
      </div>
