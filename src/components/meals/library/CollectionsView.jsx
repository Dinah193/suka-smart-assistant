    ...
    <div className="rounded-2xl border p-2 bg-white/80 backdrop-blur hover:shadow-sm transition">
      ...
      <div className="mt-2 flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="text-[11px] text-gray-600 mt-0.5 flex items-center gap-2">
            <span className="inline-flex items-center gap-1">
              <FolderOpenDot className="w-3.5 h-3.5" /> {recipes} recipe
              {recipes === 1 ? "" : "s"}
            </span>
            <span className="inline-flex items-center gap-1">
              <Link2 className="w-3.5 h-3.5" /> {atts} attributed
            </span>
          </div>
        </div>
        <div className="shrink-0 inline-flex items-center gap-1">
          ...
          <button
            className="px-2 py-1 rounded-md border hover:bg-gray-50 text-xs"
            onClick={() => onMergePick?.(c)}
            title="Merge into…"
          >
            <GitMerge className="w-3.5 h-3.5" />
          </button>
          <button
            className="px-2 py-1 rounded-md border hover:bg-rose-50 text-rose-700 text-xs"
            onClick={() => onDelete?.(c)}
            title="Delete"
          >
...
const CollectionsView = ({
  ...
  const filtered = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      if (sortBy === "count")
        return (fmtCount(a.recipeCount) - fmtCount(b.recipeCount)) * dir;
  ...
  const renameCollection = (c) => {
    ...
    const next = items.map((x) =>
      x.id === c.id ? { ...x, name, updatedAt: Date.now() } : x
    );
    applyChange(next);
    try {
      lib?.renameCollection?.(c.id, name);
    } catch {}
  ...
  const deleteCollection = (c) => {
    if (!confirm(`Delete “${c?.name}”? Recipes remain in your library.`))
      return;
  ...
  const mergeInto = (source, target) => {
    ...
    try {
      lib?.mergeCollections?.(source.id, target.id);
    } catch {}
    const next = items
      .map((x) =>
        x.id === target.id
          ? {
              ...x,
              recipeCount:
                fmtCount(x.recipeCount) + fmtCount(source.recipeCount),
              updatedAt: Date.now(),
            }
          : x
      )
    ...
    emit("library.collection.merged", {
      from: source.id,
      to: target.id,
      householdId,
    });
  ...
  const BulkBar = () => {
    ...
    const doBulkDelete = () => {
      ...
      if (
        !confirm(
          `Delete ${ids.length} collection${ids.length === 1 ? "" : "s"}?`
        )
      )
        return;
      ...
      applyChange(next);
      try {
        ids.forEach((id) => lib?.deleteCollection?.(id));
      } catch {}
      clearSelection();
    ...
    const doBulkExport = () => {
      emit("library.collections.export.requested", {
        ids,
        format: "json",
        householdId,
      });
    ...
    const pickMergeTarget = () => {
      ...
      const name = prompt(
        `Merge selected into which collection?\n\nOptions: ${names}`
      );
      ...
      <div className="sticky top-1 z-10 rounded-xl border bg-white/90 backdrop-blur px-2 py-2 mb-2 flex items-center gap-2">
        ...
        {SendToMenu ? (
          <SendToMenu
            ...
            onSent={(dest) =>
              emit("library.collections.sent", { ids, dest, householdId })
            }
          />
        ) : (
        ...
        <button
          className="ml-auto px-2 py-1 rounded-md border hover:bg-rose-50 text-rose-700 text-xs"
        ...
        <div className="mt-2 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2">
          {opts.map((o) => (
            <button
              ...
              onClick={() => {
                mergeInto(mergeSource, o);
                setMergeSource(null);
              }}
            >
              {o.name}
              <div className="text-[11px] text-gray-600">
                {fmtCount(o.recipeCount)} recipes
              </div>
            </button>
          ))}
        </div>
