...
const noopToast = (level, message) =>
  console[level === "error" ? "error" : "log"](message);
let toast = {
  success: (m) => noopToast("log", m),
  error: (m) => noopToast("error", m),
  info: (m) => noopToast("log", m),
};
try {
...
const DeciderQuickActions = ({
  ...
  const emit = (type, payload) => {
    try {
      eventBus?.emit?.(`meals.decider.quick.${type}`, payload);
    } catch {}
    try {
      automation?.runTemplate?.(`meals.decider.quick.${type}`, payload);
    } catch {}
  ...
  const doAddToPlan = async (mode = "add") => {
    ...
    try {
      ...
      toast.success(
        mode === "replace" ? "Replaced in plan." : "Added to plan."
      );
    } catch (e) {
  ...
  const doCookNow = () => {
    if (sabbathNow) {
      toast.info(
        "Sabbath Guard is active — switching to Reheat/Cold prep mode."
      );
    ...
    const payload = { decision, householdId, ts: Date.now() };
    try {
      eventBus?.emit?.("meals.session.start", payload);
    } catch {}
  ...
  const Btn = ({ title, onClick, disabled, icon: Ico, children }) => (
    <button
      type="button"
      className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 ${
  ...
  return (
    <div className={`rounded-xl border p-3 bg-white/80 backdrop-blur`}>
        ...
        <div className="min-w-0">
          ...
          <div className="text-xs text-gray-600">
            ...
            {feastLabel ? (
              <>
                {" "}
                • <span className="text-blue-700">{feastLabel}</span>
              </>
            ) : null}
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Btn
          title="Replace in plan"
          onClick={() => doAddToPlan("replace")}
          icon={RotateCw}
          disabled={busy}
        >
          Replace
        ...
        <Btn
          title="Add to Batch Cooking Queue"
          onClick={doBatchCook}
          icon={Sparkles}
        >
          Batch
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <Btn
          title={locked ? "Unlock slot" : "Lock into slot"}
          onClick={toggleLock}
        ...
        <Btn
          ...
          onClick={doCopyNextWeek}
          icon={CalendarPlus}
        >
          Next week
        </Btn>
        <Btn title="Export Household Menu" onClick={doExportMenu} icon={Upload}>
          Export menu
        </Btn>
        {decision?.estCookMins ? (
          <span className="ml-auto inline-flex items-center gap-1 text-[11px] text-gray-600">
            <Timer className="w-3.5 h-3.5" />~{decision.estCookMins} min
          </span>
        ) : null}
      </div>
      ...
      <div className="mt-2 text-[11px] text-gray-500 flex items-start gap-1.5">
        ...
        <span>
          “Cook now” respects Sabbath Guard; on feast days we’ll keep
          feast-aligned dishes visible and prefer make-ahead or reheat options.
        </span>
      </div>
    </div>
  );
