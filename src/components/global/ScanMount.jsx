...
import '../../features/scan-compare-trust/styles/scan.css';
...
const LazyScanner = React.lazy(() =>
  import('../../app/features/scan-compare-trust/components/Scanner.js').catch(() => ({ default: () => null }))
);
const LazyScanSheet = React.lazy(() =>
  import('../../app/features/scan-compare-trust/components/ScanSheet.js').catch(() => ({ default: () => null }))
);
...
function useSukaConfig() {
  ...
  return {
    sabbathGuard: cfg.sabbathGuard ?? { enabled: false, start: 'Friday 18:00', end: 'Saturday 19:00' },
    quietHours: cfg.quietHours ?? { enabled: false, start: '22:00', end: '07:00' },
  };
}
...
function isSabbathActive({ enabled, start, end }, d = new Date()) {
  ...
  const fri = new Date(d); fri.setDate(d.getDate() - ((dow + 2) % 7)); // previous Friday
  const sat = new Date(fri); sat.setDate(fri.getDate() + 1);
  const inFri = new Date(fri);
  const inSat = new Date(sat);
  return nowInRange(start || '18:00', '23:59', d) && d.toDateString() === fri.toDateString()
      || nowInRange('00:00', end || '19:00', d) && d.toDateString() === sat.toDateString();
}
...
export default function ScanMount() {
  ...
  function guardMessage() {
    ...
    if (quietHours?.enabled && nowInRange(quietHours.start, quietHours.end, now)) {
    ...
    <>
      ...
      >
        ...
        <svg width="26" height="26" viewBox="0 0 24 24" role="img" aria-hidden="true">
          ...
          <path d="M7 12h1m2 0h1m2 0h1" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
        </svg>
              ...
              <div className="inline-flex rounded-full border border-[var(--scan-border)] p-1">
                {['scan','compare','favorites'].map((k) => (
                  <button
                    key={k}
                    onClick={() => setTab(k)}
                    className={`px-3 py-1 rounded-full text-sm ${tab===k ? 'bg-[var(--scan-badge-bg)] text-[var(--scan-badge-tx)]' : 'text-[var(--scan-text)]'}`}
                    aria-pressed={tab===k}
                  >
                    {k === 'scan' ? 'Scan' : k === 'compare' ? 'Compare' : 'Favorites'}
                  </button>
                ))}
              </div>
            ...
            {tab === 'scan' && (
              <div className="scan-card p-3">
                <div className="flex items-center justify-between mb-2">
                  <h2 id="scan-sheet-title" className="text-base font-semibold">Scan Items</h2>
                  <button onClick={close} className="fav-toggle" aria-label="Close scan panel">✕</button>
                </div>

                <div className="grid grid-cols-3 gap-2">
                  <button onClick={startBarcodeScan} className="scan-chip" data-variant="success">Barcode</button>
                  <button onClick={startPhotoOCR} className="scan-chip">Photo / Doc OCR</button>
                  <button onClick={importPDF} className="scan-chip">Import PDF Ad</button>
                </div>
                ...
                <Suspense fallback={<div className="scan-skel h-32 w-full" />}>
                  ...
                  <LazyScanner onResult={(payload) => {
                    bus.emit('scan:result', { message: payload?.label || 'Item captured', payload });
                  }} />
                </Suspense>
              </div>
            )}
            ...
            {tab === 'compare' && (
              <div className="scan-card p-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={openFavorites} className="scan-chip">Favorites →</button>
                </div>
            ...
            {tab === 'favorites' && (
              <div className="scan-card p-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={openCompare} className="scan-chip">Back to Compare</button>
                </div>
                {/* Minimal empty state – hook into your real Favorites panel */}
                <div className="scan-empty">
                  Save sessions, schedules, or items while scanning to see them here.
                </div>
              </div>
            )}
          </div>
      ...
      {/* Tiny toast */}
      <div className="scan-toast" data-open={!!toast} role="status" aria-live="polite">
        {toast}
      </div>
    </>,
    portalRef.current
