...
function PlanCard({
  ...
  const mine =
    plan?.meta?.source === "user" && plan?.meta?.createdBy === userId;
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {mine && (
              <span className="text-xs text-emerald-700 bg-emerald-50 rounded-full px-1.5 py-0.5">
                Mine
              </span>
            )}
            {featured && (
              <span className="text-xs text-purple-700 bg-purple-50 rounded-full px-1.5 py-0.5">
                Featured
              </span>
            )}
          </div>
          <h4 className="mt-1 font-semibold text-sm md:text-base line-clamp-1">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              onFavoriteToggle?.(plan);
            }}
          >
            <IconHeart
              className={classNames(
                "h-5 w-5",
                plan.isFavorite ? "fill-rose-600" : "fill-gray-500"
              )}
              filled={!!plan.isFavorite}
            />
          </button>
...
export default function FavoritePicker({
  ...
  const refreshFallback = useCallback(async () => {
    ...
    try {
      const scopeMine = await router().listPlans({
        scope: "user",
        userId,
        domain: localDomain,
      ...
      const scopeFeatured = await router().listPlans({
        scope: "global",
        domain: localDomain,
        includeDeleted: false,
        limit: 1000,
      });
      const favs = await router().listFavorites({
        userId,
        domain: localDomain,
        limit: 1000,
      });
      ...
      if (tab === "mine") pool = scopeMine;
        ...
        pool = scopeMine.concat(scopeFeatured).filter((p) => favSet.has(p.id));
  ...
  const toggleFavorite = async ({ planId, domain: dom }) => {
    if (hasHook && hook?.toggleFavorite)
      return hook.toggleFavorite({ planId, domain: dom });
    if (!routerReady || !router()) return;
    const res = await router().toggleFavorite({ planId, userId, domain: dom });
    eventBus.emit?.("favorite.updated", {
      planId,
      userId,
  ...
  const adoptFeatured = async ({
    userId: uid,
    domain: dom,
    planId,
    favorite,
  }) => {
    if (hasHook && hook?.adoptFeatured)
      return hook.adoptFeatured({ userId: uid, domain: dom, planId, favorite });
    if (!routerReady || !router()) return;
    ...
    const saved = await router().savePlan(copy, {
      scope: "user",
      userId: uid,
      favorite: !!favorite,
    });
  ...
  async function handleFavoriteToggle(plan) {
    try {
      await toggleFavorite({
        planId: plan.id,
        domain: plan.domain || localDomain,
      });
      eventBus.emit?.("toast.show", {
        level: "success",
        title: plan.isFavorite
  ...
  async function handleQuickAdopt() {
    const first = items.find(
      (p) => selectedIds.has(p.id) && p.meta?.source === "featured"
    );
    if (!first) return;
    await adoptFeatured?.({
      userId,
      domain: first.domain || localDomain,
      planId: first.id,
      favorite: true,
    });
    eventBus.emit?.("toast.show", {
    ...
    <div className={classNames("w-full", className)}>
          ...
          <div className="col-span-full">
            <div className="rounded-2xl border border-dashed p-8 text-center">
              <p className="text-sm text-gray-600">
                No plans found. Try a different search, remove filters, or
                switch tabs.
              </p>
              {tab !== "featured" && (
                <p className="mt-2 text-xs text-gray-500">
                  Tip: Save any featured plan to your library using “Save to My
                  Plans.”
                </p>
              )}
            </div>
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="text-xs text-gray-500">
          {state?.snapshot?.favoriteCount ?? 0} favorites •{" "}
          {state?.snapshot?.myPlanCount ?? 0} in My Plans
        </div>
        <div className="flex items-center gap-2">
          <button
            type="button"
          ...
          <button
            ...
            disabled={[...selectedIds].every(
              (id) =>
                items.find((p) => p.id === id)?.meta?.source !== "featured"
            )}
