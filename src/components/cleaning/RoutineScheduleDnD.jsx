...
import {
  Play, Save, RefreshCcw, CalendarPlus, Sparkles,
  Undo2, Redo2, Trash2, Shield, DoorOpen, Inbox, Leaf, Info
} from "lucide-react";
...
const DEFAULT_BANK = [
  ...
  { id: "task-3", name: "Disinfect doorknobs", area: "entry", estMinutes: 3, cadence: "weekly" },
  ...
  { id: "task-6", name: "Mail to basket (no counters)", area: "entry", estMinutes: 1, tags: ["paper-inbox"] },
  { id: "task-7", name: "Crumb wipe near pet bowls", area: "kitchen", estMinutes: 4, cadence: "monthly", tags: ["bug-shield"] },
  { id: "task-8", name: "Harvest basket to sink", area: "kitchen", estMinutes: 5, tags: ["harvest"] },
];

const cadenceOptions = ["daily", "weekly", "monthly", "quarterly", "bi-annual", "annual", "none"];
...
export default function RoutineScheduleDnD() {
  ...
  const totals = useMemo(() => {
    ...
    for (const d of weekdays) {
      map[d] = (routine[d] || []).reduce((acc, t) => acc + (t.estMinutes || 3), 0);
    }
    return map;
  }, [routine]);
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && k === "z") { e.preventDefault(); routineHistory.undo(); }
      if ((e.ctrlKey || e.metaKey) && (k === "y" || (k === "z" && e.shiftKey))) { e.preventDefault(); routineHistory.redo(); }
    };
  ...
  const onDragEnd = (result) => {
    ...
    // Copy from bank → day
    if (source.droppableId === "taskBank" && weekdays.includes(destination.droppableId)) {
      const draggedTask = taskBank.find((t) => t.id === draggableId);
      ...
      routineHistory.set({
        ...routine,
        [destination.droppableId]: [...(routine[destination.droppableId] || []), newTask],
      });
      return;
    }

    // Move within/between days
    const srcDay = source.droppableId;
    const dstDay = destination.droppableId;
    if (weekdays.includes(srcDay) && weekdays.includes(dstDay)) {
      const srcList = Array.from(routine[srcDay] || []);
      const [moved] = srcList.splice(source.index, 1);
      const dstList = Array.from(routine[dstDay] || []);
  ...
  /* --------------------------------- Helpers --------------------------------- */
  const setDay = (day, list) => routineHistory.set({ ...routine, [day]: list });

  const applyCadenceToDay = (day, cadence) => {
    const list = (routine[day] || []).map((t) => ({ ...t, cadence: cadence === "none" ? null : cadence }));
    setDay(day, list);
    eventBus.emit("ui:toast", { type: "success", message: `Applied ${cadence} cadence to ${day}` });
  };
    ...
    weekdays.forEach((day) => {
      (routine[day] || []).forEach((t, i) => {
        ...
        tasks.push({
          ...
          meta: { trigger: t.trigger || "onEntry", role: t.role || "household", sabbathBlocked: !!t.sabbathBlocked, tags: t.tags || [], day },
        });
      });
    });
    return tasks;
  ...
  const scheduleCadenceTasks = (tasks) => {
    tasks.filter((t) => !!t.cadence).forEach((t) => {
      ...
      eventBus.emit("calendar:create:rrule", {
        ...
        rrule,
        tz,
        meta: { source: "RoutineScheduleDnD", cadence: t.cadence, day: t.meta?.day },
      });
    });
  };
  ...
  const handleGenerateRoutine = async () => {
    ...
    if (!tasks.length) {
      eventBus.emit("ui:toast", { type: "warning", message: "Add tasks to at least one day first." });
      return;
    }
    // Create ad-hoc plan
    if (CleaningPlanManager?.createAdhocPlan) {
      try {
        const plan = CleaningPlanManager.createAdhocPlan({
          title: `${name} (Weekly)`,
          tasks,
          meta: { source: "RoutineScheduleDnD", createdAt: new Date().toISOString() },
        });
        eventBus.emit("cleaning:plan:created", { planId: plan?.id, source: "routine-schedule" });
      } catch (e) {
        console.error(e);
        eventBus.emit("ui:toast", { type: "error", message: "Could not create cleaning plan." });
      }
    }
  ...
  const injectStrategy = async (strategyId) => {
    ...
    try {
      ...
      if (!tasks.length) {
        eventBus.emit("ui:toast", { type: "info", message: "No tasks returned from strategy." });
        return;
      }
      ...
      routineHistory.set({ ...routine, Monday: [...(routine.Monday || []), ...mapped] });
      eventBus.emit("ui:toast", { type: "success", message: "Added suggested tasks to Monday. Drag to other days as needed." });
    } catch (e) {
      console.error(e);
    ...
    <div className="p-4 md:p-6 bg-white border border-yellow-200 rounded-xl shadow-md">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold">Routine Schedule (Drag & Drop)</h2>
        </div>
        <div className="flex items-center gap-2">
          <button className="btn" onClick={() => routineHistory.undo()} disabled={!routineHistory.canUndo}>
            <Undo2 className="w-4 h-4 mr-1" /> Undo
          </button>
          <button className="btn" onClick={() => routineHistory.redo()} disabled={!routineHistory.canRedo}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        ...
        <div className="p-3 border rounded-xl flex items-center justify-between">
          <div className="text-sm text-gray-700">
            Sabbath Guard: {sabbathAware ? "On" : "Off"} {sabbathActive && <span className="ml-2 text-amber-600">(active now)</span>}
          </div>
          <button
            className="btn"
            onClick={() => {
              ...
              if (PreferencesStore.setState) {
                PreferencesStore.setState({ ...pref, sabbathAware: !pref.sabbathAware });
              }
              eventBus.emit("ui:toast", { type: "info", message: "Toggled Sabbath guard in preferences." });
            }}
          >
            Toggle
          </button>
        ...
        <div className="p-3 border rounded-xl">
          <div className="text-xs text-gray-500 mb-1">Quick Add from Strategies</div>
          <div className="flex flex-wrap gap-2">
            <button className="btn" disabled={busy} onClick={() => injectStrategy("daily-reset")}>
              <DoorOpen className="w-4 h-4 mr-1" /> Daily Reset
            </button>
            <button className="btn" disabled={busy} onClick={() => injectStrategy("bug-shield-perimeter")}>
              <Shield className="w-4 h-4 mr-1" /> Bug-Shield
            </button>
            <button className="btn" disabled={busy} onClick={() => injectStrategy("paper-inbox-zero")}>
              <Inbox className="w-4 h-4 mr-1" /> Paper Inbox
            </button>
            <button className="btn" disabled={busy} onClick={() => injectStrategy("garden-to-pantry")}>
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        {/* Task bank */}
        <DraggableTaskBank taskBank={taskBank} />
          ...
          {weekdays.map((day) => (
            <div key={day} className="border rounded-2xl p-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="text-xs text-gray-500">{totals[day]} min</div>
              </div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <select
                  ...
                  onChange={(e) => applyCadenceToDay(day, e.target.value)}
                  defaultValue="__"
                >
                ...
                <button className="btn" onClick={() => clearDay(day)}>
      ...
      <div className="mt-4 p-3 border rounded-2xl bg-white">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button className="btn" onClick={() => eventBus.emit("ui:refresh", { scope: "routine-schedule" })}>
            <RefreshCcw className="w-4 h-4 mr-1" /> Refresh
          </button>
          <div className="ml-auto text-sm text-gray-600 flex items-center gap-2">
            <Sparkles className="w-4 h-4" />
            Drag from the bank to any day. Use “Generate Routine” to create an ad-hoc plan.
          </div>
        </div>
      </div>
