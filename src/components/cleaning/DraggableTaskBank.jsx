...
/* -------------------------------------------------------------------------- */
/* react-dnd draggable card                                                   */
/* -------------------------------------------------------------------------- */

function TaskCard({ task, index, renderMeta, onClickTask, onEditTask }) {
  const [{ isDragging }, dragRef] = useDrag(
    () => ({
      type: "ROUTINE_TASK",
      item: {
        ...task,
        __source: "DraggableTaskBank",
        __index: index,
      },
      collect: (monitor) => ({
  ...
  return (
    <div
      ...
      className={`bg-white px-3 py-2 mb-2 rounded border cursor-move
                  ${
                    isDragging
                      ? "opacity-70 shadow-md border-amber-300"
      ...
      aria-label={`${task.name}${
        task.estMin ? `, ${task.estMin} minutes` : ""
      }`}
      title="Drag to a day"
    >
...
function DraggableTaskBankCore({
  taskBank,
  title = "🧰 Task Bank",
  searchable = true,
  filterable = true,
  ...
  const [q, setQ] = useState(() => {
    try {
      return localStorage.getItem("taskBank.q") || "";
    } catch {
      return "";
    }
  });
  const [deepOnly, setDeepOnly] = useState(() => {
    try {
      ...
      return defaultDeepOnly;
    }
  });
  const [showMissingOnly, setShowMissingOnly] = useState(() => {
    try {
      return JSON.parse(
        localStorage.getItem("taskBank.missingOnly") || "false"
      );
    } catch {
      return false;
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem("taskBank.q", q);
    } catch {}
  }, [q]);
  useEffect(() => {
    try {
      localStorage.setItem("taskBank.deepOnly", JSON.stringify(deepOnly));
    } catch {}
  }, [deepOnly]);
  useEffect(() => {
    try {
      localStorage.setItem(
        "taskBank.missingOnly",
        JSON.stringify(showMissingOnly)
      );
    } catch {}
  }, [showMissingOnly]);
  ...
  const renderMeta = useCallback(
    (task) => {
      ...
      const hasMissing =
        needs.missingSupplies.length || needs.missingTools.length;

      return (
        <div className="flex flex-wrap items-center gap-2 mt-1">
          {est > 0 ? (
            <Badge icon={Timer} title={`Estimated ${est} min`}>
              {est}m
            </Badge>
          ) : null}
          ...
          {hasMissing ? (
            <Badge icon={AlertTriangle} title="Requirements missing">
              {needs.missingTools.length
                ? `${needs.missingTools.length} tool`
                : ""}
              {needs.missingTools.length && needs.missingSupplies.length
                ? " • "
                : ""}
              {needs.missingSupplies.length
                ? `${needs.missingSupplies.length} supply`
                : ""}
            </Badge>
          ) : null}
        </div>
...
export default function DraggableTaskBank(props) {
  return (
    <DndProvider backend={HTML5Backend}>
      <DraggableTaskBankCore {...props} />
    </DndProvider>
  );
}
...
function EmptyState({ query }) {
  return (
    ...
    >
      {query ? (
        <>
          No tasks match <span className="font-medium">“{query}”</span>. Try
          different keywords.
        </>
      ) : (
...
(function runSelfTests() {
  try {
    ...
    console.assert(
      filtered.length === 1 && filtered[0].id === "t1",
      "[TEST] search filter by keyword"
    );
  } catch (e) {
    if (typeof console !== "undefined")
      console.warn(
        "DraggableTaskBank self-tests skipped/failed:",
        e?.message || e
      );
  }
})();
