...
// ---------------- safe requires ----------------
let eventBus = { emit: (...a) => console.debug("[RemoteQR:eventBus.emit]", ...a) };
try {
...
function emitEvent(type, data = {}) {
  const payload = { type, ts: isoNow(), source: "components.cooking.RemoteQR", data };
...
function defaultUrlBuilder(room) {
  if (typeof window === "undefined") return `/cooking/remote/${encodeURIComponent(room)}`;
...
function FallbackSvg({ text = "" }) {
  return (
    <svg
      ...
      style={{ background: "var(--sv-bg)", borderRadius: 12, border: "1px solid var(--sv-border)" }}
    >
      <defs>
      ...
      <rect x="0" y="0" width="200" height="200" fill="url(#g)" opacity="0.12" />
      ...
      <text x="100" y="175" textAnchor="middle" fontSize="12" fill="currentColor">
...
export default function RemoteQR({
  ...
  const joinUrl = useMemo(() => (url ? url : room ? urlBuilder(room) : ""), [url, room, urlBuilder]);
    ...
    (async () => {
      ...
      if (QRCodeLib?.toDataURL) {
        try {
          ...
          console.warn("[RemoteQR] QRCodeLib failed, using fallback:", er?.message || er);
          if (!alive) return;
          setDataUrl(null);
        }
      }
  ...
  const doShare = async () => {
    ...
    try {
      if (canShare()) {
        await navigator.share({ title: envelope.title, text: envelope.text, url: envelope.url });
      }
      onShareEnvelope?.(envelope);
      emitEvent("play.remote.link.shared", { room, via: canShare() ? "navigator.share" : "custom" });
    } catch (er) {
  ...
  const broadcastPrepared = () => {
    try {
      const env = { kind: "remote.qr.prepared", ts: isoNow(), room, url: joinUrl };
      eventBus.emit?.("play.control", { type: "play.control", ts: env.ts, source: "RemoteQR", data: env });
      onShareEnvelope?.(env);
    } catch {}
  };
  ...
  return (
    <div className={`sv-card sv-pad ${className}`} style={{ textAlign: "center", ...style }}>
      ...
      >
        {dataUrl ? (
          ...
          <div style={{ width: sz, height: sz, display: "grid", placeItems: "center" }}>
      ...
      {showUrl && joinUrl ? (
        <div className="sv-block sv-text-sm" style={{ wordBreak: "break-all" }}>
          <span className="sv-strong">Join URL:</span> <span className="sv-muted">{joinUrl}</span>
        </div>
      ) : null}

      <div className="sv-row sv-justify-between sv-block" style={{ flexWrap: "wrap", gap: 8 }}>
        {showCopy && (
          <button
            type="button"
      ...
      {err ? (
        <div className="sv-danger sv-text-sm sv-block">Couldn’t build QR: {String(err)}</div>
      ) : null}
    </div>
  );
}
