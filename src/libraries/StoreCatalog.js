...
(function () {
  ...
  function id(parts){ return parts.filter(Boolean).join(".").toLowerCase(); }
  function norm(s){ return String(s||"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim(); }
  function tokens(s){ return norm(s).split(/\s+/).filter(Boolean); }
  function includesAll(hay, needles){
    for (var i=0;i<needles.length;i++) if (hay.indexOf(needles[i])<0) return false;
    return true;
  }
  ...
  function convert(qty, unitFrom, unitTo){
    if (!U[unitFrom] || !U[unitTo] || U[unitFrom].base !== U[unitTo].base) return null;
  ...
  addCanonical({
    ...
    aliases: ["wheat berries","hard white wheat","hard red wheat","whole wheat berries"],
  ...
  addCanonical({
    ...
    aliases: ["fresh milled flour","freshly milled whole wheat","whole wheat flour fresh"],
    // Typical home mill yield: ~100% mass in → ~100% flour out (minus minor loss). Use 0.98 by default.
    yield: { from: "grain.wheat.berries", inUnit: "g", outUnit: "g", factor: 0.98 },
  ...
  aliases: ["ap flour","all purpose flour","all-purpose flour"],
    subs: [{ toId: "flour.whole.wheat.fresh", note: "Use fresh-milled + adjust hydration 5–10%", score: 0.55 }]
  });
  ...
  var subs = {
    "flour.ap": [{ toId: "flour.whole.wheat.fresh", note: "Hydration +5–10%, expect denser crumb", score: 0.5 }],
    "chem.bleach": [{ toId: "chem.vinegar", note: "Acid cleaner (NOT a disinfectant). Do NOT mix.", score: 0.2 }]
  };
  ...
  registerVendor("local", [
    { sku:"LOC-0001", vendor:"local", canonicalId:"grain.wheat.berries", packQty:22.68, packUnit:"kg", price:24.00, name:"Wheat Berries 50 lb (local mill)", availability:"in_stock" },
    { sku:"LOC-0002", vendor:"local", canonicalId:"yeast.instant", packQty:454, packUnit:"g", price:6.99, name:"Instant Yeast 1 lb" },
    { sku:"LOC-0003", vendor:"local", canonicalId:"salt.kosher", packQty:1360, packUnit:"g", price:3.49, name:"Kosher Salt 3 lb" },
    { sku:"LOC-0101", vendor:"local", canonicalId:"ppe.kit.clean", packQty:1, packUnit:"each", price:12.99, name:"Cleaning PPE Kit" },
    { sku:"LOC-0201", vendor:"local", canonicalId:"feed.chicken.layer", packQty:22.68, packUnit:"kg", price:16.99, name:"Layer Feed 50 lb" },
    { sku:"LOC-0202", vendor:"local", canonicalId:"mineral.goat", packQty:4540, packUnit:"g", price:19.99, name:"Goat Mineral 10 lb" },
    { sku:"LOC-0301", vendor:"local", canonicalId:"butchery.bags.vacuum", packQty:100, packUnit:"each", price:21.99, name:"Vacuum Bags (100ct)" },
    { sku:"LOC-0401", vendor:"local", canonicalId:"soil.mix.raisedbed", packQty:28.3, packUnit:"kg", price:9.99, name:"Raised Bed Mix 62.5 lb" },
    { sku:"LOC-0402", vendor:"local", canonicalId:"fert.npk.5.10.10", packQty:4.54, packUnit:"kg", price:11.99, name:"Fertilizer 5-10-10 (10 lb)" }
  ]);

  registerVendor("azure", [
    { sku:"AZ-berries-25", vendor:"azure", canonicalId:"grain.wheat.berries", packQty:11.34, packUnit:"kg", price:14.95, name:"Wheat Berries 25 lb" },
    { sku:"AZ-yeast-1lb", vendor:"azure", canonicalId:"yeast.instant", packQty:454, packUnit:"g", price:5.89, name:"Instant Yeast 1 lb" },
    { sku:"AZ-salt-3lb", vendor:"azure", canonicalId:"salt.kosher", packQty:1360, packUnit:"g", price:2.99, name:"Kosher Salt 3 lb" },
    { sku:"AZ-vac-100", vendor:"azure", canonicalId:"butchery.bags.vacuum", packQty:100, packUnit:"each", price:18.50, name:"Vacuum Bags (100ct)" }
  ]);

  registerVendor("warehouse", [
    { sku:"WH-AP50", vendor:"warehouse", canonicalId:"flour.ap", packQty:22.68, packUnit:"kg", price:18.99, name:"AP Flour 50 lb" },
    { sku:"WH-PPEKIT", vendor:"warehouse", canonicalId:"ppe.kit.clean", packQty:1, packUnit:"each", price:9.99, name:"Cleaning PPE Kit (bulk)" },
    { sku:"WH-layer-40", vendor:"warehouse", canonicalId:"feed.chicken.layer", packQty:18.14, packUnit:"kg", price:13.49, name:"Layer Feed 40 lb" },
    { sku:"WH-mineral-goat", vendor:"warehouse", canonicalId:"mineral.goat", packQty:2268, packUnit:"g", price:12.99, name:"Goat Mineral 5 lb" }
  ]);
  ...
  function findCanonicalByAliasOrId(query){
    ...
    for (var cid in canonical){
      ...
      var c = canonical[cid];
      var hay = tokens(c.label + " " + (c.aliases||[]).join(" ") + " " + (c.tags||[]).join(" "));
  ...
  function pricePerBaseUnit(sku){
    ...
    var toQtyInToBase = (U[from].base === U[to].base) ? (sku.packQty * U[from].toBase) : null;
  ...
  function chooseBestPrice(skus){
    ...
    withPPU = withPPU.filter(function(x){ return typeof x.pricePerBaseBaseUnit === "number"; });
    withPPU.sort(function(a,b){ return a.pricePerBaseBaseUnit - b.pricePerBaseBaseUnit; });
    return withPPU;
  }
  ...
  function suggestPurchases(requirement, prefs){
    ...
    // Resolve canonical
    var c = requirement.id ? canonical[requirement.id] : findCanonicalByAliasOrId(requirement.name);
    if (!c) {
      return { requirement: requirement, options: [], substitutions: [], warnings: ["unknown_item"] };
    }

    // Fresh-milled: if requirement is flour.whole.wheat.fresh and berries available, offer berry route
    var extraOptions = [];
    if (c.id === "flour.whole.wheat.fresh" && canonical["grain.wheat.berries"]) {
      // convert flour qty needed into berries qty needed via yield
      var y = c.yield || { from: "grain.wheat.berries", inUnit:"g", outUnit:"g", factor: 0.98 };
      ...
      if (needFlourBase != null && berries) {
        ...
        for (var i=0;i<rankedBerry.length;i++){
          ...
          extraOptions.push({
            kind: "alt-route",
            note: "Mill fresh flour from wheat berries (~" + round2(100*(y.factor||1)) + "% yield)",
            sku: s.sku, vendor: s.vendor, name: s.name, canonicalId: s.canonicalId,
            totalPacks: packs,
    ...
    if (prefs.vendor) {
      ...
      merged.sort(function(a,b){ return a.totalPrice - b.totalPrice; });
    }

    return {
      requirement: Object.assign({}, requirement, { id: c.id, domain: c.domain, baseUnit: c.baseUnit }),
      options: merged,
  ...
  function planToShoppingList(items, prefs){
    ...
    for (var i=0;i<(items||[]).length;i++){
      ...
      for (var n=0;n<needs.length;n++){
        ...
        var line = lines[cid] || (lines[cid] = {
          canonicalId: cid,
          label: (canonical[cid] && canonical[cid].label) || (req.name || cid),
          baseUnit: (canonical[cid] && canonical[cid].baseUnit) || req.unit || "each",
          totalNeededBase: 0,
          suggestions: []
        });
    ...
    for (var k in lines){
      ...
      var L = lines[k];
      var flat = [];
      for (var s=0;s<L.suggestions.length;s++) flat = flat.concat(L.suggestions[s]);
  ...
  function pickPacksToCover(options, needBase, baseUnit){
    ...
    var ranked = options.slice(0).filter(function(o){
      return typeof o.totalPrice === "number" && typeof o.coverQtyBase === "number";
    });
    ...
    var totalPrice = 0, cover = 0;
    for (var j=0;j<chosen.length;j++){ totalPrice += chosen[j].totalPrice; cover += chosen[j].coverQtyBase; }
    return { packs: chosen, totalPrice: round2(totalPrice), coverQtyBase: round2(cover), needBase: round2(needBase), unit: baseUnit };
  }
  ...
  function mealBakingKitFreshMilled(servings){
    ...
    return needs.map(function(x){ return suggestPurchases(x, { vendor: null }); });
  }
  ...
  function cleaningStarterKit(){
    ...
    return needs.map(function(x){ return suggestPurchases(x, {}); });
  }
  ...
  function buildShortageEventFromPlan(planMissingLines){
    ...
    return {
      ...
      payload: {
        ...
        items: planMissingLines.map(function(m){ return { id: m.id, name: m.name, qtyNeeded: m.qty, unit: m.unit }; })
      }
    };
  }
  ...
  function inferDomainFromLines(lines){
    ...
    for (var i=0;i<(lines||[]).length;i++){
      var c = lines[i].id ? canonical[lines[i].id] : findCanonicalByAliasOrId(lines[i].name);
  ...
  var api = {
    ...
    getCanonical: function(idOrName){ return idOrName && canonical[idOrName] || findCanonicalByAliasOrId(idOrName); },
    listCanonical: function(){ return Object.keys(canonical).map(function(k){ return canonical[k]; }); },
