...
(function () {
  ...
  try { ConflictHeuristics = require("@/libraries/ConflictHeuristics"); } catch (e) {}

  var scheduleHelpers = { now: function(){ return Date.now(); } };
  ...
  var TONES = { danger: "danger", warning: "warning", info: "info", success: "success" };
  ...
  function registerCard(type, builder) { registry[type] = builder; }
  function listTypes() { return Object.keys(registry); }
  ...
  function mkKey(parts) { return (parts || []).filter(Boolean).join("|").toLowerCase(); }
  ...
  function baseCard(seed) {
    ...
    return Object.assign({
      ...
      key: seed.key || seed.id || ("nba:"+Math.random().toString(36).slice(2)),
  ...
  function filterExpired(cards) {
    var now = scheduleHelpers.now ? scheduleHelpers.now() : Date.now();
    return cards.filter(function(c){ return !c.expiresAt || c.expiresAt > now; });
  }
  ...
  function applyDomainCaps(cards) {
    ...
    Object.keys(byD).forEach(function(d){
      ...
      arr.sort(function(a,b){
        if (PRIOR[b.priority] !== PRIOR[a.priority]) return PRIOR[b.priority]-PRIOR[a.priority];
  ...
  function throttle(cards, ttlMs) {
    ...
    for (var i=0;i<cards.length;i++){
      ...
      if (!last || (now - last) > (ttlMs || 60*1000)) { // 60s min re-show window
  ...
  function emitShown(cards){
    for (var i=0;i<cards.length;i++){
      try { eventBus.emit("nba.card.shown", { key: cards[i].key, domain: cards[i].domain, tags: cards[i].tags, priority: cards[i].priority }); } catch(e){}
    }
  }

  function emitDismiss(card, reason){
    try { eventBus.emit("nba.card.dismissed", { key: card.key, reason: reason || "user" }); } catch(e){}
  }
  ...
  function wireActionInvocation(action){
    ...
    return function(){
      try {
        eventBus.emit("nba.action.invoked", { label: action.label, intent: action.intent || "cta" });
  ...
  registerCard("conflict", function(ctx){
    ...
    for (var i=0;i<conflicts.length;i++){
      var c = conflicts[i];
      var tone = (c.kind === "biohazard") ? TONES.danger
               : (c.kind === "weather" || c.kind === "withhold") ? TONES.warning
               : TONES.info;

      var icon = ICONS[c.kind] || ICONS.info;
      var key = mkKey(["conflict", c.kind, c.domain, (c.affected||[]).join(",")]);

      var built = baseCard({
        id: "card:"+key,
        key: key,
        title: c.title || ("Resolve " + c.kind + " conflict"),
        subtitle: (c.domain||"plan").toUpperCase() + " • " + (c.kind || "conflict"),
        body: c.rationale || "",
  ...
  registerCard("shortage", function(ctx){
    ...
    for (var i=0;i<events.length;i++){
      ...
      var domain = ev.payload && ev.payload.domain || ctx.domain || DOMAINS.meals;
      var items = (ev.payload && ev.payload.items) || [];
      if (!items.length) continue;

      var list = items.slice(0, 5).map(function(it){
        return (it.qtyNeeded ? (it.qtyNeeded+"× ") : "") + (it.name || it.sku || "Item");
      }).join(", ");

      var key = mkKey(["shortage", domain, items.map(function(x){return x.name||x.sku;}).join(",")]);
        ...
        actions: [
          {
            ...
            variant: "primary",
            emit: { name: "grocerylist.requested", payload: { domain: domain } },
            intent: "cta"
          },
          {
            label: "Swap items",
            variant: "secondary",
            emit: { name: "planner.alternate.requested", payload: { items: items.map(function(x){return x.id;}) } },
            intent: "option"
          }
        ]
      }));
  ...
  registerCard("withholdResume", function(ctx){
    ...
    for (var i=0;i<events.length;i++){
      ...
      out.push(baseCard({
        ...
        actions: [
          { label: "Resume now", variant: "primary", emit: { name: "session.resume.requested", payload: { domain: domain } }, intent: "cta" },
          { label: "Adjust reminder", variant: "secondary", emit: { name: "reminder.adjust.requested", payload: { until: until } }, intent: "option" }
        ]
      }));
    }
    return out;
  ...
  registerCard("planDraft", function(ctx){
    ...
    return [baseCard({
      ...
      tone: TONES.info,
      actions: [
        { label: "Draft it for me", variant: "primary", emit: { name: "mealplan.draft.requested", payload: { domain: domain } }, intent: "cta" },
        { label: "Collect tasks first", variant: "secondary", emit: { name: "prep.tasks.requested", payload: { domain: domain } }, intent: "option" }
      ]
    })];
  });
  ...
  registerCard("weather", function(ctx){
    ...
    for (var i=0;i<events.length;i++){
      ...
      out.push(baseCard({
        ...
        body: conf.rationale || "Outdoor conditions may reduce success or safety.",
        ...
        tone: TONES.warning,
        actions: [
          { label: "Find safe window", variant: "primary", emit: { name: "planner.schedule.safeWindow.requested", payload: { itemId: (conf.affected||[])[0] } }, intent: "cta" },
          { label: "Add PPE note", variant: "secondary", emit: { name: "prep.tasks.requested", payload: { domain: conf.domain, tasks: ["PPE: weather protection"] } }, intent: "option" }
        ]
      }));
    }
    return out;
  ...
  function buildCardsFromConflicts(conflictsOrHints, opts){
    opts = opts || {};
    var ctx = { conflicts: normalizeConflicts(conflictsOrHints), domain: opts.domain, plan: opts.plan };
    var cards = [];
  ...
  function buildCardsFromEvents(events, opts){
    ...
    for (var i=0;i<types.length;i++){
      ...
      try { cards = cards.concat(b(ctx) || []); } catch(e){ console.warn("[NbaCards] builder failed:", types[i], e && e.message); }
    }
    // Optional “empty state” planDraft
    var planDraftB = registry["planDraft"];
    if (planDraftB) {
      try { cards = cards.concat(planDraftB({ domain: opts.domain, plan: opts.plan }) || []); } catch(e){}
    }
    return cards;
  }
