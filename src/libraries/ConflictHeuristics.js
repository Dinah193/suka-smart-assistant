...
(function () {
  ...
  try {
    estimateEngine =
      require("@/engines/estimates/estimateEngine") || estimateEngine;
  } catch (e) {
    /* noop */
  }
  ...
  var placementRules = {
    ...
    detectApplianceClash: function () {
      return [];
    },
  };
  try {
    placementRules =
      require("@/engines/deciders/placementRules") || placementRules;
  } catch (e) {
    /* noop */
  }

  var workPrepConsolidation = {
    detectResourceOverlaps: function () {
      return [];
    },
  };
  try {
    workPrepConsolidation =
      require("@/engines/prep/workPrepConsolidation") || workPrepConsolidation;
  } catch (e) {
    /* noop */
  }
  ...
  function windowFromItem(item) {
    ...
    if (
      item &&
      item.timeWindow &&
      typeof item.timeWindow.start === "number" &&
      typeof item.timeWindow.end === "number"
    ) {
  ...
  function registerHeuristic(name, fn) {
    registry[name] = fn;
  }
  ...
  function analyze(plan, options) {
    ...
    var ctx = {
      ...
      helpers: {
        ...
        workPrepConsolidation: workPrepConsolidation,
        estimateEngine: estimateEngine,
      },
      advisors: {
    ...
    for (var i = 0; i < names.length; i++) {
      try {
        ...
        console.warn(
          "[ConflictHeuristics] heuristic failed:",
          names[i],
          e && e.message
        );
      }
  ...
  function applyAutoFixes(plan, conflicts) {
    ...
    var map = {};
    for (var x = 0; x < plan.items.length; x++)
      map[plan.items[x].id] = plan.items[x];
    ...
    var resultItems = [];
    for (var key in map)
      if (map.hasOwnProperty(key)) resultItems.push(map[key]);
    return {
      patchedPlan: Object.assign({}, plan, { items: resultItems }),
      patches: patches,
    };
  }
  ...
  registerHeuristic("time.overlap", function (ctx) {
    ...
    var overlaps = scheduleHelpers.overlapWindows(
      items.map(function (it) {
        return { id: it.id, window: windowFromItem(it), title: it.title };
      })
    );
    var out = [];
    for (var i = 0; i < overlaps.length; i++) {
      ...
      var a = items.find(function (x) {
        return x.id === pair.a.id;
      });
      var b = items.find(function (x) {
        return x.id === pair.b.id;
      });
      if (!a || !b) continue;
      ...
      var conflict = {
        ...
        rationale:
          "Two tasks overlap in time, risking execution quality and throughput.",
        affected: [a.id, b.id],
        suggestions: [
          {
                ...
                apply: function (target) {
                  ...
                  return Object.assign({}, target, {
                    ...
                    durationMin: Math.round(
                      (shifted.end - shifted.start) / 60000
                    ),
                  });
                },
              };
          ...
          },
          {
            ...
            intent: "option",
            emit: {
              name: "planner.option.selected",
              payload: { strategy: "defer-execute", items: [a.id] },
            },
          },
        ],
      };
  ...
  registerHeuristic("appliance.clash", function (ctx) {
    var items = ctx.plan.items || [];
    var clashes =
      (placementRules.detectApplianceClash &&
        placementRules.detectApplianceClash(items)) ||
      [];
    var out = [];
        ...
        suggestions: [
          ...
          },
          {
            ...
            emit: {
              ...
              payload: { appliance: c.appliance, items: c.ids },
            },
          },
        ],
      });
  ...
  registerHeuristic("weather.risk", function (ctx) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      var assess = ctx.advisors.WeatherAdvisor.assess(
        it,
        ctx.options.weather || null
      );
      ...
      var domainMod =
        ctx.domain === "garden" || ctx.domain === "animals" ? 10 : 0;
      ...
      out.push({
        ...
        rationale:
          (assess.reasons && assess.reasons.join("; ")) ||
            ...
            patch: function () {
              return {
                ...
                apply: function (target) {
                  ...
                  if (notes.indexOf("[PPE]") === -1)
                    notes = "[PPE] Weather protection required. " + notes;
  ...
  registerHeuristic("biohazard.safety", function (ctx) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      var domainMod =
        ctx.domain === "meals" || ctx.domain === "animals" ? 10 : 0;
      ...
      out.push({
        ...
        rationale:
          (assess.reasons && assess.reasons.join("; ")) ||
          "Potential contamination or unsafe handling flow.",
        affected: [it.id],
        suggestions: [
          {
            ...
            emit: {
              ...
              payload: {
                domain: ctx.domain,
                tasks: ["Sanitize surfaces", "Change gloves"],
              },
            },
          },
          {
            ...
            patch: function () {
              return {
                ...
                apply: function (target) {
                  ...
                  var notes = target.notes || "";
                  if (notes.indexOf("[FLOW]") === -1)
                    notes =
                      "[FLOW] Separate prep/execute; enforce sanitation buffer. " +
                      notes;
                  return Object.assign({}, target, { notes: notes });
                },
              };
  ...
  registerHeuristic("withhold.windows", function (ctx) {
    var items = ctx.plan.items || [];
    var withholds =
      ctx.advisors.SafetyAdvisor.withholdWindows(items, ctx.plan) || [];
    var out = [];
    for (var i = 0; i < withholds.length; i++) {
      ...
      var domainMod =
        ctx.domain === "animals" || ctx.domain === "meals" ? 10 : 0;
        ...
        suggestions: [
          {
            ...
            patch: function () {
              return {
                ...
                apply: function (target) {
                  var notes = target.notes || "";
                  if (notes.indexOf("[REMIND]") === -1)
                    notes = "[REMIND] Resume at withhold end. " + notes;
                  return Object.assign({}, target, { notes: notes });
                },
              };
            },
          },
          {
            ...
            intent: "option",
            emit: {
              name: "session.pause.requested",
              payload: { reason: "withhold", until: w.end, items: [w.itemId] },
            },
          },
        ],
      });
  ...
  registerHeuristic("resource.overlap", function (ctx) {
    var items = ctx.plan.items || [];
    var overlaps =
      ctx.helpers.workPrepConsolidation.detectResourceOverlaps(items) || [];
    var out = [];
    for (var i = 0; i < overlaps.length; i++) {
      ...
      out.push({
        ...
        rationale:
          "Multiple tasks share a constrained resource at the same time.",
        affected: o.ids || [],
        suggestions: [
          {
  ...
  function score(conflict) {
    if (!conflict || typeof conflict.score === "number")
      return (conflict && conflict.score) || 0;
