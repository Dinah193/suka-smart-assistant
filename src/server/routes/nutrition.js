...
const scrapeNutritionIfMissing =
  (await softImport('src/server/services/nutrition/scrapeNutritionIfMissing.js')) ||
  (await softImport('src/server/services/ScraperOrchestrator.js'))?.enqueueNutritionJob ||
  null;
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const triggerScrape = req.query.scrape === '1' || req.query.scrape === 'true';
    const waitMode = (req.query.wait || '').toString(); // e.g., "short" to re-check once after enqueue

    if (!raw.trim()) {
      return json(res, 400, { ok: false, error: 'Missing query parameter "name"' });
    }
    ...
    if (triggerScrape && scrapeNutritionIfMissing) {
      ...
      if (waitMode === 'short') {
        ...
        if (recheck?.ok && recheck.data) {
          emit('nutrition.api.lookup.hit', { normalizedName, note: 'post-scrape-recheck' });
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    return json(res, 404, { ok: false, error: 'Not found', reason: 'NOT_FOUND' });
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    return json(res, 400, { ok: false, error: 'Provide either ?id= or ?name=' });
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const scrapeResult = await safeScrape({ normalizedName, hint: raw, force: true });
...
async function safeGetFromDB({ id, name }) {
  try {
    if (!getNutritionFromDB) {
      emit('nutrition.api.lookup.error', { step: 'getNutritionFromDB', message: 'service unavailable' });
    ...
    emit('nutrition.api.lookup.error', { step: 'getNutritionFromDB', message: err?.message || 'db error' });
    return { ok: false, data: null };
  }
}
...
async function safeScrape({ normalizedName, hint, force = false }) {
  try {
    if (!scrapeNutritionIfMissing) {
      emit('nutrition.scrape.skipped', { normalizedName, reason: 'SCRAPER_UNAVAILABLE' });
      return { ok: false };
    }
    ...
    emit('nutrition.api.lookup.error', { step: 'scrape', message: err?.message || 'scrape error', normalizedName });
    return { ok: false };
  }
}
