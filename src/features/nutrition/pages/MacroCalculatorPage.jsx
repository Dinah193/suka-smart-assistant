...
function KeyBtn({ children, onClick, className = "", title }) {
  return (
    <button onClick={onClick} title={title} className={`${btnBase} ${className}`}>
      {children}
    </button>
  );
}
...
function computeTotals({ netCarb, fiber, sugarAlcohol, protein, fat }, opts) {
  const { includeFiberCalories, fiberCalPerGram, includeSACalories, saCalPerGram } = opts;
...
function normalizeFromOFF(product) {
  ...
  const name = product.product_name || product.generic_name || product.product_name_en || "Food";
  ...
  return { label, netCarb: net, fiber: fib, sugarAlcohol: sa, protein: pr, fat: ft, saType: "generic" };
}

const OFF_BASE = "https://world.openfoodfacts.org/api/v2";
async function fetchOpenFoodFactsUPC(upc) {
  const res = await fetch(`${OFF_BASE}/product/${encodeURIComponent(upc)}.json`);
...
async function searchByNameOFF(query) {
  const url = `${OFF_BASE}/search?search_terms=${encodeURIComponent(query)}&page_size=10&json=1`;
  const res = await fetch(url);
  if (!res.ok) throw new Error("OFF search failed");
  const data = await res.json();
  const products = data?.products || [];
  return products.map((p) => ({ code: p.code, label: [p.brands || "", p.product_name || "Food"].filter(Boolean).join(" – ") }));
}

/************************** Barcode scanner modal **************************/
function BarcodeScanModal({ open, onClose = () => {}, onDetected = () => {}, formats = ["ean-13","ean-8","upc-a","upc-e"], preferredFacingMode = "environment", throttleMs = 150, autoClose = true, allowManual = true, initialUPC = "", showTorch = false, onError }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setSecureHint(isSecure ? "" : "Tip: Camera access usually requires HTTPS (or localhost).");
    } catch {
      setSecureHint("");
    }
  }, [open]);
      ...
      <div className="w-full max-w-lg rounded-3xl overflow-hidden bg-gradient-to-b from-slate-100 to-slate-200 border border-slate-300 shadow-2xl">
        <div className="flex items-center justify-between p-3 border-b">
          ...
          <div className="flex items-center gap-2">
            {torchSupported && (
              <button className={btnBase} onClick={() => toggleTorch(!torchOn)} title="Toggle flashlight">
        ...
        <div className="p-3 space-y-3">
          ...
          {secureHint && <div className="text-xs text-slate-500">{secureHint}</div>}
          ...
          {allowManual && (
            <div className="flex w-full gap-2">
              <Input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && validUPC && (onDetected(manualUPC), autoClose && onClose())
                }
                autoFocus={!!err}
                className="flex-1"
              />
...
const MacroPercentCalculator = forwardRef(function MacroPercentCalculator(
  ...
  const [manual, setManual] = useState({ netCarb: 0, fiber: 0, sugarAlcohol: 0, protein: 0, fat: 0 });
  const [manualAdd, setManualAdd] = useState({ netCarb: "", fiber: "", sugarAlcohol: "", protein: "", fat: "" });
  const [entries, setEntries] = useState([]); // {label, netCarb, fiber, sugarAlcohol, protein, fat, servings}

  const [includeFiberCalories, setIncludeFiberCalories] = useState(settings?.includeFiberCalories ?? true);
  const [fiberCalPerGram, setFiberCalPerGram] = useState(settings?.fiberCalPerGram ?? 2);
  const [includeSACalories, setIncludeSACalories] = useState(settings?.includeSACalories ?? true);
  const [saCalPerGram, setSACalPerGram] = useState(settings?.saCalPerGram ?? 2.4);
  ...
  useImperativeHandle(ref, () => ({
    ...
    clear: () => {
      setEntries([]);
      setManual({ netCarb: 0, fiber: 0, sugarAlcohol: 0, protein: 0, fat: 0 });
    },
    getSnapshot: () => ({ totals, calories: calcs, pct, entries }),
  }));

  const inc = (key, by) => setManual((m) => ({ ...m, [key]: Math.max(0, clampNum(m[key]) + by) }));
  const onAbs = (key, v) => setManual((m) => ({ ...m, [key]: Math.max(0, clampNum(v)) }));
  ...
  const calcs = useMemo(
    ...
    [totals, includeFiberCalories, fiberCalPerGram, includeSACalories, saCalPerGram]
  );
    ...
    <div className="w-full grid lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-4">
        <div className={card}>
          ...
          {ROWS.map((f) => (
            <div key={f.key} className="grid grid-cols-[max-content_12rem_14rem_auto] items-center gap-2 py-1">
              <label className="w-36 text-sm text-slate-700 text-right pr-3">{f.label}</label>
          ...
          <div className="mt-3 flex gap-2">
            <KeyBtn onClick={() => setManual({ netCarb: 0, fiber: 0, sugarAlcohol: 0, protein: 0, fat: 0 })}>Clear manual</KeyBtn>
          </div>
        </div>
        ...
        <div className={card}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm text-slate-600 w-32">Sugar alcohol cal/g</span>
              <Input
                type="number"
                step="0.1"
                value={saCalPerGram}
      ...
      <div className="space-y-4">
        <div className={cardSoft}>
          ...
          <div className="grid grid-cols-2 gap-2 text-sm text-slate-700">
            <div>
              Net Carbs: <span className="font-semibold">{totals.netCarb.toFixed(1)} g</span>
            </div>
            <div>
              Protein: <span className="font-semibold">{totals.protein.toFixed(1)} g</span>
            </div>
            <div>
              Fat: <span className="font-semibold">{totals.fat.toFixed(1)} g</span>
            </div>
            <div>
              Fiber: <span className="font-semibold">{totals.fiber.toFixed(1)} g</span>
            </div>
            <div>
              Sugar Alcohols: <span className="font-semibold">{totals.sugarAlcohol.toFixed(1)} g</span>
            </div>
          </div>
          <div className="mt-3 text-sm text-slate-700">
            Calories total: <span className="font-bold">{calcs.totalCalories.toFixed(0)} kcal</span>
          </div>
          <div className="mt-2 grid grid-cols-3 gap-2 text-sm">
            <div className="rounded-xl p-2 border bg-white">
              <div className="text-slate-500">Net Carb %</div>
              <div className="text-lg font-bold">{(pct.carbsPct * 100).toFixed(1)}%</div>
            </div>
            <div className="rounded-xl p-2 border bg-white">
              <div className="text-slate-500">Protein %</div>
              <div className="text-lg font-bold">{(pct.proteinPct * 100).toFixed(1)}%</div>
            </div>
            <div className="rounded-xl p-2 border bg-white">
              <div className="text-slate-500">Fat %</div>
              <div className="text-lg font-bold">{(pct.fatPct * 100).toFixed(1)}%</div>
            </div>
          </div>
        </div>
            ...
            <ul className="space-y-2">
              {entries.map((e, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 text-sm">
                    ...
                    <SmallInput
                      ...
                      onChange={(ev) => {
                        ...
                        setEntries((prev) =>
                          prev.map((x, i) => (i === idx ? { ...x, servings: Math.max(0, v) } : x))
                        );
                      }}
                      className="w-20"
                    />
                  ...
                  <KeyBtn className="text-xs" onClick={() => setEntries((prev) => prev.filter((_, i) => i !== idx))}>
                    Remove
                  </KeyBtn>
                  ...
                  <details className="text-xs text-slate-600">
                    ...
                    <div>
                      net {e.netCarb}g, fiber {e.fiber}g, SA {e.sugarAlcohol}g, protein {e.protein}g, fat {e.fat}g
                    </div>
                  </details>
                </li>
              ))}
...
function NutritionImportBridge({ calcRef, enableBarcode = true, enableSearch = true, fdcProxyUrl = null, onImported = () => {} }) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-3">
      <div className="flex flex-wrap items-center gap-3">
        {enableBarcode && <KeyBtn onClick={() => setScanOpen(true)}>Scan Barcode</KeyBtn>}
        <div className="flex items-center gap-2">
          <Input
          ...
          />
          <KeyBtn onClick={() => doLookup(upc)}>{busy ? "Looking..." : "Lookup"}</KeyBtn>
        </div>
        {enableSearch && (
          <div className="flex items-center gap-2">
            <Input
              placeholder="Search name (Open Food Facts)"
              onKeyDown={(e) => e.key === "Enter" && doSearch(e.currentTarget.value)}
      ...
      <BarcodeScanModal open={scanOpen} onClose={() => setScanOpen(false)} onDetected={doLookup} />
    </div>
  );
}
...
function mapRecipeToMacroEntry(recipe) {
  ...
  const netCarb =
    recipe.netCarbs != null ? recipe.netCarbs : Math.max(0, (recipe.totalCarbs ?? 0) - fiber - sa);
  return {
...
export default function MacroCalculatorPage() {
  ...
  function simpleSummary(prev, cur) {
    ...
    if (Math.abs(tcA - tcB) > 0.5) lines.push(`Calories total: ${fmtK(tcA)} → ${fmtK(tcB)}`);
  ...
  const importFromMealPlan = () => {
    let plan = null;
    if (typeof window !== "undefined" && window.sukaMealPlan) plan = window.sukaMealPlan;
  ...
  return (
    <div className="space-y-6 p-4 max-w-6xl mx-auto">
      ...
      <NutritionImportBridge calcRef={calcRef} enableBarcode enableSearch onImported={() => {}} />

      {/* Calculator */}
      <MacroPercentCalculator ref={calcRef} settings={DEFAULT_SETTINGS} onTotalsChange={handleTotalsChange} />

      {/* Meal Plan import panel */}
      <section className={card}>
        <div className="flex items-center justify-between">
          <h2 className="font-semibold">Import from Meal Plan</h2>
          <KeyBtn onClick={() => setImportOpen((s) => !s)}>{importOpen ? "Hide" : "Open"}</KeyBtn>
        </div>
        {importOpen && (
          <div className="mt-3 space-y-3">
            <p className="text-sm text-slate-600">
              Option A: set <code>window.sukaMealPlan</code> in devtools to your plan object then click “Import
              Plan”.
              <br />
