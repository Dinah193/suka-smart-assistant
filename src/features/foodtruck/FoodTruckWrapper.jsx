...
// - Designed to integrate later with foodTruckShim.js events
...
const DEFAULT_FONTS = {
  ...
  linkHelp:
    "Optional: load Google Fonts in your app's <head> for custom faces.",
  ...
  sections: [
    ...
    },
    {
      ...
      items: [
        {
          id: uid(),
          name: "Crisp Masala Fries",
          desc: "tempered spice finish",
          price: 4,
        },
        {
          id: uid(),
          name: "Mango Lassi Cup",
          desc: "cultured, bright",
          price: 5,
          badge: "popular",
        },
      ],
...
export function validateMenu(menu) {
  ...
  if (!Array.isArray(menu.sections))
    errors.push("menu.sections: array required");
  (menu.sections || []).forEach((s, si) => {
    if (!s.title) errors.push(`sections[${si}].title: required`);
    if (!Array.isArray(s.items))
      errors.push(`sections[${si}].items: array required`);
...
export default function FoodTruckWrapper({ initialMenu }) {
        ...
        (() => {
          try {
            const old = JSON.parse(
              localStorage.getItem("suka.foodtruck.wrapper.v1")
            );
  ...
  const updateTheme = (key, val) =>
    setData((d) => ({ ...d, theme: { ...d.theme, [key]: val } }));
  ...
  const setHeadingFont = (stack) =>
    setData((d) => ({
      ...d,
      fonts: { ...(d.fonts || DEFAULT_FONTS), headings: stack },
    }));
  ...
  const addSection = () =>
    setData((d) => ({
      ...
      sections: [
        ...d.sections,
        { id: uid(), title: "New Section", note: "", items: [] },
      ],
    }));
  ...
  const moveSection = (sid, dir) =>
    setData((d) => {
      ...
      const j = Math.max(
        0,
        Math.min(d.sections.length - 1, i + (dir === "up" ? -1 : 1))
      );
      ...
      sections: d.sections.map((s) =>
        s.id === sid
          ? {
              ...
              items: [
                ...s.items,
                { id: uid(), name: "New Item", desc: "", price: 0 },
              ],
            }
          : s
      ),
  ...
  const moveItem = (sid, iid, dir) =>
    setData((d) => {
      ...
      const j = Math.max(
        0,
        Math.min(sec.items.length - 1, i + (dir === "up" ? -1 : 1))
      );
  ...
  const togglePrices = () =>
    setData((d) => ({ ...d, showPrices: !d.showPrices }));
  const setCurrency = (currency) => setData((d) => ({ ...d, currency }));
  const addFooterNote = () =>
    setData((d) => ({
      ...d,
      footerNotes: [...(d.footerNotes || []), "New note"],
    }));
  const removeFooterNote = (idx) =>
    setData((d) => ({
      ...d,
      footerNotes: d.footerNotes.filter((_, i) => i !== idx),
    }));
  const updateFooterNote = (idx, text) =>
    setData((d) => ({
      ...d,
      footerNotes: d.footerNotes.map((t, i) => (i === idx ? text : t)),
    }));
    ...
    >
        ...
        <div className="rounded-2xl shadow p-4 bg-white/80 border border-black/5">
          ...
          <div className="mb-4">
            ...
            <div className="space-y-3">
              <FontSelect
                label="Base font"
                value={data.fonts?.base || DEFAULT_FONTS.base}
                options={FONT_CHOICES}
                onChange={setBaseFont}
              />
              <div className="flex items-center gap-2">
                <input
                  ...
                  type="checkbox"
                  className="h-4 w-4"
                  checked={data.fonts?.lockHeadingsToBase ?? true}
                  onChange={toggleLockHeadings}
                />
                <label htmlFor="lockHeadings" className="text-sm">
                  Headings use base font
                </label>
              </div>
              <FontSelect
                label="Heading font"
                value={
                  data.fonts?.headings || data.fonts?.base || DEFAULT_FONTS.base
                }
                options={FONT_CHOICES}
                onChange={setHeadingFont}
                disabled={data.fonts?.lockHeadingsToBase}
              />
          ...
          <div className="mb-2 flex items-center justify-between">
            ...
            <button
              className="inline-flex items-center px-3 py-2 rounded-xl border bg-white hover:bg-gray-50"
              onClick={addSection}
            >
            ...
            {data.sections.map((s) => (
              <div key={s.id} className="rounded-xl border p-3 bg-white/70">
                ...
                <input
                  ...
                  value={s.note || ""}
                  onChange={(e) =>
                    updateSection(s.id, { note: e.target.value })
                  }
                />
                <div className="mt-2">
                    ...
                    {s.items.map((it) => (
                      ...
                      >
                        <input
                          ...
                          value={it.name}
                          onChange={(e) =>
                            updateItem(s.id, it.id, { name: e.target.value })
                          }
                        />
                        <input
                          className="col-span-6 rounded-lg border px-2 py-1"
                          placeholder="Description"
                          value={it.desc || ""}
                          onChange={(e) =>
                            updateItem(s.id, it.id, { desc: e.target.value })
                          }
                        />
                        <div className="col-span-2 flex items-center gap-2">
                          ...
                          <input
                            ...
                            value={it.badge || ""}
                            onChange={(e) =>
                              updateItem(s.id, it.id, { badge: e.target.value })
                            }
                          />
                          <button
                            title="Up"
                            className="px-2 py-1 rounded-lg border"
                            onClick={() => moveItem(s.id, it.id, "up")}
                          >
                            ↑
                          </button>
                          <button
                            title="Down"
                            className="px-2 py-1 rounded-lg border"
                            onClick={() => moveItem(s.id, it.id, "down")}
                          >
                            ↓
                          </button>
                          <button
      ...
      >
        <div
          ...
          style={{ background: data.theme.bg }}
        >
        ...
        >
          <div className="absolute inset-0 p-6">
            <div className="absolute top-4 right-4">
              <button
                className="px-4 py-2 rounded-xl border bg-white/80"
                onClick={() => setFullscreen(false)}
              >
                <Minimize2 size={16} className="inline mr-2" />
                Exit
              </button>
            </div>
  ...
  return (
    <div className={`w-full ${fullscreen ? "h-full" : ""} `}>
      ...
      <div className="relative overflow-hidden">
        <div
          className="px-6 py-5 md:px-10 md:py-8 grid grid-cols-12 gap-4 items-center"
          ...
          <div className="col-span-3 md:col-span-2 flex items-center gap-3">
            {data.logoDataUrl ? (
              <img
                src={data.logoDataUrl}
                alt="logo"
                className="h-16 w-16 md:h-20 md:w-20 rounded-2xl object-contain bg-white/10 p-2"
              />
            ) : (
      ...
      {/* Sections */}
      <div
        className={`px-6 md:px-10 ${compact ? "py-4" : "py-8"}`}
        style={{ fontFamily: baseFont }}
      >
        <div
...
export function __selfTest_FoodTruckWrapper() {
  ...
  results.push({
    name: "reorder up",
    pass:
      JSON.stringify(reorder(["a", "b", "c"], 1, 0)) ===
      JSON.stringify(["b", "a", "c"]),
  });
  results.push({
    name: "reorder down",
    pass:
      JSON.stringify(reorder(["a", "b", "c"], 1, 2)) ===
  ...
  results.push({
    name: "reorder clamps",
    pass:
      JSON.stringify(reorder(["a", "b", "c"], 10, -5)) ===
      JSON.stringify(["c", "a", "b"]),
  });
  // Test: formatPrice
  results.push({
    name: "format price $",
    pass: formatPrice("$", 8) === "$8.00",
  });
  results.push({
    name: "format price edge",
    pass: formatPrice("$", "") === "$0.00",
  });
  // NEW: formatPrice non-numeric
  results.push({
    name: "format price non-numeric",
    pass: formatPrice("$", "abc") === "$0.00",
  });
  // NEW: formatPrice euro
  results.push({
    name: "format price €",
    pass: formatPrice("€", 3.5) === "€3.50",
  });
  // Test: validate default menu
  results.push({
    name: "validate default menu",
    pass: validateMenu(DEFAULT_MENU).length === 0,
  });
  // Test: validate missing fields
  results.push({
    name: "validate missing title",
    pass: validateMenu({ ...DEFAULT_MENU, truckName: "" }).includes(
      "menu.truckName: required"
    ),
  });
  // NEW: validate sections must be array
  results.push({
    name: "validate sections array",
    pass: validateMenu({ ...DEFAULT_MENU, sections: null }).includes(
      "menu.sections: array required"
    ),
  });
  ...
  const tmp = {
    ...
    fonts: {
      ...
      lockHeadingsToBase: false,
    },
  };
  const okFonts = !validateMenu(tmp).some((e) =>
    e.startsWith("menu.fonts.base")
  );
  ...
  results.push({
    name: "validate fonts missing base",
    pass: validateMenu(missingFonts).includes("menu.fonts.base: required"),
  });
