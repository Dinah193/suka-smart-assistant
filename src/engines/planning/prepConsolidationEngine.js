...
(function () {
  ...
  function intersect(a, b) {
    var setA = {}; var out = [];
    for (var i=0;i<(a||[]).length;i++) setA[String(a[i]).toLowerCase()] = 1;
  ...
  function timeOverlap(a, b, slackMinutes) {
    ...
    var aStart = asDate(a.start || a.windowStart || a.slot && a.slot.start || a.date);
    var aEnd   = asDate(a.end || a.windowEnd || a.slot && a.slot.end || (aStart && new Date(aStart.getTime() + (Number(a.estMinutes||a.duration||30)*MIN))));
    var bStart = asDate(b.start || b.windowStart || b.slot && b.slot.start || b.date);
    var bEnd   = asDate(b.end || b.windowEnd || b.slot && b.slot.end || (bStart && new Date(bStart.getTime() + (Number(b.estMinutes||b.duration||30)*MIN))));
    if (!aStart || !bStart) return false;
    return !( (aEnd.getTime()+slack) <= bStart.getTime() || (bEnd.getTime()+slack) <= aStart.getTime() );
  }

  function sameDay(a, b) {
    var A = asDate(a), B = asDate(b);
    return A.getFullYear()===B.getFullYear() && A.getMonth()===B.getMonth() && A.getDate()===B.getDate();
  }

  function locationAffinity(a, b) {
    // If zones/areas match → 1, if room matches → 0.8, else 0.0
    var az = (a.zone||"").toLowerCase(), bz = (b.zone||"").toLowerCase();
    var ar = (a.room||"").toLowerCase(), br = (b.room||"").toLowerCase();
  ...
  function sanitationRisk(a, b) {
    ...
    var aFlags = (a.flags||[]).map(function(x){return String(x).toLowerCase();});
    var bFlags = (b.flags||[]).map(function(x){return String(x).toLowerCase();});
    var aRaw = aFlags.indexOf("raw-meat")>=0 || aFlags.indexOf("raw-fish")>=0;
    var bRaw = bFlags.indexOf("raw-meat")>=0 || bFlags.indexOf("raw-fish")>=0;
    var aProduce = aFlags.indexOf("produce")>=0 || aFlags.indexOf("ready-to-eat")>=0;
    var bProduce = bFlags.indexOf("produce")>=0 || bFlags.indexOf("ready-to-eat")>=0;
  ...
  function buildGraph(tasks, ctx) {
    ...
    for (var i=0;i<tasks.length;i++) {
      nodes.push({ id: tasks[i].id, idx: i, domain: tasks[i].domain, title: tasks[i].title });
    }
    for (var a=0;a<tasks.length;a++) {
      for (var b=a+1;b<tasks.length;b++) {
        ...
        if (w >= ((ctx && ctx.minPairScore) || 0.35)) {
          edges.push({ a: a, b: b, w: w, sanitation: sanitationRisk(tasks[a], tasks[b]) });
        }
      }
    }
  ...
  function clusterize(tasks, ctx) {
    ...
    function unite(x,y){ var rx=find(x), ry=find(y); if(rx!==ry) parent[ry]=rx; }
  ...
  function buildPrepChecklist(cluster) {
    ...
    for (var a=0;a<cluster.length;a++){
      for (var b=a+1;b<cluster.length;b++){
        if (sanitationRisk(cluster[a], cluster[b])) { needsSanitize = true; break; }
      }
      if (needsSanitize) break;
    }
    if (needsSanitize) {
      steps.unshift({ label: "Prep sanitize station (soap + sanitizer, fresh towels)", domain: "cleaning" });
      steps.push({ label: "Sanitize surfaces/tools after raw handling", domain: "cleaning" });
      if (consumables.indexOf("sanitizer")<0) consumables.push("sanitizer");
      if (consumables.indexOf("towels")<0) consumables.push("towels");
      if (tools.indexOf("cutting board - raw")<0) tools.push("cutting board - raw");
      if (tools.indexOf("cutting board - produce")<0) tools.push("cutting board - produce");
    }
    ...
    for (var i2=0;i2<cluster.length;i2++) if (cluster[i2].domain === "cleaning") { hasCleaning = true; break; }
    if (hasCleaning) {
      steps.unshift({ label: "Stage cleaning caddy (spray, microfiber, gloves)", domain: "cleaning" });
    }

    // 4) Garden/animal bundles
    var hasGarden = false, hasAnimal = false;
    for (var i3=0;i3<cluster.length;i3++){
      if (cluster[i3].domain==="garden") hasGarden = true;
      if (cluster[i3].domain==="animal") hasAnimal = true;
    }
    if (hasGarden) steps.unshift({ label: "Harvest tote ready (shears, bins, labels)", domain: "garden" });
    if (hasAnimal) steps.unshift({ label: "Butchery PPE & labels staged", domain: "animal" });
  ...
  function summarizeCluster(cluster) {
    ...
    var earliest = asDate(cluster[0].start), latest = asDate(cluster[0].end);
    var domains = {};
    for (var i=0;i<cluster.length;i++){
      var t = cluster[i];
  ...
  function clusterToSuggestion(cluster, ctx) {
    ...
    var suggestion = {
      ...
      nbActions: [
        ...
        { label: "Open Prep Checklist Generator", action: "OPEN_PREP_CHECKLIST" },
    ...
    var tools = []; var apps = []; var ings = [];
    for (var i=0;i<cluster.length;i++){
      tools = tools.concat(cluster[i].tools||[]);
      apps = apps.concat(cluster[i].appliances||[]);
      ings = ings.concat(cluster[i].ingredients||[]);
    }
    tools = uniq(tools); apps = uniq(apps); ings = uniq(ings);

    if (apps.length) suggestion.reasons.push("Shared appliances: " + apps.join(", "));
    if (tools.length) suggestion.reasons.push("Shared tools: " + tools.join(", "));
    if (ings.length) suggestion.reasons.push("Shared ingredients: " + ings.slice(0,4).join(", ") + (ings.length>4?"…":""));
    suggestion.reasons.push("Time overlap/proximity within ~" + ((ctx && ctx.windowSlackMinutes) || 20) + " minutes.");
  ...
  function analyze(tasks, ctx) {
    ...
    if (!Array.isArray(tasks) || !tasks.length) { out.reasons.push("No tasks provided"); return out; }
    ...
    if (!norm.length) { out.reasons.push("No valid tasks"); return out; }
    ...
    for (var dk=0; dk<dayKeys.length; dk++){
      ...
      for (var n=0;n<res.graph.nodes.length;n++) {
        ...
        globalGraph.nodes.push({ id: node.id, idx: node.idx + offset, domain: node.domain, title: node.title });
      }
      for (var e=0;e<res.graph.edges.length;e++) {
        var edge = res.graph.edges[e];
        globalGraph.edges.push({ a: edge.a + offset, b: edge.b + offset, w: edge.w, sanitation: edge.sanitation });
      }
      ...
      for (var c=0;c<clusters.length;c++){
        ...
        if (cluster.length === 1) {
          ...
          var heavy = Number(singleton.estMinutes || 0) >= 45 || (singleton.appliances||[]).length>0;
          if (!heavy) continue;
        }
    ...
    try {
      if (ctx && ctx.emitEvents && eventBus && typeof eventBus.emit === "function") {
  ...
  function mergeChecklists(list) {
    var steps = []; var tools = []; var consumables = []; var appliances = []; var ingredients = [];
    ...
    var seen = {}; var stepOut = [];
    for (var s=0;s<steps.length;s++){
      var lbl = steps[s].label;
      if (!seen[lbl]) { seen[lbl]=1; stepOut.push(steps[s]); }
    }
    return {
      steps: stepOut,
      tools: uniq(tools),
