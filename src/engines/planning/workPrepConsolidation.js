...
(function () {
  ...
  function loadPrefs(ctx) {
    ...
    var ctxPrefs =
      ctx && ctx.preferences
        ? { prep: ctx.preferences.prep, resources: ctx.preferences.resources }
        : {};
    var runtime = {};
    try {
      var pr =
        automation && automation.get
          ? automation.get("workprep.preferences")
          : null;
  ...
  function idOf(it) {
    return it.id || it._id || it.slug || it.title || it.name || null;
  }
  function slotStartOf(it) {
    return asDate(it && it.slot && (it.slot.start || it.slot.date || it.slot));
  }
  function slotEndOf(it) {
    ...
    var mins = Number(
      it.totalTimeMinutes ||
        ...
        (it.time &&
          (it.time.total ||
            Number(it.time.prep || 0) + Number(it.time.cook || 0))) ||
        it.durationMin ||
        fallbackMin
    );
  ...
  function extractResources(item) {
    ...
    var tags = item.tags || [];
    res.zone =
      extractTagValue(tags, "zone") ||
      (item.domain === "garden" ? "outdoor" : "indoor");
    res.species =
      extractTagValue(tags, "species") ||
      (Array.isArray(item.species) && item.species[0]) ||
      null;
    ...
    if (item.oven && (defined(item.oven.tempF) || defined(item.oven.tempC))) {
      res.oven = {
        tempF: defined(item.oven.tempF) ? Number(item.oven.tempF) : null,
    ...
    if (Array.isArray(item.chemicals))
      res.chemicals = item.chemicals.map(function (c) {
        return norm(c.name || c);
      });
    ...
    if (Array.isArray(item.steps)) {
      for (var i = 0; i < item.steps.length; i++) {
        ...
        if (
          s.surface &&
          /floor|counter|glass|stainless|toilet|sink/.test(s.surface)
        )
          uniqPush(res, "tools", "microfiber cloth");
        if (s.timer && s.timer.durationSec) uniqPush(res, "tools", "timer");
        if (/mop|bucket/i.test(s.text || ""))
          uniqPush(res, "tools", "mop & bucket");
      }
    }
    ...
    if (item.domain === "garden") {
      ...
      if (/trellis|stake/i.test(String(item.kind || "")))
        uniqPush(res, "tools", "stakes/trellis");
      if (/fertil/i.test(String(item.kind || "")))
        uniqPush(res, "tools", "fertilizer scoop");
    }
    ...
    for (var i = 0; i < timeline.length; i++) {
      ...
      for (var ai = 0; ai < A.res.appliances.length; ai++) {
        ...
        for (var j = i + 1; j < timeline.length; j++) {
          ...
          if (
            B.res.appliances.indexOf(app) >= 0 &&
            overlap(aStart, aEnd, B.start, B.end)
          ) {
            overlaps.push({
              type: "appliance",
              resource: app,
              ...
              fixes: [
                {
                  label: "Shift later by +30m",
                  payload: { type: "SHIFT_MINUTES", id: B.id, minutes: 30 },
                },
                {
                  label: "Start earlier by -30m",
                  payload: { type: "SHIFT_MINUTES", id: A.id, minutes: -30 },
                },
              ],
            });
          }
        ...
        for (var j2 = i + 1; j2 < timeline.length; j2++) {
          ...
          if (B2.res.zone === zone && overlap(aStart, aEnd, B2.start, B2.end)) {
            overlaps.push({
              ...
              fixes: [
                {
                  label: "Pick alternate zone",
                  payload: { type: "PICK_ZONE", id: B2.id },
                },
                {
                  label: "Shift by +30m",
                  payload: { type: "SHIFT_MINUTES", id: B2.id, minutes: 30 },
                },
              ],
            });
          }
      ...
      if (A.res.species) {
        ...
        for (var j3 = i + 1; j3 < timeline.length; j3++) {
          var B3 = timeline[j3];
          if (
            norm(B3.res.species) === sp &&
            overlap(aStart, aEnd, B3.start, B3.end)
          ) {
            overlaps.push({
              type: "species",
              resource: sp,
              ...
              fixes: [
                {
                  label: "Stagger by 20m",
                  payload: { type: "SHIFT_MINUTES", id: B3.id, minutes: 20 },
                },
                {
                  label: "Assign different handler",
                  payload: {
                    type: "ASSIGN_ROLE",
                    role: "assistant",
                    id: B3.id,
      ...
      if (A.res.bed) {
        ...
        for (var j4 = i + 1; j4 < timeline.length; j4++) {
          ...
          if (
            norm(B4.res.bed) === bed &&
            overlap(aStart, aEnd, B4.start, B4.end)
          ) {
            overlaps.push({
              type: "bed",
              resource: bed,
              ...
              fixes: [
                {
                  label: "Split tasks (sequence)",
                  payload: { type: "SEQUENCE", ids: [A.id, B4.id] },
                },
                {
                  label: "Move B to next free window",
                  payload: { type: "AUTOSCHEDULE_NEXT", id: B4.id },
                },
              ],
            });
          }
      ...
      for (var ri = 0; ri < A.res.roles.length; ri++) {
        ...
        for (var j5 = i + 1; j5 < timeline.length; j5++) {
          var B5 = timeline[j5];
          if (
            B5.res.roles.indexOf(role) >= 0 &&
            overlap(aStart, aEnd, B5.start, B5.end)
          ) {
            overlaps.push({
              type: "role",
              resource: role,
              ...
              fixes: [
                ...
                {
                  label: "Shift B by +15m",
                  payload: { type: "SHIFT_MINUTES", id: B5.id, minutes: 15 },
                },
              ],
            });
          }
  ...
  function consolidatePreps(timeline, prefs) {
    ...
    for (var i = 0; i < ovenUsers.length; i++) {
      ...
      if (group.length > 1) {
        ...
        var temps = group
          .map(function (g) {
            return (g.res.oven && g.res.oven.tempF) || null;
          })
          .filter(defined);
        var target = temps.length
          ? Math.round(
        ...
        for (var k = 0; k < temps.length; k++)
          spread = Math.max(spread, Math.abs(temps[k] - target));

        var actions = [
          {
            kind: "preheat",
            text: "Preheat oven" + (target ? " to " + target + "°F" : ""),
            at: new Date(windowStart.getTime() - 10 * 60000),
          },
        ];
        if (spread > (prefs.prep.ovenTempToleranceF || 25)) {
          actions.push({
            kind: "notice",
            text: "Oven temperature mismatch across items; consider sequence or split batches.",
          });
        }
        var appliesTo = group.map(function (g) {
          return g.id;
        });
    ...
    var cleaners = timeline.filter(function (t) {
      return (
        t.domain === "cleaning" || (t.res.chemicals && t.res.chemicals.length)
      );
    });
    if (cleaners.length) {
      // group by zone & proximity
      cleaners.sort(function (a, b) {
        return a.start - b.start;
      });
      var visited = {};
      for (var c = 0; c < cleaners.length; c++) {
        ...
        for (var d = c + 1; d < cleaners.length; d++) {
          ...
          var near =
            Math.abs(D.start - C.start) <=
            (prefs.prep.proximityMinutes || 30) * 60000;
          if (sameZone && near) {
            groupC.push(D);
            visited[d] = true;
          } else if (
            D.start - C.start >
            (prefs.prep.proximityMinutes || 30) * 60000 * 2
          )
            break;
        }
        ...
        if (groupC.length > 1) {
          ...
          for (var gx = 0; gx < groupC.length; gx++) {
            var t = groupC[gx];
            for (var ci = 0; ci < t.res.chemicals.length; ci++) {
              chems[t.res.chemicals[ci]] = true;
            }
            for (var pi = 0; pi < t.res.ppe.length; pi++) {
              ppe[t.res.ppe[pi]] = true;
            }
          }
          ...
          if (chemKeys.length) {
            actionsC.push({
              kind: "stage",
              text: "Stage chemicals: " + chemKeys.join(", "),
              at: new Date(start.getTime() - 5 * 60000),
            });
            // ensure dwell timers exist downstream
            actionsC.push({
              kind: "timer-sanity",
              text:
                "Ensure dwell timers set (" +
                (prefs.prep.defaultDwellSec || 600) +
                "s) for disinfectant steps.",
            });
          }
          if (ppeKeys.length) {
            actionsC.push({
              kind: "stage",
              text: "Stage PPE: " + ppeKeys.join(", "),
              at: new Date(start.getTime() - 5 * 60000),
            });
          }
          ...
          bundles.push({
            ...
            actions: actionsC,
            appliesTo: groupC.map(function (g) {
              return g.id;
            }),
          });
        }
    ...
    var garden = timeline.filter(function (t) {
      return t.domain === "garden";
    });
    if (garden.length) {
      ...
      for (var gi = 0; gi < garden.length; gi++) {
        ...
        for (var gj = gi + 1; gj < garden.length; gj++) {
          ...
          var H = garden[gj];
          var near =
            Math.abs(H.start - G.start) <=
            (prefs.prep.proximityMinutes || 30) * 60000;
          var sameBed =
            G.res.bed && H.res.bed && norm(G.res.bed) === norm(H.res.bed);
          if (near || sameBed) {
            groupG.push(H);
            used[gj] = true;
          } else if (
        ...
        if (groupG.length > 1) {
          ...
          for (var tgi = 0; tgi < groupG.length; tgi++) {
            ...
            for (var tt = 0; tt < rr.tools.length; tt++)
              toolsSet[rr.tools[tt]] = true;
          }
          ...
          bundles.push({
            ...
            window: {
              start: groupG[0].start,
              end: groupG[groupG.length - 1].end,
            },
            actions: [
              {
                kind: "stage",
                text:
                  "Stage garden tools: " +
                  (toolList.join(", ") || "hand tools"),
                at: new Date(groupG[0].start.getTime() - 10 * 60000),
              },
            ],
            appliesTo: groupG.map(function (g) {
              return g.id;
            }),
          });
        }
    ...
    var withRoles = timeline.filter(function (t) {
      return t.res.roles && t.res.roles.length;
    });
    if (withRoles.length) {
        ...
        if (
          ...
          bundles.push({
            ...
            actions: [
              {
                kind: "brief",
                text: "Brief '" + overlapRole + "' once for adjacent tasks.",
              },
            ],
            appliesTo: [X.id, Y.id],
          });
  ...
  function consolidate(plan, ctx) {
    ...
    for (var i = 0; i < detection.overlaps.length; i++) {
      ...
      if (o.type === "appliance" && o.resource === "oven") {
        suggestions.push(
          "Oven conflict detected — consolidate preheat or stagger starts by 30m."
        );
      } else if (o.type === "zone") {
        suggestions.push(
          "Zone '" + o.resource + "' is busy — try alternate zone or shift."
        );
      } else if (o.type === "species") {
        suggestions.push(
          "Species overlap — assign assistant or stagger tasks."
        );
      } else if (o.type === "role") {
        suggestions.push(
          "Role '" +
            o.resource +
            "' is double-booked — assign alternate or reschedule."
        );
      } else if (o.type === "bed") {
        suggestions.push(
          "Garden bed '" +
            o.resource +
            "' double-booked — sequence tasks or move one."
        );
      }
    }
    if (
      bundles.some(function (b) {
        return b.type === "cleaning-station";
      })
    ) {
      suggestions.push(
        "Combine cleaning runs in the same zone and reuse staged PPE/chemicals."
      );
    }
    if (
      bundles.some(function (b) {
        return b.type === "garden-staging";
      })
    ) {
