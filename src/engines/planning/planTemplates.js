...
(function () {
  ...
  function idOf(x) {
    return (x && (x.id || x._id || x.slug || x.title)) || null;
  }
  ...
  function registerTemplate(tpl) {
    ...
    try {
      if (eventBus && typeof eventBus.emit === "function") {
        eventBus.emit("templates:registered", {
          id: tpl.id,
          version: tpl.version || 1,
          domain: tpl.domain || "generic",
        });
      }
    } catch (e) {}
    return tpl;
  ...
  function loadRuntimeTemplates() {
    try {
      ...
      for (var i = 0; i < packs.length; i++) {
        try {
          registerTemplate(packs[i]);
        } catch (e) {
          logger.warn("[planTemplates] bad runtime template", e);
        }
      }
    } catch (e) {
  ...
  function selectMealCandidate(pool, ctx) {
    ...
    return scored && scored.length
      ? pool.find(function (r) {
          return idOf(r) === scored[0].id;
        }) || pool[0]
      : pool[0];
  }
  ...
  function applyTemplate(opts) {
    ...
    if (!opts || !opts.templateId) {
      out.reasons.push("No templateId");
      return out;
    }

    var tpl = getTemplate(opts.templateId);
    if (!tpl) {
      out.reasons.push("Template not found: " + opts.templateId);
      return out;
    }
    ...
    try {
      if (typeof tpl.generator === "function") {
        intents =
          tpl.generator({ startDate: start, days: days, ctx: ctx }) || [];
      }
    } catch (e) {
    ...
    for (var i = 0; i < intents.length; i++) {
      ...
      var guard = sabbathGuard(
        intent.slot && intent.slot.start,
        ctx.tz,
        !!ctx.preferences && !!ctx.preferences.sabbathWriteProtect
      );
      if (guard && (intent.type === "COOK" || intent.type === "CLEAN")) {
        out.blocked.push({
          intent: intent,
          reasons: ["Sabbath guard: protected time"],
        });
        continue;
      }
      ...
      if (tpl.domain === "meals" && !recipe) {
        ...
        if (!recipe) {
          out.blocked.push({
            intent: intent,
            reasons: ["No candidate recipes available"],
          });
          continue;
        }
      }
      ...
      if (
        tpl.domain === "meals" &&
        placement &&
        typeof placement.canPlace === "function"
      ) {
        ...
        if (check.reservations && check.reservations.length) {
          for (var r = 0; r < check.reservations.length; r++) {
            ...
            ctx.plan.push({
              recipeId:
                "RESV:" + (check.reservations[r].forRecipeId || idOf(recipe)),
        ...
        if (!opts.dryRun && typeof placement.registerPlacement === "function") {
          try {
            placement.registerPlacement(recipe, intent.slot, ctx);
          } catch (e) {}
        }
      }
    ...
    try {
      if (eventBus && typeof eventBus.emit === "function") {
        eventBus.emit("templates:apply:done", {
          id: tpl.id,
  ...
  registerTemplate({
    ...
    meta: {
      ...
      description:
        "Balanced spread that respects time/budget/diet, rotates protein/cuisine, auto-reserves leftovers.",
  ...
  registerTemplate({
    ...
    meta: {
      ...
      description:
        "Large Sunday dinner + midweek leftovers lunches; quick dinners Mon-Thu.",
    ...
    generator: function (args) {
      ...
      for (var i = 0; i < 7; i++) {
        ...
        if (d.getDay() === 0) {
          ...
          out.push({
            type: "COOK",
            slot: { start: addDays(d, 5), mealType: "dinner" },
            meta: { coldPreferred: true },
          });
          break;
        }
      }
