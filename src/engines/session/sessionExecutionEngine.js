...
(function () {
  ...
  function sabbathActiveFor(sess){
    ...
    if (sess && sess.runbook && sess.runbook.meta && sess.runbook.meta.sabbathGuard) return true;
    var g = _cfgSabbath(); return !!(g && g.enabled);
  }

  /* ------------------------------- Event helpers ----------------------------- */
  function emit(evt, payload){
    try { if (eventBus && eventBus.emit) eventBus.emit(evt, payload); } catch (e) {}
  }
  function on(evt, handler){
    try { if (eventBus && eventBus.on) eventBus.on(evt, handler); } catch (e) {}
  }
  function off(evt, handler){
    try { if (eventBus && eventBus.off) eventBus.off(evt, handler); } catch (e) {}
  }
  ...
  function gateGuards(runbook){
    ...
    if (guards.sabbathActive) {
      emit("guard:sabbath", { runbookId: runbook.id, title: runbook.title, when: nowISO() });
    }
  ...
  function gateHazards(runbook){
    ...
    if (hazards.length) {
      emit("nba:hint", { type: "SAFETY", label: "Hazards: " + hazards.join(", "), runbookId: runbook.id });
    }
    if (ppe.length) {
      emit("nba:hint", { type: "PPE", label: "PPE: " + ppe.join(", "), runbookId: runbook.id });
    }
  }
  ...
  function buildTimerPayload(step, sessionId){
    ...
    return {
      id: sessionId + ":tmr:" + String((step && step.id) || Math.random().toString(36).slice(2)),
      groupId: sessionId,
      label: (step && step.timer && step.timer.label) || (step && step.label) || "Timer",
  ...
  /* ----------------------------- Session lifecycle --------------------------- */
  function _calcTotalSteps(runbook){ return (runbook && runbook.steps && runbook.steps.length) || 0; }
  ...
  function createSession(runbook){
    ...
    analytics.track && analytics.track("session/created", {
      id: runbook.id, domain: runbook.domain, minutes: Number(runbook.estimatedMinutes || 0) || null
    });
  ...
  function doDevice(step, sess){
    emit("device:command", { sessionId: sess.id, device: step.device || {}, label: step.label });
  }

  function _emitPrepWindow(step, sess){
    var isCandidate = !!(step && (step.emitAsPrepCandidate || step.parallelizable));
    if (step && step.type === "TIMER" && isCandidate) {
      emit("prep:candidate", {
        ...
        area: (sess.runbook && (sess.runbook.meta && (sess.runbook.meta.area || sess.runbook.meta.dish))) || ""
      });
    }
  }
  ...
  function doDelay(step, sess){
    ...
    var so = Number(step && step.startOffsetSec); if (!isFinite(so) || so < 0) so = 0;

    var t = {
      id: sess.id + ":delay:" + String(step.id || Math.random().toString(36).slice(2)),
      groupId: sess.id,
  ...
  function doReminder(step, sess){
    ...
    emit("schedule:reminder:create", {
      id: sess.id + ":rem:" + String(step.id || Math.random().toString(36).slice(2)),
      title: (step && step.reminder && step.reminder.label) || step.label || "Reminder",
      when: whenISO,
  ...
  function doCheck(step, sess){
    ...
    emit("check:prompt", { sessionId: sess.id, stepId: step.id, label: step.label || "Check" });
  }
  ...
  function _applyOnDoneFor(step, sess){
    ...
    for (var a=0;a<actions.length;a++){
      var act = actions[a];
      if (act.type === "NOTE") emit("session:note", { sessionId: sess.id, label: act.label });
    }
  }
  ...
  function finalize(sess){
    ...
    for (var i=0;i<cl.length;i++){
      emit("nba:hint", { type:"CLEANUP", label: cl[i].label, sessionId: sess.id });
    }
    ...
    analytics.track && analytics.track("session/done", { id: sess.id, domain: sess.runbook && sess.runbook.domain });
  }
  ...
  /* ------------------------------- Public API -------------------------------- */
  function createSession(runbook){ return restoreOrCreate(runbook, (runbook && runbook.meta && runbook.meta.resumeState) || null); }

  function start(sessId){
    var sess = SESSIONS[sessId]; if (!sess) return null;
  ...
  function skip(sessId, n){
    ...
    for (var i=0;i<count;i++){ advance(sess); }
  ...
  function startAt(sessId, index){
    var sess = SESSIONS[sessId]; if (!sess) return null;
    var total = (sess.runbook && sess.runbook.steps && sess.runbook.steps.length) || 0;
  ...
  function getResumeState(sessId){
    var sess = SESSIONS[sessId]; if (!sess) return null;
    return { index: sess.idx, done: sess.progress && sess.progress.done, ts: nowISO() };
  }
