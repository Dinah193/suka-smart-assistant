...
try {
  // @ts-ignore
  PersonalFoodStandards =
    require("@/components/tier1/components/PersonalFoodStandardsForm").API ||
    null;
} catch {}
...
try {
  // @ts-ignore
  scheduleHelpers =
    require("@/engines/scheduleHelpers").default ||
    require("@/engines/scheduleHelpers") ||
    null;
} catch {}
...
const DEFAULT_WEIGHTS = {
  meals: { onHandPct: 0.38, timeFit: 0.22, budgetFit: 0.2, dietMatch: 0.2 },
  garden: { seasonFit: 0.35, frostFit: 0.25, bedFit: 0.2, effortFit: 0.2 },
  cleaning: {
    supplyOnHand: 0.35,
    timeFit: 0.2,
    ppeCompliance: 0.25,
...
function estimateItemCost(ing) {
  ...
  if (ing?.quantity) {
    const qty = toBaseQuantity(ing.quantity, ing.unit);
    const band =
      ing.category === "meat" ? 3 : ing.category === "produce" ? 0.7 : 1.2;
...
function getDietProfile(context = {}) {
  ...
  try {
    const pfs = PersonalFoodStandards?.get?.();
    if (pfs) return pfs;
  } catch {}
...
function computeOnHandPct(recipe, context = {}) {
  ...
  const has = (name) => {
    ...
    for (const alt of subs) {
      ...
      if (altKey in inv && Number(inv[altKey]?.quantity || inv[altKey]) > 0)
        return altKey;
    }
    return null;
  };
  ...
  for (const ing of ings) {
    ...
    if (required && available) {
      if (available >= required) have += 1;
        ...
        have += CLAMP(available / required, 0, 1);
        reasons.push(
          `Partial on-hand for ${ing.name} (${available}/${required})`
        );
      }
    } else {
      have += 1;
  ...
  if (value === 1)
    reasons.push("All ingredients on-hand (allowing valid substitutions).");
...
function computeBudgetFit(recipe, context = {}) {
  ...
  if (!cps && !budget) {
    reasons.push("No budget/cost; neutral (0.6).");
    return { value: 0.6, reasons };
  }
  if (!budget) {
  ...
  if (cps <= budget) {
    ...
    reasons.push(
      `At/under budget: $${cps.toFixed(2)} ≤ $${budget.toFixed(2)}/serv.`
    );
    return { value: CLAMP(0.9 + bonus, 0, 1), reasons };
  }
...
function computeDietMatch(recipe, context = {}) {
  ...
  const allowBonus = allow.length
    ? allow.filter((tag) => rTags.includes(tag)).length
    : 0;
  ...
  if (profile.macros && recipe.nutrition) {
    ...
    for (const k of ["kcal", "protein", "fat", "carbs"]) {
      if (
        typeof n[k] === "number" &&
        typeof (tgt[k]?.target ?? tgt[k]) === "number"
      ) {
    ...
    if (dims > 0) {
      macroScore = CLAMP(sum / dims, 0, 1);
      reasons.push(`Macro alignment ~${Math.round(macroScore * 100)}%.`);
    }
  }
  let v = macroScore;
  ...
  if (v <= 0.4 && (hasAvoid || ingConflict))
    reasons.push("Fails diet rules; consider swaps.");
  return { value: v, reasons };
}
...
function gardenSeasonFit(task, context = {}) {
  ...
  const lastFrost = settings?.garden?.frost?.last
    ? new Date(settings.garden.frost.last)
    : null;
  const firstFrost = settings?.garden?.frost?.first
    ? new Date(settings.garden.frost.first)
    : null;
...
function gardenBedFit(task, context = {}) {
  ...
  try {
    ...
    if (typeof busy === "boolean") {
      if (!busy) {
        reasons.push("Bed free in requested window.");
        return { value: 0.95, reasons };
      }
      reasons.push("Bed busy; trying next window.");
      const next = scheduleHelpers?.nextWindow?.({
        scope: "garden",
        bedId: task.bedId || task.bedName,
        minutes: task.durationMin || 30,
        from: task.targetDate ? new Date(task.targetDate) : getNow(),
      });
...
function gardenEffortFit(task, context = {}) {
  ...
  return computeTimeFit(
    ...
    { timeWindowMinutes: context.timeWindowMinutes || context.timeWindow }
  );
}
...
function cleaningSupplyOnHand(item, context = {}) {
  ...
  (item.tags || []).forEach((t) => {
    if (/quat|bleach|glass-cleaner/.test(t)) needs.add(normName(t));
  });

  if (!needs.size) {
    reasons.push("No specific supplies listed; neutral (0.7).");
...
function cleaningPPECompliance(item) {
  ...
  if (!required.size) {
    reasons.push("No PPE required; full compliance.");
    return { value: 1, reasons };
  }
...
function cleaningDwellCoverage(item) {
  ...
  const hasDwell = steps.some((s) =>
    Number(s.dwellSec || s.timer?.durationSec)
  );
  const value = hasDwell ? 0.95 : 0.55;
  reasons.push(
    hasDwell
      ? "Dwell timers present in steps."
      : "No dwell timing; add timers for disinfectants."
  );
  return { value, reasons };
}
...
function cleaningTimeFit(item, context = {}) {
  const minutes =
    ...
    (Array.isArray(item.steps)
      ? item.steps.reduce((a, s) => a + Number(s.durationMin || 0), 0)
      : 0) ||
    0;
  return computeTimeFit(
...
function animalEquipOnHand(proc, context = {}) {
  ...
  const inv = context.inventory || InventoryMonitor?.getSnapshot?.() || {};
  const needs = new Set(
    [...(proc.equipment || []), ...(proc.ppe || [])].map(normName)
  );
  if (!needs.size) {
    reasons.push("No equipment listed; neutral (0.7).");
    return { value: 0.7, reasons };
  }
  let have = 0;
  needs.forEach((n) => {
  ...
  reasons.push(
    v === 1
      ? "All equipment/PPE on-hand."
      : `Missing ${needs.size - have} item(s).`
  );
  return { value: v, reasons };
}
function animalRoleCoverage(proc) {
  ...
  const stepRoles = new Set(
    (proc.steps || []).map((s) => s.role).filter(Boolean)
  );
  const missing = [...stepRoles].filter((r) => !roles.has(r));
  if (missing.length) {
    reasons.push(`Role(s) not assigned: ${missing.join(", ")}.`);
    return { value: 0.55, reasons };
  }
...
function animalSafetyCompliance(proc) {
  ...
  const hasSharps = hazards.some((h) =>
    /sharps/.test(h.code || h.description || "")
  );
  const hasBio = hazards.some((h) =>
    /bio|zoon/.test(h.code || h.description || "")
  );
  let v = 0.8;
  if (hasSharps || hasBio) v = 0.9;
  if (
    (proc.preChecks || []).some((c) =>
      /PPE|sanitize|disinfect/i.test(c.text || c)
    )
  )
    v = Math.min(1, v + 0.05);
...
function animalTimeFit(proc, context = {}) {
  const minutes =
    ...
    (Array.isArray(proc.steps)
      ? proc.steps.reduce((a, s) => a + Number(s.durationMin || 0), 0)
      : 0) ||
    0;
  return computeTimeFit(
    { totalTimeMinutes: minutes },
...
function aggregateScore(parts, weights) {
  ...
  const score = CLAMP(
    keys.reduce(
      (acc, k) => acc + (parts[k]?.value || 0) * (weights[k] || 0),
      0
    ),
    0,
    1
  );
  const reasons = keys.flatMap((k) =>
    (parts[k]?.reasons || []).map((r) => `${k}: ${r}`)
  );
  ...
  for (const k of keys) {
    ...
    if (v < 0.7) {
      if (k === "supplyOnHand" || k === "equipOnHand" || k === "onHandPct")
        nextBestActions.push("Open: Inventory to review missing items.");
      if (k === "timeFit" || k === "effortFit")
        nextBestActions.push("Try: Reschedule or split into smaller slots.");
      if (k === "frostFit")
        nextBestActions.push("Adjust: Move to frost-safe date.");
      if (k === "ppeCompliance")
        nextBestActions.push("Check: PPE list before starting.");
      if (k === "dwellCoverage")
        nextBestActions.push("Add: Dwell timers to steps.");
      if (k === "safetyCompliance")
        nextBestActions.push("Review: Hazards & controls.");
    }
  }
...
function inferDomain(item) {
  ...
  if (
    (item?.kind &&
      /sow|transplant|harvest|prune|weed|water|fertiliz|trellis|thin|seed/i.test(
        item.kind
      )) ||
    item?.bedName ||
    item?.cropName
    ...
    return "garden";
  if (item?.chemicals || item?.roomType || (item?.ppe && !item?.species))
    return "cleaning";
...
function scoreItem(item, context = {}) {
  ...
  if (domain === "meals") {
    ...
    id =
      id ||
      `${item.kind || "task"}:${item.cropName || "crop"}:${
        item.bedName || "bed"
      }`;
  ...
  const signals = Object.fromEntries(
    Object.entries(parts).map(([k, v]) => [k, v.value])
  );
  ...
  const result = {
    ...
    weights,
    reasons: agg.reasons,
    nextBestActions: agg.nextBestActions,
  };
    ...
    if (domain === "meals") {
      if (result.score >= 0.82 && (signals.onHandPct ?? 1) < 0.7) {
        eventBus.emit?.("nba:hint", {
          type: "SHOPPING_MICRO",
          recipeId: id,
          reason: "High score but missing items.",
        });
      }
    }
  } catch {}
