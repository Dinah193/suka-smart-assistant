...
const Z = Object.freeze({
  0.50: 0.000, // median ~ mean for symmetric approx
...
export function estimateStepsPERT(steps = []) {
  ...
  for (let idx = 0; idx < steps.length; idx++) {
    ...
    per.push({ idx: idx + 1, label: String(s.label || `Step ${idx + 1}`), ...r.stat });
    sumE += r.stat.E;
    sumVar += r.stat.variance;
  }
  const sigma = Math.sqrt(sumVar);
  return { ok: true, stat: { E: sumE, variance: sumVar, sigma, steps: per, bands: confidenceBands(sumE, sigma) } };
}
...
export function estimatePathPERT({ nodes = [], edges = [] } = {}) {
  ...
  for (const e of edges) {
    ...
    if (!adj.has(u) || !adj.has(v)) return { ok: false, error: `edge references unknown node (${u}→${v})` };
    adj.get(u).push(v);
  ...
  for (const id of topo) {
    ...
    if (!bestE.has(id)) { bestE.set(id, E); bestVar.set(id, variance); }
    else { bestE.set(id, bestE.get(id) + E); bestVar.set(id, bestVar.get(id) + variance); }
    // Relax edges
    for (const v of adj.get(id)) {
      const candE = bestE.get(id) + sMap.get(v).E;
      const candVar = bestVar.get(id) + sMap.get(v).variance;
      if (!bestE.has(v) || candE > bestE.get(v)) {
        bestE.set(v, candE); bestVar.set(v, candVar); prev.set(v, id);
      }
    }
  }

  // Sink with largest E
  let sink = null, maxE = -Infinity;
  for (const id of topo) {
    if ((adj.get(id) || []).length === 0) {
      const e = bestE.get(id) ?? sMap.get(id).E;
      if (e > maxE) { maxE = e; sink = id; }
    }
  }
...
export function initPERT() {
  ...
  eventBus.on?.(Events?.SESSION_DRAFT_READY || "session/draftReady", ({ data }) => {
    try { estimateSessionDraft(data?.draft || data); } catch {}
  }, { priority: -2 });
}
...
function coerceOMP(s) {
  ...
  const O = pickNum(s.o, s.optimistic, hoursToMin(s.o_h), scaleAround(s.estMin, 0.8), scaleAround(s.minutes, 0.8));
  const M = pickNum(s.m, s.mostLikely, hoursToMin(s.m_h), s.estMin, s.minutes);
  const P = pickNum(s.p, s.pessimistic, hoursToMin(s.p_h), scaleAround(s.estMin, 1.25), scaleAround(s.minutes, 1.25));
...
function hoursToMin(h) { return isFinitePos(h) ? h * 60 : undefined; }
function scaleAround(n, f) { return isFinitePos(n) ? n * f : undefined; }
function pickNum(...vals) { for (const v of vals) if (isFinitePos(v)) return +v; return undefined; }
function clampPos(n) { return isFinitePos(n) ? Math.max(0.1, +n) : undefined; }
