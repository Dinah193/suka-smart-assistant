...
export function initMonteCarlo() {
  ...
  eventBus.on?.(
    ...
    ({ data }) => {
      try {
        simulateSessionDraft(data?.draft || data, { runs: 3000 });
      } catch {}
    },
    { priority: -3 }
  );
...
function normalizeOptions({
  ...
  return {
    ...
    dist,
    lambda,
    seed: seed != null ? seed | 0 : hashSeed(`${Date.now()}`),
  };
}
...
function coerceOMP(s = {}, idx = 0) {
  ...
  const P = firstNum(
    ...
    s.pessimistic,
    hoursToMin(s.p_h),
    scaleAround(s.estMin, 1.6),
    scaleAround(s.minutes, 1.6),
    M * 1.5
  );
...
function hoursToMin(h) {
  return isFinite(h) ? h * 60 : undefined;
}
function scaleAround(n, f) {
  return isFinite(n) ? n * f : undefined;
}
function firstNum(...vals) {
  for (const v of vals) if (isFinite(v)) return +v;
  return undefined;
}
function isFinite(n) {
...
function clampPos(n) {
  return isFinite(n) ? Math.max(0.1, +n) : 1;
}
function small(x) {
  try {
...
function summarizeSamples(arr /* Float64Array */) {
  ...
  const pct = (p) =>
    sorted[Math.max(0, Math.min(n - 1, Math.floor(p * (n - 1))))];
  const p50 = pct(0.5),
    p80 = pct(0.8),
    p90 = pct(0.9),
    p95 = pct(0.95);
