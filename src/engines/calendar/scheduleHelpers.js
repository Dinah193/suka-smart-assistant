...
(function () {
  ...
  try {
    ...
    if (inv) {
      Inventory.has = function (skuOrName) {
        try {
          return !!inv.hasItem && !!inv.hasItem(skuOrName);
        } catch (_) {
          return false;
        }
      };
  ...
  try {
    ...
    if (weatherSvc) {
      Weather.forecast = function (args) {
        try {
          return weatherSvc.getWindowForecast(args) || [];
        } catch (_) {
          return [];
        }
      };
      Weather.now = function (args) {
        try {
          return weatherSvc.getCurrent(args) || null;
        } catch (_) {
          return null;
        }
      };
  ...
  try {
    SpeciesProfiles =
      require("@/data/species.profiles.json") || SpeciesProfiles;
  } catch (_) {}
  var CropProfiles = {};
  try {
    CropProfiles = require("@/data/crop.profiles.json") || CropProfiles;
  } catch (_) {}
  ...
  function id(prefix) {
    return (prefix || "rem") + ":" + Math.random().toString(36).slice(2, 10);
  }
  ...
  function planPPE(task) {
    ...
    if (kind === "butchery" || flags.indexOf("raw-meat") >= 0)
      extra = extra.concat([
        "gloves",
        "apron",
    ...
    if (kind === "animal" && flags.indexOf("biohazard") >= 0)
      extra = extra.concat(["gloves", "eye protection", "respirator (N95)"]);
    if (kind === "cleaning" && flags.indexOf("caustic") >= 0)
      extra = extra.concat([
        "gloves",
        "eye protection",
        "respirator (chem)",
    ...
    if (
      kind === "garden" &&
      (flags.indexOf("spray") >= 0 || flags.indexOf("pesticide") >= 0)
    )
      extra = extra.concat([
        "gloves",
        "eye protection",
    ...
    if (flags.indexOf("sharp") >= 0)
      extra = extra.concat(["cut-resistant gloves"]);

    // Profile-based PPE (species or crop)
    var species = task.species && String(task.species).toLowerCase();
    if (
      species &&
      SpeciesProfiles[species] &&
      Array.isArray(SpeciesProfiles[species].ppe)
    ) {
    ...
    if (kind === "butchery") {
      notes.push(
        "Confirm sanitation station is set and chill chain timers are armed."
      );
    }
  ...
  function schedulePPEReminder(task, session) {
    ...
    var msg =
      ...
      (task.title || "Task") +
      "\n• Required: " +
      (ppePlan.required.join(", ") || "—");
    if (ppePlan.missing.length)
    ...
    var payload = {
      ...
      tags: ["ppe", "safety", "nba"],
      linkTo: {
        sessionId: ctx.id || read(task, "sessionId") || null,
        taskId: task.id || null,
      },
      data: {
        type: "ppe",
        required: ppePlan.required,
        missing: ppePlan.missing,
      },
    };

    try {
      (ReminderManager.schedule || automation.schedule || function () {})(
        payload
      );
    } catch (_) {}
    try {
      eventBus.emit("reminder.created", payload);
    } catch (_) {}
    try {
  ...
  function planWeather(task) {
    ...
    var windowEnd = toISO(
      task.endAt || addMinutes(windowStart, task.durationMin || 60)
    );
    var windowMins = Math.max(
      5,
    ...
    var avoidRain =
      !!env.avoidRain ||
      (task.kind === "garden" && (task.flags || []).indexOf("spray") >= 0);
    var avoidHighWind =
      !!env.avoidHighWind ||
      (task.kind === "garden" && (task.flags || []).indexOf("spray") >= 0);
    ...
    for (var i = 0; i < fc.length; i++) {
      ...
      var precipChance =
        typeof p.precipChance === "number" ? p.precipChance : 0;
      ...
      if (tempMin != null && tempC != null && tempC < tempMin) {
        issues.push({
          at: p.at,
          type: "cold",
          msg: "Temp " + tempC + "°C below min " + tempMin + "°C.",
        });
      }
      if (tempMax != null && tempC != null && tempC > tempMax) {
        issues.push({
          at: p.at,
          type: "heat",
          msg: "Temp " + tempC + "°C above max " + tempMax + "°C.",
        });
      }
    }
    ...
    return {
      issues: issues,
      window: { start: windowStart, end: windowEnd, minutes: windowMins },
    };
  }
  ...
  function scheduleWeatherAlerts(task, session) {
    ...
    if (!plan.issues.length) {
      ...
      var sensitive =
        (task.kind === "garden" &&
          (task.flags || []).some(function (f) {
            return f === "spray" || f === "transplant";
          })) ||
        task.kind === "butchery" ||
        (task.kind === "animal" &&
          (task.flags || []).indexOf("heat-stress-risk") >= 0);
      if (sensitive) {
        var msg =
          "[Weather OK] " +
          (task.title || "Task") +
          " — no forecasted conflicts in the planned window (" +
          fmtDuration(plan.window.minutes) +
          ").";
        var payloadOk = {
          id: id("wxok"),
        ...
        try {
          eventBus.emit("reminder.created", payloadOk);
        } catch (_) {}
    ...
    for (var i = 0; i < plan.issues.length; i++) {
      notes.push(
        "• " +
          plan.issues[i].msg +
          " (" +
          (plan.issues[i].at || "time TBA") +
          ")"
      );
    }
    var msgWarn =
      "[Weather Risk] " + (task.title || "Task") + "\n" + notes.join("\n");
    var payloadWarn = {
      id: id("wx"),
    ...
    try {
      eventBus.emit("reminder.created", payloadWarn);
    } catch (_) {}
    try {
      eventBus.emit("nba.suggestion.created", {
        id: payloadWarn.id,
        ...
        action: {
          type: "openWeatherSmartRescheduler",
          params: { window: plan.window, issues: plan.issues },
        },
      });
    } catch (_) {}
  ...
  function resolveWithholdDays(task) {
    ...
    if (w.until) {
      ...
      var now = new Date(
        read(task, "appliedAt", task.startAt || automation.nowISO())
      );
      var diff = Math.ceil((d - now) / 86400000);
      return clamp(diff, 0, 365);
    }
    var domain =
      task.kind === "garden" || w.type === "harvest" ? "garden" : "animal";
    var src = (w.source || "").toLowerCase();
    if (
      src &&
      WithholdRegistry[domain] &&
      WithholdRegistry[domain][src] != null
    ) {
  ...
  function scheduleWithholdReminders(task, session) {
    ...
    var product = (
      w.type || (task.kind === "garden" ? "harvest" : "meat")
    ).toLowerCase();
    var note =
      "[Withhold Active] " +
      (task.title || "Task") +
    ...
    var reminders = [
      ...
      },
      {
        ...
        message:
          "[Withhold Clear] You can now resume use of " +
          product.toUpperCase() +
          ".",
        tags: ["withhold", "clear"],
        linkTo: {
    ...
    try {
      eventBus.emit("inventory.hold.created", {
        product: product,
        until: untilISO,
        reason: "withhold",
        refTaskId: task.id || null,
      });
    } catch (_) {}
    try {
      eventBus.emit("calendar.block.range", {
        start: appliedISO,
        end: untilISO,
        label: "WITHHOLD: " + product.toUpperCase(),
        color: "warning",
      });
    } catch (_) {}
    ...
    for (var i = 0; i < reminders.length; i++) {
      ...
      try {
        eventBus.emit("reminder.created", reminders[i]);
      } catch (_) {}
    }
  ...
  function scheduleAll(task, session) {
    ...
    var out = [];
    try {
      out.push(schedulePPEReminder(task, session));
    } catch (e) {
      console.warn("PPE schedule error:", e && e.message);
    }
    try {
      var wx = scheduleWeatherAlerts(task, session);
      if (wx && wx.length) out = out.concat(wx);
    } catch (e2) {
      console.warn("Weather schedule error:", e2 && e2.message);
    }
    try {
      var wh = scheduleWithholdReminders(task, session);
      if (wh && wh.length) out = out.concat(wh);
    } catch (e3) {
      console.warn("Withhold schedule error:", e3 && e3.message);
    }
