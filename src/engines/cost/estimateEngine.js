...
(function () {
  ...
  try { eventBus = (require("@/services/eventBus") || {}).eventBus || eventBus; } catch (_) {}

  // Price/Inventory sources (all optional, used opportunistically)
  var Inventory = {
    findBySku: function(){ return null; },
    priceFor: function(){ return null; } // { price, currency, packQty, packUnit, lastUpdated }
  };
  try {
    var inv = require("@/managers/InventoryMonitor");
    if (inv) {
      Inventory.findBySku = function(sku){ try { return inv.getItemBySKU && inv.getItemBySKU(sku); } catch(_){ return null; } };
      Inventory.priceFor = function(q){ try { return inv.getPriceFor && inv.getPriceFor(q); } catch(_){ return null; } };
    }
  } catch(_) {}

  var PriceBook = { lookup: function(){ return null; } };
  try { PriceBook = require("@/services/pricing/priceBook") || PriceBook; } catch (_) {}

  var SupplierCatalog = { find: function(){ return null; } };
  try { SupplierCatalog = require("@/services/suppliers/catalog") || SupplierCatalog; } catch(_){}

  var Settings = {
    currency: "USD",
    utilityRates: { // sensible defaults; override in SettingsStore
      electricity_kwh: 0.15,
      natural_gas_therm: 1.20,
      water_1000gal: 5.00
    },
    tz: "America/New_York",
    locale: "en-US",
  ...
  try {
    ...
    Settings.currency = (settings.get && settings.get("currency")) || Settings.currency;
  ...
  var StoreSelector = { getActiveStoreId: function(){ return null; } };
  try { StoreSelector = require("@/stores/StoreSelector") || StoreSelector; } catch(_){}

  // ----------------------------- Tiny Utils -----------------------------
  function toNum(n, def){ n = Number(n); return isFinite(n) ? n : (def || 0); }
  function round2(n){ return Math.round((toNum(n,0) + Number.EPSILON) * 100) / 100; }
  function id(prefix){ return (prefix||"est")+":"+Math.random().toString(36).slice(2,10); }
  function pick(a,b,c){ return a!=null ? a : (b!=null ? b : c); }
  ...
  var U = (function(){
    ...
    var toBase = function(qty, unit){
      ...
      switch(unit){
        case "g": return { qty:n, unit:"g", dim:"mass" };
        case "kg": return { qty:n*1000, unit:"g", dim:"mass" };
        case "oz": return { qty:n*gramsPerOunce, unit:"g", dim:"mass" };
        case "lb": return { qty:n*gPerLb, unit:"g", dim:"mass" };
        case "ml": return { qty:n, unit:"ml", dim:"vol" };
        case "l": return { qty:n*mlPerL, unit:"ml", dim:"vol" };
        case "floz": return { qty:n*mlPerFlOz, unit:"ml", dim:"vol" };
        case "tsp": return { qty:n*4.92892, unit:"ml", dim:"vol" };
        case "tbsp": return { qty:n*14.7868, unit:"ml", dim:"vol" };
        case "cup": return { qty:n*236.588, unit:"ml", dim:"vol" };
        case "ea": return { qty:n, unit:"ea", dim:"count" };
        case "seeds": return { qty:n, unit:"seeds", dim:"count" };
        default:
    ...
    var fromBase = function(base, targetUnit){
      ...
      var q = toNum(base.qty,0), u = base.unit, dim = base.dim;
      if (!targetUnit) return { qty:q, unit:u };
      // mass targets
      if (dim==="mass"){
        switch(targetUnit){
          case "g": return { qty:q, unit:"g" };
          case "kg": return { qty:q/1000, unit:"kg" };
          case "oz": return { qty:q/28.349523125, unit:"oz" };
          case "lb": return { qty:q/453.59237, unit:"lb" };
        }
      }
      // volume targets
      if (dim==="vol"){
        switch(targetUnit){
          case "ml": return { qty:q, unit:"ml" };
          case "l": return { qty:q/1000, unit:"l" };
          case "floz": return { qty:q/29.5735295625, unit:"floz" };
          case "tsp": return { qty:q/4.92892, unit:"tsp" };
          case "tbsp": return { qty:q/14.7868, unit:"tbsp" };
          case "cup": return { qty:q/236.588, unit:"cup" };
        }
      }
      // count targets
      if (dim==="count"){
        if (targetUnit==="ea" || targetUnit==="seeds") return { qty:q, unit:targetUnit };
      }
    ...
    return { toBase: toBase, fromBase: fromBase, unitCost: unitCost, packsNeeded: packsNeeded };
  })();
  ...
  var CommodityDefaults = {
    detergent: { // concentrated cleaners
      unit: "ml",
    ...
    feed_layer: { // chicken layer feed
      unit: "lb",
      basePricePerLB: 0.40 // ~$20 for 50 lb
    },
    feed_broiler: {
      unit: "lb",
      basePricePerLB: 0.45
  ...
  function inferCommodityKey(line){
    ...
    if (g.indexOf("feed")>=0) {
      ...
      if (name.indexOf("sheep")>=0 || name.indexOf("ewe")>=0) return "feed_sheep";
      return "feed_layer";
    }
    if (g.indexOf("seed")>=0 || name.indexOf("seed")>=0) return "seed_generic";
    if (g.indexOf("chem")>=0 || g.indexOf("clean")>=0) {
      ...
      if (name.indexOf("detergent")>=0 || name.indexOf("soap")>=0) return "detergent";
      return "detergent";
    }
    if (name.indexOf("vac bag")>=0 || name.indexOf("vacuum bag")>=0) return "bags_vac";
  ...
  function priceFromCommodityDefault(line){
    ...
    if (key === "seed_generic"){
      ...
      // feed priced by lb
      var lb = U.fromBase(base, "lb"); lb = lb ? lb.qty : qty;
      ...
      var ml = U.fromBase(base, "ml"); ml = ml ? ml.qty : qty;
      var ppm = d.basePricePerML || 0.003;
      var cx = d.concentrationX || 1;
      price = (ppm * ml) / Math.max(cx,1);
    }

    return { price: round2(price), currency: Settings.currency, source: "commodity-default", key: key };
  }

  // ----------------------------- Price Resolution -----------------------------
  function resolvePrice(line, opts){
    opts = opts || {};
    var storeId = pick(line.storeId, (opts.storeId!=null?opts.storeId:null), (StoreSelector.getActiveStoreId && StoreSelector.getActiveStoreId()) || null);
    ...
    if (Inventory && Inventory.priceFor){
      try {
        var invPrice = Inventory.priceFor({ sku: line.sku, name: line.name, storeId: storeId });
    ...
    if (Array.isArray(line.alt)) {
      for (var i=0;i<line.alt.length;i++){
        ...
        if (altResolved) { altResolved.substitution = true; return altResolved; }
      }
    }
    ...
    return normalizeLineCost(line, { price: 0, currency: Settings.currency, source: "unknown" });
  }
  ...
  function normalizeLineCost(line, priceRef){
    ...
    return {
      ...
      meta: Object.assign({}, priceRef.meta || {}, { substitution: !!line.substitution })
    };
  }
  ...
  function estimateUtilities(task, opts){
    opts = opts || {};
    var rates = Object.assign({}, Settings.utilityRates, opts.utilityRates || {});
    ...
    function pushItem(label, cost, meta){
      items.push({ id:id("util"), label:label, cost:round2(cost), currency:Settings.currency, meta:meta||{} });
      total += cost;
    }
    ...
    if (a.btu && a.fuel === "gas"){
      ...
      pushItem("Natural Gas ("+round2(therms)+" therms)", costG, { therms:therms });
    }
    ...
    if (litersUsed){
      ...
      pushItem("Water ("+round2(litersUsed)+" L)", costW, { liters:litersUsed });
    }
  ...
  function coalesceResources(resources){
    ...
    for (var i=0;i<resources.length;i++){
      var r = resources[i] || {};
      var k = (r.sku ? ("sku:"+r.sku) : ("name:"+lower(r.name||""))) + "|"+lower(r.unit||"ea");
  ...
  function estimateLineItemsFromTask(task, opts){
    ...
    for (var i=0;i<resources.length;i++){
      ...
      if (task.meta && task.meta.concentration && r.name){
        var key = Object.keys(task.meta.concentration).find(function(k){ return lower(r.name).indexOf(lower(k))>=0; });
        if (key){
          ...
          if (factor>1){
            ...
            var back = U.fromBase(base, r.unit) || { qty: r.qty / factor, unit: r.unit };
            r.qty = back.qty;
          }
        }
      }

      var normalized = resolvePrice(r, { storeId: pick(task.storeId, task.location && task.location.storeId, null) });
  ...
  function estimateTaskCost(task, opts){
    ...
    try {
      ...
      var subtotal = 0;
      for (var i=0;i<lines.length;i++) subtotal += toNum(lines[i].lineTotal, 0);
      ...
      if (kind==="cooking" || kind==="butchery") {
        ...
        var hasBags = lines.some(function(li){ return (li.group||"").toLowerCase().indexOf("bag")>=0; });
        if (!hasBags){ overhead += 0.50; overheadNotes.push("Packaging disposables"); }
      }

      if (kind==="garden"){
        // seedling mortality +10% cost factor
        var seedCost = lines.filter(function(li){ return (li.group||"").toLowerCase().indexOf("seed")>=0; })
                            .reduce(function(s,li){return s+toNum(li.lineTotal,0);},0);
        if (seedCost>0){ var add = round2(seedCost*0.10); overhead += add; overheadNotes.push("Seed/seedling loss buffer"); }
      }

      if (kind==="cleaning"){
        // rags/filters assumed small cost
        overhead += 0.25; overheadNotes.push("Consumables buffer");
      }

      // Taxes (simple %; override via SettingsStore.utilityRates.sales_tax or dedicated Tax service)
      var taxRate = (Settings.utilityRates && Settings.utilityRates.sales_tax) ? toNum(Settings.utilityRates.sales_tax, 0) : 0;
      ...
      try { eventBus.emit("estimate.created", result); } catch(_){}
      try {
        eventBus.emit("nba.suggestion.created", {
          ...
          action: { type: "openEstimateBreakdown", params: { estimateId: result.id } }
        });
      } catch(_){}
  ...
  function estimateSessionCost(tasks, opts){
    ...
    var subtotal = 0, utilitiesTotal = 0, tax = 0, overhead = 0;
    for (var i=0;i<tasks.length;i++){
      var est = estimateTaskCost(tasks[i], opts);
      combined.perTask.push(est);
      combined.items = combined.items.concat(est.items);
      combined.utilities.items = combined.utilities.items.concat(est.utilities.items);
    ...
    try { eventBus.emit("estimate.session.created", combined); } catch(_){}
    return combined;
  }
  ...
  function mergeLineItems(items){
    ...
    for (var i=0;i<items.length;i++){
      var li = items[i] || {};
      var key = (li.sku ? "sku:"+li.sku : "name:"+lower(li.name||"")) + "|"+lower(li.unit||"ea")+"|"+(li.storeId||"any");
      if (!map[key]) map[key] = Object.assign({}, li);
        ...
        map[key].lineTotal = round2(toNum(map[key].lineTotal,0) + toNum(li.lineTotal,0));
      }
    }
    return Object.keys(map).map(function(k){ return map[k]; });
  }
  ...
  // UMD-ish export
  try { module.exports = api; } catch (_) { this.estimateEngine = api; }
})();
