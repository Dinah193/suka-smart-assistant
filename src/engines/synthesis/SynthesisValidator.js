 ...
 * }>}
 */
export async function validate({ imports = [], readinessSteps = [], sessionSuggestions = [], context = {} } = {}, options = {}) {
  ...
  for (const imp of imports) {
    ...
    byImport[imp.id || imp.title || `imp:${Math.random().toString(36).slice(2)}`] = {
  ...
  if (!ok) {
    ...
    blockers.push(`Readiness coverage below 100% (${Math.round(score * 100)}%).`);
    if (allMissing.length) blockers.push(`Missing: ${Array.from(new Set(allMissing)).join(', ')}`);
  ...
  if (options.commit && Array.isArray(sessionSuggestions) && sessionSuggestions.length > 0) {
    const commitRes = await commitSessionsBuilt(sessionSuggestions, options.planId);
    if (commitRes.ok) {
      ...
      await exportToHubIfEnabled({
        ...
        ts: nowISO(),
        source: SOURCE,
        data: { planId: options.planId || null, count: sessionSuggestions.length },
      });
    } else {
      emit('synthesis.validation.commit.error', { planId: options.planId || null, message: commitRes.error });
      // Commit failure should not flip validation result to false
    }
  }

  emit('synthesis.validation.passed', { score: 1, planId: options.planId || null });
...
export function registerRequirement(rule) {
  ...
  emit('synthesis.validation.rule.registered', { id: rule.id, domain: rule.domain || '*' });
}
...
function detectRequirements(imp, ctx) {
  ...
  for (const rule of REQUIREMENT_RULES.values()) {
    try {
      if (rule.domain && rule.domain !== (imp.domain || '').toLowerCase()) continue;
      const needed = typeof rule.when === 'function' ? !!rule.when({ item: imp, ctx }) : true;
      if (!needed) continue;
...
function isRequirementSatisfied(req, { stepIndex, importItem }) {
  try {
    if (typeof req.satisfiedBy === 'function') {
      return !!req.satisfiedBy({ steps: stepIndex.all, stepIndex, item: importItem });
    }
  } catch {
...
function bootstrapDefaultRequirements() {
  ...
  REQUIREMENT_RULES.set('thaw-protein', {
    ...
    when: ({ item }) =>
      hasAny(item?.items, (x) => /(chicken|beef|pork|fish|lamb)/i.test(x.name) && x.state === 'frozen'),
  });
  ...
  REQUIREMENT_RULES.set('soak-beans', {
    ...
    when: ({ item }) =>
      hasAny(item?.items, (x) => /(chickpea|garbanzo|bean)/i.test(x.name) && x.state === 'dried'),
  });
...
function buildRequirementFromRule(rule, item) {
  return {
    ...
    mitigation: rule.mitigation || `Add a “${rule.label || humanize(rule.id)}” step.`,
...
function scoreCoverage(byImport) {
  ...
  const score = requiredTotal === 0 ? 1 : clamp01(satisfiedTotal / requiredTotal);
...
function indexSteps(steps) {
  ...
  for (const s of steps) {
    ...
    for (const token of ['thaw', 'soak', 'preheat', 'stock', 'sterilize', 'clean']) {
...
async function commitSessionsBuilt(sessions, planId) {
  try {
    const store =
      ...
      (await softImport('src/services/sessions/SessionsStore.js'));
    ...
    if (typeof updater !== 'function') return { ok: false, error: 'No updateMany/bulkUpsert in SessionsStore' };
    ...
    return res?.ok !== false ? { ok: true } : { ok: false, error: res?.error || 'store error' };
  } catch (err) {
