...
import { CheckCircle2, XCircle, Clock, Circle, AlertTriangle, Info, RefreshCw, Filter } from "lucide-react";
...
export default function ReadinessGateCard({
  ...
  const filtered = useMemo(() => {
    ...
    switch (localFilter) {
      case "failed":  return arr.filter((g) => g.status === "failed" || (g.checks || []).some(c => c && c.pass === false));
      case "pending": return arr.filter((g) => g.status === "pending");
      default:        return arr;
    }
  }, [gates, localFilter]);
    ...
    <div className="w-full rounded-2xl border border-slate-200 bg-white shadow-sm">
      <Header
        ...
        onRefresh={() => {
          if (typeof onRefresh === "function") onRefresh();
          try {
            eventBus.emit({ type: "ui.readinessGate.refresh", ts: isoNow(), source: SRC, data: {} });
          } catch (_) {}
        }}
      />
          ...
          {filtered.map((g) => (
            <GateRow
              ...
              onInspect={() => {
                ...
                try {
                  eventBus.emit({ type: "ui.readinessGate.inspect", ts: isoNow(), source: SRC, data: { id: g.id, sessionId: g.sessionId } });
                } catch (_) {}
              }}
            />
          ))}
    ...
    <div className="flex items-center justify-between px-4 py-3 bg-slate-50 rounded-t-2xl">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-slate-500">
          {fmtDate(now, timezone)} <span className="text-slate-400">({timezone})</span>
        </span>
      </div>
      <div className="flex items-center gap-2">
...
function FilterTabs({ value, onChange }) {
  ...
  return (
    <div className="flex gap-1">
      <button className={`${base} ${value === "pending" ? active : idle}`} onClick={() => onChange("pending")}>Pending</button>
      <button className={`${base} ${value === "failed" ? active : idle}`} onClick={() => onChange("failed")}>Failed</button>
      <button className={`${base} ${value === "all" ? active : idle}`} onClick={() => onChange("all")}>All</button>
    </div>
  );
}
    ...
    <li className="px-4 py-3 hover:bg-slate-50">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-[56px]">
          <span className={`inline-flex items-center justify-center text-[10px] font-semibold rounded-md px-2 py-1 ${b.bg} ${b.text}`}>
            {gate.gate?.label || "T-x"}
          </span>
        </div>
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span className="text-slate-800 text-sm font-medium line-clamp-1">{gate.sessionTitle || "(untitled session)"}</span>
            <span className={`text-[10px] rounded px-1.5 py-0.5 ${statusChip.bg} ${statusChip.text}`}>{statusChip.label}</span>
            <span className="text-[11px] text-slate-500">{fmtTime(start, timezone)} <span className="text-slate-400">({dueIn})</span></span>
          </div>
          ...
          <div className="mt-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5">
            ...
            {(gate.checks || []).length === 0 && (
              <div className="text-[12px] text-slate-500 flex items-center gap-1">
                <Info className="w-3.5 h-3.5" /> No checks attached to this gate.
              </div>
            )}
          </div>
        </div>
...
function CheckPill({ check }) {
  ...
  const bg = pass ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
           check.severity === "error" ? "bg-rose-50 text-rose-700 border-rose-200" :
           "bg-amber-50 text-amber-700 border-amber-200";
  return (
    <div className={`flex items-center gap-1.5 text-[12px] border rounded-md px-2 py-1 ${bg}`} title={check.detail || ""}>
      <Sev className="w-3.5 h-3.5" />
      <span className="truncate">{check.label || "Check"}</span>
      {check.actionSuggestion && !pass && (
        <span className="truncate text-[11px] opacity-80">• {check.actionSuggestion}</span>
      )}
    </div>
  );
}
...
function badge(domain) {
  switch (String(domain || "general").toLowerCase()) {
    case "cooking":      return { bg: "bg-emerald-100", text: "text-emerald-800" };
    case "cleaning":     return { bg: "bg-sky-100", text: "text-sky-800" };
    case "garden":       return { bg: "bg-lime-100", text: "text-lime-800" };
    case "animal":       return { bg: "bg-amber-100", text: "text-amber-900" };
    case "preservation": return { bg: "bg-fuchsia-100", text: "text-fuchsia-800" };
    case "storehouse":   return { bg: "bg-indigo-100", text: "text-indigo-800" };
...
function chip(status) {
  switch (status) {
    case "passed":  return { label: "Passed",  bg: "bg-emerald-100", text: "text-emerald-800" };
    case "failed":  return { label: "Failed",  bg: "bg-rose-100",    text: "text-rose-800" };
    default:        return { label: "Pending", bg: "bg-slate-100",   text: "text-slate-800" };
  }
}
...
function deriveStatus(gate) {
  ...
  if (checks.some(c => c && c.pass === false && (c.severity === "error" || c.severity === "warn"))) return "failed";
  if (checks.length > 0 && checks.every(c => c && c.pass === true)) return "passed";
  return "pending";
}

function fmtDate(d, tz) {
  const dtf = new Intl.DateTimeFormat("en-US", { timeZone: tz, month: "short", day: "2-digit" });
  return dtf.format(d);
}
