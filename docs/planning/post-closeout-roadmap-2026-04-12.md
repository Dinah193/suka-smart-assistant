...
- Verified post-merge baseline for PR #49 by polling main CI smoke-e2e to terminal completion (rerun required after a stalled in-progress run).
- Sprint B approvals audit slice: added integration coverage in `_tests_/plannerUnifiedFeed.contract.test.js` asserting approval audit-log transition persistence (`pending_approval->active->completed`) and deterministic notification precedence (`approval.requested` action-required before `approval.decided` informational).
- Sprint C kickoff slice: added cross-household project-space APIs in `src/server/routes/planners.js` for project create/list plus milestone, contribution, and membership trust updates (`/api/planners/community/projects*`).
- Sprint C kickoff coverage: added `_tests_/communityProjectSpaces.contract.test.js` to validate deterministic project schema behavior for milestones, contribution logs, trusted membership promotion, and filtered retrieval by visibility/trust/member household.
- Sprint C follow-up privacy/governance matrix: enforced project visibility semantics (`household_only`/`trusted`/`public`) with viewer-aware filtering in `/api/planners/community/projects` and dispute listing visibility guards.
- Sprint C follow-up trust-mode enforcement: enforced `invite_only`/`trusted_only`/`open` contribution and milestone mutation gates on project routes with deterministic `403` contracts for unauthorized actors.
- Sprint C follow-up moderation disputes: added project dispute report/list APIs (`/api/planners/community/projects/:id/disputes*`) with approval workflow linkage and status synchronization coverage when approvals transition.
- Sprint C follow-up coverage: extended `_tests_/communityProjectSpaces.contract.test.js` and added `_tests_/communityProjectDisputes.contract.test.js` for privacy matrix, trust-mode enforcement, and moderation dispute lifecycle assertions.